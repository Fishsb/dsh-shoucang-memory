...
> **v5.1（2026-09-10 架构体检实锤，ACT-020）：`profiles` 的 target 用词与宿主写门对齐。** v4 起本文档与宿主契约均把 target 写作裸名 `USER|AGENT`，而宿主写门 `writeProfileLine` 只接受文件名 `USER.md`/`AGENT.md` → **蒸馏 profiles 通道自 v15（09-08）上线以来每一次写入都被拒**（审计 `gate-reject` 里 `target=USER`/`AGENT`；`USER.md` 停更于 09-02 为佐证）。现两侧统一为 `USER.md|AGENT.md`；宿主另加**归一化**（额外接受 USER/AGENT、带或不带 `.md`、大小写差异，模型输出漂移一律在宿主侧收敛而非拒收）与**拒因明细**（非画像目标 / 格式违规 / 容量超限含阈值与算式 / replace 未逐字命中），并把画像容量从硬编码 3,000 改为按目标读容量门（`capUser`/`capAgent`，与面板/write_gate 同源）。
...
> **树由模型自动维护（2026-09-10 拍板）**：结构决策归模型——蒸馏写侧并入/裂 L2；深睡 treeOps 通道（v1 落地见下行）允许模型输出结构操作 JSON（split/merge/rename/move/attach-clone 全谱最终形态），宿主只守不变量（锚/指针不悬空/无孤儿/归档可回滚/write_gate/health/幂等）。层义为模型规范非硬白名单。v5.4"深睡不新建/不合并小节"禁令在 treeOps v1 落地时解除（分裂新 ### 仍归蒸馏写侧）。
> **treeOps v1（2026-09-10 落地，src/treeops.ts）**：操作集=rename/merge 两档（split/move/attach-clone 后续档）——模型提提案（file/oldTitle/newTitle/keepTitle/dropTitle 逐字取自材料「现行树节清单」），宿主执行并守不变量（归档 audit/treeops/ 可回滚、锚存在、指针集内重写不悬空、无孤儿、write_gate、幂等）；DEFAULT_DISTILL_PROMPT 的 treeOps 判据句对齐列为后续档。
> v1（ADR-0004 内嵌）→ v2：加第一层归属路由 → v3（2026-09-06 用户拍板）：判定锚从类别改为粒度——两库的区别不是主题类别，而是粒度与功能：记忆库=粗粒度方向指引，项目卡库=细粒度规范/事实 → **v3.1（2026-09-06 用户拍板）：格式传递**——`newIndex`/`appends.text` 内嵌记忆库 spec §8 教程式与索引行模板（**条目格式权威 = memory-whitelist-spec.md §8**），蒸馏产出不再凭示意自由发挥。
