...
- **深度睡眠归纳 pass（L0 原则层唯一写入口；2026-09-08，睡眠=回想巩固当天的记忆）**：**宿主自动触发**（守藏蒸馏器 10min 巡检：全部任务会话停滞 ≥3 小时 → 自动执行一次，每轮停滞窗口至多一次；审计轮可手工兜底同一 pass）。
  停滞判定=**会话活跃状态机**（不靠单点时间戳）：任意事件→RUNNING，turn/end(completed)→ENDED；RUNNING 无事件 ≥3h → PROBING（采样转录文件两次比对 mtime/size）→ 输出增长=**正常长任务**（刷新水位，不睡）/ 无增长且会话在=**STALLED 疑似卡住**（不阻塞睡眠，发审计告警需人工确认）/ 无增长且会话已消失=**EXIT 异常退出**（计入停滞）/ 探针不可用或探测异常=无法确认，**按停滞处理→正常睡**。**只有确认「长线任务正在推进」才拦住睡眠**（用户拍板口径 2026-09-08）；停滞计时一律沿用最后事件时刻，不刷新成当前时间（否则永远睡不着）
