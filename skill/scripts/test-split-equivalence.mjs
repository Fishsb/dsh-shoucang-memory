...
/* ══ 仓级单写者锁（2026-09-25 · 实测事故驱动）════════════════════════════════════
 * **为什么必须加锁**：本件是**会改真实源码树 + 重建产物**的那一类门禁（上面已自述"约 40 秒"）。
 *   而 `check-runner.mjs` 无任何互斥（实测 `lockfile|LOCK|flock|mutex|pidfile` 0 命中）⇒
 *   两处同时跑门禁时，本件的反例注入会被另一处的读取/构建夹在中间。
 * **实测后果（2026-09-25）**：`panes-toggles.js` 的 `renderTogglesSched` 被替换成反例注释后
 *   **未被还原**、`styles.js` 停在 `--sc-nav-w:4px` ⇒ `client.js` 成了**反例态产物**，
 *   `test-panel-view-contract` 缺 13 项而红，且**全部依赖 client.js 的门禁读到的是被破坏过的产品**。
 *   本件件头 :62-70 早就记载过这个"一旦污染就永远无法自愈"的形态，只是当时**没把并发列为触发路径**。
 * ⇒ 修法：进本件先取 `repo-lock`（与 `bank-lock` 同族、同纪律，见 `scripts/repo-lock.mjs` 件头）。
 *   **fail-closed**：等不到锁（缺省 90s）即**拒跑退 1**，绝不静默放行 —— 放行就等于让两个 build 交叠。 */
const { acquireRepoLock, releaseRepoLock } = await import('./repo-lock.mjs')
const lockHandle = acquireRepoLock(root, { note: 'test-split-equivalence', waitMs: Number(process.env.SHOUCANG_REPO_LOCK_WAIT_MS || 90000) })
if (!lockHandle) {
  console.log('❌ 未能取得仓级写锁（另一处正在跑会写源码树的门禁）⇒ **拒跑**（fail-closed，绝不并发改源码）')
  console.log('   ⇒ 说明：本件要临时改写 src-client/ 并重建 client.js；与另一个构建交叠会留下反例污染。')
  console.log('   ⇒ 退回动作：等对方跑完重试；或显式放宽等待 SHOUCANG_REPO_LOCK_WAIT_MS=<ms>。')
  process.exit(1)
}
...
/* **还原语义（2026-09-25 重写）**：原兜底在"锚点缺失"分支调 `git checkout --` 两源文件 ——
 *   但那会**连带抹掉别人未提交的改动**（本仓跨会话在途改动是常态；实测本件目标两件当时是 CLEAN
 *   才没出事，属**潜伏**风险）。改为：**进入本件第一件事就是把自己的工作副本写到仓外**，
 *   兜底一律从该副本还原 ⇒ 只动本件碰过的两个文件、且只恢复到"本件开始时的样子"，
 *   既不依赖 git，也不会越界影响他人。 */
const SNAP_DIR = join(tmpdir(), `sc-split-equiv-${process.pid}-${Date.now().toString(36)}`)
mkdirSync(SNAP_DIR, { recursive: true })
const SNAP_TOGGLE = join(SNAP_DIR, 'panes-toggles.js')
const SNAP_STYLES = join(SNAP_DIR, 'styles.js')
const SET_PATH = join(root, 'src-client', 'styles.js')
copyFileSync(TARGET, SNAP_TOGGLE)
copyFileSync(SET_PATH, SNAP_STYLES)
/* 兜底用：把两个源文件恢复成「本件开始时」的样子（逐字节）。**不碰 git，不动他人改动**。 */
const restoreFromSnapshot = () => {
  try {
    copyFileSync(SNAP_TOGGLE, TARGET)
    copyFileSync(SNAP_STYLES, SET_PATH)
    return true
  } catch (e) {
    console.log('❌ 从工作副本还原失败：' + String(e?.message || e).slice(0, 160))
    console.log(`   ⇒ 工作副本仍在 ${SNAP_DIR}（未删，可人工恢复）`)
    return false
  }
}
...
try {
  ...
  if (!restored) {
    if (exitedEarly) {
      ...
      writeFileSync(TARGET, orig)
      writeFileSync(SET_PATH, readFileSync(SNAP_STYLES))
      console.log('⚠ 异常路径：已强制还原 panes-toggles.js + styles.js')
    }
    build()
  }
  /* 释放仓锁 + 清工作副本（锁最后释放：build() 也写产物）。 */
  try { rmSync(SNAP_DIR, { recursive: true, force: true }) } catch { /* 留证优先，不因清理失败而红 */ }
  releaseRepoLock(lockHandle)
}
