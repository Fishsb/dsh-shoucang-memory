...
- 记忆数据只存**你本机**的 DSH 家目录（默认 `~/.dsh/suite/memory`，可用 `MEMORY_ROOT` 覆盖），不入库、不发布、不上报
- 无遥测、无外部依赖服务——除你自己配置的 LLM / 向量端点外不发任何请求
- 仓库带「公开树隐私门」机检 `scripts/check-public-tree.mjs`——**推送前须手动执行且必须 PASS**（仓内**无** git hook、无 CI 自动跑，别当成已自动化）
...
改少量文件时不必等全量（236 件串行约 2 分钟）——按需选择：
...
且子集摘要自带「非全量」标记。`--fast` 跳过在册慢件（`{ slow: true }`，实测 >5s，如 `test-split-equivalence`
