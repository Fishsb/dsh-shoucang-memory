...
- 🧠 **长期记忆库** — 原则层 + 索引层 + 详情层分层结构，每次会话自动加载，越用越懂你
- 🏭 **会话蒸馏** — 会话空闲后自动唤醒，LLM 子代理裁决「什么值得记」，教程式浓缩写回记忆库
- 🌙 **深度睡眠** — 全部会话停滞后自动归纳「原则层」，把当天记忆提炼成可迁移的通用经验
- 👤 **双画像通道** — USER.md（用户画像）+ AGENT.md（agent 自我认知），蒸馏与深睡双入口持续维护
- 🖥️ **设置面板** — 侧栏入口：记忆库浏览、蒸馏统计、参数调节、深度睡眠视图，深浅色跟随宿主主题
- 🛡️ **白名单门禁** — 写入过闸：不符白名单一律不存，宁缺毋滥

> 守藏（shoucang）：取「善于收藏、守而不忘」之意。
...
| 层 | 内容 |
|---|---|
| `PRINCIPLES.md` | 原则层（L0）— 深度睡眠归纳的跨任务通用经验，容量硬限 1,000 字符，唯一写入口 = 深睡归纳 |
| `MEMORY.md` | 知识索引（L1）— 工具用法 / 工作流 / 教训 / 环境事实，一行一条指向详情 |
| `USER.md` / `AGENT.md` | 画像索引（L1）— 你的偏好习惯 / agent 自身纪律，蒸馏 + 深睡双通道维护 |
...
全部会话停滞 ≥ 3 小时（可配）自动运行一次归纳 pass：当天痕迹 → 归纳子代理提炼原则 → write_gate 校验 → `PRINCIPLES.md` 原子落盘（同时维护 USER/AGENT 画像）。
...
- **记忆板块** — 三索引 + 原则层容量卡、notes 分类浏览、小节直达、蒸馏统计与趋势
