...
> 守藏（shoucang）：取「善于收藏、守而不忘」之意。**定位 = DSH 的长期记忆底座**（服务助理的记忆能力，而非"本插件是助理"）：蒸馏=学习，深度睡眠=反思，replace 更新=迭代。伙伴关系与人设实现属另一插件，不在本仓。
...
| 层 | 内容 |
|---|---|
| `AGENT.md` | agent 自我画像（成长档案）— 角色定位 / 学习史 / 稳定做法 / 常犯错误 / **`[原则]` 习得原则**（深度睡眠归纳的跨任务通用经验，v16 并入此文件）/ **`[路径]` 任务路径**（v17，对标 AWM：可复用任务类型的变量化步骤序列），容量 3,000 字符 |
...
全部会话停滞 ≥ 3 小时（可配）自动运行一次归纳 pass：自上次归纳以来的新痕迹（无痕迹则本轮不睡）→ 归纳子代理提炼「习得原则」与「通用任务路径（`[路径]` 行，v17，具体值变量化）」与画像更新 → write_gate 校验 → 以 `[原则]`/`[路径]` 索引行原子写入 `AGENT.md`（**反思双通道**：认识自己 + 认识用户，同步维护 USER 画像）。归纳材料含窗口内任务运行成败统计（v17，ExpeL 式成败对比信号，定位真因用）。
