...
- **🔴 授权推进轮（N-1/N-2/N-3）：嵌入通道恢复 + 投影归位 + 深睡闭环复验 —— 揭出「通道修好 ≠ 判定可用」的第三层（2026-09-23 · ACT-367 / ADR-368 / ADR-369）**：
  - **N-1 嵌入通道恢复（环境面，实证）**：判因发现 `Ollama` 主进程不在、**11434 未监听**，但留着一个**孤儿 `llama-server`（PID 14068，09-22 起，父进程已死）** —— 它占着模型却**不服务 11434**。
    ...
    孤儿经判因（父死 + 零连接 + 已被新 worker 取代）后终止，**回收 1.7GB 内存 + 一个 GPU 上下文**（8GB 卡），服务不受影响。
    ⚠ **观测证据**：`check-relevance-live` 由红转绿（此前 `A4 向量不可用 provider=unreachable`）。
  - **N-2 投影归位（含库内副本）**：`criteria.md` / `criteria-gate.json` 两投影过期 ⇒ 重生成；**并按 `check-deploy-sync` 同步库内 `<bank>/engine/` 副本**
    （该门本就是为"库内跑的是旧判据却报成功"而立）。⇒ `check-criteria` / `check-carriers` / `check-deploy-sync` **三红转绿**。
  - **N-3 深睡闭环复验（本轮最重要的结论）**：手动触发一轮深睡（`POST /deep-sleep/trigger`，chunk 1/5 completed）。**回流链修复已真机生效**：
    新落 `yield.verdict` 行的 `sid` 由**死会话 `6be5ac2e`**（09-16 末次活动）变为**近期会话 `41cf9478`** —— 取样修复的直接证明；长形 sid 也能读到该行 ✅。
    ⚠ **但 `yieldUnjudged` 仍 10/10，全库 `judged=true` 的 mcl-step = 0**。取证到**判定器输入**才看清根因：
    `buildYieldRequest` 唯一能给的判别信息是 `nextTools`（工具名序列），而本批 10 条**全是 `pwsh→write→pwsh` 这类无信息重复**
    —— `nextAction` 字段**全仓零产生者**（`harvest-access.mjs:227` 只落 `nextTools`，正是原注释写的"采集层只落工具名 ⇒ 通常为空"）。
    ⇒ 模型按提示词「判不准就写 null，**不要猜**」**诚实全判 null**。**这不是 fail-closed 生效，而是「输入无判别力 ⇒ 判定器无法工作」**
    —— 比"通道不通"更隐蔽：通道是通的、判定在跑、字段也齐全。**判据质量的下一步不在回流侧，而在采集侧（补 `nextAction`）。**
  - **同轮修掉我造成的 3 处红**（如实记）：① 深睡提交 `3e1e691` 新增的 `AGENT.md:201` 原则行**引入即带悬空指针**
    （`§契约内容准入` 指向 `flows.md`，实际小节在 `lessons.md`）⇒ 触发 `check-section-refs` 棘轮（baseline 0 → partial 1）；已改为双文件指针，partial 归一 0。
    ② 我编辑 `AGENT.md` 忘了重镜像 ⇒ 影子库失配（12681 vs 12664 字符）⇒ 走 `mirrorFile` 重镜像（`roundTrip=true`），`check-record-parity` 转绿。
    ③ criteria 投影与库内副本同源缺口（见 N-2）。
  - **验收**：全量门禁 **232 pass / 1 xfail / 1 skip / 3 fail**；剩余 3 红经三重取证（mtime 09-17/19/20 早于本轮 · 基线 diff 为他人新增 · A3c 是代理断言）
    **无一属本轮改动面**：`test-panel-view-contract` + `check-i18n-keys`（他人会话在 `panel-view-baseline.json` 加了 `capacityEnforce` 期望但渲染未接完）+ `test-split-equivalence`（A3c 纯为前者代理）。
    部署面 **320/320 sha 一致** · 热重载 fiber active · 特性探针 **118 项齐全**。

- **🔴 回流链复验：前一轮「回流通道真机生效 ✅」结论**被推翻** —— 三处静默失效叠加，通道建成**但读侧永不命中**；本轮逐条修（2026-09-23 · ACT-363 / ADR-366）**：
  **前一轮只证了「写侧落了行」**（`yield.verdict` 真机 1 行），**从未验证读侧能否认领它**。本轮补验，结果**认不到**：
  - **① 键口径两侧相反（真缺陷）**：真机 `agent.id` 是**长形** `session-<uuid>`（活体面板实测 `sysBlockLastSid="session-7de99a"` + 轨迹 `renSame`），
    而读侧 `mcl.ts:588` 用 `slice(-8)`、写侧（全量审计行）用 `slice(0, 8)` ⇒ 同一会话**两个键** ⇒ 永不相等。
    端到端反证（直接跑安装副本函数）：喂长形 ⇒ `undefined`；喂短形 ⇒ 才读到那行。
    ⇒ 收敛为**单一实现** `recall-yield#shortSidOf`（剥 `session-` 前缀后取**前 8 位**，与全量审计行同源），读写两侧复用。
  - **② 取样器恒指向已死会话（真缺陷）**：`deepsleep-run.ts:209` 原按 `idleSteps` 降序取前 10 —— 而该键被采集器封顶 3，
    实测分布 `{0:948, 1:20, 2:26, 3:5739}` 即 **85.2% 并列最大** ⇒ 排序键**饱和**，稳定排序取到的是**最老的 10 行**
    （实测全为 2026-09-15，sid `6be5ac2e` 末次活动 09-16）⇒ 回流行恒写给**死会话**，活会话按自己的 sid 取（跨会话不继承）⇒ **仍然落空**。
    ⇒ 新增单一实现 `recall-yield#sampleYieldRounds`（**筛 `idleSteps>0` + 按时间序取最近**，正是原注释已写明的意图）。
  - **③ 尾读窗口小于字节窗（真缺陷）**：`readLatestLedgerRow` 缺省 `perVolume=200`，而 `readTailLines` 字节窗 256KB ≈ 730 行
    ⇒ **读了却丢弃约 73%**；叠加台账 4.8 行/分增长 ⇒ 约 42 分钟后距末尾必超 200。
    同日实测同一行：`perVolume=200` **MISS**、`300` **HIT** ⇒ 一个**每分钟都在发生**的时间差即可让回流静默失效。
    ⇒ 缺省改 **1000**（与字节窗同量级，**零新增读盘**，只把已读到的真正用起来）。
  - **④ 测试盲区一并修（变异自证抓出）**：`test-mcl` 夹具用**短形** `sess-m3-1`，与生产**长形**不同 ⇒ 测不出跨侧错配。
    改为生产形状 `session-<8hex>-uuid`；新增 M3L 场景（长形下回流必须命中）。
    ⚠ **变异自证暴露更深一层**：两侧共用同一函数后，M3L-1/2 **改任何口径都仍绿**（已实测）——
    共用实现能**消除错配**，但**不足以证明口径正确**⇒ 必须把**约定取值本身**写成字面量断言（M3L-4）。
    另：`test-recall-yield` ⑥ 原断言（`ev.sid === 'abcdef12'`，注"与读侧 `slice(-8)` 同口径"）**把缺陷当预期**，一并订正。
  - **判据（三处各带反向自证）**：`test-recall-yield` ⑧-1/⑧-2 · `test-ledger-compact` E1–E5 · `test-mcl` M3L-0..4。
    变异实测：改回 `slice(-8)` ⇒ M3L-4 红；改回 `perVolume=200` ⇒ E1 红；改回 `idleSteps` 排序 ⇒ ⑧-2 红（**三条都能抓住**）。
  - **⚠ 诚实边界**：本轮的「回流可用」仍是**函数级 + 真库读数**证据，**尚未**观察到「真机深睡产出 verdict ⇒ 活会话据此换向」的完整闭环
    （需下一轮深睡，且上游 Ollama 嵌入通道自 18:09 起不可达 ⇒ 判定多半仍 `unjudged`）。

- **🔴 消费链中层「判据哑火」根治：判断者替换（`acted` → 轮级模型判）+ 补上缺失的结论回流（2026-09-23 · 用户授权「按方案执行」）**：
  圆桌会议 8 席 3 轮定稿（`docs/consumption-chain-mimesis-plan-2026-09-23.md`）后落地。**病症**（全三档台账实测）：
  中层「元推理监测→控制」**机制在线但信号已死** —— D5（09-22）后 `zeroGain` 恒 0 ⇒ `switchSource` 永假
  ⇒ `mcl-switch` **结构性不可达**（后段 2570 行 judge 全 0），而它在读数上与"一切顺利"**完全同形、全程零告警**。
  **真根因**：`acted = evs.some(e => e.type === 'tool/call')` 取的是**整个会话**快照（`mcl.ts:723/757`）
  ⇒ 语义 =「本会话是否**曾经**调用过任何工具」⇒ 会话内调过一次即**恒 true**（实测 1356 true / **0** false）
  ⇒ `foldZeroGain` 无条件归零。**实证**：D5 前 `zeroGain` 有 39 种取值 / `≥2` 行 180 / `switchSource=true` 114 行；
  D5 后全 0 ⇒ **不是"从未触发"，是 D5 把它关死的**。
  - **① 判断者替换（用户拍板原则「代码/向量不足够判断时积极介入模型判断」）**：`foldZeroGain` 的**唯一驱动者**
    改为**轮级模型判**（`helped` / `not-helped` / `unjudged` 三态）；`echoed`（词面代理）**降为纯观测、不驱动计数**；
    **未判 ⇒ 计数不动**（fail-closed，不据"没有证据"换向）。**删** `acted` 主信号与 `nextZeroGain`（对外零调用方）。
    ⚠ **不做"窗口化 `acted`"**：那只是把会话级弱代理换成窗口级弱代理（采集器同口径实测 85%–99% 非空）
    ⇒ 仓内反复剿灭的「假绿换假绿」。
  - **② 补上缺失的回流边（实测：本无此通道）**：`mcl-switch` 10 行与 `yieldSwitchSemantic=true` 14 次**跨日错开**，
    配对 60s/3600s 均 **0/10**（86400s 的 10/10 是同日巧合）；且 `mcl.ts` 此前**全文零 `readFileSync`** ⇒ 离线判定
    对步内**零影响**。现：离线判定落一条 `yield.verdict` 到**既有统一台账**（**不新开 `.jsonl` 流** ——
    `check-observability` 的 `BASELINE=1` 只许减不许增），步内**每步跨档读一次**。
  - **③ 三态字段（照 ACT-352 范式）**：新增 `yieldUnjudged` / `yieldError` —— **键缺席 = 判定完成**，
    有值即原因（`unreadable`/`insufficient-rows`/`unparsable`/`threw`）。判因（obs 实测）：59 行真机判定里
    这两个字段**根本不存在**，「未判 10」只活在 `yieldNote` **中文串**里 ⇒「判不准」与「判不了」**不可分辨**。
    ⚠ 消费侧**必须**写 `'yieldError' in o`（`undefined` 会被 `event-envelope` 静默丢键，两形态同形）。
  - **④ 台账 IO 归位**：新增 `ledger-compact#readLatestLedgerRow`（**跨档 + 有界尾读**，热路径不全量读 48k 行）
    与 `appendLedgerLine`（写侧单一出口，内置 fail-safe）。此前 `mcl.ts` 若自持路径会撞 `check-ledger-read`
    —— **门禁是对的**（只读主档会静默丢轮转历史，G13 已因此漏网 6 处），故改设计而非加例外。
  - **⑤ 未判可分辨（实测补的洞）**：`fast` / `material-step` / `decide-none` / `inject-msg` 四条「本步没判」的
    分支原先**不带 `judged`** ⇒ 与"判了没进展"在读数上同形。现一律如实标 `judged:false`
    （`test-mcl` 新增 M3N 场景自证：**无回流 ⇒ 零换向**，旧语义此处会恒真）。
  - **判据**：`check-yield-reflow`（**新登记**，含 9 条自证：正例绿/反例红/误报防线/「行数多≠有判别力」）
    —— R1 结构可触发性 · R2 出口可触发性 · R3 回流配对率，**锚事件锚点、禁墙钟**
    （实测墙钟窗会把"又哑了一天"藏进绿里）。今日按 **xfail 声明制**登记（缺陷在 + 数据待产生）；
    **转绿即 XPASS ⇒ 退 1（施工完成信号，非回归失败）**。`test-recall-yield` 重写（33 条，含 3 组反例自证）；
    `test-mcl` 增至 60 条（M3 场景改由**回流真驱动** + 新增 M3-8/M3N 反向自证）。
  - **零抬基线**：`check-module-growth` PASS；新增逻辑落 `recall-yield`/`ledger-compact` 纯函数区
    （`deepsleep-run` 曾达 605 行触阈，按门禁正道"新功能落新模块"抽出，未 `--rebase`）。
    三件导出阈顶格件（`panel-shared`/`deepsleep-core`/`targets` 35/35）**均不在改动面**。

- **🟢 容量门册三补完：开关贯通子链 + 写入原语 CAS/可验 + 三处可观测面（2026-09-22 · ADR-334 · 用户授权「全部推进」）**：
  上一轮把册零/册二/册四落地后，核查发现**四项未落地**，本轮全部补齐。**均按架构层收口，非逐条打补丁**：
  - **① 开关贯通到子进程链（治「关了一半的门」）**：`SHOUCANG_CAP_STRICT` 在三处子进程调用点**语义分叉** ——
    两个自动路径（`capEnv` / `deepsleep-apply`）**从不注入**该变量，而面板路径用 `{...process.env}` **继承环境**
    ⇒ 部署侧一旦设了 `=1`，"**人在面板上手工改文件**也会被自动容量闸拒"，且**无任何信号**。
    **正解不是加第二个 env，而是复用这个既有变量**（它在两份门脚本里已实现已测试，语义与开关同构）：
    `capEnv` 映射开关 → `'1'`/`'0'`；`panel-inject`（人工路径）与 `treeops`（结构维护路径）**显式置 `'0'`**
    —— 把"该路径不参与自动容量阻断"写成**代码里的显式事实**，不再交给宿主环境碰运气。四点全覆盖。
  - **② 写入原语补 CAS（治"没有协议"）**：`AGENT.md` 有两个**都不取库锁**的写者（画像通道 / 原则通道），
    此前各自"读 → 改 → rename"，**末写者胜**且双方都报成功。新增 `atomicWriteFileCas`——写前确认
    "盘上仍是我读到的基线"，否则**拒写退让**（`failed` ⇒ 既有水位回滚重蒸通路，**不静默覆盖**）。
    两个写者均已接入；原则通道的 CAS 预检与 rename **收敛为单一失败出口**（`commitWithCas`）。
  - **③ 可验层补逐字回读（治"自称可验却只验长度"）**：`section-rewrite` 头注自称"③ 可验"，
    而实现只比 `statSync().size` ⇒ 内容被并发覆盖但**长度相同**时检测不出。两个原语
    （`atomicWriteFile` / `gatedWriteFile`）**同批**改为逐字回读比对（不同批会分叉成两套"可验"语义）。
  - **④ 三处可观测面接消费方（治"采了不显示"）**：
    · `otherChannels` 增 `byChannel` 明细 + `zeroLanded` —— 原只有 `{tried,done}` 两个汇总数字，
      **答不了"哪个通道零落地"**（实测画像通道有提案零落地的轮次被判 `landed=true` **29 轮**，
      而告警台账 **0 行从未响过**）。类型与归约抽到新件 `channel-plan.ts`（解 `deepsleep-core` 导出棘轮 37>35），
      `deepsleep-core` **再导出**保零迁移。
    · `memory-reconcile` 拒绝率增 `byChannel`（原分母用**全体** writeEvents ⇒ 画像 167 次真拒被摊薄成 **7.4%**）+
      修正原式**量纲错误**（分子是"拒绝行数"、分母是"事件数"）。
    · `panel-memory` 增 `gateRejectsDetail`（按 target 分组 + 最近一次时间/原因）—— 原 `gateRejects`
      在 `src-client/*` **零命中**（采了没人看）。三者均在「运行观测」页**渲染**。
  - **⑤ 结构收口（打补丁的反面样本）**：CAS 首版写成"预检 + rename"**两处并列失败出口**，
    被 `test-wiring-gate-ast`（要求失败出口**结构性唯一**）判红 ⇒ **收敛结构**，而非把判据期望值从 1 放宽到 2。
    同理 `applyPrinciples` 顶破 I1 棘轮（122 行）⇒ 按领域接缝抽 `readPrincipleOrigin`（落盘链的读端，
    与写端 `commitAccepted` 成对）。
  - **⑥ 文案**：`panes-memory-detail` / `i18n-dict-memory` 残留的「超限由 write_gate 拒写」改为
    随开关态如实描述（i18n 双验收基线随之更新）。
  - **判据**：新增 `check-capacity-wiring`（四点接线 + 三处可观测面，含 `--selftest` 三反例）与
    `verify-cas-live`（CAS 真机拦截/放行/可验三态，真库零改动）；`CHECKS` **213 件**。
  - **验收**：仓内 **212 pass · 0 fail** · 部署 **320/320** 一致 · 热重载 active · 两条真机探针 PASS ·
    `ui-geo-regress` **121 PASS / 0 FAIL** · 公开树 PASS。
  - **仍未做**（如实声明）：存量处置（`AGENT.md` 332% / `MEMORY.md` 914%）—— 方案 §7-O2 已裁定**单列一轮**，
    理由是超容量主体是 principles 通道（它仍在放行）⇒ 此刻裁剪会被对冲。
- **🟢 容量门改为可关闭的开关（2026-09-22 · ADR-333 · 用户授权「开始落地」）**：
  用户原话「**容量门不是硬拒绝门槛，做成开关功能，要可以关闭硬拒绝门槛功能**」。
  - **① 口径统一（册零）**：画像容量判据原用 `body.length`（**含空白**），而面板与两个门脚本用
    `replace(/\s+/g,'').length`（**去空白**）⇒ 实测同一份 `USER.md` 面板报 **87.3%**、写门按 **98.9%**
    判（Δ**348**）——**两个数互相矛盾，面板那根进度条在结构上显示不出该故障**。现统一走
    `capacityCharsOf`（**单一实现**，落 `budget-override.ts` 的额度域纯函数区）。
    ⚠ 首版落在 `targets.ts` 被 `audit-architecture` 的**导出棘轮**挡下（36>35），按仓内既有出路移位。
  - **② 新增开关 `capacityEnforce`（册二）**：`false`（**缺省**）= 超限**照写**并落一条 `capacity-over`
    留痕；`true` = 超限拒写（= 改造前画像行为）。缺省取 `false` 的三条理由：四条写入路径里**三条现状已是不阻断**
    （`cap_memory` 913.9% 照写 / principles 已写 112 行 / 索引行）、用户 2026-09-16 已就索引行判定
    「直接拒绝不符合意图，提醒就可以」⇒ 本条是**推广**该判定、取 `true` 等于"修完默认仍坏"。
    **只关容量这一支**：指针（exit 2）/格式（4）/凭据（5）三道门照旧硬拒。
  - **③ 留痕不可删**：拒写态原有的 `gate-reject` 审计是画像通道**唯一的失败信号**（真机 167 条）；
    不阻断后**换名 `capacity-over` 继续落一行** —— 否则开关会亲手删掉该信号（"让失败不可观测"）。
  - **④ 文案与承诺修正（册四）**：设置页三框原文案无条件声称「写入超限被拒」，而实测四条路径只有一条真拒
    ⇒ 改为**随开关态**如实描述；`deepsleep-apply.ts` 里**用户可见**的错因串 `超限=原则间合并（本轮跳过）`
    通向一个**全仓零实现**的出口（承诺 2 处 / 实现 0 处），改为如实描述「本轮拒收」并指向**真实存在**的
    `consolidateTree`。
  - **⑤ 结构收口**：`panes-toggles.js` 因新增控件达 616 行 ≥600，按 `check-module-growth` 门禁原文
    「**新功能应落新模块**」抽出 `panes-capacity.js`（② 记忆与容量 pane）；`numSetting` / `enumSetting`
    两个控件下沉到中立层 `ui-kit.js` —— 首版留在 `panes-toggles.js` 导致 `panes-toggles ↔ panes-capacity`
    **静态循环**，被前端分层门判红（实测）。
  - **⑥ 两个真 bug（门禁全绿也没发现，由真机探针抓出）**：开关语义首版写成 `=== false ? false : true`
    ⇒ **键未设时判"阻断"，与"缺省不阻断"正好相反**；修一版又写成 `=== true` ⇒ **坏值静默变成"不阻断"**
    （放宽而非保守）。最终四态逐一手算：缺键⇒不阻断 · `false`⇒不阻断 · `true`⇒阻断 · **坏值⇒保守阻断**。
    ⚠ `typecheck` / `build` / 全部 208 项门禁对该错**全绿** —— 只有真机行为探针抓到，已登记为常驻判据。
  - **判据**：`probe-capacity-gate.mjs`（静态形态 + 真库零改动）+ `verify-capacity-live.mjs`
    （走**安装副本真实代码**实测四态判决）；两条均登记 `CHECKS`（**210 件**）。
  - **边界**：**不改真源数据**（探针只写副本，真库前后 sha256 相同，已断言）；存量
    （`AGENT.md` 332% / `MEMORY.md` 914%）**本轮不动**，单列后续方案 —— 理由是超容量主体是 principles 通道
    （`[原则]`96+`[路径]`19 vs 画像行 13），**该通道仍在放行** ⇒ 任何裁剪会被对冲。
- **🔴 会话方案落地核查后的四件收口（2026-09-22 · ADR-328 · 用户授权「全部处理」）**：
  - **① 回归修复：样本迁移漏迁 2 个消费者 ⇒ 两个工具已跑不动**（`4fa315a` 的副作用）。
    上一轮清公开树原文时把 `eval-gate.mjs` 的样本从**字面量**改为**运行期构建器** `buildSamples`，
    **9 个兄弟消费者都改了 import，漏了 2 个**：`_memory/audit/baseline-lab.mjs`（`indexOf('const SAMPLES = [')`
    ⇒ `anchor < 0` ⇒ **exit 2**）与 `baseline-lab-verify.mjs`（`.match(/const SAMPLES = …/)[1]` ⇒ **null[1] 崩**）。
    **两条盲区叠加导致全绿**：① 它们住 `_memory/`（gitignore，不在 `git ls-files` 面内）；
    ② 它们**未登记 `CHECKS`**（仓规则 6「未登记 = 等于没写」⇒ `npm test` 永不碰）。
    后果不止"工具坏了"——**那两件正是「TF-IDF+LR 基线 vs laya」那笔账的工具** ⇒ 账从"没做"变成"做不了"。
    **修**：两件改**同源 import**（与 9 个兄弟件同式）+ **登记进 `CHECKS`**（含 skip 语义）。
    **新门 `check-script-consumers`**（12 例 / 6 反例）守两条：① **无安全网的源码文本取数** ——
    读源码抠 `const X = ` 声明、且该文件**不在 `CHECKS` 表内**（产者改写法即静默失配，无一层会响）；
    ② **import 面漂移** —— 具名导入必须在产者导出集里真实存在（仓规则 6 原话此前只有人肉纪律）。
    ⚠ 判据① **必须带「不在 CHECKS 表内」**：本仓 `check-*/audit-*/test-*` 本就以"读源码做断言"为业（正当，
    每次 `npm test` 都跑）；**首版漏此条，实测 9 处里 7 处假红** —— 真病灶不是"读文本"，是「**读文本 + 没有安全网**」。
  - **② 评估样本 T1「同一类被双写两个键」+ B1 恒判多数类（两个真缺陷，同族「代理指标非判据」）**：
    库内实测 `MEMORY.md` 标签分布 `{lesson:361, 教训:102, 环境:93, tool:83, flow:41, env:34, 经验:3}` ——
    `lesson`/`教训`（同显「教训」）、`env`/`环境`（同显「环境」）是**同一个类的两种写法**
    （显示层 `tag-label.js:22` 早已记此撞名），而样本器把它们当成**互斥的两类**，后果三重：
    ① 选项集出现**同义重复**（一个答案占两个位置，稀释随机基线）；
    ② 多数类被人为劈半（`15/20 = 75.0%` 判过 `<90%` 判据，而**归一后 20/20 = 100%** 单类退化**本该判红**）；
    ③ **读数被掩盖**：B1「TF-IDF+LR 75.0%」实测预测分布 `{lesson:20}`（**恒判一个类**）⇒
    恰等于多数类基线、**零判别力**，却看起来像个像样的分数（同族：`switchSource 95.7% 恒真`）。
    **修**：取样**前置归一**（复用**既有**显示层映射 `tagLabelZh`，**不另造同义表**），归并后再封顶/出选项；
    `check-eval-samples` 增 **⑤′ 类数下限 ≥2**（1 类 ⇒ 零判别力）+ **⑤″ 选项无异义双写**（各含反例自证）。
    **均衡后 225 条实测揭示真相**：B1 T1 = **33.3%**（不是 75%，多数类基线 36%）。
  - **➂ 同一处的第三个缺陷：Laya 的类定义表停在旧词表 ⇒ 读数不可解读**。
    `baseline-lab.mjs` 的 `T1_DEFS` 硬编码 `原则/经验/路径/边界/认知`（= **AGENT.md** 五类），
    而样本已于 09-21 切到 **MEMORY.md 词表**（归一后 `教训/环境/工具/流程/经验`）⇒ **覆盖面 1/5**，
    四个类在 criteria 里只拿到「键名本身」当定义 ⇒ **Laya T1 实测 0.0%（20/20 全判「经验」）**。
    **那不是"模型不行"，是喂进去的类定义属于另一个词表**（同族：`formatConstraintLine`「模型不知道有上限」）。
    **修**：补齐定义表 + 加**覆盖门** `assertT1DefsCover`（定义须覆盖全部实际选项，且**定义 ≠ 键名本身**，
    不足即 **exit 2** ⇒ 读数不可解读，而不是照常跑出一个好看的错数）。
    ⚠ 试过"从注册表 `carriers.tags[].note` 派生"⇒ **实为溯源台账**（"医嘱脚本 AGENT 侧既有标签…一并登记"），
    拿它当类定义是**假修**，故如实保留局部语义表 + 覆盖门。修后 Laya T1 **0.0% → 18.7%**。
  - **④ 槽位预算三键：「接面板」只接到 HTTP，没接到 UI（半截落地）**。
    `closure-plan` 册二交付物写「接上 `/set` + `/config` + **契约表**」，实测**只通服务面**：
    `POST /set` 三键各 **200** · 越界 **400** · `/config#global` 三键可读；但 **`src-client/` 对三键零引用**
    （`git grep injectBudgetChars -- src-client` ⇒ 无命中），而注入页 13 个设置**全是硬编码键**、无通用编辑器
    ⇒ **旋钮从"不存在"变成"只有 curl 能拧"**。另：契约表里**没有三键条目**（落在通用 `/set` 之下）
    ⇒ `check-panel-contract`「三向一致」对它们是**空过**，不是"验过"。
    **修**：① 注入页补 **2 个数值控件 + 1 个对象控件**（`injectLevelCaps` 逐档输入、整体 JSON 写回 ——
    对象键不得走 `parseInt`）；② `panel-contract.ts` 增 **`SET_SCALAR_KEYS`（30 键）** 并登记进契约；
    ③ `check-budget-override` 增 **⑦a 有控件 / ⑦b 在契约里 / ⑦c 契约 ⟷ 源码 `allowed` 逐键一致**（30 ⟷ 30）。
    ⚠ ⑦c 是必需的：**本件初版手抄即漏 6 键**（`injection.*` 容量门与 `embedding.dimension`），当场被纠出。
    面板视图基线随之**刻意更新**（3 项新增，diff 逐条核对后固化）。
  - **⑤ D5：收益信号换源 + 三态化（用户拍板项）**。`topicEcho`（词面代理）真机 **true = 0/3119 恒 false**
    ⇒ 本链只递增不归零 ⇒ 换向恒真（曾 94.6%）。但**朴素换源有害**：真实动作 `nextTools` 实测
    **4985/5931 = 84.0% 非空**（近恒定）⇒ 单用它会把换向出口**变哑**。仓内已知的**真收益判据**在
    `deepsleep-run#judgeYieldRounds`（离线、按轮、子代理语义判），**不适合每步**。
    ⇒ 落**三态 + 显式留痕**（不把弱信号包装成强判据）：`acted=true` ⇒ 归零（还在推进，不该换向）；
    `acted=false` ⇒ 递增（停滞，才是该换向的形态）；**两路都取不到 ⇒ 计数不动**
    （宁可不动，也不据"无证据"换向 —— 与 J5/U3 既有保守口径「未判 ⇒ 不换向」同旨）。
    新增 `recall-yield#foldZeroGain`（**单一实现**，纯函数）+ 审计落 `acted`/`yieldSignal`
    ⇒ 「用了哪个信号」可事后分辨（输入量可见化）。`planStepJudgement` 改吃**已折减**的 `zeroGain`；
    旧入口 `nextZeroGain` **保留为薄封装**（旧调用方零迁移，行为逐字不变）。
    `test-recall-yield` 增 **⑥ 组 15 条**（含 2 组反例自证：① 无证据当作"没动作"⇒ 会据缺失误换向；② 只看词面 ⇒ 必然换向）。
  - **⑥ 清理**：`git reflog expire --expire=now --all && git gc --prune=now` 清掉悬空 `v0.3.1` tag 对象
    （本地/远端 tags 皆 0、无 Release；对象已先备份到 `~/.dsh/backups/tag-cleanup-20260922/`）。
    `git fsck --full` 由报 `dangling tag` 转为**静默**；仓完好（158 提交 · 928 文件 · 密钥扫描 0 命中）。
...
- **🔴 ACT-355 · ADR-358：棘轮「可再生额度」**从根因消灭** + 扫描面守卫单一实现收口完成（2026-09-23 · 用户「全部做，从根源解决问题」授权）**：
  ...
  - **① 根因＝`--rebase` 无条件抬 `base`，与件头注释「棘轮只许降」自相矛盾** ⇒ **`--rebase` 缺省只许收紧**
    （`base := min(旧, 实测)`；`hwm := max(旧, 实测)` 仅作观测），**抬 base 须显式 `--raise`**，且报告**逐条大声打印**
    「抬了哪几条、多少行」（R3③ 知情要求）。⇒ 上限**恒为 `base+15`，rebase 再也不能换取新空间**。
    实测（会审指出的序列）：`base=578` + 实测 590 ⇒ 缺省 rebase 后 **`{base:578, hwm:590}`（base 不动）**；
    `--raise` 才 `{590,590}` 并打印「⚠ 抬高了 N 条基线」。**自证含成对反例**（缺省与 `--raise` 结果**必不同**，防恒真）。
  - **② 扫描面守卫单一实现收口完成**：`lib-scan-scope.mjs` 的 `IS_MAIN` 由**字符串比较**改为 **`realpathSync` 归一**
    —— 修前经 junction 实测**输出 0 行 / exit 0**（判红能力整体消失，比假绿更隐蔽），修后 **23 行**（= 真实路径）。
    `audit-architecture` **成为第 5 个消费者**：目录缺席由 **ENOENT 抛栈**改为**可解释判因**「扫描面不可用」+ exit 1
    （正撞库明文要求的「读不出 ≠ 不存在」）。⚠ 据实留档：**递归是它的语义**，只借库的三态判因、**不把判定搬进库**
    （会审 steelman 实测：`accept` 只收条目名，搬进去会把已修好的判据**改回假绿**）。
  - **③ 三态不得混同**：`check-i18n-registered` 原把**部署形态缺席**与**断链**混用同一个 `exit 3` ⇒ 现分开：
    目录缺席 ⇒ skip(3)；**目录可读却 0 份词表（断链）⇒ 判红(1)**。实测三态分流 A/B/C 各不相同。
  - **④ 制度面**：`audit-architecture` 的 `T` 补**收紧提示**（不判红）；`thresholds.note` **去掉硬编码条数**
    （原写「13 项里 12 项」，实测 **21 条 / 13 条未预注册**）+ 订正读口（`check-threshold-control` 已取代**已删**的
    `check-threshold-consumed`）+ 重生成投影（`check-criteria` PASS）。⚠ **明确不做**：把工程常量登记进
    `thresholds.entries`（会审三席一致否：明排除、登记即红或造假读口、`deepEq` 与「只许收紧」方向相反、**0 消费点**）。
  - **⑤ 顺带收口**：`--print`/`--rebase`/自证全部纳入 `IS_MAIN` 守卫（此前被 import 时**会改写被 import 件的 FREEZE**）；
    `lib-scan-scope.mjs` **从未被 git 跟踪**却已被 4 件 import 且登记 CHECKS ⇒ 本轮 **`git add` 入库**（状态 `A`），
    消除「部分提交致新克隆崩」风险（**未暂存任何他人文件**）；三份文档的陈旧读数订正为 585/578 并标注「引用前现跑」。
  - **判据**：`check-module-growth --selftest`（含 5 条根因成对反例 + 6 条 rebase/CRLF）· `audit-architecture --selftest`
    **8/8**（新增 ⑥⑥′⑥″ 目录缺席三例）· `lib-scan-scope --selftest` · `check-threshold-registry` PASS · `check-criteria` PASS。
  - **边界**：未抬任何基线；`--raise` 路径**本轮未对真源执行**（只在隔离副本验证）；全部改动可 `git revert`。

- **🟢 ACT-352：异常态塌缩成 0 的**病根在 `src/`**——治因，非再修症状（2026-09-23）**：
  - **判因（承接 ACT-349，追到病根）**：ACT-349 我在**读侧**（`essence-review-stability`）识别并排除异常纪元，
    那是**症状修复**。本轮追到**产出侧**：`src/deepsleep-run.ts` 的 `base` 以
    `approved: 0, rejected: 0` 为初值，而**两条异常分支**（`全部批次无法解析` / `调用异常`）**直接返回 base**
    ⇒ **三种语义塌缩成同一个数**：① 「判了全否」（**真值** 0）② 「全部批次无法解析」（**异常**）③ 「调用异常」（**异常**）。
    这正是本仓「失败不可观测」族的**病根**（同族先例：`vec.ts` 六类失败塌缩成 `null`、`sleep-selfcheck` 把"未跑"记成 ok）。
  - **修法（治因）**：新增**显式 `error` 字段**（`undefined` = 判定完成；`'unparsable'|'threw'` = 未完成及原因）。
    · **不动既有数值字段**（`approved` 仍为 0，向后兼容）；
    · **两条异常分支**均设 `error`；**同步版与批量版同改**（`:233` / `:260` 两条路径，保持语义一致）；
    · **落审计**（`releaseSemanticError` 字段）⇒ **历史行从此可回溯**，消费侧不必再靠 `note` 文本匹配
      （**文本匹配是代理指标**，本仓明令慎用）。
  - **消费侧同步改判据优先级**（`essence-review-stability.mjs`）：① **首选显式字段** →
    ② 文本匹配（**仅回退给历史行**，它们写在字段存在之前）→ ③ 末位兜底。
    **同时收敛一处重复实现**：`isEpochError` 原在 `--decompose` 与 `--from-ledger` **各写一份**
    （同语义两处重实现）⇒ 现为**模块级唯一实现**。
  - **验收**：`npm run typecheck` **零错**；`build:host` **exit 0**（改动已落 `lib/deepsleep-run.js`
    的 `releaseSemanticError` / `'unparsable'`）；两路径复核读数**不变**（仍「排除 1 个异常纪元 · 极差 16.3pp」）
    ⇒ **行为等价、语义更精确**。
  - ⚠ **`build:client` 未过，与本次改动无关**：报错在 `src-client/i18n-dict-pane-run.js:23`（**其他会话在途**，
    该文件 `git status` 为 `M`）—— **我对 `src-client/` 零改动**；`build:host`（我的改动所在侧）**成功**。

- **🟢 ACT-349：异常纪元被计成「通过率 0%」——把极差虚增一倍（2026-09-23 · **真缺陷，非补丁**）**：
  - **发现路径**：盘点剩余任务时跑 `essence-review-stability --from-ledger`，得「通过率极差 **37.2pp**（阈值 10pp）
    ⇒ 未收敛」。核对读数时发现**账不平**：末轮 `候选 113 · 通过 0 · 否 0 · 未判 0`——**113 ≠ 0+0+0**。
  - **根因（实测到原文）**：该轮的 `releaseSemanticNote` 明写
    「**全部批次无法解析（`stop=error`×12 · 输出 0/0/0/… 字符）**」⇒
    **`approved=0` 不是"模型判了全否"，而是"12 批全失败"**。
    而旧实现直接算 `approved / candidates` ⇒ **把异常退化成 0%**，与"真 0 通过"**不可分辨**
    （「失败不可观测」族标准形态；同族先例：`vec.ts` 把六类失败塌缩成 `null`、`sleep-selfcheck` 把"未跑"记成 ok）。
    **后果（量化）**：该行把极差从真值 **16.3pp 虚增到 37.2pp** —— **判读被污染了一倍**。
  - **修法（治因）**：新增 `isEpochError()` 判据（`note` 含 `全部批次无法解析|stop=error`，
    **或** `候选>0 而 approved+rejected==0`）⇒ 异常纪元**归 error 档、不进通过率序列**；
    但**单独列出**（`⚠ 排除 N 个异常纪元` + 逐条原因），**不静默丢弃**。
    **两条路径同修**（`--decompose` 与 `--from-ledger`——首版只修了前者，后者是我最初跑的那条）。
  - **验收（实测）**：修前 `极差 37.2pp` ⇒ 修后 **`极差 16.3pp`**，且打印「排除 1 个异常纪元」及原因。
    **结论不变**（仍"未收敛 ⇒ 不得接线自动执行"），但**依据现在是干净的**：
    波动区间 20.9%~37.2%，轮1→2 模型波动占 **94%**、轮2→3 占 **87%**（候选变化只能解释 ≤2.1pp）。
  - **附带（同轮）**：该件**此前未登记 CHECKS**（属 obs 席报的「有判据无消费者」族），
    而它是 **S-P5c 释放接线的前置门** ⇒ 已登记**只读路径** `--from-ledger`
    （⚠ **不登记裸跑**：不带开关时它会**触发 8 个并发子代理跑真深睡**，违反「检查件不得写真库」；实测单跑 >300s）。
    CHECKS **230 → 231**，`AGENTS.md` 同步。

- **🟢 非确定性门真根因修复：零命中提示行随运行时资源状态出现（2026-09-23 · ACT-348 · **真修，非补丁**）**：
  - **判因（用户当轮「不要打补丁」）**：`inject-baseline-diff` **全量跑偶发 FAIL / 单跑恒 PASS**。
    按纪律**连跑 6 次 + 12 次高频采样**辨明：骨架与原始哈希在时间上恒定 ⇒ **不是抖动、不是并发**。
  - **根因（**经两轮定位** · 第一轮是中途产物，据实留档）**：
    **① 中途发现的真缺陷（非根因，已单独修）**：`normalize` 的**空行不参与压缩** ⇒
      `热取前 N 条` 的 N 随库增长（**实测基线 7 → 当前 8**）⇒ 该段行数变 ⇒ 后续行错位 ⇒
      某些占位块**被空行分隔**（基线）而**没有**（当前）⇒ 同一结构算出不同骨架。
      修法：**先剔空行再压缩**（空行不承载结构）。
      **同时删掉一处恒真判据**：压缩循环首条写 `out[last] === m.replace(/\d+/g,'') && out[last] === m`，
      而 `m` **已是占位符**（不含数字）⇒ 前半**恒 true** ⇒ 该行**等价于**紧随其后的第二条（冗余）。
    **② 真根因（修①后仍 FAIL，据此深挖到分支源码，实测到字符号）**：新差异是**当前多一行**
      `（本步相关性零命中，以下为位置式基线）` —— 其产者 `src/relevance-supply.ts:216`：
      `if (!trace || !trace.zeroHit || !String(q||'').trim()) return ''`，
      而 `zeroHit` 只在 `rows.length === 0`（**桥 ∧ 词法都取不回**）时为真（同件 `:198-203`）。
      ⇒ **该行出现与否取决于"桥（向量 embedding）当时能否取回行"** = **运行时资源状态**
        （本机 Ollama 在忙 ⇒ 桥超时 ⇒ 零命中 ⇒ 多这一行），**不是注入结构变了**。
      **实测对照（同一 case、同一时刻）**：**单跑 10 次全不含**该行（字符恒 4395）；
        **全量跑时含**（~500–600s 期间其它门禁同样在打真机端点/占资源）⇒ **这就是"只在全量跑时红"的机制**。
  - **修法（治因，非加容错）**：该行**整行剔除**（不是换占位符 —— 基线与当前**一行有一行无**，
    换占位符仍差一行；而它的存在性由运行时资源状态决定，**不属结构骨架**）。
    与件内既有先例同类（「热取前 N 条」「成长横幅数字」）。
  - **验收（三向，缺一不算）**：① **独立自证件** `test-inject-baseline-normalize.mjs`（**原样提取** `normalize`，
    双向各锁，含 **A7/A8 反向对**：「数字变不报」但「**整行被删必报**」）**全绿** ⇒ **结构变仍必报**；
    ② 主路径**连跑 5 次全 PASS**；③ **全量门禁复跑**（见下）。
  - **文案修复（同轮 · 本轮立即发挥价值）**：行尾注原只看 `sameRaw` ⇒ **骨架不一致时仍打印「不算失败」**
    （读数与判定相反）；PASS 行原写「**逐字节一致**」而实际判的是**归一化骨架**。两者已改为**与判定同源**——
    **正是这次修复让全量失败时明说「骨架不一致 ⇒ 判失败」并给出差异位置，根因才得以定位**。

- **🟢 ~~非确定性门真根因修复：空行未参与归一~~（已被上方取代 · 留档勿引）**：
  - **判因（用户当轮「不要打补丁」）**：`inject-baseline-diff` **全量跑偶发 FAIL / 单跑恒 PASS**。
    按纪律**连跑 6 次 + 12 次高频采样**辨明：**骨架与原始哈希在时间上恒定** ⇒ **不是抖动、不是并发**。
  - **根因定位（实测到字符号，非推断）**：从**失败件的落盘日志**（`check-runner` 保存的失败输出）读到
    首个差异 **@ 字符 464**：**基线** `（<omission-note>）\n\n[环·<row>]`（占位行与环记录段间**有空行**）
    vs **当前** `（<omission-note>）\n[环·<row>]`（**无空行**）。
    **成因链**：`热取前 N 条` 的 **N 随库增长变**（**实测基线 7 → 当前 8**）⇒ 该段**实际行数**随之变
    ⇒ 之后的行**整体错位** ⇒ 某些占位块**恰好被空行分隔**（基线）而**没有**（当前）。
    而**空行不在占位行正则里 ⇒ 不被压缩** ⇒ **同一结构因条数不同而算出不同骨架** ⇒ 假红。
    ⚠ 这解释了「为何只在全量跑时」：全量跑耗时 ~500s，期间**其它会话仍在写库** ⇒ 库跨过某个条数阈值；
    单跑 ~0.3s 撞不上。
  - **修法（改的是归一的"缺失性质"，不是给门加减震）**：**先剔空行再压缩** ——
    空行**不承载结构**（段存在性由标题行表达），留着它只会把"活库条数"混进骨架。
    这与本件既定语义（头注「只守结构骨架，不守活的记忆内容」）**同向**。
  - **同时删掉一处恒真判据**：原压缩循环的第一条判断写
    `out[last] === m.replace(/\d+/g,'') && out[last] === m`，而 `m` **已是占位符**（不含数字）
    ⇒ `m.replace(...) === m` **恒为 true** ⇒ 该行**等价于**紧随其后的第二条（**冗余**，且让读者以为有两种压缩路径）。
  - **验收（三向，缺一不算）**：① **独立自证件** `test-inject-baseline-normalize.mjs`（**原样提取** `normalize`，
    双向各锁）**全绿** ⇒ **结构变仍必报**（未因剔空行而失明）；② 主路径**连跑 5 次全 PASS**；
    ③ **全量门禁复跑**（见下）。
  - **同轮（第一批）**：`inject-baseline-diff` 的**文案说谎**修复 —— 行尾注原只看 `sameRaw`，
    于是**骨架不一致时仍打印「不算失败」**（读数与判定相反）；PASS 行原写「**逐字节一致**」而实际判的是**归一化骨架**。
    两者均已改为**与判定同源**（与 `check-l0-conflict-wiring` ② 同族）。**该文案修复本轮立刻发挥价值**：
    全量失败时它明说「骨架不一致 ⇒ 判失败」并给出差异位置，**才使根因得以定位**。

- **🟢 非确定性门的真根因：文案说谎 + 活库×枚举归一的固有边界（2026-09-23 · ACT-348）**：
  - **判因（用户当轮「不要打补丁」）**：`inject-baseline-diff` 全量跑时偶发 FAIL、单跑恒 PASS
    （**同一天实测两次**：一跑 `1 fail`、复跑 `PASS`）。按本仓纪律**连跑 6 次辨确定性** ⇒ 全 PASS
    且骨架哈希恒定 `fea54a77f051`。**未动被守对象**（记忆库）。
  - **① 修的是「文案说谎」，不是给门加容错**（这是「不打补丁」的具体落实）：
    行尾注原只看 `sameRaw` ⇒ **骨架也不一致（`ok=false`）时仍打印**
    「**内容已变**：记忆库是活的，**不算失败**」，而它**同一条已计 `bad++` 并最终 exit 1**
    ⇒ **读数与判定相反**（"失败被文案淡化"，本仓「失败不可观测」的反向形态）。
    修法：**先看骨架判定**（它才是判据），只在 `ok===true && !sameRaw` 时才说"内容变、不算失败"。
    同理修 PASS 行：原写「N 个 case **逐字节一致**」，而本件判的是**归一化后的骨架**
    （若真逐字节，库一长就必红）⇒ 改为照实说「**结构骨架一致**（归一化后）」。
    **与 `check-l0-conflict-wiring` ② 同族**（已登记为「门禁文案须同源」教训）。
  - **② 把真根因显式化（**不**再加归一化规则 —— 那是打补丁）**：该件的归一化是**枚举式**的
    （覆盖已知 5 类活数据行）；**活库若长出新的活数据形态**（新段种/新横幅/新计数行），
    该行未被归一 ⇒ **报骨架不一致**。⇒ 已在**件头写明这是「活库 × 枚举归一」的固有边界，不是抖动**，
    并给出**正解**：发现新形态时**把该形态归一**（修判据），**不是**给门加减震/容错
    （那会把真骨架变一并静音 —— 本仓明令「未证实的容错等于掩盖真漂移」）。
    并指明**判别路径**：本件自带独立自证件（原样提取 `normalize`，双向各锁）——
    **若本件报红而自证件全绿** ⇒ 按上述正解补归一。
  - **③ ADR-350（同轮 · 计数闸真实未决项）**：`feature:可配置评估通道` 计数闸 **8/3**，
    核查其 8 次记录的实际内容后判定：那是**一个功能的完整施工序列**（方案档→面板化→模型选择器→
    样本重建→T2 接线→标签归一），**不是同一死胡同反复补丁** ⇒ 计数闸此处是**代理指标失真**。
    但它确有**真实未决项**：通道全部建好（含 `distill-write.ts:580` 真接线 + 86 条判据）而
    `evalEnabled` 长期 `false`，「**已建好却默认关闭**」这一态从未被显式裁决 ⇒ 就该未决项出决策
    （**明确 fail-closed 是有意终态** + 开启方式），**不为清零而补条**。

- **🟢 完成判据真机取证 + 治理侧三值归位（2026-09-23 · ACT-347）**：
  - **① 本仓自述的完成判据**（「认知环双通道在真实会话中可观测生效」）**真机已实证** ——
    推翻治理模型里长期挂着的「**缺真机样本**」（受 `mcl.ts` 6 字门槛限制）：
    · `phase:'judge'` **2799 行**，**首现 2026-09-21T20:06:47.403Z**（正是 `mcl.ts` 注释记的那条真机实证时间），持续到本次复验的 **08:11:11Z**；
    · **慢通道**（§0s 曾明确「B3/B4 仍待真机慢通道样本」）**5984 行**，其中 `judge` **1128** / `inject` **766**；
    · 环内换向出口 `mcl-switch` **10 行**；§0s 自带复验命令 `m3-live-measure.mjs` **全绿**，
      且其 **B1 成本判据成立**（`judge` 行带 `sim` = **0** ⇒ 每步轻判定**零嵌入**，未被护栏限制）。
    ⇒ **三值各说各话**被消除：`OPEN-ITEMS §0s` 说"已闭环" · `nav doing` 说"缺样本" · **真台账 2799 行** ⇒ 以真台账为准。
  - **② §0t 阈值校准：仪器已修、结论被噪声阻塞**（复验原文）：
    `mcl-calibrate` 稳定性自证显示 **T=0.58 带宽 12.5pp / T=0.59 带宽 13.8pp**，
    均**远大于**与预注册目标的距离（1.2pp）⇒ **不宜据此调值**（同 §0t 结论）。
    ⇒ 完成判据的第二半从此记为「**仪器已修、结论被噪声阻塞**」，而非"待办未做"。
  - **③ 治理侧归位（两处漂移，均为本轮引入/过期）**：
    · **STALE 1 → 0**：删掉 `check-scan-scope.mjs`（上一轮）后**未摘登记** ⇒ `nav_graph` 报 STALE。已修。
    · **主线向量过期**：`doing`/`exitCondition` 原写「缺真机样本」「影子阈值待校准」⇒ 已按真机读数改写（`nav_set`）。
    ⚠ 登记落点**必须带 `shoucang/` 前缀**（仓在 `D:\FF\shoucang`、治理根在 `D:\FF`）—— 漏写即假 STALE。
  - **④ `check-l0-conflict-wiring` ② 文案缺陷（真缺陷 · 判据正确但误导读者）**：
    原提示把「**已登记为不可达**」与「**未登记**」混在同一句（`—— 从不传：days30, sessions（须登记 … 或删除）`）——
    而二者**实测早已登记**在 `wiring.unreachableValues`。⇒ 读者会以为有未办事项。
    **修法**：按实际状态**分开措辞**（已登记 ⇒ 陈述事实；未登记 ⇒ 才是待办，且**只有它判红**）。
    **反例自证**（实测）：临时从注册表摘掉 `days30` ⇒ 精确报「⚠ **未传且未登记**：days30」并 **exit 1**；还原 ⇒ PASS。

- **🟢 收尾轮：门禁运行历史（默认关闭）+ 清理报告态冗余 + 撤回一次越棘轮（2026-09-23 · ACT-346）**：
  - **① 门禁运行历史 `gate-runs.jsonl`（**默认关闭** · fail-closed）**：治「**唯一现在完全做不成**」的判据 ——
    `check-runner` 此前**不留历史**（失败件输出落 `tmpdir()/shoucang-runner-<name>.log`，**固定名、下次即覆盖**
    ⇒ 本轮**已真实丢失一次 `eval-gate` 证据**）⇒ 两个缺口：**恒真门禁查不出**（"该件历史上是否从未红过"）
    与**低频抖动查不动**。落盘内容：逐件一行 `{at,file,code,verdict,durationMs}` + 一轮汇总行；失败件全文
    另存 `gate-logs/<时间戳>__<件名>.log`（**带时间戳不再覆盖**）。
    ⚠ **默认关闭的理由（据实留档：我这一步撞上了棘轮）**：`check-observability` 是**棘轮**
    （`BASELINE = 1`，DS4「单一事件源」，**只许收紧**）；新增一条 suite 域事件流使其变 **2 > 1**
    ⇒ **实测当场红**（`❌ 事件流 2 > 基线 1`，**门禁正确抓到我**）。
    而两种"绕过"都不可接受：**抬基线**＝越过冻结棘轮（**R3**）；**改归 `state` 族**＝**标签说谎**
    （本流是 append-only，不是"整体重写/key upsert"）。
    ⇒ 故**默认不落盘**（不产生未登记流、不动棘轮），能力以**显式开关**保留：设 `SHOUCANG_GATE_RUNS=1`
    即生效（**代码已就绪**，两态实测：默认 4→4 行不变 / 开关打开 4→6 行）。**留待用户拍板**是否新增此流并抬基线。
  - **② 删除 `check-scan-scope.mjs`（145 行 · 报告态冗余）**：它恒 `exit 0`（不判 FAIL）、16.5s/次，
    结论是「**判不了**」（五版判据全被证伪）。其价值（候选清单）已被 **B 方向的件内自证**取代
    ⇒ 属我引入的冗余，**自己清**。CHECKS **231 → 230**，`AGENTS.md` 同步。
  - **③ ⚠ 撤回一次我自己引入的治理漂移**：`nav_node` 登记 `lib-scan-scope.mjs` 时**漏了 `shoucang/` 前缀**
    （仓在 `D:\FF\shoucang`、治理根在 `D:\FF`）⇒ `nav_graph` 报 **STALE**。
    **这正是本仓既有未决承诺那条**（「nav 工件登记路径是否改成 `shoucang/` 前缀」）—— 实测确认**必须带前缀**。
    修正后 **STALE 0**。

- **🟢 B 方向：空扫判据下沉到件内自证（2026-09-23 · ACT-344 · 五版元门禁失败后的正解）**：
  - **判因（承接上一轮的五版失败）**：外围元门禁连试五版都判不了「某件是否真读 `src/`」——
    该事实**从进程外部不可判定**。用户选定 **B 方向**：**判据下沉到各件自己的 `--selftest`**
    （件内知识、可复现、不依赖外部推断）。
  - **① `lib-scan-scope.mjs` 增 `selftestScanGuard()`**：把「空扫自证」抽成**可复用模式**（防 23 份手写副本，
    那又是补丁）。件只声明 `probe(dir)`（**它自己的扫描函数**），helper 在隔离夹具上验三态
    （有内容 / 空 / 缺席）并**反证 probe 非恒真**（④ 用恒真 probe 作反例）。
  - **② 逐件实证后确定适用集 = 3 件，不是 44 件**（此前我按"文本含 src"列过 44/23 件清单，**那两版都是错的**）：
    真正会「空扫 ⇒ 结论恒真 ⇒ 误判通过」的**确切形态**只有 `existsSync(X) ? readdirSync(X)… : []`，
    全仓共 **4 件**具此形态，其中扫描面指向 `src/` 者 **3 件**：
    · `check-bridges` ✅ 已接原语 + 件内自证（4 例）
    · `check-observability` ✅ 已接原语（正向 PASS / 空扫反向**可解释判红 exit 1**）
    · `check-field-usage` ✅ 已加件内自证（4 例，用 `selftestScanGuard`）
    · `check-ledger-read` —— **判定为不需要**（该段自述「报告态」，空集不影响正确性；
      接上去会把报告态变判红 = **扩大**语义而非收口）⇒ 已在其源码就地写明判据
    · `check-test-self-restore` —— **不需要**（扫描面是 `scripts/` 登记表，非 `src/`）
  - **③ `audit-architecture` 与 `check-i18n-attr-literal` 经复核判定「无需改动」**：
    两者**已经**是 B 方向的正解形态（件内 `--selftest` 已含空扫断言：前者 5 例含"空目录必红 +
    真 src 不回归 + 有文件但 0 模块"，后者 6 例含三态守卫）。
    ⇒ **我先前把它们列为"待收口"是误判**，据实撤回（判据应看**件内是否自证**，而非"是否 import 了某个库"）。
  - **④ ⚠ 本件自身的两个新坑（据实留档）**：
    · **库文件的 selftest 被 import 时抢先执行**：`lib-scan-scope` 只判 `process.argv.includes('--selftest')`
      ⇒ 消费方 `check-bridges --selftest` 会**先跑库的自证并 exit**（症状：跑 A 看到 B 的输出）。
      修法：**必须判"是否被直接执行"**（`import.meta.url === argv[1]`）——
      **副作用代码必须分清"被 import"与"被直接执行"**。
    · **JSDoc 注释块混用了 `//` 与 ` * `** ⇒ 语法错误（`SyntaxError: Unexpected token '*'`）。

- **🟢 收口轮：扫描面语义收敛为单一实现（2026-09-23 · ACT-343 · 用户当轮「**注意要避免打补丁问题**」）**：
  - **判因（用户点破的正是它）**：同一道「空扫不得判绿」守卫，我此前**逐件打了四遍补丁**
    （`audit-architecture` / `check-bridges` / `check-field-usage` / `check-i18n-attr-literal`），措辞各异
    —— **那正是"打补丁"本身**，也是本仓反复记账的「同一语义多处重实现」。
  - **① 新增 `scripts/lib-scan-scope.mjs`（扫描面语义的单一实现）**：沿用仓内既有先例
    `lib-client-src.mjs`（前端源码统一读取口径）的范式 —— `lib-*.mjs` = 被多个 `check-*` 消费的单一事实源。
    收敛**四态**：`ok` / `empty`（空集不得判绿 ⇒ 判红）/ `skip`（按设计可缺 ⇒ exit 3，**不算通过**）/
    `error`（存在但读不出 ⇒ 判红）。
    ⚠ **核心不变量**：**「读不出」绝不与「不存在」合并** —— 修前多处 `catch { … exit(3) }` 把两者混为一谈，
    人读到的结论（"正常跳过"）与真实原因（"读不出来"）不一致（「失败不可观测」族）。
    自证 **21 例**（三态 × 边界 + accept 过滤后为空的判定 + 两条恒真反例 + guardScanScope 三态）。
  - **② 新增 `scripts/check-scan-scope.mjs`**：**但它的结论是「判不了」** ——
    目标是"找出谁真读 `src/`"，经**五版判据**全部被证伪（文本正则 → sentinel → 件数 → ENOTDIR → 输出证据，
      误报 4 件 ↔ 漏报至 1 件来回摆）。根因：**该事实从进程外部不可判定**，五版都在用**代理**
      （文本/退出码/输出措辞）替代它 —— 又一次「代理指标非判据」，而这次是我在同一件工具里连犯五遍。
    ⇒ 按本仓「报告态」处置（同 `check-installed-sync` / `check-test-self-restore` v4「推断不如登记」）：
      **恒 exit 0**，只给**可靠事实**（挖空假根下的退出码）+ 疑似提示，交人复核。
      **正解**是**把空扫判据写进各件自己的 `--selftest`**（范式：`check-bridges --selftest`）。
  - **③ `check-bridges` 迁移为消费原语**（等价验证：正向 PASS + 自证 9 例含空扫反向证伪）。
    ⚠ **收口只完成 1/4**：`audit-architecture`（7 处）/ `check-field-usage`（1 处）/
      `check-i18n-attr-literal`（1 处）**仍内联** —— 未迁移部分**如实记账**，不当作已完成。
  - **④ ⚠ 本轮我自己踩的坑（第三次同族，据实留档）**：
    · **守卫挡住了自证**：把守卫放在 `--selftest` 分支**之前** ⇒ `--selftest` 直接跑主路径并 exit
      （自证永远跑不到）。**位置纪律**：守卫要挡住**真实判据**，但**不得挡住自证**——
      与 `check-field-usage`「守卫须在一切 IO 之前」是同一纪律的两面。
    · **相对 import 在 tmp 里解析不了**：自证把变异副本写进 `mkdtemp()`，而副本 `import './lib-scan-scope.mjs'`
      ⇒ `ERR_MODULE_NOT_FOUND`（自证当场抓到）。改为"**同目录临时副本**"（跑完即删）。

- **🟢 全量检查 W1 余项（续）：系统性空扫扫描（全量而非抽样）+ 我自己的两处施工失误（2026-09-23 · ACT-341）**：
  - **① 把「抽样」升级为「全量证据」**：前一轮只抽查 5 件，本轮改用**系统性变异扫描**——
    在 tmp 假根（`src/` 与 `src-client/` 置空）里**逐一实跑全部 72 件 check-\***（30 件做目录扫描，已排除含真库写入者），
    分类为「假绿 / 诚实判红或崩溃 / 需精核」。**结论：`check-bridges` 是唯一真假绿**（上轮已修）；
    24 件空扫时非 0（判红或崩溃）；4 件经精核实为**误报**（扫描面非 `src/`、属报告态、或已有 exit 3 守卫）。
  - **② `check-i18n-attr-literal` 扫描面守卫加固**：修前 `catch (e) { console.log('无 src-client —— 跳过'); exit(3) }`
    —— **`e` 被吞且未分类** ⇒ 权限不足 / IO 错误 / 路径损坏**全都显示成"目录不存在"**
    （本仓「失败不可观测」族）。修后三向分明：**真不存在 ⇒ skip 3**（诚实跳过不算通过）·
    **存在但读不出 ⇒ 判红 1** · **存在但 0 个可扫件 ⇒ 判红 1**（空集不得判绿）。自证 6 例。
  - **③ ⚠ 我自己的两处失误（据实留档）**：
    · **用 PowerShell `Set-Content -Encoding UTF8` 改这个 .mjs** ⇒ **引入 BOM 且中文字符串 GBK 化**，
      文件语法损坏（首字节实测 `239,187,191`，正文 `锛氭棤`）——**正是本仓 `check-suite-config-read` 头注释记录过的同一手法**。
      已 `git checkout` 恢复，改用 **edit 工具**重做（首字节 `35,33,47` = `#!/`，中文完好）。
      **教训重申：文本改动一律用编辑工具，不用 shell 重定向**。
    · **自证段第一版放在文件末尾** ⇒ `process.exit(0)` 在其之前 ⇒ **自证永不可达**
      （实测 `--selftest` 直接跑了主路径并 exit 0）。已提到主路径之前。
      **教训：`"加了自证" ≠ "自证会跑"` —— 与同轮 `check-field-usage` 的「守卫位置」是同一课**。

- **🟢 全量检查 W1 余项：空扫假绿的第 2 例 + 守卫位置教训（2026-09-23 · ACT-341）**：
  - **① `check-bridges` 空扫判绿（受控证明）**：`:53` 的 `existsSync(SRC) ? … : []` 在 `src/` 缺席时给出**空扫描面**
    ⇒ 发布/消费两侧计数都是 0 ⇒ **恰好等于基线 0** ⇒ 打 `PASS`。危害：本件是「唯一 composition root」的**棘轮仪表**，
    仪表在扫描面失效那天仍报 `✅ 边数 0` ⇒ **"没有桥"与"没扫到"不可分辨**（本仓最忌的「代理指标非判据」）。
    实测（变异副本把 `SRC` 指向空目录）：修前 `PASS（未新增惰性桥边）` exit 0。
    **修法**：`SCAN_EMPTY` 断言置于**任何计数之前**（否则"0 边"会先被当成"= 基线 0"判绿）+ 空扫时给出判因与退回动作。
    **自证扩到 9 例**（原 6 例角色判定 + 新增 3 例：空扫必红 / 判因可解释 / **真 src 不回归**，用子进程变异副本实证）。
  - **② ⚠ 「守卫的位置比守卫本身更重要」（本轮实测教训）**：给 `check-field-usage` 补空扫守卫时，
    **首版只护了 `readdirSync(srcDir)`，而上一行的 `readFileSync(criteria.json)` 早已先崩** ——
    从空目录跑仍是 ENOENT 堆栈。⇒ 修正为**把守卫提到一切 IO 之前**。
    **教训固化**：加固扫描面时，必须确认**它之前没有任何 IO**（"加了守卫"≠"守卫生效"）。
    修后语义从「脚本坏了（堆栈）」变为「**扫描面不可用（可解释的红 + 退回动作）**」。
  - **排除项（实证后判定无需改，非遗漏）**：`check-deferred-queue`（扫描面全在 `mkdtemp` 合成夹具内，无外部面）·
    `check-observability` / `check-shared-fn`（空扫时**硬崩或 FAIL** ⇒ 能表达失败，非假绿）·
    `check-field-usage`（属①类"崩溃型"，已按②加固）。

- **🟢 全量检查 W1：激活 11 件「写了却从不运行」的自证 + 补记录库回滚点（2026-09-23 · ACT-338/340 · 承接 ADR-339 两层分离）**：
  - **① 激活 11 件 `--selftest`**：obs 席实测全仓 **38 件实现了 `--selftest`**，而 CHECKS 只登记 **25 件**
    ⇒ **13 件自证「写了但从不运行」**。依仓规规则 6「未登记 = 等于没写」，它们等于不存在。
    逐件实跑 2 次筛确定性 + 审副作用后**登记 11 件**（门禁 **214 → 225 pass**）：
    `check-claim-alignment` · `check-cue-space` · `check-i18n-keys` · `check-i18n-scan` · `check-journal-privacy`
    · `check-judge-kind` · `check-relevance-live` · `check-section-refs` · `effective-directions`
    · `inject-baseline-diff` · `test-i18n-render`。
    **剔除 2 件（有判因，非遗漏）**：`check-runner.mjs`（其 `--selftest` 会**递归跑全量门禁** ⇒ 登记即自引用且耗时翻倍）·
    `migrate-cue-keys.mjs`（**一次性迁移器**，写盘+真库 ⇒ 把"迁移动作"纳入常驻门禁是错的）。
  - **② 新增 `scripts/record-snapshot.mjs`（记录库时点快照）**：data 席实测 `.records/` **无可用回滚点**
    （唯一备份是 9 天前且**不含 `records.jsonl` 副本**）⇒ 方案档把「先补回滚点」列为 **W2（动真源）的硬前置**。
    本件落**仓外** `~/.dsh/backups/`（库是隐私区，仓内快照会随仓公开）；写入前算 sha256、写完逐一回验
    （沿用本仓「先留档再改、留档失败即拒改」纪律）；`--list` / `--verify` / `--rotate N`（缺省留 10 份）；
    **库不存在 ⇒ exit 3**（不凭空造快照）。判据 **9 条**，全在 `mkdtemp` 合成库上（**绝不碰真库**）。
    已实测建立首份快照并双向校验 sha256 一致。
  - **⚠ 施工中一次真实操作失误（据实留档，已修正并转化为判据）**：清理带缺陷的旧快照目录时用了
    PowerShell `Get-ChildItem -Filter "sc-records-snapshot-*."` —— **尾点被 `-Filter` 规范化** ⇒ 该模式
    **匹配了全部快照**，**误删 3 份**（含刚建立的那份）。**库本身未被触碰，快照随即重建并校验一致**。
    ⇒ 两条教训已固化成判据：① 该脚本 `--selftest` ⑤ 断言**目录名必须恰为 14 位纯数字**（防尾点复发）；
    ② **清理/删除一律走精确路径或脚本自身的轮转，不用 PowerShell `-Filter` 通配**（本仓记忆库早有
    「通配/名字推导的命中集执行前须显式列举」这条原则，本次是又一实例）。
  - **架构侧**：同轮 `nav_decide` 出 **ADR-339「两层分离」**（计数闸 3/3 触发的第一性原理决策）——
    **判据层**（章程/方案档：只排顺序与验收、不承载待办）与**执行层**（`check-runner` CHECKS + `OPEN-ITEMS`）
    分离；本锚点此后不接受"再来一册"式增量，新发现走「先落 OPEN-ITEMS → 按波次施工 → 证据回写」三步。

- **🟢 全量检查 W0：三项「会掩盖其他问题」的门禁缺陷（2026-09-23 · 圆桌会审六席交叉复核 · ACT-337）**：
  起因是圆桌会议「全量检查章程」的六席交叉复核——**结论不是推测，各自带回受控证伪**。
  本轮修其中三项（余下两项属 R3，列待拍板）。**每项自带反向证伪并登记 CHECKS**（依仓规「未登记 = 没写」）：
  - **① D-A 空扫平凡通过（最重）**：`audit-architecture --gate --dir <空目录>` 报 `0 模块` 却打
    「✅ 全部在阈值内」且 **exit 0** —— 空集上「无违规」是**恒真命题**。危害：它掩盖「判据不可证伪」**本身**，
    而这是章程排序的**第①层**；它同时是本件 `--dir` **变异副本反向证伪手法自身的守卫**（路径写错 ⇒ 扫不到 ⇒ 静默判绿）。
    **修法**：加 `EMPTY_SCAN` 非空断言（gate 下判红 exit 1；报告态打印「不携带信息」警示）。
    **判据** `audit-architecture --selftest`（5 条：空目录必红 / 判因可解释 / **真 src 不回归** /
    **「有文件但 0 模块」也必红** —— 防「目录非空即判绿」）。
  - **② D-D 扫描面漏 `.roundtable`**：外部进程在该目录写一个 **14B 带 BOM 的 json** 就使**仓级门禁**翻红
    （受控复现 0→1→0），而该目录 `git check-ignore` 命中 ⇒ **不入库的东西却在扫**。
    **修法**：`check-suite-config-read.mjs:122` 补 `.roundtable` —— 这是**对齐**同胞三件
    （`check-hardcode.mjs:58` / `check-public-content.mjs:118` / `check-script-consumers.mjs:70` **本就含它**），
    **非新增豁免**。另加 ⑤b（隔离区必须全排除）+ ⑤c 反例自证（防 ⑤b 恒真）。
    **边界**：治因（四份实现收成一份）**未做** —— 四者扫描面**按设计各不相同**
    （`check-hardcode` 有意扫 `lib/`，因本仓 `lib/` 随仓提交）⇒ 只能收口「必须排除的隔离区」这个**交集**，属 R3。
  - **③ D-C 对账门口径误标**：`check-record-parity` 把 `String.length`（UTF-16 码元数）打成 `B`（字节），
    真库差 **2.36×**（`notes/lessons.md` **313,849 字节** vs **132,922 字符**）⇒ 读数会让人误判"文件被截断"；
    且「逐字节一致」实为**字符串相等**（结论不错、标签说谎）。
    **修法**：双口径输出（字符数 + `statSync().size` 真字节）。
    ⚠ **字段名未改**：`mdBytes`/`storeBytes` 是**跨件契约**（另被 `record-sync.mjs` · `panel-arch.ts` → 面板 UI ·
    `ui-geo-regress.mjs` 真机几何夹具消费）⇒ 改名须连带改 3 件 + UI 契约，属 R3。
    **判据** `check-record-parity --selftest`（6 条，含**反例**：注入裸 `${….length}B` 必须判红 ⇒ 防恒真；
    与**边界**：正确双口径写法不得判红 ⇒ 防过严）。
  - **④ D-B skip 不得被误读为通过**：摘要形态 `PASS（… · 1 skip）` 会被人读成"全绿"，而本轮那 1 件 skip 恰是
    **G2 中文精度闸** `eval-gate`（`exit=3`）——该面**从未被验过**却混在同一句 PASS 里。
    **修法**：终端与 JSON **双面**列名 —— 终端新增「⏭ SKIP 项（…该面未验，不得计入通过）：<件名>」段
    （与 XFAIL 同待遇，件名恒列）；JSON 新增 `skip[]` 与 **`verified`** 布尔（`exit 0` 不再等于"全部已验证"）。
    **退出码语义未触碰**（skip 仍不判失败）——只修"读的人会怎么理解"。
    自证：真建探针 + 临时插 CHECKS 首行 ⇒ 断言渲染/摘要/列名三段 + **还原逐字节一致**（6 pass）。
  - **过程留档（据实）**：施工中自证**两次假红**并修掉 —— D-C 的 ③ 判据首版扫全文 ⇒ 命中**自己的订正注释**；
    改剥注释后仍红 ⇒ 因**断言名与反例样本本身就是被禁文本**（自指）⇒ 改为**只扫 `console.log` 实参**（真输出面）。
    **教训：禁止某文本形态的自证不能扫全文件**。
  - **未做（列拍板）**：D-E（证伪覆盖率棘轮 / 22 件缺空扫护栏 / 门禁运行历史落盘）属 R3；
    D-D 治因（四份扫描面收成一份）属 R3；D-C 字段改名属跨件契约变更。
    ⇒ 三项**均已同步 `AGENTS.md` 的 CHECKS 计数（213 → 215）**，`check-arch-sync` ⑦ 复跑转绿。
  - ⚠ **本轮非确定性门实例**：`inject-baseline-diff` 全量跑时红、**单跑 3 次全 ✅**（报「记忆库是活的」却仍 FAIL）
    —— 与 obs 席记录的「至少 2 件门禁非确定性」同型；**跨会话并发跑门禁会互相污染**（本轮已实测）。
...
- **🔴 棘轮劣化面会审「档一」：`--rebase` 不再销毁基线的历史依据 + 容差吃掉多少从此可见（2026-09-23 · ACT-351 · 5 席圆桌会审 + 用户具名授权「档一」）**：
  圆桌会审（5 席，orchestrated）对主持人三条主张独立取证，**推翻三处**（登记项 23→实为 **21**；「三个目标文件全在途」→ 实际**只有 `src/scheduler.ts`**；
  ③「因库不递归所以没接」→ **不可复现**，`src/`、`src-client/` 子目录**各 0**），并实证两条独
  立真缺陷。用户授权只做「档一」= 下列前两项（单文件、未碰墙、`git revert` 即净），其余落 `docs/OPEN-ITEMS.md §0v` 待授权。
  - **① `--rebase` 曾整段替换 FREEZE 表 ⇒ 静默吞掉历史注释**：原正则 `/(const FREEZE = \{)[\s\S]*?(\n\})/`
    实测把该段 **28 行 → 4 行**，`IR1 册五 E2` / `UI1/U2` 棘轮轨迹**全灭**——而那些注释是「基线**为什么是这个数**」
    的唯一依据 ⇒ 每次 rebase 等于销毁判据的来源。**改为逐行就地改值**：只重写值段，**行内与整行注释原样保留**。
    ⚠ 改造期**实测踩到两个坑并留档**：**(a) CRLF** —— JS 正则 `.` **不匹配 `\r`**，`(.*)$` 使带 `\r` 的条目行**整体不匹配**
    ⇒ 既不改写也不记 `seen` ⇒ rebase 后**条目重复（3→6）**，且**静默**；**(b) LF 夹具掩盖了它** —— 首版自证只用 LF
    合成样本、全绿，真跑 CRLF 才暴露 ⇒ 补 **CRLF 用例**后先红。这两条正是「**夹具绿 ≠ 真数据绿**」。
  - **② 容差被静默吃掉、且永不归还 ⇒ FREEZE 改 `{ base, hwm }` 双水位**：原设计只有 `base` 一个数，`base + SLACK(15)`
    是「**每次 rebase 重置回满**」的可再生额度。实测真形：`851215f` 把 styles 730→726，`467206f` 又把它长到 729
    而该提交**只改 body.js 一行基线**（-626/+613）⇒ **+3 自 2026-09-17 永久固化在静默区**；`scheduler.ts` 同形状是
    **第二次**（`9b8bdb2`/`17db03b`/`41fd7f8` 跨 3 提交/3 天实测均 617 / 基线 611）。
    ⇒ 新增 `hwm` = **历史最高实测水位**（与 `base` 同样**只许降**）；`eaten = hwm − base` 即「**已吃掉、尚未归还**」的额度，
    **每次运行逐行打印**。⚠ **判红语义零变更**（改造前红的仍红、绿的仍绿；`hwm` **只产账、不产红**）；
    兼容**裸数字**旧形态（等价 `{base:N,hwm:N}`）。当前读数：**已吃额度合计 12 行**（styles 3 / body 2 / scheduler 7）。
  - **③ 顺带收口一处既有假绿（非本轮引入）**：`check-module-growth.mjs` 顶层含 `process.exit`，而被
    `check-pane-sections.mjs:29` **直接 import** ⇒ 被 import 时**抢先执行并退出** ⇒
    `node scripts/check-pane-sections.mjs --selftest` 输出 36 行**全是本件的断言**，pane 自己的 **6 条断言一条没跑**
    （退出码来自本件）⇒ **假绿**。补 `IS_MAIN` 守卫（范式照 `lib-scan-scope.mjs:222-231`），修后 pane 真跑自己的 6 条。
  - **④ 独立复核席（禁自产自审 · 不同席）抓出并已修的两处自造缺陷**：本轮改动由「独立验证」席复核，**抓到我引入的两个真缺陷**——
    **(a) `IS_MAIN` 在别名路径下整件静默 `exit 0`**：首版照抄先例的**字符串**比较，经 junction（`<仓根上级>/sc-jt → <仓根>`）
    实测判 false ⇒ 文件体**一行不执行**（输出 0 行 / exit 0）⇒ **判红能力整体消失，比旧假绿更隐蔽**。修为 `realpathSync()` 归一
    （实测可穿透 junction：修后 17 行 = 真实路径）。⚠ 该模式**继承自 `lib-scan-scope` 先例**（同 junction 下它亦 0 行），先例另行登记。
    **(b) 自证驱动副本而非真实现**：首版把 rebase 逻辑**复制**进 `--selftest`，复核席对**真** `--rebase` 注入三种缺陷，
    自证**全部逃逸**（验的是副本）。修为抽 `rewriteFreezeBlock` 为**导出单一实现**、真路径与自证共用；
    变异真实现 ⇒ 三处**语义有效**变异全部抓住（修前全部逃逸）。
  - **判据**：`check-module-growth --selftest`（原 13 条 → **新增 9 条**：双水位 3 + 账 4 + rebase 注释留存 4 + CRLF 2，
    含「旧实现必吞注释」反例）+ 变异测试（M-G/M-H2/M-I2/M-J 全部红）；
    `check-runner` 中本轮相关件全绿。⚠ 全量唯一 FAIL 是 `check-i18n-keys`，属**其他会话在途**的 `src-client/panes-capacity.js`
    容量门 UI 缺 EN 词条，与本轮改动**无交集**（该门只扫 `src-client/`，不读本轮任一文件）。
  - **边界自检**：本轮**未**抬任何基线、未跑 `--rebase` 改真源、未碰在途 `M` 文件（`check-module-growth.mjs` 施工前实测 clean）。
  - **残留（已登记 `docs/OPEN-ITEMS.md §0v` 序 9–12）**：`eaten` 欠账**不产生红**（「账可见 ≠ 会清偿」；可再生额度未被治，
    实测 `715绿→716红→rebase→730绿→731红` 逐字复现，累计吃 31 行而判红从不提前）· `--print`/`--rebase` 仍在 import 期执行（今不可达）·
