...
- **🔴 公开树真实记忆库原文：全域清理（**45 处 / 6 件**）—— 上一条只清了 `scripts/`，而重灾面根本不在那儿（2026-09-21 续）**：
  - **为什么会漏**：上一条的 `check-public-content` 首版**只扫 `scripts/`**且**只扫已跟踪文件**。本轮自查抓到**两个盲区**：
    ...
    `deliverables/`（38）· `docs/`（2）· `CHANGELOG.md`（1）· 其余（1），而 **`scripts/` 恰好 0 处**；
    ⚠ 这正与 `AGENTS.md` 记载的历史泄露成因一致（"把 `/inject/preview` 输出复制进**文档**"）——**重灾面从来是文档面**。
    ② **只扫 `git ls-files` ⇒ 门漏了自己**：新写的 `check-public-content.mjs` 尚未 `git add`，
    而它自身就带一行真库原文当反例探针 ⇒ 首版实跑"PASS"**把它自己漏掉了**。
  - **修**：扫描面扩为**整棵公开树**（已跟踪 + `--others --exclude-standard`，排除 `_memory/`/`devref/`/`lib/`/`.roundtable/`）；
    自证语料改为**合成基准**（本件自身零真库内容，并加一条断言钉住这一点）。
  - **清理（每处均按"正文脱敏、保结构"处置）**：`deliverables/inject-parity-2026-09-14.md` **38 处**
    （两份**完整注入面 dump**：A 48 行 / B 49 行 → 改为**保形占位**：行数、`- ` 前缀有无、段标题位置**全部不变**
    ⇒ §0/§1 依赖的结构事实**仍可逐条复核**：实测 A 带前缀 **41** / B **12**，与 §1 第 3 条所载**完全一致**）·
    `deliverables/engineering-assurance/inject-recall-chain-2026-09-11.md` **2 处** · `docs/sleep-granularity-plan-2026-09-15.md` **2 处** ·
    `CHANGELOG.md` **1 处** · `deliverables/tessa-test-plan.md` **1 处** —— 明细行一律换成**形态描述**（保留"标签/指针/来源"三类结构信息，去掉正文）。
  - **判据**：`check-public-content` 实跑 **45 处 → 0 处**；`check-public-tree` 仍 PASS；`check-runner` **201 pass / 0 xfail / 1 skip（exit 0）**。
  - ⚠ **残余风险（如实记）**：这些文件**已在远端公开过**，本次只清**当前版本**；历史面按 `docs/PUBLISH-POLICY.md` §5 的既定处置
    （2026-09-15 已做过一次历史重置）**不重复重置**，且重置**不保证消除已有副本/抓取**。
- **🔴 归因取样窗口被 M3a 的高频行挤空 —— 修（2026-09-21 · ADR-324 · 用户授权「全部自己决策」）**：M3a（ADR-303）新增的 `phase:'judge'` 行**不带 `missReason`**，而 J3/U1 归因取样按「末 **60 条 `mcl-step`**」倒扫 ⇒ `judge` 占切点后总量的 **77.7%** ⇒ **窗口内可用样本 = 0**（门槛 30）⇒ 归因判定**再次结构性不可达**（与已修的「上限 20 < 门槛 30」同型，成因相反）。
  · **实测（复刻运行时取样，非采信落账值）**：按现取法倒取 60 条 ⇒ **60 条全是 `judge`**、`samplesFromMclRows(...) = 0`；且用同一复刻函数回测各历史深睡时刻，与当时落账值**同量级**（21/14/19/21/25/20/12/20 vs 16/19/22/24/21/13/16/18）⇒ **复刻口径正确**。
  · **修法（判据修正，非调参）**：窗口**不按"产者是谁 / 行数多少"截断，而按消费谓词收满即停** —— 新增 `recall-diagnosis#isAttributionRow`（消费谓词单一实现）+ `#takeAttributionScan`（窗口唯一实现，带扫描上限 4000 行防坏档无界扫描）；`deepsleep-run` 取样改为调用它。⚠ 我最初拟的是"按 `phase === 'inject'` 过滤后再截 60 条"——**实测只得 27 条仍 < 门槛**（窗口内 ok:未命中 ≈ 33:27）⇒ **按产者名过滤是脆弱解（产者会变），按消费谓词收满是收敛解**。
  · **判据**：`check-attribution-samples` 扩为 **③ 结构（窗口走唯一实现 / 谓词含消费谓词 / 无行数截断 / 谓词同源 / **返回已解析行**）+ ④ 行为（真跑 `lib/`：100 条噪音打头仍收满已解析样本；反例：谓词退化 ⇒ 收回噪音）**。**先红实证**：两条变异各令 exit 1，还原后**字节级一致**。修复后端到端实测样本 **0 → 30**。
  · ⚠ **同时修掉我自己在该修复里犯的一个错**：首版 `takeAttributionScan` 把 parse 与 pred 合成一个（返回**原始行**）而下游 `samplesFromMclRows` 要**对象** ⇒ **样本恒 0，而当时上面四条结构断言全绿** ⇒ 补 ④ 行为级断言（"门只看接线了没、没看过出来的东西对不对"）。
- **🔴 公开树内真实记忆库原文清理 + 语义级补门（2026-09-21 · 隐私红线既成事实）**：`check-public-tree` 按**路径/模式**扫（邮箱/盘符/用户名…），**不查正文语义** ⇒ 「把记忆库原文复制进 `scripts/`」它**结构性看不见**。
  · **实测两批共 21 处**：① `eval-gate.mjs` 内嵌 **20 条**样本，其中 **16 条**与运行库画像文件**逐字重合**；② 另有 **5 件**（`test-granularity-converge` / `test-inject-dedup` / `test-process-supply` / `test-targets` / `threshold-scan`）把库内索引行原文当夹具。
  · **成因单一**：都是「**复制真库内容当夹具/样本**」—— 而夹具只需**形状**（有标签/有指针/两行不同），**不需要真文本**。⇒ 两批全部换**合成占位**，相关测试仍全绿（判据强度不变）。
  · **`eval-gate` 迁至运行期取材**：改调 `eval-samples.mjs#buildSamples`（构建器内**零记忆内容**，产物落已 gitignore 的 `_memory/audit/`），样本量缺省 **8/任务**（原 90/任务 ⇒ 540 次调用超时限）。⚠ **准确率不可与旧值比**：旧 20 条样本实测三处结构性失效（字面泄漏 8/20 · 镜像对 8/20 · 真独立仅 4）⇒ 其 95% **不可解读**；迁移后诚实读数 **类型化 37.5% vs 基线 44.4%**（`--n 8`）。
  · **新门 `check-public-content`**（登记 `CHECKS` **200 → 202**）：① 与库文**逐字重合 = 0**（判据是"在库文里真出现"，**不是**"长得像索引行"——后者全是合成夹具，会误报）② 反例自证**实跑**（塞真库整行 ⇒ 必命中）③ 库不可达 ⇒ **exit 3 skip，不算通过**。
- **面板回落值 ⟷ 运行态缺省：消「同一开关两套口径」（2026-09-21 · S4）**：`/config` 的 `global` 回落原为**硬编码字面量**，而运行时缺省来自**注册表**（`scheduler.ts` 的 zod 写 `default(SURFACE.…)`）⇒ 两套口径各自演化**必然打架**。
  · **实测**：`mclFamiliarThreshold` 回落 **0.65** vs 注册表 **0.58** · `injectProfileRows` 回落 **3** vs `carriers.profile` **6** · `scoreWeights` 回落 **'legacy'** vs 运行态 **'v2'** ⇒ 当 `scheduler.json` **无该键**时 **面板显示 A、运行时按 B**（"假可控"）。真机验证：修后面板报 `injectProfileRows=6` / `scoreWeights=v2`，与运行态一致。
  · **修**：回落一律读 `SURFACE.*` / `SCORE.*`（同 `scheduler.ts` 同源）。⚠ **注册表未声明的 `activity*` 四项如实保留字面量** —— 我一度改成 `SURFACE.activity?.x`，**那是引用一个不存在的字段**（"为统一而造的假引用"，比字面量更坏），已撤回并在注释里如实标注它是**已知的第二处口径**。
  · **新门 `check-panel-fallback`**（登记 `CHECKS`）：① 无陈旧字面量 ② 真引用注册表路径 ③ **无假引用**（用实际 import 的 SURFACE 做键存在性检查，不靠文本猜）；反例自证 4 例含 2 反例。
  · **探针假读数两处**（`_memory/audit/j3u1-rate.mjs`）：① `Number(r.attributionAgrees)` 把 `null`（**未判**）算成 **0** ⇒ 打出「累计 0/625 = 0.0%」的**假结论**（曾据此误判"字段没被正确写"）⇒ 现按三态分账（**已判 1 行 / 未判 51 行**，未判**不进分母**；真值 0/30）② 切点不带时区 ⇒ JS 按本机 UTC+8 解析而台账是 UTC ⇒ 切点凭空前移 8 小时、造出**假红**（我复算 E-05 时踩到）⇒ 一律 ISO 字符串比较。
...
- **🔴 查出既有真缺陷：账实不符（1 条泄漏）· 未修 · 门禁保持红（2026-09-16）**：`test-usage-truth` B2 报
  ...
  泄漏行 = **一条 `[原则]` 索引行（正文含库内原文 ⇒ 此处不复述；格式 = 「标签 · 主题 · 概况 → 指针」）**；
...
- **注入槽位额度的**面板可调 + 覆盖链**（2026-09-21 · S3 · 用户授权「全部处理」）**：三册（`memClass`/`gate`/`budgetChars`）的**代码面**其实早已落地，真缺口是**面板调不到** —— `/set` 白名单 27 键里 `injection.budgetChars` / `injection.levelCaps` / `injection.situation.*` **零命中**。
  ...
  · **三条防线缺一不可**（本次实测两次）① 白名单（能调）② 映射（写得进）③ **类型**（写对）——`injectLevelCaps` 是**对象**，落进 `Number(value) || 0` 会**静默写 0 而 API 仍回 200**（本仓已记两次的坑的**第三种形态**）⇒ 显式 `JSON.parse` + 逐档范围校验，非法即 **400**。
  · **端到端实证（真机，非读码推断）**：`POST /set injectBudgetChars=1800` ⇒ 200 且落盘 ⇒ 面板读数 `source: registry → override` ⇒ **`/inject/stats` 的 `budgetTotal` 4000 → 1800 且 `overBudget: true`** ⇒ **本会话注入头部随即变为「本步受预算 1800 字符约束省略 96 行」**（旋钮作用到了我自己的注入面 —— 这是"真接入"最硬的证据）。回滚（删键）后三项全部回到 4000/registry/false。越界值与对象非法值**均 400**。
  · **无覆盖时行为逐值等价**：由 `check-budget-override` ④（真跑 `lib/` 的**行为**断言，非文本）+ 真机 `inject-baseline-diff` **3 case 逐字节一致**双守。
  · **结构合规**：就地补覆盖链曾把 `panel-shared.ts` 顶到 **868 > 冻结上限 846**，按该门**唯一出路**「新功能落新模块」抽为 `budget-override.ts` 的 `resolveSupplyBudget` ⇒ 回落 **837 ≤ 846**（抽出**同时是收紧**；抬基线属 R3，未做）。
  · **判据**：新件 `check-budget-override`（登记 `CHECKS` **202 → 204**，含 `--selftest` 4 例含 2 反例）守四组：消费侧真接线（且**无旧式直读残留**）· 范围单一事实源 · 对象类型防线 · **无覆盖时行为等价 + 覆盖真生效/越界夹取**；`check-carriers` **⑤b 扩为四类**（新增"对象"类，判据是"有显式 `JSON.parse(value)` 分支"而非看键名 —— 键名会骗人）；模块数 **103 → 104**（`AGENTS.md`/`ARCHITECTURE.md` 同步）。

- **「先判后 cap」重复判定修复：同一任务的重复捕获不再重置轮级态**（2026-09-21 · §13-P2 归因 · 真机两对同 sim 的 inject 行）
  · **现象**：E-05 修好「并发 check-then-act」之后，**真机仍出现同一轮判两次** —— `20:06:47.403 / .537`
    （同 sid·step·`sim=0.589`，**134ms** 差）与 `19:37:38.246 / .367` 同形；**两对都落在 step 1**、`mcl-ready` 只一条。
  · **根因（**不是**并发）**：本机宿主把 `user/message` **落在当轮 pre-step 之后**是常态（`mcl.ts` 原注释已记载该顺序）⇒
    `captureTaskText` 的「轮级重置」把**刚判定的 `channel` 清空** ⇒ 早判入口**再判一次**。这是**顺序性重复**
    （两次调用**不重叠**）⇒ E-05 的 `deciding` **并发闸对它无效** —— 这正是"修了之后仍见重复"的原因。
    同因另有两处副作用：重复捕获会推 `taskTextAt`（⇒ 下一步 `freshTurn` 再为真 ⇒ **又判一次**）与
    `ledger.newTask`（⇒ 增量去重窗口**白走一轮**）。
  · **修**：新增纯函数 `shouldResetTurn(prevText, nextText)`（**可机检**）—— **同文本 ⇒ 同一任务的重复捕获 ⇒ 直接返回**
    （不重置、不推 `taskTextAt`、不推 `newTask`）；换文本才是新任务（**既有语义不破**）。
    新增 trace 标记 **`cap-dup`** 使重复可观测（此前重复**无痕迹**）。
  · **判据**：`test-mcl-race.mjs` 增 **E10 + E11a-c**（先判后 cap 只判一次 + 判据真值表）⇒ **13 PASS**；
    `test-mcl.mjs` **56 PASS** 未回退。`check-installed-features` +1 标记（117→**118**）。
    ⚠ **诚实标注**：E10 **修复前也通过**（夹具里早判入口的 `void decideTurn` 被其 `.catch` 静默吞掉、复现不出真机时序）
    ⇒ **不作"先红"证据**；真机前向判据 = 修复后 `phase:'inject'` 的**同 sid·step·sim 且 ≤2s 对**应为 **0**。
- **M3「频率分离」落地（M3a 判定/注入解耦 + M3b 环内换向出口）**（2026-09-21 · 方案⟷验收见 `docs/evaluation-channel-plan.md` §11/§12）
  ...
  · **判据**：`test-mcl.mjs` 增 **M3-1..M3-7**（每步有判定行 ≥ 步数 / 注入行**恰 1** / 判定行必带原因 /
    换向**有且仅一行** / 触发于 `zeroGain≥2` / 出过换向不再引导 / 主链未断）**+ M3E-1/M3E-2**
    （**轻判定零嵌入**：起本地**计数服务器**当嵌入端点 ⇒ 完整判定那步 2 次请求、**后续 3 步合计 0 次**）
    ⇒ **56 PASS / 0 FAIL**；`test-mcl-race`（E-05）**9 PASS** 未回退。`check-installed-features` +1 标记（116→**117**）。
    ⚠ **B1 为什么不用真机数**：真机窗口内 `phase:'judge'` 行 **N=0**（重载后只有 2 字短指令，低于
    `mcl.ts:224` 的 `text.length < 6` 门槛 ⇒ 未进判定段）⇒ 真机**未取证**；而"代码看起来没调"只是**代理指标**。
    故把判据落在**可复现的请求计数**上（详见 `docs/OPEN-ITEMS.md` §0s）。
