...
**最后更新**：2026-09-21（round 12 · **三批**：
**① 评估通道 M1/M2/M4 收口五层**（提交 `25fe40c` + 推送 + pin 跟进；方案档新增 §10 施工进度总表）→
**② §0q MCL 幂等闸竞态**：真机取证**推翻**方案档旧记「E-05 真机未撞上」（实测 1859 组 / 1655 会话 / 3997 次多跑，**当日仍在发生**）
**并已修复**（`st.deciding`「同址检查 + 置位」· `test-mcl-race.mjs` 9 条**先红→后绿** · 提交 `c026391`）；同批**订正一条长期误标**：
`src/mcl.ts` **不是冻结件**（实测 `check-module-growth` 冻结名单仅 3 件、mcl.ts 代码 456 行 < 600）⇒ 改它**不涉棘轮** →
**③ §0r 影子库对账红**：已定位到**行号** —— `notes/*.md` 的增量 `appends` **零镜像调用**（`mirrorShadow` 全仓仅覆盖画像行与 MEMORY.md），
只靠批量 `mirrorAll` 兜 ⇒ 存在**固有窗口**；修法要写记忆库 `.records/` ⇒ 按 R2 ③ **不可自主施工，待授权**）
...
| **门为何判绿（根因）** | `produces()` 的 `asValue` / `inTernary` 两条正则**在类型声明行上就命中** —— 实测命中处（**均为原始文件行号**）：`src/criteria.ts:73`（即 `export type L0Conflict = 'none' \| 'coexist' \| 'supersede'`）与 `src/criteria.ts:117`（`const CONF = {…}` 行）。`:\s*[^\n;]{0,60}'coexist'[^\n;]{0,60}(?:\)\|,\|;)` 的 `:\s*` 把 **`type X =` 之后的联合类型**当成了「赋值右值」。⇒ **判据抓的是"字面量在类型里被声明"，恰是注释里被明确排除的那一类**（⚠ 取证脚本读的是**剥注释后**的源，故其内部行号（32/75）**小于**读者打开文件时看到的行号（73/117）；本节一律用**原始文件行号**） |
...
## 0t. 🟠 **阈值校准的读数不稳定：同一个 T，样本一换读数漂 5.8pp ⇒ 先修仪器，再谈调值**（2026-09-21 round 12 · **已取证 · 待拍板**）

> **性质**：这条问的不是"某个数该改成多少"，而是**判据层的稳定性**。本仓完成判据的第二半是
> 「影子阈值以**已去污染**样本完成校准」；本次复核发现：**校准读数本身对样本构成高度敏感**。
> **态**：**已取证**（读数皆真机实测）；**值不动**（阈值是 **2026-09-20 的用户裁决** ⇒ R3 ④）⇒ **收敛为一句待拍板**。

| 面 | 事实（逐条实测 · 2026-09-21） |
|---|---|
| **复检条件早已满足** | 登记制（`criteria.json#surface.mcl.familiarThreshold` 的 note）要求含 `preregisteredCriterion`+`samples`+`conclusion`+`recheck`；本值上次校准基于 **4217** 样本，而 `recall-diagnose` 的可归因样本现为 **5916**（远超复检阈 **N≥300**）⇒ **到达复检条件** |
| **今日细校准读数** | `node scripts/mcl-calibrate.mjs`：样本 14193 步 · 可归因 5916 · sim 分位 p50=0.548 / p90=0.641 / p99=0.730；扫阈 `0.58→33.5%` · **`0.60→26.9%`** · `0.62→16.1%` ⇒ **机检结论：应上调至 0.6**（预注册目标 27.5%，距 0.6pp） |
| **⚠ 真问题：同一 T，样本一换读数就漂** | **`T=0.58` 的过线率**：4217 样本时 **27.7%**（2026-09-20 据此拍板 0.58）→ **5916 样本时 33.5%**（**+5.8pp**）。而 `0.58→0.60` 的差只值 **6.6pp** ⇒ **"最优值"落在样本敏感性噪声带内**。⇒ 在会漂的读数上调旋钮，正是本仓反复剿的「**代理指标当判据**」 |
| **两判据"冲突"其实是分工（已澄清）** | `recall-diagnose` 给 `lower-threshold`（阈值层 1675=28.3% ≥ 召回层 358=6.1%），细校准给 `RAISE`。⚠ 但前者**自己写明**：「**本读数是对账基线，非调参依据**：实测与细校准方向相反（**一致率=0**）。改阈值以 `criteria.json#thresholds.mcl.familiarThreshold` 的细校准结论为准。」⇒ **不是判据漂移，是分工**（对账基线 vs 调参依据），且**权威已指定** |
| **⚠ 我上一句话的自我订正** | 我曾据校准器的提示说「瓶颈在**召回层**」——**错**。那 66% 的 `hit 为空` 是 **8297 条旧数据（无 `missReason` 字段）**，诊断器**明确拒绝推断**（"任何猜出来的分布都会误导后续决策"）；**可归因的 5916 里召回层只占 6.1%**，瓶颈在**阈值层 28.3%** |
| **我的判断（架构级）** | **先修仪器、再谈调值**：读数对样本构成敏感 ⇒ 应先让仪器**报出稳定性**（例如按「有命中」子集分层 + 给稳定带，而不是只给一个"最接近目标"的 T），否则每次复检都会得出一个**随样本漂**的"最优值" |
| **✅ 已收口（2026-09-21 · §13-P4）：结论落在噪声带 ⇒ **不调值**，并把稳定性做成常驻读数** | 给 `scripts/mcl-calibrate.mjs` 增**稳定性自证**块（全样本 / 前半 / 后半 / 最近 1500 四条子样本 + **带宽**），实测（5916 样本）：<br>`T=0.58` 33.5% / 26.5% / 40.4% / **58.3%** ⇒ **带宽 31.8pp**；`T=0.60` 26.9% / 20.9% / 33.0% / **53.2%** ⇒ **带宽 32.3pp**。<br>⇒ **带宽（≈32pp）远大于「最优 T」与目标 27.5% 的距离（0.6–6.6pp）** ⇒ **「应上调至 0.60」不是稳定结论**（最近样本上同一 T 就是 53.2%）。<br>⇒ **结论：保持 0.58 不动** —— 在噪声带上调值，正是本仓「代理指标当判据」的同型。仪器判据：`stable = 各子样本带宽 < 该 T 距目标距离`（现已内置并在输出中明示）。 |
| **唯一待拍板的一句话（仍留）** | 若你仍要按"最接近预注册目标"上调 0.60，我照办（回滚：`/set mclFamiliarThreshold 0.58`，热生效）；但**我的判断是保持**，依据即上行的带宽读数。 |

**复验命令**：`node scripts/mcl-calibrate.mjs`（细校准 · **权威**）· `node scripts/recall-diagnose.mjs`（对账基线 · 非调参依据）· `node scripts/verify-open-items.mjs`（OPEN-1/2/3）

---

## 0s. 🟠 **「重载报 OK」≠「新码在跑」：M3 真机生效性未取证（N=0），活体 trace 显示判定链不达判定段**（2026-09-21 round 12 · **已登记，待取证**）

> **性质**：与 §0n/§0o/§0p/§0q **同族**（*机制自证 ≠ 判据*）—— 这次自证的是**五层清单的层③**：
> `dev_reload_package` 回报「热重载完成 · 重建 1 fiber · before/after 均 active」，**而那是代理指标**。
> **态**：**已登记、未定性** —— 现有证据**不足以下"新码没生效"的结论**（见下「为什么不下结论」），
> 只能确证「**尚未观测到生效**」。**在拿到证据前，本条只登记不判。**

| 面 | 事实（逐条实测 · 2026-09-21） |
|---|---|
| **主判据：新码特征零出现** | M3a 给 `compliance` 行新增了 `switchStop` 字段（**新码独有**）。全时段 `mcl-step` **13254 行**、其中 `compliance` **7007 行** ⇒ **带 `switchStop` 的 = 0**；每步判定行 `phase:'judge'` = **0 / 13254**。 |
| **行级时间线** | 本会话（sid `20faa603`）最后一条 `mcl-step` 在 **19:37:38Z**（2 条 `inject`）；M3 重载在 **~19:44Z** ⇒ **重载后 0 行**。 |
| **活体面板读数** | `GET /api/shoucang-panel/mcl/status`：当前纤维 **`steps:20`**（钩子**在收事件**）、`sysBlockCalls:20`，但 **`sessions:0 · tasks:0 · slow:0 · fast:0 · injected:0`**，`trace` 全为 `renSame` + **`pre0`**（`mcl.ts` 的"该步无任务文本 ⇒ 直接 return"）⇒ **判定链在"无任务文本"处提前返回，从未走到判定段**。 |
| **为什么不下结论（诚实标注）** | ⚠ trace 里的 sid 是 **`3abf5ac7`**（**不是本会话** `20faa603`）⇒ 那 20 次 `pre0` 属于**另一个会话**；而本会话在重载后**零样本**。⇒ **"重载后新码是否生效"目前无法判定**（N=0），须等新会话/新任务文本进入判定段后再读。 |
| **⚠ 我自己的探针在此处假绿（自查抓到并已修）** | 取证件 `_memory/audit/m3-live-measure.mjs` 第一版：`judge` 行 = 0 时判据 `judgeWithSim === 0` **平凡成立** ⇒ 打出「✅ 成本成立」。⇒ **已修为零样本显式判"未取证"**、不得给 ✅ —— 本仓 §0o 同型（**输入为零 ⇒ 跳过断言 ⇒ 通过**），亦是记忆里那条「**输入量须可见化 · N=0 显式记 0**」的同型。 |
| **层③ 的判据缺口（本条的架构价值）** | 「重建 1 fiber / active」是**机制自证**；行为级变更的层③判据应是「**新码特征出现在活体行里**」（本文例：`switchStop` 字段 或 `phase:'judge'` 行）。⇒ 建议把五层清单的层③ 由"重载过"细化为「**重载过 + 新码特征活体可见**」。 |
| **更简约的解释（代码事实 · 优先于下面那条）** | `mcl.ts:224`：`if (text.length < 6) return` —— **任务文本有 6 字门槛**。而重载后本会话收到的用户消息是「**继续**」（**2 字**）⇒ **根本不会被捕获** ⇒ 每步在 `pre0` 返回 ⇒ **零样本是预期结果**，**无需"投递断了"来解释**（trace 里没有 `cap` 也因此**不构成证据**）。⇒ 我上一版把"重载静默停摆"的嫌疑**估高了**，此处订正。 |
| **✅ 已闭环（2026-09-21 20:06–20:08Z · 真机）** | 一条约 42 字的指令进来后：**`phase:'judge'` 行 7 条**（sid `20faa603`，**step 1..7 各一条** ⇒ **判定频率 = 每步**），其中 step1 `note=decide-none`（注入分支出口）、step2..7 `note=fast`；**7 条全部不带 `sim`** ⇒ **每步判定零嵌入在真机亦成立**；面板 `trace` 由 `pre0` 转 **`pre`（任务文本 42 字）**、`sessions=1 / tasks=1`（此前 0/0）。⇒ **结论：「重载没坏」** —— 此前零样本的**唯一原因是短指令低于 `mcl.ts:224` 的 6 字门槛**；我把它当缺陷候选是**估高了**。 |
| **本条的真实价值（保留）** | 不是"无采样"，而是**两条判据设计教训**：① **`N=0` 是"未取证"、不是证据**（我据它一度怀疑"重载静默停摆"）；② **层③「重载过」是机制自证**，行为级变更应判「**新码特征活体可见**」——本条的判据即 `phase:'judge'` 行 与 trace 的 `pre`。 |
| **仍开口（转 §0q-侧）** | 慢通道样本（有材料注入 + 回引判定 + 换向出口）本回合未出现（本回合是 `fast` 通道）⇒ **B3/B4 仍待真机慢通道样本**，不在本条范围。 |

**复验命令**：`node _memory/audit/m3-live-measure.mjs`（看 `judge` 行数与 `switchStop` 出现与否）·
读面板 `/api/shoucang-panel/mcl/status`（看 `trace` 是否出现 `cap`/`pre`；恒为 `pre0` 即**未达判定段**）

---

## 0r. 🔴 **影子库对账红：`notes/*.md` 的增量写入**从不镜像** —— 只有批量 `mirrorAll` 兜，中间是固有窗口**（2026-09-21 round 12 · **已定位到行号，未修**）
...
## 0q. 🔴 **MCL 幂等闸竞态：「窗口存在但真机未撞上」的旧记载被推翻 —— 今日仍在发生（3997 次多跑判定 / 1655 会话）**（2026-09-21 round 12 · **已取证 → 已修**）

> **性质**：与 §0n / §0p **同族**（*机制在，但它实际做的与它声称的不是一回事*）——区别是本条的声明写在**注释**里：
> `mcl.ts:484` 原文「靠 `st.channel` **幂等**，不会判两次」。读数证明那句话**不成立**。
> **态**：**已取证 → 已修（2026-09-21）**。⚠ **本条初版把 `mcl.ts` 当「冻结件」——经复验不成立**：
> `check-module-growth` 的**冻结名单只有 3 个**（`src-client/styles.js` 726 / `src-client/body.js` 613 /
> `src/scheduler.ts` 578，实测输出），`src/mcl.ts` **不在其中** —— 该门口径是**代码行数**（`stripCommentsLite` 剥注释），
> mcl.ts 实测 **456 行 < 600 阈值**（物理 682 行里 226 行是注释；⚠ 该件 `--print` 表头写「物理行」是**误标**）。
> ⇒ **增行不触发冻结棘轮**，修它的门槛只剩「M3 前置须具名授权」那一条（方案档 §9），**与棘轮无关**。
> **修法已落**：`decideTurn` 幂等闸改「**同址检查 + 置位**」（新增 `st.deciding`），判定主体抽 `decideTurnInner`、
> `finally` 释放；判据 `scripts/test-mcl-race.mjs`（9 条，**先红确定性**：修复前实得 2 次判定，修复后 1 次）。
> **取证件**：`_memory/audit/e05-race-probe{,2,3,4,5}.mjs`（只读，私区，不入公开树）。

| 面 | 事实（逐条实测 · 2026-09-21） |
|---|---|
| **竞态在哪（读码可证）** | `decideTurn` 的幂等闸在 `src/mcl.ts:493`（`if (st.channel) return none`），而 `st.channel` 直到 **`:519`** 才赋值 —— 中间隔着**两个 await**（`:497` 融合召回 + `:504` 语义相似度）。**两条入口都调它**：① `session/event` 的 `user/message` **早判**（`:408`，**fire-and-forget `void`**，step **硬编码 1**）② `agent/pre-step`（`:415`；其 `:601` 的 `if (!st.channel)` **同样跨 await**）。⇒ step===1 时两条入口**可同时越过闸门**（JS 单线程只在 await 处让出 ⇒ check-then-act 跨 await 即非原子）。 |
| **真机判据（本件的判据设计）** | 每次判定落一条 `kind:'mcl-step'`（含 `sid`/`step`/`sim`/`injected`/`dupSkipped`）。⇒ **同 sid + 同 `sim` + 时间 ≤2s** = 同一次计算被执行两次。⚠ **v1 的判据（同 `(sid,step)` 多行）不具区分力**——`step` 是**按轮重置**的，长会话每轮都产生 step=1 ⇒ 同键多行是**平凡**的（实测这类"多行组"3091 个，其中**多数是不同轮次**）。⇒ v2 改用**时间间隔**才把信号分离出来。**这条本身是方法教训：判据须先能证伪平凡解释。** |
| **实测规模** | **1859 组**重复执行 · 涉及 **1655 个 sid** · **3997 次多跑判定**。 |
| **时新性（决定性）** | 按日分布自 2026-09-13 起持续存在，**直到今日 2026-09-21 当日 923 次**；最近一次 `2026-09-21T17:38:20Z`，而当时**最新台账行是 `17:48:11Z`**（差 10 分钟）⇒ **现行缺陷，非历史遗迹、非已修事件**。 |
| **指纹（原样两行）** | 同一 `(sid, step=1)` 在 30–150ms 内两行、**`sim` 完全相同**：`injected=441 dupSkipped=0 viaSystem=1 topicsN=3` + `injected=0 dupSkipped=3 topicsN=0`（例：`697a44d4` @16:49:51.717 / .833）。后到那次 `freshRows=0`。 |
| **真危害 = 0（如实标注）** | **同一决策注入两次 = 0 组** —— 被 `SupplyLedger` 增量台账兜住（`:530 seen` / `:539 mark`）：后到者见全量已见 ⇒ `injected=0`。⇒ **危害不是"同份材料进两次上下文"**。 |
| **真实代价（这才是危害）** | ① 每次多跑一遍**融合召回 + 嵌入相似度**（`recallRanked` + `semanticSim`，后者是嵌入调用）⇒ 3997 次重复算力/延迟；② **计数器失真**：`counters.slow++` / `lastChannel` / `lastSim` 等会 +2 ⇒ `/mcl/status` 与审计计数**偏高**（可解释 `mcl-step` 行数远多于判定轮数）；③ 幂等保证是**时序侥幸**（早判先跑完 ⇒ 后到者去重），**换负载/换模型即可能翻转成真双注入**——当前"零双注入"不能当作安全证据。 |
| **推翻的既有记载** | ① 方案档 `docs/evaluation-channel-plan.md` §8「**E-05 真机验证**：窗口存在但未撞上」⇒ **不成立**；② 同档 **A3** 判据「每次判定有且仅有一行；同 sid+step 不重复」⇒ 真机 **FALSE**（1859 组两行）；③ `mcl.ts:484` 注释自称的幂等 ⇒ **仅时序侥幸**。 |
| **为何是 M3 的硬前置** | M3 要把判定频率提到**每步**。现有每条判定平均多跑 ≈1 次 ⇒ **不先修竞态就做 M3，等于把 2× 空转放大到每步**。原方案只写"须先解 E-05 竞态"，本件补上**量化依据**。 |
| **修法（**已实施** · 2026-09-21）** | **同步占位**（原方案照此落地）：幂等闸改「**同址检查 + 置位**」——`if (st.channel \|\| st.deciding) return none; st.deciding = true`；判定主体抽 `decideTurnInner`，`finally` 释放占位（异常/提前 return 也释放，不会把会话永久锁死）。JS 单线程 ⇒ 检查与置位之间**无 await ⇒ 原子**。⚠ 初版称「会增行 ⇒ 触发冻结棘轮」—— **不成立**（订正见「态」段）。**判据**：`scripts/test-mcl-race.mjs`（**9 条** · 登记 `CHECKS` **198**）**先红→后绿**（修复前实得 2 次判定）；**真机前向判据** = 部署后 `_memory/audit/e05-race-probe4.mjs` 的**新增**命中应为 **0 组**（历史 1859 组是既成事实，不回溯）。 |

**复验命令**：`node _memory/audit/e05-race-probe5.mjs`（现：命中 3997 · 当日 923 · 最近 17:38:20Z）。
：声明侧失读 ⇒ 静默降级为「两处一致」仍报「三处一致」**（2026-09-20 round 10 · 已修 + 反例自证）

> **性质**：与 §0m 同族（**判据/读数存在，但实际没读到**），但这次出问题的是**门自身** ——
> 它是防「pin 声明领先于 lock」的那道门，**恰在声明侧读不到时它静默失能**。

| 面 | 事实（实测） |
|---|---|
| **触发** | 本轮改 profile pin（`f802ab77` → `5948e3b`）时，用 `Set-Content -Encoding UTF8` 写入 ⇒ **引入 UTF-8 BOM**（旧文件无 BOM：备份首字节 `7B 0A 20 20` vs 我的 `EF BB BF 7B`） |
| **病症** | `JSON.parse` 抛 `Unexpected token '﻿'` ⇒ 声明侧 `shas: []` + `error` **被记进 `sources`**，但**判定只取 `all = sources.flatMap(shas)`** ⇒ **声明侧整侧被静默排除**，剩下 lock + `.modules.yaml`（两者同为 `f802ab77`）⇒ 打印 **「PASS（三处一致 @ f802ab77）」** |
| **为什么危险** | ① **「三处」实际只读了两处** —— 读者会以为 pin 已核对；② 失读的正是**声明侧**，即"声明领先于 lock"这条结构性现象**唯一的观测点** ⇒ 门在该报红时给了绿；③ 这是本仓反复剿的形态：**把"坏掉了"显示成"没那么坏"**（与本件要防的 pin 脱钩是**同一后果**：仓内绿与装上去的是哪一代脱钩） |
| **既有防线为何没拦住** | 门内有 `parseMiss` 静默失效防御，但它只覆盖 **lock / .modules.yaml** 两处（判据：文件含依赖名却抽不出 SHA）；**声明侧走的是 `try/catch`，把异常收进 `error` 字段后无人再判** |
| **修** | 声明侧解析失败 ⇒ **立即 `exit 1`**（文件/代码问题，非环境问题），**不再降级判定**；理由写进注释：**读不到声明值 ≠ 声明值一致** |
| **反例自证** | `--selftest` 增 **2 例**：① 带 BOM 的 JSON **必须**解析失败（否则修法无意义）② **剥 BOM 后可解析**（证明修法是「去 BOM」而非「放弃解析」） |
| **修后如实报真况** | `声明 8ebf797c / 锁定 f802ab77 / 实装 f802ab77` ⇒ **⚠ 三处不一致**（报告态 exit 0，`--strict` 才红）—— 与 `AGENTS.md` 记的「每次推送后声明领先 lock」**结构性现象一致**（pin 随提交前进，lock 只在下一次 `pnpm install` 更新） |
| **顺带自愈** | 已剥离 profile `package.json` 的 BOM（首字节回 `7B`），pin 值本身正确（`8ebf797c…`） |
| **⚠ 未做（环境侧，须择时手动）** | lock / `.modules.yaml` 归位需在 profile 跑 `pnpm install`（会顺带归位**所有**声明领先的依赖，实测曾把 `project-nav` 0.10.4→0.10.6）。**本轮未做** —— 属环境侧操作且会动 profile，按 §云端同步清单第⑤层"纯文本改 pin（安全）· pnpm install 宜择时手动" |
...
## 0k. 🔴 **「假旋钮」第 5 例 + 守它的门禁自身恒真**：`ingest.granularity.splitLaw`（2026-09-20 round 9）

> **一句话**：这一项**登记了、投影了、门禁也全绿**，而 `R`/`K` **在所有实现面都零消费者** ——
> 守着它的那道判据是**结构性恒真**，恰好把真缺陷盖住。

| 面 | 事实（实测） |
|---|---|
| **真缺陷** | 消费点全是**裸字面量**：`src/panel-observe.ts:440` 写死 `const R = 1000`；`src/distill.ts#DEFAULT_DISTILL_PROMPT` 写死 `> 1000 字 / > 6 条`；`src/deepsleep-core.ts#DEEP_SLEEP_PROMPT` 写死 `R=1000` ⇒ **改注册表 + gen:criteria + 门禁全绿，而提示词与面板判据零变化** |
| **为什么没被发现** | 旧判据 ③ = 「键名在**整份** `criteria-gate.json` 里出现过」。而投影里**顶层 `R`/`K`**（=`reg.health.R/K`，体检口径）与 **`granularity.R/K`**（=分裂律）**同名同值** ⇒ 实测**把整个 `granularity` 支删掉仍判"有"**（命中的是顶层那个）⇒ **恒真** |
| **同族定位** | **第 5 例**（前 4：门4 零样本实为输入链断 · S-P1b′ 纪元 3/10 实为单档读 · `inject-dedup-probe` 写了从不跑 · J3/U1 上限<门槛恒真）⇒ 已固化为可复用判据形态：**凡"登记/投影在"就当"被消费"，必查"谁按哪个路径读"** |
| **修实现** | 新增 `src/criteria.ts#splitLawOf()`（唯一读口，`paramOf` 读注册表真源）；三消费点全改读它 |
| **修判据 ③** | 归属**不猜**，问**生成器自己**：解析 `gen-criteria.mjs#buildGate` 的「支名→行 id」映射，要求「支下有此键 + 值等于登记值 + 读者按该支路径取值」三条合取。⚠ 第一版改法曾用「路径深度 ≥2」，**被 selftest 当场抓出两处判错**（`notesWarn` 是真消费者却恰在顶层；`dedup.R`/`granularity.R` 同名同值互相冒充）⇒ 作废 |
| **修判据 ④（新）** | **运行期抵达**：`await import()` 编译产物断言两处提示词含注册表值 —— `tsc` **不内联**模板表达式（实测仍保留 `${SPLIT.R}` 文本），只 grep 源码会漏 |
| **先红→后绿** | 门 ③ 接线前**红**（悬空：投影支 `granularity` · 键在但无人按该支路径读）⇒ 接线后**绿** |
| **变异实证** | 注册表 `R` 改 `1234` → `gen:criteria` + `build:host` → 两处提示词运行期**真含 1234**；还原后注册表**字节级一致** |
| **判据** | `check-threshold-control`（`--selftest` **11 例 / 7 反例**）· `check-threshold-registry` 21/21 · `check-criteria` PASS · `check-runner` **188 pass / 0 fail** |

---

## 0j. 🔴 **OPEN-1 的结论被新样本推翻：0.55 已过冲，最优是 0.58**（2026-09-20 round 9）

> **性质**：与 §0f/§0g/§0h 同族 —— 不是"新增待办"，而是**已有结论的真伪问题**。
> OPEN-1 此前以「✅ 闭环 · 维持 0.55 不改」结案，依据是 **120 条样本**下的细校准。

**复检触发早已满足**：`OPEN-1` 的 `recheck` 原文写「近期样本 ≥300 时重跑」；本轮实测 **带 `missReason` 的行 = 4217 条**。

**同判据、同工具、只换样本量 ⇒ 结论翻转**（预注册判据 = 目标覆盖率 **27.5%**，出处 `criteria.json#surface.mcl.note`）：

| 样本量 | T=0.55 过线率 | 细校准结论 |
|---|---|---|
| **120**（当时） | **27.3%**（距目标 0.2pp） | ✅ **维持 0.55 不改** |
| **4217**（本轮） | **42.1%**（**过冲 14.6pp**） | 🔴 **应上调至 0.58**（27.7%，距目标 0.2pp） |

**扫描表（本轮实测）**：

```
T      sim 过线        其中快通道   被标签门挡掉
0.50   8355 (79.8%)     3031         5324
0.55   4405 (42.1%)     2815         1590      ← 现值
0.58   2896 (27.7%)     1912          984      ← 最接近目标
0.60   2332 (22.3%)     1505          827
0.65    633 ( 6.0%)      520          113
```

**⇒ 结论：应上调至 0.58**（0.55 在 4217 条样本上已明显过冲）。

**✅ 已落地（2026-09-20 round 10 · 用户授权「继续全部收尾」）**：注册表 `surface.mcl.familiarThreshold` 与**运行态 `~/.dsh/suite/scheduler.json`** 两处同改为 **0.58**。⚠ **关键是两处都改**：只改注册表**不改真机行为** —— 校准器当时报的是「当前值 0.55（scheduler.json 热覆盖）」，即**显式热覆盖优先于注册表缺省**；只动注册表 + `gen:criteria` + 门禁全绿，而注入行为零变化（正是本轮全程在剿的「假绿」形态）。复验：`node scripts/mcl-calibrate.mjs` ⇒ 「当前值 **0.58（判据源）** · 结论：**保持当前值**」。回滚：`/set mclFamiliarThreshold 0.55`（热生效）。

**最初只出结论不改配置的两条理由（其中一条现已解除、一条定案不变）**：
1. ~~**改 `mclFamiliarThreshold` 是行为变更**（直接改注入/召回行为），按 R1 须具名授权~~ ⇒ **已获授权**（本轮落地）。
2. **两个信号仍相反** —— `recall-diagnose` 的 **coarse `advice` 给 `lower-threshold`**，而细校准给「上调」。
   §0 早已定案**以细校准为准**（coarse 是粗方向、细校准是预注册判据）；本轮**不改这条定案**，只是把新读数落档。

**为什么这个翻转值得记档**：它同时印证两条仓内原则 ——
① **`[原则] 代理指标非判据`**：coarse `advice` 与细校准方向相反，说明 coarse 是代理；
② **`[原则] 样本足够前不可外推`**：120 条时"恰好命中 27.3%"是**巧合级**贴合，样本量上去后同一阈值过冲 14.6pp
   ⇒ 与 `S-P1b′` 的「纪元 3/10」（真值 46）是**同一类病**：**低样本下的"最优"不可信**。

**复验**：`node scripts/mcl-calibrate.mjs`（看「预注册判据」段）。
...
**✅ 已落地（round 10 · 用户授权「继续全部收尾」）**：判定改为按**文件数**（`traceFiles < minTraces`）——
名实不符从"已登记"变为"已消除"。**已具名陈述的代价**：那 25 轮越过 51% 的入睡纪元**不再入睡**
（其材料是"背景上下文"而非"新增知识"，而 S-P1b″ 定案的轴正是「与新增材料同源」）；**不丢数据**
（跳过即滑窗，未来文件 mtime 必晚于水位 ⇒ 后续纪元仍捕获）。判据 `test-epoch-watermark` **⑥**（3 断言，
**剥注释后判定**，防"只改注释不改行为"的假修）；**变异实证**：改回 `traces.length < minTraces` ⇒ exit 1（2 红）。
...
| 项 | 判据（原文口径） | 实测 | 结论 |
|---|---|---|---|
| **OPEN-1**<br>D2 两条件分流 · D4 零贡献率改善 | 样本 ≥30 ⇒ `advice` 落 `fix-recall`/`lower-threshold`/`enable-embed`；**方向由数据推导，不猜** | 🔴 **判定已被新样本推翻（2026-09-20 round 9）—— 详见 §0j**：当时样本 **120** ⇒ 细校准「**维持 0.55**（覆盖率 27.3%，距目标 0.2pp）」。**本轮样本累积到 4217 条**（复检触发 N≥300 早已满足）⇒ 同一预注册判据（目标覆盖率 **27.5%**）下：**T=0.55 的过线率已达 42.1%**（**过冲 14.6pp**），而 **T=0.58 ⇒ 27.7%（距目标 0.2pp）** ⇒ **结论翻转为「应上调至 0.58」**。<br>⚠ **这是"低样本下的结论不可外推"的又一实例**（同族：S-P1b′ 的"3/10 纪元"）。⚠ **本条只出结论，未改配置**（改 `mclFamiliarThreshold` 是**行为变更**，且 `recall-diagnose` 的 coarse `advice` 仍给 `lower-threshold` —— **两者相反**，见 §0 原分析：**以细校准为准**） | `node scripts/mcl-calibrate.mjs` | 目标覆盖率 27.5% ⇒ 最优 **T=0.58（27.7%，距 0.2pp）**；现值 0.55 ⇒ **过冲 14.6pp** | ⏳ **待拍板**（是否上调 0.55 → 0.58；属行为变更） |
...
|---|---|---|---|---|---|
| **OPEN-1** | D2 两条件分流 · D4 零贡献率改善 | ~~MCL 归因样本 6 / 30~~ → **120 → 4217** | `node scripts/mcl-calibrate.mjs` | 见 §0；**预注册目标覆盖率 27.5%**，< 30 样本 ⇒ `insufficient-data` **不猜方向** | ✅ **复检已执行（2026-09-20 round 9）· 结论翻转** ⇒ 见 §0j；**是否改配置待拍板** |
...
| **S-P1b′** | ⏳ **两前置**均已满足，**只差新纪元产生 `traceFiles` 样本**（2026-09-20 round 9 复核）** | 本条原写「纪元 **3/10**；**且须先解决 S-P1b″ 的口径问题**」—— **两前置现在都不再是障碍**：① **纪元数**：G13 修复跨档读后真值 **46 个不同纪元**（早已 ≥10；原"3/10"是**单档读**的假读数）；② **口径**：`S-P1b″` 已**定案**（正确轴 = 窗口内痕迹文件数）。**本轮已把正确轴接进校准器**：`epoch-calibrate` 新增第三口径报告 —— `traceFiles: {n, missing, p25, p50, p75, p90, min, max}`，且**缺字段的旧纪元一律不计入**（旧行没有该字段）。**当前读数**：`n=0 · missing=47` ⇒ **N=0 显式记 0**（字段自本轮才落审计，须等**新深睡纪元**）。<br>⚠ **不得用 `spanHours` 或 `materialChars` 代替**（各有硬反证，见 S-P1b″）；也**不得把 `missing` 当 0**（那是"输入量不可见"的老毛病） | `node scripts/epoch-calibrate.mjs`（看「痕迹文件数分位」段）· `--json` 看 `traceFiles.n` | ① 纪元 ≥10 ✅ ② 口径定案 ✅ ③ **`traceFiles.n ≥ 10` 后**按该轴分位预注册取值；不足 ⇒ 显式写 `insufficient-data` | ⏳ **等新纪元落账**（触发 = `traceFiles.n ≥ 10`；字段已就位，只需深睡真跑若干轮） |
...
| **J3/U1-调用点** | ✅ **根因已定位并修（2026-09-20 round 9 第二次复核）—— 不是"样本不足"，是**断链**** | 🔴 **上一版判定（"卡在单纪元样本量、结构性等待"）被本轮读码推翻**：真根因是**同一函数内两处字面量互相打架** —— 取样本 `samplesFromMclRows(missRows,`**`20`**`)` 而上限卡 **20**，判定 `if (samples.length <`**`30`**`)` 门槛 **30** ⇒ **上限 20 恒 < 门槛 30** ⇒ 门槛分支**结构性恒真** ⇒ 后面那段**真调子代理**的代码（整个 J3/U1 的目的）**从未执行过一次**。<br>**真机证据（跨档 49 深睡行 · 35 带 `attribution*` 字段）**：`attributionSamples` **min 0 / max 18 / ≥30 者 0 条**；出非 `insufficient` verdict **0 条**。⚠ **`insufficient` 的字面（"未命中样本 N < 30"）与 N 一路 6/10/11/14/17 一路"看着在涨"的表象**，使断链**读起来就是"样本在积累"** —— 与「门4 零样本实为断链」「S-P1b′ 纪元 3/10 实为单档读」**同族（第 4 例）**：**"样本不足"与"结构性不可达"在读数上长得一模一样**。<br>**修**：两处字面量提为**具名常量** `ATTRIBUTION_SAMPLE_LIMIT` / `ATTRIBUTION_MIN_SAMPLES`（同文件模块级，互引关系写进注释与门禁） | `node scripts/check-attribution-samples.mjs`（**入 `CHECKS`**，先红已钉） | ① 不变量 `LIMIT >= MIN`（**从源码取值解析，不采信注释**）；② 门槛分支之后**确有** `subagents.start(`（防"把恒真改成恒假"的假修）；③ 反例自证 4 例含「真机修前形态 20/30」 | ✅ 已修 2026-09-20 · **先红实证**：变异 `LIMIT 30→20` ⇒ 本件 exit 1 · 还原 ⇒ exit 0 且**字节级还原** |
| **J3/U1-一致率（剩余未知）** | 🟡 **修复后才有"单纪元样本够不够"这个问题** | 修前该问题**问不出口**（分支恒真，永远走不到取材充分性）⇒ 上一版把"断链"误答成"等样本"。**修后的真未知**：单纪元窗口内"未命中样本"能否达 30（历史读数 max 18，但那批读数本身受上限 20 截断 ⇒ **旧数据不能用来判新门槛**）。**观测口**：修复后新纪元的 `attributionSamples` 分布（≥30 的行数与 `verdict != insufficient` 行数） | `POST /deepsleep/trigger` → 核审计 `attribution*` 五字段 | ① 样本 ≥30 时**真跑出 verdict**；② `agrees` 与细校准对账；③ 对账不通过 ⇒ **以细校准为准** | ⏳ 待新纪元落账（**不得再用修前数据外推**） |
...
| **S-J4** | 🟡 **基线已测出（2026-09-20 round 9）—— 「先测真实假冷率」这一步已完成；升级本身仍待前置** | **基线读数（`activity-calibrate` · 只读真库）**：① **假冷率 = 3/21（14.3%）** —— 判 `cold` 的 21 条里，真实 `coldDays(44)` 内仍有命中的只有 3 条。⚠ **与历史登记值 26/49 不可直接相减**：旧值在 `harvest` 接线前、且含记录错漏污染（口径已换）。② **记录错漏 75/119（63.0%）** —— 真实事件数 > 记录 `hits` 的条目占比（如 `notes/env.md｜插件更新与依赖 rec=6 real=14`）⇒ **这才是活性判定的主要失真源**，比天数阈值本身更该先治。③ **真实天距分位**：p50 **0.8** · p75 **1.7** · p90 **1.7** · p95 **9.6** · max **10.8**（天）—— 远小于现阈值 `14/44/90`。④ **真实 hits30 分位**：p50 **14** · p75 **48** · p90 **48** · max **102**（现 `hotHits=23` 落在 p50–p75 之间）。⚠ **为什么仍不动阈值**：这些分位是"**现行判定下被读到的**条目"的分布（受判定自身筛选）⇒ 直接按 p75 定值 = 用被污染样本定新阈值（同族：低样本下的"最优"不可信，见 §0j） | `node scripts/activity-calibrate.mjs` · `node scripts/memory-reconcile.mjs` | ① 假冷率基线 ✅ **3/21 = 14.3%**（本轮）② 记录错漏率须先降（现 **63%**）③ 安全红线节不被判冷（现未违） | ⏳ 待**记录错漏**治理（**阈值不是瓶颈**） |
