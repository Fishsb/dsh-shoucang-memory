...
> **当前状态（2026-09-15 复核）**：五阶段（S0–S4）+ S4R + S4X + S4Y + **UI1 面板架构整理** 的**可施工项全部完成**；
> 下表 3 项**均为"待外部条件"**（数据 / 时间），**不依赖代码** —— 其中 **OPEN-1 的条件经实测已满足**（见下）。

**最后更新**：2026-09-15

---

## 0. 条件变更（2026-09-15 实测）

| 项 | 变化 |
|---|---|
| **OPEN-1** | 🔔 **阻塞条件已满足**：`node scripts/recall-diagnose.mjs` 实测 **可归因样本 119（≥30 阈值）**，`advice` **已可推导** = **`lower-threshold`**（依据：阈值层 **27** ≥ 召回层 **12** ⇒ 瓶颈在 `mclFamiliarThreshold`，非词表/索引）。<br>⚠ **尚不宜直接改值**：判据只给**方向**不给**步长**；且本项自身写明「**不猜方向**」—— 改值须配**可测量的前后对比**（定步长 → 改 → 累积新样本 → 复测阈值层/召回层占比）。**下一步 = 定标实验**，不是拍一个数。 |
| **OPEN-2** | ⏳ 可复核（**需触发一轮深睡**）：判据是「≥1 条原则的 `← 源:` 落在 `replayHits` 候选内」。本轮未做（触发的副作用是**真的改写记忆库**，非只读复核）。 |
| **OPEN-3** | ⏳ **仍等库龄**（主库 2026-09-13 重建，窗口 90 天 ⇒ 约 **2026-12** 之后）。`forgetCandidates = 0` 是**正确期望**，非故障。 |

---

## 1. 待条件项（不可由 agent 自行推进）

| # | 项 | 差什么 | 复验命令 | 判据 | 何时可做 |
|---|---|---|---|---|---|
| **OPEN-1** | **D2 两条件分流 · D4 零贡献率改善** | MCL 归因样本 **6 / 30** | `node scripts/recall-diagnose.mjs` | `advice` 落 `fix-recall` / `lower-threshold` / `enable-embed` 之一。<br>**< 30 样本 ⇒ 输出 `insufficient-data`，不猜方向**（猜出来的调参会误导） | 样本自动累积（本会话已 0 → 6） |
| **OPEN-2** | **L1 跨日二次激活的「因果」** | 输入 **10** 条（10/10 真跨日）+ 产出 **3** 条，但产出是"原则"、**未直接消费 replay 候选** | `curl "http://127.0.0.1:3080/api/shoucang-panel/deepsleep/trigger"` ⇒ 再查 `deep-sleep` 审计的 `replayHits` | **≥1 条原则的 `← 源:` 落在 `replayHits` 候选内**。<br>材料现已带「跨 N 天」标注（S4Y 修）⇒ 可据此判 | **下一轮深睡**即可（手动触发亦可） |
| **OPEN-3** | **L3 `forget` 候选消费** | **库龄** —— 主库 2026-09-13 才重建，而窗口是 **90 天** | 同 `recall-diagnose`，或直查 `deep-sleep` 审计的 `forgetCandidates` | `> 0` ⇒ 应见 `forgetArchived` / `forgetKept` 消费；<br>`= 0` ⇒ **正确期望**（空 ≠ 故障，**禁止硬消费造数据**） | 约 **2026-12** 之后 |
