...
| **S-P4d** | ✅ **注入面回归保护（AC-G.5）已落地（2026-09-20 round 9 补登记）** | 判因：`inject-dedup-probe.mjs` **文件早已存在且能跑（exit 0），但从未登记进 `CHECKS`** ⇒ 按仓规则 6「**未登记 = 等于没写**」，它**从未在 `npm test` 里跑过**（同族先例：`test-deepsleep-verdict` / `test-watermark-guard` / `test-atomic-write` 三件曾写了从不运行）。本轮已登记（`check-runner` **185 → 186 件**） | `node scripts/check-runner.mjs`（含 `inject-dedup-probe`）· `node scripts/inject-baseline-diff.mjs` | ① 方向条目数不减少 ② 误注入率不升 ③ 被消重行不在注入面 ④ 关闭即逐字节回基线 —— 四条全过 | ✅ **已完成 2026-09-20** |
