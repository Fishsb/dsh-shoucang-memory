...
> **态**：**已取证、未修**。修它要动**冻结件** `src/mcl.ts`（681 行 ≥600，`check-module-growth` 冻结阈）
> ⇒ 增行即触发棘轮、抬基线属 **R3** ⇒ 按仓规**如实登记而非顺手改**。
...
| **修法（待授权 · 只给方案不实施）** | **最小改 = 同步占位**：把幂等闸从"事后赋值"改为"**进入即占位**"——`:493` 处 `if (st.deciding \|\| st.channel) return none; st.deciding = true`，函数各出口复位。JS 单线程 ⇒ check-then-set 之间**无 await ⇒ 原子**。⚠ **会增行** ⇒ 触发冻结棘轮（抬基线属 R3）；或写成**净零行**（替换既有行而非新增）。**判据现成**：`_memory/audit/e05-race-probe4.mjs` 修后应报 **0 组**（先红→后绿，反例已具备）。 |
