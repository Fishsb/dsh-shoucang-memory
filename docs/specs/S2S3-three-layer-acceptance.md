...
| **G13** | **UI 可见**：面板「睡眠汇报」视图渲染（日历式日期列表 + 该日汇报 + 统计卡），**DOM 几何与图证双判据** | 册四 | `ui-geo-regress --shots` + 契约门禁 | ❌ 无此视图 |
...
| **UI 判据（双轨）** | **图证**（`--shots`：图内可见日期列表与统计卡）+ **DOM 几何**（日期项高 >0、在视口内、统计卡数值与 `/sleep/issues` 返回**逐值一致**）——引用仓内既有纪律：**纯像素判断对折叠线以下系统性失明**（S-P1d 定案） |
