...
| **U5-b** | ✅ `test-split-equivalence.mjs` —— 事后**无法取"拆分前"DOM**（旧版本不在工作树）⇒ 改为**证明两个渲染门确实能抓到拆分破坏**：A1 不渲染 sched tab ⇒ 结构门报"缺失"（**34→21**）· A2 `--sc-nav-w` 压 4px ⇒ 几何门 **FAIL** · A3 两源文件**逐字节还原** · 复原后两门回绿。详见 §2′ |
| **U5-c** | ✅ 由 U5-b 的 **A1/A2 两条反例**覆盖（原独立卡的意图已实现）；另 U5-a 的 400 反例覆盖其等价意图 |
...
_建立 2026-09-15 · 收尾 2026-09-15 · UI1 阶段验收记录（**U0/U1/U2/U3/U5（含 a/b/c）全部完成**；
`body.js` **5476 → 626 有效行**，**`<800` 已达成**；门禁 **108 pass**；
六层核：仓内绿 · 部署 sha1 · 热重载 · 特性探针 · 对称 pin · 公开树）。_
