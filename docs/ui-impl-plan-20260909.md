...
> ⚠ **时点文档（2026-09-09）——文中「自建 9915 桥」路线已于 2026-09-11 被取代**：现行向量承载 = **Ollama `bge-m3`** @ `http://127.0.0.1:11434/v1`（本机免 key、OpenAI 兼容）。本文保留原样作为**当时**的施工图；正文中「探测 9915 health」「`provider: DmlExecutionProvider`」等描述**不代表现状**（现行 provider 探测见 `panel.ts` 的泛化端点判定与 `/vector/status2`）。
