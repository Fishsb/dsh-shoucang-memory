...
> **状态：✅ 已实现**（M0–M3：`b339814`/`bf2bf87`；ADR-126）。本档保留为判因与设计记录；实施进度见 `memory-core-roadmap.md`。**注**：M2 实施时把计划的 `write-ledger.jsonl` **合并进统一台账** `audit/ledger.jsonl`（`type=write.*`），以本档以外的总纲决议为准。
