...
> ⚠ **时点文档（2026-09-10）——文中「自建 9915 桥」路线已于 2026-09-11 被取代**：现行向量承载 = **Ollama `bge-m3`** @ `http://127.0.0.1:11434/v1`（本机免 key、OpenAI 兼容 `/v1/embeddings`），见 `ARCHITECTURE.md` §Provider 与 `settings-guide.md` 的 `embedBaseUrl` 行。旧自建桥（`bge-m3-openai-server-gpu.py` + nssm 服务 `dsh-bge-embed` :9915）已随 nssm 卸载停用；**模型资产仍在 `D:\AI\models\bge-m3`，随时可复活**。本文保留原样作为**当时**的方案记录，正文中的 9915、端口探测清单、Provider 预设等**不代表现状**。
