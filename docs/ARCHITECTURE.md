...
> `src/` 共 **68 模块**（**排除生成物 `criteria.generated.ts`**，与 `audit-fnspan` 同口径；
...
> 手写明细必然漂移（实测：本处曾写「55 模块」而其分项相加只有 **53**，**文档自身都不自洽**；
...
4. 架构文档**由人工重生成**——原「按指纹过期重生成（指纹 = src/** + 本文）」机制已随治理插件 0.10 移除；
   **「判过期」仍无机检，但「声明是否与实测一致」自 2026-09-14 起有部分机检**（`check-arch-sync` 六项，见页首）。
   ...
   （2026-09-13 本会话把 44 → **55 模块**的归属一次性补齐，2026-09-14 复核再校正为 **62**——见 §3；
