...
> 并由治理模型登记落点做偏移检测；**文档与代码不一致已无机器兜底，属人工纪律**。
...
| **M7 深睡** | `deepsleep-machine.ts`（会话状态机 FSM，阶段 D 自装配层迁出）+ 其余 `deepsleep-*` 9 件 | 停滞≥3h 归纳 [原则]/[路径] | 窗口痕迹预算 12000+18000；done 才推水位 |
| **M8 注入** | `panel-shared.ts` 的 `buildHotMemoryText` + `mcl.ts`（慢通道线索化召回） | 会话开头热记忆 / 任务首步线索注入 | 预算 3000 字符；delta 预留字节 |
| **M9 向量** | vec.ts `recallRanked` | 语义召回（融合） | 见 §5 |
| **M10 自省** | `assistant_capabilities` | 能力面查询 | 只读，无 token |

> **载体归属口径（2026-09-12 根治 A–D 后，取代此前的单体表述）**：`src/` 共 **44 模块** 按领域分家 ——
> **入口/装配 6 件**（`index` / `panel` / `distill` / `scheduler` / `mcl` / `deepsleep`，各 ≤120 行，
> 由 `audit-wiring` 的 I1 棘轮守）+ **panel 域 5 件** + **distill 域 15 件** + **deepsleep 域 9 件** +
> **基础设施 6 件**（`targets` / `criteria` / `criteria.generated` / `activity` / `treeops` / `vec`）+
> **3 个 `-share` 惰性桥**（`scheduler-share` / `deepsleep-share` / `mcl-share`）。
...
4. 架构文档**由人工重生成**——原「按指纹过期重生成（指纹 = src/** + 本文）」机制已随治理插件 0.10 移除，
   **不再有机器判过期兜底**；动了模块归属就同步改本文，并由 nav 治理模型登记落点做偏移检测。
