...
const ROW_OVERRIDES = [
  ...
  /* ── 空壳落点收口（2026-09-19 · docs/pointer-supply-plan.md §2.3 待决项 · §12 处置记录）──
   * 病灶：这 4 行的指针**能解析**（agg=exists）但落点是**正文 0 非空行**的小节
   *   ⇒ 走不到本件既有的 missing/partial 分支（`agg === 'exists' ⇒ continue`），
   *   故必须用**逐行覆盖**（覆盖先于一切，含"能解析但锚错"行 —— 错锚比文件级指针更糟）。
   * 逐条取证（依据 = 目标节正文与行内概况逐条对上）：
   *   · 批改前先核字节（行号口径不可靠/删除集须零交集不可动块/原子替换复验）↔ §完整性核验
   *     （"不信 cmdlet 行号…以字节偏移+内容规则为唯一判据…禁区前后哈希…临时文件校验后原子替换"）3/3 对上；
   *   · git 推送与代理回退（直连超时/代理在听/单命令 -c http.proxy）↔ env.md §网络与代理
   *     （"git 直连 github 被阻断时恢复推送：分通道诊断/探代理端口/单条命令回退/勿写全局"）3/3 对上；
   *   · 不可达结论须多通道（单通道失败只算样本/须列已测通道）↔ lessons.md §假失败
   *     （"多通道鉴权矩阵探活；头错配把可用通道假报 401/500 ⇒ 误判死条目"）2/2 对上；
   *   · 架构档机检与落点（六工具/新鲜度/指纹过期/落点漂移）**在 notes 内无单一承载节**
   *     ⇒ 按本件既有处置档**降级为文件级指针**（知识仍在索引行里，按需 grep 文件）。 */
  { match: '批改前先核字节', to: 'notes/lessons.md §完整性核验' },
  { match: '架构档机检与落点', to: 'notes/flows.md' },
  { match: 'git 推送与代理回退', to: 'notes/env.md §网络与代理' },
  { match: '不可达结论须多通道', to: 'notes/lessons.md §假失败' },
]
