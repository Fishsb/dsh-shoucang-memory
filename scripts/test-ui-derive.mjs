...
/* v9 严格对齐（2026-09-13）：面板 SUITE_STATUS 已改为原型文案「已装配/未装配」，口径差异移入 title ⇒ 金标准同步 */
const gSuiteText = (s) => (s === 'both' || s === 'injected' || s === 'profile') ? '已装配' : '未装配'
const gSuiteKind = (s) => s === 'missing' ? 'error' : (s === 'both' || s === 'injected' || s === 'profile') ? 'ok' : 'info'
...
/* VEC_RECALL 于 2026-09-13 v9 严格对齐时删除（其唯一使用者「向量召回」区块按原型移除）⇒ 收敛为四张 */
const d7 = ['VEC_DOWN', 'VEC_LABEL', 'CAP_PCT', 'SUITE_STATUS'].filter((k) => src.indexOf('var ' + k) < 0)
d7.length === 0 ? ok('D7 四张映射表齐备（VEC_DOWN/VEC_LABEL/CAP_PCT/SUITE_STATUS）')
  : bad('D7 缺表：' + d7.join(', '))
...
const DECIDERS = ['providerKind', 'providerDown', 'vecLabel', 'capKind', 'suiteStatus', 'vectorOn', 'has', 'count']
...
/* 词边界：原正则 `bText` 是子串匹配，会误伤 `subText` 之类的参数名（2026-09-13 UI.kpi 新增即被误报） */
const e4 = hit(/querySelectorAll\('span'\)\[1\]|\bbText\b/)
