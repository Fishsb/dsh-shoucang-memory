...
// ⑧ 归档可回滚证据
//   ⚠ **确定性判据**（2026-09-14 修）：原为 `af.length === 1` —— 而归档文件名只到**秒**
//     （`treeops-YYYYMMDD-HHmmss`）：同一秒内多次调用**并入同一文件**（追加，证据不丢）、
//     跨秒则分成两个文件 ⇒ **按文件个数断言 = 时间边界假红**（实测踩到：02:51:28 与 02:51:29 两份，
//     测试判红，而当次运行完全正确）。改为**校验全部归档文件的并集**——这才是它真正要验的两件事。
const arch = join(root, 'audit', 'treeops')
const af = existsSync(arch) ? readdirSync(arch).filter((f) => f.endsWith('.jsonl')) : []
ok('⑧ 归档至少写入一份', af.length >= 1, af.join(','))
const recs = af.flatMap((f) => {
  try { return readFileSync(join(arch, f), 'utf8').trim().split('\n').filter(Boolean).map((l) => JSON.parse(l)) } catch { return [] }
})
...
ok('⑧ 归档无空文件（写了就有内容）', af.every((f) => readFileSync(join(arch, f), 'utf8').trim().length > 0))
