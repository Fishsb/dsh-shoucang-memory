...
const STREAMS = [
  ...
  /* 承诺结算链 · 册二（2026-09-20）**结算留档流**：为何**不并入台账**（登记要求的理由）——
   *  ① **它是"可还原"的载体，不是事件**：每行存被改承诺的**整条原文**（含 `before` 全文），
   *     读侧语义 = 按键（`id`/`opHash`）取原文供回滚核对；并入台账会把"原文备份"摊进事实流。
   *  ② **与 `rollback/<opHash>.json` 同族**（那是 revise 的留档，逐 op 一文件、非 jsonl ⇒ 不在本表口径内），
   *     本流是结算的孪生留档（多条一批 ⇒ 一行一条）。同一纪律：**先留档再改**。
   *  ③ 它是**库内** `audit/session-review/` 的产物（数据属库），与 suite 台账分域。 */
  ['settle-rollback.jsonl', 'keyed', '承诺结算**留档**（被改承诺原文 + 证据 + opHash；"先留档再改"的判据载体，`revive` 回滚用）', 'bank'],
