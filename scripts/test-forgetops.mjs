...
import { applyForgetOps, sectionExists } from '../lib/forgetops.js'
...
// ⑦ 审计留痕
// ⚠ 2026-09-14 修（**时间边界假红**）：原断写死 `files.length === 1`，而审计文件名只到**秒**
//   （`forgetops-<YYYYMMDD-HHmmss>.jsonl`）⇒ 本测试内部多次调用若**跨秒**就会产生 2 个文件 ⇒ 偶发红
//   （实测一次跨秒：`…-123836.jsonl` 与 `…-123837.jsonl`）。**同型缺陷今天已在 `test-treeops-split.mjs` 修过**
//   （其提交原文：「归档判据改确定性（按文件个数断言=时间边界假红）」）——本例是同一类的漏网。
//   修法同法：**不断言文件个数**，改为**读取全部审计文件并汇总断言**（语义不变、对时间不敏感）。
...
if (files.length) {
  const recs = files.flatMap((f) => readFileSync(join(fo, f), 'utf8').trim().split('\n').filter(Boolean).map((l) => JSON.parse(l)))
