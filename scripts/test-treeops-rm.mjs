...
// 用 xfail 而非 skipped/红灯的理由见本文件 `xfail` 定义处；G-22 定级由 team-lead 裁定。
{
  ...
  const after = readNote(root)
  // S4Z（2026-09-14）**G-22 已修** —— merge 改为与 rename **同口径**（`src/treeops.ts` 只匹配 L2/L3）。
  //   原 `xfail` 双向锁如上设计**如期翻成 XPASS** 提醒"行为已变、缺陷需重审" ⇒ 现改为**普通断言**。
  ok('M6-e merge 不得匹配 L4（#### 丁）⇒ 应 skipped 且字节不变',
...
console.log('\n未覆盖声明（不假装覆盖）：① gate 拒路径（玩具库无 memory_write_gate.mjs，恒 absent）' +
  ...
  '④ ~~merge 侧 L4（####）行为不锁~~ ⇒ **G-22 已修并转为普通断言**（2026-09-14：merge 与 rename 同口径，只匹配 L2/L3）；' +
  'xfail 双向锁机制保留（供未来"已知缺陷、暂不修"的契约断言使用）')
