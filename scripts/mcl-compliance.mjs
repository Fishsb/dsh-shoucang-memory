 ...
 * 退出码：0 = 达标或样本不足（报告）· 1 = 样本充足且**合规率显著下降** · 3 = 无数据/样本不足
 *
...
const judgedAll = rows.filter((r) => r.kind === 'mcl-step' && r.channel === 'slow' && typeof r.compliant === 'boolean')
...
const group = (pred) => {
  ...
  const okN = g.filter((r) => r.compliant === true).length
...
console.log(`认知环引用合规率 · 数据源 legacy mcl-audit ∪ 统一台账里的 mcl.*（${auditDir}）`)
console.log(`  窗口：${SINCE ? `≥ ${SINCE}` : '全量（⚠ 含 2026-09-13 之前的行——那时合规步不落账，分母不存在）'} · 判定样本 ${judged.length}/${judgedAll.length}`)
console.log(`  材料走**消息面**（viaSystem≠1）  n=${byMsg.n}  合规 ${byMsg.compliant}  合规率 ${pct(byMsg.rate)}`)
console.log(`  材料走**systemPrompt 段**（=1） n=${bySys.n}  合规 ${bySys.compliant}  合规率 ${pct(bySys.rate)}`)
