...
/* ── 1. 原地抽取 CSS ── */
/* 稳定锚点 + 引号无关（S3）：打包后局部变量名会变、引号会被规范化，锚点必须与二者无关 */
const mCss = /window\.__SC_CSS__\s*=\s*(\[[\s\S]*?\]\.join\(\s*['"]['"]\s*\))/.exec(text)
if (!mCss) { console.error('未找到 CSS 数组稳定锚点 window.__SC_CSS__ —— client.js 结构变了？'); process.exit(1) }
const start = mCss.index
const end = mCss.index + mCss[0].length
...
const jsSrc = text.slice(0, start) + text.slice(end) // end 已含锚点表达式结尾（S3 稳定锚点）
...
{
  /* 引号无关 + 覆盖模板字面量（S3）：产物经 esbuild 重排后拼接可能是 "sc-log-" + lv 或 `sc-log-${lv}` */
  const res = [/['"]([^'"]*)['"]\s*\+/g, /`([^`$]*)\$\{/g]
  res.forEach((re) => {
    let m
    while ((m = re.exec(jsSrc)) !== null) {
      const seg = m[1].trim().split(/\s+/).pop()
