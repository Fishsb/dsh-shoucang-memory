...
const CHECKS = [
  ...
  // ACT-283（2026-09-21）**可配置评估通道 · G2 闸门**（中文精度 + 分档可用性）：
  //   判因：方案首要未知量是「把记忆库**中文**材料交给类型化决策通道，精度够不够」——官方只说 CJK
  //     "handled but **not equally well**"，全仓与全网**零中文实测** ⇒ 不可推断，只能实测。
  //   形态：零依赖 · 零 src 改动 · 后端 = 任意 OpenAI 兼容 `/chat/completions`（缺省本机 Ollama，**无需 key**）。
  //   判据：G2-a 类型化准确率 ≥ 同批样本/同后端**基线**×90%；G2-b 高置信子集正确率 ≥ 低置信
  //     （B 档无真概率 ⇒ 只验**排序性**，不验校准）。
  //   ⚠ 后端不可达 ⇒ exit 3 判 **skip**（不算通过，与 `inject-baseline-diff` 同口径）。
  //   ⚠ **校准性不在本件范围**（须 A 档真概率）；本件如实标注，不冒充。
  //   首跑实测（qwen3:8b @ 本机 Ollama）：类型化 **19/20 = 95.0%** · 基线 17/20 = 85.0% · G2-a/G2-b 均 PASS。
  ['scripts/eval-gate.mjs', { slow: true }],
  // M0′（2026-09-21 · ACT-289）**评估样本集质量门**（17 条 · 无网络 · 现场从真库构建）：
  //   判因：前代样本集（内嵌于 `eval-gate.mjs`）实测三处结构性失效，使 G2 的 95% 不可解读：
  //     ① **字面泄漏** 8/20 —— T1 材料以 `[原则]` 开头而答案就是 `原则`，**一条正则拿满分**；
  //     ② **镜像对** 8/20 —— T2 同 state 配一正一反，词袋基线被 state 主导，留一时必然预测
  //        **孪生兄弟标签**（实测 6/8 精确命中）⇒ 基线 0/8 是伪影而非实力；
  //     ③ 真独立样本仅剩 4 条，推不出结论。
  //   本件双向守：**正**断言（无泄漏/无镜像/选项合法/来源可溯/无重复/小节名非复合）+
  //   **反例自证**（喂带 `[标签]` 与被构孪生对的样本必须变红 ⇒ 断言语义有效、非恒真）。
  //   ⚠ 类不均由本件**显式报出**（AGENT.md 实测 原则 82/路径 17/边界 2/认知 1/经验 1）
  //     —— 不报多数类基线的准确率数字无法解读。
  //   ⚠ 记忆库不可达 ⇒ exit 3 判 **skip**（不算通过）；样本含私人内容 ⇒ 缺省不回显类名。
  ['scripts/check-eval-samples.mjs'],
  // M1（2026-09-21 · ACT-283）**评估通道行为判据**（43 条 · 纯判据，不发真实网络请求）：
  //   判因：本仓 `vec.ts:240-260` 把**六类失败全塌缩成 `return null`**，归因只看配置位 ⇒
  //     真机 `missReason` 中 `embed-off = 0`（"调了但失败"与"没调"**账上同形**）。
  //   本件从第一天起就**分态**，并把 G1 出网条款落成可执行判据：
  //     A 出网闸（loopback 走**解析 hostname**，非正则）· B 七态归因 · C state 白名单（拒项如实计数）·
  //     D 严格解析（值越域/NaN 一律拒，不猜）· E 档位 const union + 缺省 fail-closed。
  //   ⚠ A7/A8 是**实测锚点**：`panel-shared.ts:803` 的 `isLocalBase` 会把
  //     `http://127.0.0.1.evil.com/v1` 与 `http://localhost:11434@evil.com/v1` 误判为 LOCAL，本件必判 remote。
  ['scripts/test-eval-channel.mjs'],
  // M5′（2026-09-21 · ACT-293）**评估通道「真实消费者」行为判据**（28 条 · 无网络）：
  //   判因：方案档 A4 明写「新件**有真实消费者** + 登记」，而实测 `evaluate-channel` **全仓零命中**
  //     ⇒ M1/M2 建的通道"能跑但没人用"。方案 §4-M3 指定的落点（改冻结件 `mcl.ts` 每步判定）
  //     须先解 E-05 竞态，故本步另选**零冻结件、零竞态**的真消费面：
  //     **蒸馏索引行落盘成功后**的只读语义复核（`admitIndexRow` 只做机械准入，从无语义判断）。
  //   本件守**接线与契约**（能力另证，见下）；三条最易变"假接线"处各带断言：
  //     ① 关闭态零接线（`evalEnabled!==true` ⇒ 钩子 undefined ⇒ 逐字节一致）
  //     ② 端基址空 / 拆不出主题概况 ⇒ **未判**（off，不放行也不误拒）
  //     ③ 任何异常 ⇒ 放行且**不打断主线**；后端不可达 ⇒ 仍落账（分态不塌缩为 null）
  //   + F 组 A4 正判据：消费点存在 `dep.auditIndexLine(`、装配 `ingestAuditOf(`、I1 抽出形态 `writeDepsOf(`
  //   ⚠ **能力（是否够准）不在本件**：三模型真库对照（假阳性 qwen3.5:2b 25.0%@101ms /
  //     qwen3:8b 50.0%@987ms / v4.1-flash 15.0%@3315ms）由 `_memory/audit/consumer-compare.mjs` 回答
  //     （私人区，含真实记忆内容，不入公开树）。
  ['scripts/test-eval-ingest-audit.mjs'],
  ['scripts/test-model-config.mjs'],
  // S-P4d / S-P4e′-落点（**2026-09-20 round 9 补登记** · 守「跨形态消重的收益点」）：
  //   判因：`inject-dedup-probe.mjs` **文件早已存在且能跑（exit 0），但从未登记进 `CHECKS`**
  //     ⇒ 按仓规则 6「**未登记 = 等于没写**」，它从未在 `npm test` 里跑过（同族先例：
  //     `test-deepsleep-verdict` / `test-watermark-guard` / `test-atomic-write` 三件曾写了从不运行）。
  //   它守的正是 S-P4e′-落点 与 **S-P4d（AC-G.5 注入面回归保护）**的收益判据 —— 四条：
  //     ① 被消重的行**不在注入面**（0/1 条仍在 ⇒ 消重在**配额之前**生效，IR1 册五 E5）
  //     ② **额度被用上**（画像行 ≥ 6；旧状事后过滤 ⇒ 槽位空置、省 0 行）
  //     ③ 每条 skip 带 `by` 与 `sim`（可解释）
  //     ④ **关闭即逐字节回基线**（Buffer 级相等）—— 失败开放
  //   ⚠ 向量不可用 / 宿主端点不可达 ⇒ 该脚本自行降级；此处不额外判 skip（其输出已显式说明）。
  ['scripts/inject-dedup-probe.mjs'],
  ...
  // 「收益回流可触发性」（2026-09-23 · 消费链拟态落地方案 §5.2 · 圆桌会议 8 席 3 轮定稿）。
  //   守的是一类**会掩盖其他问题**的缺陷：判据**机制在线但信号已死** ——
  //   D5（09-22）后中层换向出口**结构性哑火**（`zeroGain` 恒 0 ⇒ `switchSource` 永假 ⇒ `mcl-switch` 不可达），
  //   而它在读数上与"一切顺利"**完全同形**、全程零告警（真机 D5 后 2570 行 judge 全 0）。
  //   三条判据（**锚结构量、禁墙钟**——实测墙钟窗会把"又哑了一天"藏进绿里）：
  //     ① R1 结构可触发性（slow 桶 `zeroGain` 取值数 ≥2 且 `≥2` 行 >0），**只看事件锚点之后**
  //        （全档混算会被 D5 前的 39 种取值掩盖成绿 —— 本件实测踩过，故按会议 §1 记账纪律分段并列）；
  //     ② R2 出口可触发性（锚点后 `mcl-switch` >0）；③ R3 回流配对率（防"回流没通、计数自己动了"）。
  //   ⚠ **今日应红**（缺陷在，且回流行尚未产生数据）⇒ 按 xfail **声明制**登记；
  //     转绿即 XPASS ⇒ 本件退 **1**（= 施工完成信号，**不是**回归失败）。
  //   R3 无数据时**判 skip**（exit 3）——「缺席 ≠ 干净」，不假装通过。
  ['scripts/check-yield-reflow.mjs', { xfail: true }],
  ['scripts/check-yield-reflow.mjs', '--selftest'],
