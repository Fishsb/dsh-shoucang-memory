...
/** 节数下限（防"合并标记"绕过）。
 *  **棘轮**：只许在**内容真的迁出**时下调，且必须写明原因（否则它就是"删标记"的后门）。
 *  变更史：15（U2-B 分区块时） → **11**（UI1 收尾 · 2026-09-15）——
 *    原因：U2 把 24 个 render 迁出 `body.js` 后，**多个节的内容整体搬走**，
 *    残留的空壳标记已被清理（不清则门形同虚设：0 行的节也"达标"）。现 11 节全部有实体内容。
 *  仍有效力：把 2 个节合并 ⇒ 10 < 11 ⇒ 立即报（"合并标记"式绕过依旧被堵）。 */
const BASELINE_SECTIONS = 11
/**
