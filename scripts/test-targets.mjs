...
console.log('== G. suiteAssemblyMatrix（装配矩阵单一实现）==')
{
  ...
  ok(sch.includes('suiteAssemblyMatrix') && sch.includes('suiteScan'),
    'scheduler 一侧：调用单一实现，并经 suiteScan 暴露（装入 composition root）')
  // G0 composition root 收尾（2026-09-13）：panel 不再经 `scheduler-share` 取矩阵，改由 root 的句柄盒显式传入；
  //   并把「三条惰性桥已全退役」钉成断言（lib 中不得再出现任何 `-share.js` 引用 —— 复现即 FAIL）。
  ok(/scheduler\.current/.test(pan),
    'panel 一侧从 composition root 句柄盒取矩阵（不再经 scheduler-share 桥）')
  const shareRefs = readdirSync(join(repoRoot, 'lib')).filter((f) => f.endsWith('.js'))
    .filter((f) => /-share\.js/.test(readFileSync(join(repoRoot, 'lib', f), 'utf8')))
  ok(shareRefs.length === 0, `三条惰性桥已全退役（lib 中无任何 -share.js 引用；实测 ${shareRefs.length} 件${shareRefs.length ? `：${shareRefs.join(',')}` : ''}）`)
}
