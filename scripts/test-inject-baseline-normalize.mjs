...
// A7（2026-09-21 补 · §13-P3）：**成长横幅的两个数字变** ⇒ 不报。
//   判因（实测）：门翻红时**首差落在字符 34** —— `本轮提存 7 条`→`0 条`、`标出 18 条（… 9 条）`→`10 条（… 1 条）`，
//   而本工具既定语义是「只守结构骨架，不守活的记忆内容」⇒ 这两个数字属活数据（同「热取前 N 条」先例）。
//   ⚠ 横幅**段标题**（`🧠 最近成长…`）与**行存在性**仍受骨架约束（删该行/改标题仍报，见 A8/B 方向）。
ok(normalize(base) === normalize(base.replace('本轮提存 7 条', '本轮提存 0 条').replace('标出 18 条未被真读（疑似召回面 9 条）', '标出 10 条未被真读（疑似召回面 1 条）')),
  'A7 成长横幅数字变 ⇒ 不报（同「热取前 N 条」先例）')
ok(normalize(base) !== normalize(base.replace('  标出 18 条未被真读（疑似召回面 9 条）\n', '')),
  'A8 成长横幅**整行被删** ⇒ 报（防归一化过宽致假绿）')
