...
const ctxQ = { agent: { session: { snapshotEvents: () => [{ type: 'user/message', data: { content: [{ text: '任务路径 流程经验' }], source: { kind: 'user' } } }] } } }
...
// A8：**来源过滤护栏**（P0 · 2026-09-13）—— 宿主注入块（MCL 慢通道材料 / 运行态快照 / 后台回执）同样以
//   `user/message` 下发，绝不能被当成"最近用户消息"去做召回（否则等于**用注入物召回它自己**，实测旁证：
//   两个不同问题召回出同一批 6 行）。判别口径 = `data.source.kind === 'user'`，与 `mcl.ts#captureFromEvent`、
//   `distill-activation.ts`（ACT-024 结构性去污染）**同一口径**；A8b 验「注入块在后面也不顶掉真用户消息」。
...
ok(gatedMixed.length > 0, `A8b 注入块排在真用户消息之后时仍取**真用户消息**（实测命中 ${gatedMixed.join(',') || '无'}）`)
