...
| T1-d **轮换** | ⏸ **用户裁决：暂不轮换**（2026-09-17） | **由用户拍板**（我按技能硬性安全门请示后，用户选择暂不轮换）。状态与依据：① 实测该账户 `403 INSUFFICIENT_BALANCE` ⇒ 密钥**当前无法调用**，风险暂为 0；② 自动化轮换须**杀掉 23 个 Chrome 进程**（会丢未保存工作）⇒ 代价 > 当前收益；③ **充值前处理即可**。触发条件写明：**一旦该账户充值或密钥出现在新位置 ⇒ 立即轮换**。<br>最短路径（零干扰）：登录 `api.dshapi.icu/dashboard` → 删旧密钥 → 建新 |
...
| # | 待办 | 状态 / 阻塞 |
|---|---|---|
| 1 | **T1 密钥轮换** | ⏸ **用户裁决「暂不轮换」**（2026-09-17）。已完成：有效性探测（**密钥有效**）· 明文副本清零（**4/4**）· 审计与备份。**触发条件**：该账户充值，或密钥出现在新位置 ⇒ 立即轮换（控制台 `api.dshapi.icu/dashboard`，删旧建新） |
