...
export function createBankApi(dep: BankDeps) {
  return {
    bankGitScript,
    runSelfCheck: (...a: Tail<Parameters<typeof runSelfCheck>>) => runSelfCheck(dep, ...a),
    bankSnapshot: (...a: Tail<Parameters<typeof bankSnapshot>>) => bankSnapshot(dep, ...a),
  }
}
...
const runSelfCheck = async (dep: BankDeps, trigger: 'deep-sleep' | 'timer' | 'manual'): Promise<{ verdict?: string; adjustments: string[] } | null> => {
  try {
  ...
  } catch (e) { dep.infra.log(`selfcheck(${trigger}) 失败（不影响主流程）：${String((e as Error).message).slice(0, 80)}`); return null }
}

/** v2 库 git 版本化快照（写后触发；失败静默——版本化是增强不是主流程依赖） */
const bankSnapshot = async (dep: BankDeps, label: string): Promise<void> => {
  if (dep.config.bankGit === false) return
  try {
