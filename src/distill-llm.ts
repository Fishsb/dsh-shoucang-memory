...
export function createLlmApi(d: LlmDeps) {
  return {
    validateProvider: (...a: Tail<Parameters<typeof validateProvider>>) => validateProvider(d, ...a),
    resolveLlm: (...a: Tail<Parameters<typeof resolveLlm>>) => resolveLlm(d, ...a),
    probeScriptPath,
    locateTranscript: (...a: Tail<Parameters<typeof locateTranscript>>) => locateTranscript(d, ...a),
    resolveWorkspace: (...a: Tail<Parameters<typeof resolveWorkspace>>) => resolveWorkspace(d, ...a),
  }
}
export type LlmApi = ReturnType<typeof createLlmApi>

// 蒸馏与深睡**共享**（两侧都写）⇒ 以对象引用传递，传值快照会让两侧计数脱钩
const validateProvider = (d: LlmDeps, ): void => {
  if (!(d.config.llmProvider && d.config.llmModel) || d.llmState.providerFailCount >= 2) return
  try {
    const llm = d.ctx.llm as any
...
const resolveLlm = (d: LlmDeps, sp: string, sm: string): { provider: string; model: string } | null => {
  ...
  if (d.config.llmProvider && d.config.llmModel) return { provider: d.config.llmProvider, model: d.config.llmModel }
  return null
}
...
const resolveWorkspace = async (d: LlmDeps, sid: string): Promise<string | null> => {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    try { file = await locateTranscript(d, sid) } catch { file = null }
    if (!file) d.log(`ws 反解: 转录定位失败 attempt=${attempt} sid=${sid.slice(0, 18)}`)
    if (file) {
      // ① 权威：转录首行 session.cwd
      try {
      ...
      } catch (e) { d.log(`ws 反解: cwd 探针异常 ${String((e as Error)?.message || e).slice(0, 80)}`) }
