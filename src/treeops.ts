...
function loadActivityStatus(memRoot: string): Map<string, { status: string; retired: boolean }> {
  const out = new Map<string, { status: string; retired: boolean }>()
  try {
    for (const l of readFileSync(join(memRoot, 'audit', 'activity.jsonl'), 'utf8').split(/\r?\n/)) {
      if (!l.trim()) continue
      try {
        const o = JSON.parse(l) as { f?: string; s?: string; status?: string; retired?: boolean }
        const f = String(o.f || '').replace(/^notes\//, '').replace(/\.md$/, '')
        const s = coreName(String(o.s || ''))
        if (f && s) out.set(`${f}::${s.toLowerCase()}`, { status: String(o.status || ''), retired: !!o.retired })
      } catch { /* 坏行跳过 */ }
    }
...
export async function applyForgetOps(
  memRoot: string,
  ops: ForgetOp[],
  hooksIn?: TreeOpsHooks,
): Promise<{ archived: number; kept: number; skipped: number }> {
  const hooks: TreeOpsHooks = hooksIn || { audit: () => { /* */ }, log: () => { /* */ } }
  ...
  for (const raw of list) {
    try {
      const action = String((raw && (raw as any).action) || '')
      const file = normalizeNotesFile((raw as any)?.file)
      const section = String((raw as any)?.section || '').trim()
      if (!file || !section) { record({ action, file: (raw as any)?.file, section, outcome: 'skipped', reason: 'file/section 非法' }); res.skipped++; continue }
      const core = coreName(section)
      if (!core) { record({ action, file: `notes/${file}`, section, outcome: 'skipped', reason: '核心名缺失' }); res.skipped++; continue }

      // keep：仅留痕（负真值），不动文件
      if (action === 'keep') {
        record({ action: 'keep', file: `notes/${file}`, section: core, reason: String((raw as any)?.reason || '').slice(0, 160) })
        res.kept++
        continue
      }
      if (action !== 'archive') {
      ...
      if (!lines) { record({ action, file: `notes/${file}`, section: core, outcome: 'skipped', reason: 'notes 文件缺失/不可读' }); res.skipped++; continue }
      const hit = matchSection(parseSections(lines), core)
      if (!hit) { record({ action, file: `notes/${file}`, section: core, outcome: 'skipped', reason: '小节未找到或匹配歧义' }); res.skipped++; continue }
      const sec = hit.sec
      // 守卫①：叶子节（防孤儿树枝）
      if (!sec.leaf) { record({ action, file: `notes/${file}`, section: core, outcome: 'skipped', reason: '非叶子节（含更深标题）——防孤儿树枝' }); res.skipped++; continue }
      // 守卫③（幂等）：已是 stub
      if (sec.body.some((l) => STUB_RE.test(l))) { record({ action, file: `notes/${file}`, section: core, outcome: 'skipped', reason: '已是归档 stub（幂等）' }); res.skipped++; continue }
      ...
      record({
        ...
        activity: { status: st.status, retired: st.retired }, orphan: !!(raw as any)?.orphan,
        reason: String((raw as any)?.reason || '').slice(0, 160),
      ...
      try {
        ...
        record({ action, file: `notes/${file}`, section: core, outcome: 'skipped', reason: `归档写失败: ${String((e as Error)?.message || e).slice(0, 60)}` })
        res.skipped++
        continue
      }
      ...
      safeLog(hooks, `forgetops: op 异常（跳过）: ${String((e as Error)?.message || e).slice(0, 160)}`)
      res.skipped++
    }
  }
