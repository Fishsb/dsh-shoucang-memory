...
export interface TreeOp {
  ...
  parts?: Array<{ title: string; start: string }>
}
...
function runNodeBin(args: string[], opts?: { cwd?: string; env?: Record<string, string>; timeout?: number }): Promise<RunResult> {
  return new Promise((resolve) => {
    ...
    const child = spawn(process.execPath || 'node', args, {
      cwd: opts?.cwd, env: env as NodeJS.ProcessEnv, windowsHide: true,
    })
...
export const coreName = (t: string): string => String(t || '').trim().replace(CORE_TAIL_RE, '').trim()
...
export function parseSections(ls: string[]): Section[] {
...
export function normalizeNotesFile(f: unknown): string | null {
 ...
 *
 *  ⚠ **容量开关的显式声明**（ADR-333 册三 · B3，2026-09-22）：本处是**结构维护路径**
 *  （treeOps 的 rename/merge ⇒ 整文件索引改写），语义上**不是知识写入**，不应被容量闸拦。
 *  ⇒ 显式置 `SHOUCANG_CAP_STRICT='0'`，与 `panel-inject.gateWrite`（人工路径）**同一取向**：
 *    凡"非自动写入"的路径，其容量语义由**本行代码显式声明**，不交给宿主环境碰运气。
 *  ⚠ 指针（exit 2）/格式（4）/凭据（5）三道门**照旧生效**——本行只关容量这一支。
 *  ⚠ 三个 `SHOUCANG_CAP_*` 的透传保留（脚本仍按正确阈值**计算并告警**，只是不阻断）。 */
async function runIndexGate(memRoot: string, target: string, tmp: string): Promise<'pass' | 'fail' | 'absent'> {
  ...
  const env: Record<string, string> = { MEMORY_ROOT: memRoot, SHOUCANG_CAP_STRICT: '0' }
...
export function safeAudit(hooks: TreeOpsHooks, o: Record<string, unknown>): void { try { hooks.audit(o) } catch { /* */ } }
export function safeLog(hooks: TreeOpsHooks, m: string): void { try { hooks.log(m) } catch { /* */ } }
...
export async function applyTreeOps(memRoot: string, ops: TreeOp[], hooksIn?: TreeOpsHooks): Promise<TreeOpsResult> {
  ...
  const archive = (o: Record<string, unknown>): void => {
    try {
      appendFileSync(ctx.archFile, envelope(o, `archive.${String((o as { op?: string; action?: string; kind?: string }).op || (o as { action?: string }).action || (o as { kind?: string }).kind || 'event')}`), 'utf8')
      ctx.archived++
...
// ═══ forgetOps（2026-09-11 · 认知对照 P0「主动遗忘」）═══
// 定位：`activity.ts` 每轮产出「遗忘候选」（cold 且 ≥90 天零命中），此前**无消费端**（只写 audit/*.md 无人读）
// → 容量单调增长，只能靠 write_gate 硬顶。本通道让模型对该清单做取舍，但**只允许 archive / keep，禁止直删**：
//   - archive = 该节**正文**移入 `notes/archive/<file>`，原位留 2 行 stub（**索引指针仍有效**、可一键恢复、容量真降）；
//   - keep    = 保留并留理由（负真值留痕，防下轮重复提议同一节）。
// 守卫（三条，任一不满足即跳过）：① 目标是叶子节（无更深标题，防孤儿树枝）；② activity.jsonl 中该节为 cold/retired
// （证明确是候选，防误伤热节）；③ 正文不得已是 stub（幂等）。
// 不变量与 treeops 同：归档可回滚 / 幂等 / tmp+rename 原子落盘 / 防御式不抛。

// 主动遗忘（ForgetOp / loadActivityStatus / sectionExists / openForgetArchive / applyForgetOps）
// 已于 2026-09-14 阶段 4 迁至 ./forgetops.ts —— 领域边界见 AGENTS.md「结构操作 + 主动遗忘」。
...
type ParsedSplit = { kind: 'split'; file: string; title: string; core: string; parts: Array<{ title: string; core: string; start: string }> }
type ParsedOp = ParsedRename | ParsedMerge | ParsedSplit
...
function parseOp(raw: TreeOp): ParsedOp | string {
  ...
  if (action === 'split') {
    ...
    if (!title) return 'split 缺 title'
    if (!TITLE_OK(title)) return 'split title 非法（禁 #/`/换行/指针语法字符）'
    const core = coreName(title)
    if (!core) return 'split title 核心名缺失'
    const rawParts = Array.isArray((raw as any).parts) ? (raw as any).parts : []
    if (rawParts.length < 2) return 'split parts < 2（拆一层至少 2 个子节）'
    if (rawParts.length > SPLIT_K_MAX) return `split parts > ${SPLIT_K_MAX}（§8.1 扇出目标 K，防横向膨胀）`
    const parts: ParsedSplit['parts'] = []
    const seen = new Set<string>()
    for (const p of rawParts) {
      const t = String((p && (p as any).title) || '').trim()
      const s = String((p && (p as any).start) || '').trim()
      if (!t) return 'split part 缺 title'
      if (!TITLE_OK(t)) return 'split part title 非法'
      const c = coreName(t)
      if (!c) return 'split part title 核心名缺失'
      if (seen.has(c.toLowerCase())) return 'split part 重名（核心名重复）'
      if (!s) return `split part「${t}」缺 start（边界锚=子节首行原文，逐字取自材料）`
...
export function matchSection(sections: Section[], kw: string): { sec: Section; exact: boolean } | null {
 ...
 */
const OP_SCOPE = { rename: [2, 3], merge: [2, 3], splitTarget: [2], splitConflict: [3] } as const

/** 按 op 取节范围（**唯一 filter 落点**；口径见 `OP_SCOPE`） */
export const scopeOf = (sections: Section[], op: keyof typeof OP_SCOPE): Section[] =>
  sections.filter((s) => (OP_SCOPE[op] as readonly number[]).includes(s.level))
...
async function runRename(ctx: Ctx, v: ParsedRename, archive: (o: Record<string, unknown>) => void): Promise<{ applied: number; skipped: number }> {
  ...
  // 只匹配 ## 与 ### 开头标题行（v1 口径）
  const scope = scopeOf(sections, 'rename')
 ...
 */
async function runSplit(ctx: Ctx, v: ParsedSplit, archive: (o: Record<string, unknown>) => void): Promise<{ applied: number; skipped: number }> {
  const { memRoot, hooks } = ctx
  const notePath = join(memRoot, 'notes', v.file)
  const lines = readLines(notePath)
  if (!lines) {
  ...
  const hit = matchSection(scopeOf(sections, 'splitTarget'), v.title)
  if (!hit) {
  ...
  for (const p of v.parts) {
    const found: number[] = []
    for (let q = sec.idx + 1; q < sec.end; q++) if (lines[q].trim() === p.start) found.push(q)
    if (found.length !== 1) {
      archive({
  ...
  const scopes = scopeOf(sections, 'splitConflict')
  for (const p of v.parts) {
  ...
  const out = lines.slice()
  for (let i = marks.length - 1; i >= 0; i--) out.splice(marks[i], 0, `### ${v.parts[i].title}`)
...
async function runMerge(ctx: Ctx, v: ParsedMerge, archive: (o: Record<string, unknown>) => void): Promise<{ applied: number; skipped: number }> {
  ...
  const sections = parseSections(lines)
  // S4Z：G-22 的真修 —— 范围口径**只在 `OP_SCOPE` 声明一处**（原先 rename/merge 各自手写 ⇒ merge 漏过滤）
  const mergeScope = scopeOf(sections, 'merge')
  const keep = matchSection(mergeScope, v.keepTitle)
  const drop = matchSection(mergeScope, v.dropTitle)
  if (!keep || !drop) {
