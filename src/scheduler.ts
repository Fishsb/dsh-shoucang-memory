...
/** schedulerShare 装配面。**单一实现**：此前 enableDistill 开/关两条分支各写了一份
 *  完全相同的对象（15 行 ×2），改一处漏一处就会让 panel /suite 与 /distill/config 读数漂移。 */
function schedulerShareApiOf(config: Config, llmModels: LlmModelsFn) {
  return {
...
function assembleMcl(ctx: Context, config: Config): void {
  try {
...
export function applyScheduler(ctx: Context, config: Config): void {
  ...
  schedulerShare.api = schedulerShareApiOf(config, llmModels)
  assembleMcl(ctx, config)
}
