...
export interface Config extends ProbeConfigFields, EvalConfigFields, ModelConfigFields {
  ...
  injectRelevance: boolean // U3 转正：注入相关性重排开关（panel 早已读取）
  injectFreshSlots: number // U3 转正：新鲜度保底槽位数
  // ═══ v2.2（ADR-130）层模型开关 ═══
  injectProfileRows: number // P 层画像行每档注入上限（缺省 3；0=关闭=回滚到「画像行不注入」）
  scoreWeights: string // 打分公式：'legacy'（缺省，现行 relevance+activity）| 'v2'（α_rel/α_imp/α_rec）
  shadowScore: boolean // 影子打分：并行计算新公式并写统一台账 type=score.shadow（不改排序）
  ...
  /** P2b（2026-09-13）：慢通道材料改挂 systemPrompt 段（缺省 false ＝ 现状零行为变化） */
  mclMaterialInSystem: boolean
  mclTopK: number // 慢通道指针条数（缺省 3）
  mclAudit: boolean // 审计流开关（写统一台账 knowledgeRoot()/audit/ledger.jsonl，type=mcl*）
}
...
export const Config: any = z.object({
  ...
  // ADR-333（2026-09-22 用户拍板「容量门不是硬拒绝门槛，做成开关功能，要可以关闭硬拒绝门槛功能」）。
  //   缺省 **false = 不阻断**（照写 + 落 `capacity-over` 留痕）。三条理由：
  //   ① 四条写入路径里**三条现状已是不阻断**（cap_memory / principles / 索引行）⇒ 与多数现状一致；
  //   ② 用户 2026-09-16 已就索引行判定「直接拒绝不符合意图，提醒就可以」⇒ 本条是**推广**该判定；
  //   ③ 取 true 等于"修完默认仍坏"（USER.md 现 98.9%/2971 字符，照旧一条写不进）。
  //   ⚠ 只关**容量**这一支：指针（exit 2）/格式（4）/凭据（5）三道门**不受影响**，照旧硬拒。
  capacityEnforce: z.boolean().default(false).description('容量门是否阻断写入（缺省 false=超限照写并留痕 capacity-over；true=超限拒写，恢复改造前画像行为）'),
 ...
 */
function applySuiteConfigFile(config: Config, warn?: (m: string) => void): void {
  try {
    ...
    try { allowed = new Set(Object.keys(Config({}) as object)) } catch { return /* schema 自检异常 ⇒ 不合并（纯缺省最安全） */ }
    for (const [k, v] of Object.entries(raw)) {
      if (v === undefined) continue
      if (!allowed.has(k)) { warn?.(`[shoucang] scheduler.json 未知键已忽略：${k}`); continue }
      try {
        const probe = Config({ [k]: v } as any) as Record<string, unknown>
        if (k in probe) (config as any)[k] = probe[k]
        else warn?.(`[shoucang] scheduler.json 键 ${k} 未被 schema 接受，已忽略`)
      } catch (e) {
...
function recallTool(config: Config) {
  return   defineTool({
...
function associateTool(config: Config) {
  return defineTool({
    name: 'shoucang_associate',
    description:
      '守藏联想候选（**只提案不判**）：在库内 notes 小节之间用**向量**找「异域同构」——语义高度相近、来自**不同文件**、且**词面几乎不重叠**（即同一道理出现在两个领域）。返回候选对 + 证据（相似度 / 共词），**由调用方模型判断是否真成立**并决定是否落环（association-ring 记账）。适用：收尾沉淀、跨主题回顾、卡住时找「别处也讲过类似的事」。',
    parameters: {
    ...
    async execute(args: any) {
      ...
      if (!s.proposals.length) return `shoucang_associate：${s.reason || '无候选'}`
...
function capabilitiesTool(ctx: Context, config: Config) {
  return   defineTool({
...
function distillOptionsOf(config: Config) {
  return {
  /* ⚠ 2026-09-25：`'node'` → `process.execPath`。该值经 distillOptionsOf 下传，最终在
   *   `distill-proc.ts:22 spawn(nodeBin || 'node', …)` 使用 ⇒ 裸名在系统 node 缺失时
   *   抛 ENOENT，**整条蒸馏链静默不跑**（实测本机正是该状态）。宿主自己即 node 进程。 */
  ...
  // ADR-333：容量门开关**必须在此显式映射**（本件 :671-673 已记载该形态的坑：
  //   「schema 有 ≠ 运行时 config 有 —— 新增键必须两处都加，否则功能接线了却开不了」）。
  capacityEnforce: config.capacityEnforce,
...
function schedulerShareApiOf(config: Config, llmModels: LlmModelsFn): SchedulerApi {
  return {
...
function assembleMcl(ctx: Context, config: Config, comp?: CompositionHandles): void {
  try {
    ...
    if (mcl && comp) comp.mcl.current = { status: () => mcl.status() }
  } catch (e) {
    ctx.logger?.warn?.(`[shoucang] MCL 装配失败（认知环跳过）：${String((e as Error)?.message || e).slice(0, 120)}`)
  }
}

export function applyScheduler(ctx: Context, config: Config, comp?: CompositionHandles): void {
