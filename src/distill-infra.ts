...
const audit = (d: InfraDeps, o: Record<string, unknown>): void => { try { mkdirSync(dirname(d.ledgerFile), { recursive: true }); appendFileSync(d.ledgerFile, envelope(o, `audit.${String((o as { kind?: string }).kind || 'event')}`)) } catch { /* 静默 */ } }
