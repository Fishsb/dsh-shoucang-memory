...
//
// ⚠⚠ **本件当前只做"候选与计划"这一半，不做不可逆释放**（遵 [原则] 契约须描述现状）：
//   一次真正的释放必须同时具备三件保护，而它们**尚未实现**：
//     ① **语义复核**（字面自足 ≠ 语义自足；须交模型独立判一次）
//     ② 执行侧**归档可回滚**（复用 `forgetops` 的 archive+stub，不另造）
//     ③ **未自足者零释放**（那条判据已在此实现）
//   ⇒ 故本件**只产出计划**（`planRelease`），由调用方记入审计可见化；**不写任何文件**。
//   把"计划"与"执行"分开还有一个好处：**计划可被反复观测**（每轮深睡的候选数变化即趋势信号），
//   而执行是不可逆的 —— 先让计划跑够，再谈执行。
 ...
 */
export function planRelease(memRoot: string): ReleasePlan {
  let text = ''
  try { text = readFileSync(join(memRoot, JUDGMENT_FILE), 'utf8') } catch {
  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    if (!selfContained) { blocked++; continue } // 判据③：未自足**永不入候选**
 ...
 * ⚠ **语义门仍未实现**（字面自足 ≠ 语义自足）⇒ `applyRelease` **不传语义清单即零释放**（fail-closed）。 */

/** 从指针解析归档目标：`→ notes/<file> §<section>` ⇒ `{ file, section }`（解析不出 ⇒ null）。 */
export function parsePointerOf(row: string): { file: string; section: string } | null {
...
export interface ReleaseOp { op: 'archive'; file: string; section: string; action: 'archive'; why: string }

/**
 * 把释放计划转成 **forgetOps 同形 op 列表**（**只提案，不执行**）。两道门在本件内：
 *   · **无指针/解析不出 ⇒ 不产 op**（指针是释放的唯一入口）；② **未自足 ⇒ 不产 op**（判据③再断言一次）。
 */
export function releaseOpsFromPlan(plan: ReleasePlan): { ops: ReleaseOp[]; skipped: number; note: string } {
  const ops: ReleaseOp[] = []
  let skipped = 0
  for (const c of plan.candidates) {
    const p = parsePointerOf(c.row)
    if (!p) { skipped++; continue }
    if (!(c.descChars >= RELEASE_MIN_ACTION_CHARS)) { skipped++; continue }
...
export async function applyRelease(
  memRoot: string,
  proposed: readonly ReleaseOp[],
  semanticApproved: readonly string[],
  applyOps: (ops: ReadonlyArray<{ op: string; file: string; section: string; action: string }>) => Promise<{ archived: number; kept: number; skipped: number }>,
  opts?: { maxPerRun?: number },
): Promise<{ released: number; skipped: number; reasons: string[] }> {
  const reasons: string[] = []
  const max = Math.max(1, Number(opts?.maxPerRun) || RELEASE_MAX_PER_RUN)
  if (!semanticApproved.length) return { released: 0, skipped: proposed.length, reasons: ['语义复核清单为空 ⇒ **零释放**（语义门未实现，fail-closed）'] }
  const fresh = planRelease(memRoot)
  const approved = new Set(semanticApproved.map((s) => String(s).trim()))
  const pass: Array<{ op: string; file: string; section: string; action: string }> = []
  let skipped = 0
  for (const o of proposed.slice(0, max)) {
    ...
    if (!row) { skipped++; reasons.push(`${o.file} §${o.section}：**当下复核未通过**（不再自足/指针消失）⇒ 不释放`); continue }
    if (!approved.has(row.row.trim())) { skipped++; reasons.push(`${o.file} §${o.section}：**不在语义复核通过清单** ⇒ 不释放`); continue }
    pass.push(o)
  }
  if (!pass.length) return { released: 0, skipped, reasons }
...
export function buildSemanticReviewRequest(plan: ReleasePlan, opts?: { offset?: number; limit?: number }): string {
  ...
  lines.push('')
  for (const { c, i } of batch) lines.push(`- [${i}]（该行说明段 ${c.descChars} 字）${c.row.slice(0, 200)}`)
  lines.push('')
...
export function buildSemanticReviewBatches(plan: ReleasePlan, size = 10): string[] {
  const n = Math.max(1, Number(size) || 10)
  const out: string[] = []
  for (let off = 0; off < plan.candidates.length; off += n) out.push(buildSemanticReviewRequest(plan, { offset: off, limit: n }))
  return out.length ? out : [buildSemanticReviewRequest(plan, { offset: 0, limit: n })]
}

export interface SemanticReview { i: number; usable: boolean | null; why: string; quote: string }

/** 严格解析（**整批原子**：结构不合法 ⇒ 整体 null —— 半批会静默改变"通过清单"）。
 *  ⚠ `quote` **缺失/过短不算结构错** ⇒ 记空串，交由 `semanticApprovedRows` **按项判未判**
 *    （**接地失败是逐条事实，不是整批结构错**；整批作废会白烧整批判定）。 */
export function parseSemanticReview(text: string): SemanticReview[] | null {
  const m = /\[[\s\S]*\]/.exec(String(text || ''))
  if (!m) return null
  let arr: unknown
  try { arr = JSON.parse(m[0]) } catch { return null }
  if (!Array.isArray(arr) || !arr.length) return null
  const out: SemanticReview[] = []
  for (const x of arr) {
    const o = x as { i?: unknown; usable?: unknown; why?: unknown; quote?: unknown }
    if (typeof o?.i !== 'number' || !Number.isFinite(o.i)) return null
    const u = o.usable
    if (!(u === true || u === false || u === null)) return null
    out.push({ i: o.i, usable: u as boolean | null, why: String(o.why || '').slice(0, 80), quote: String(o.quote || '') })
  }
  return out
}
 ...
 */
export function semanticApprovedRows(plan: ReleasePlan, review: readonly SemanticReview[]): { approved: string[]; rejected: number; unjudged: number; ungrounded: number } {
  const byIdx = new Map(review.map((r) => [r.i, r]))
  const approved: string[] = []
  let rejected = 0, unjudged = 0, ungrounded = 0
  plan.candidates.forEach((c, i) => {
    const r = byIdx.get(i)
    if (!r) { unjudged++; return }
    const q = String(r.quote || '').trim()
    if (q.length < 4 || !c.row.includes(q)) { ungrounded++; unjudged++; return }   // 接地失败 ⇒ 未判（不通过）
    if (r.usable === true) approved.push(c.row)
    else { if (r.usable === null) unjudged++; else rejected++ }
  })
...
export function intersectApprovals(a: readonly string[], b: readonly string[]): string[] {
  const setB = new Set(b.map((x) => x.trim()))
  const seen = new Set<string>()
  const out: string[] = []
  for (const x of a) { const k = x.trim(); if (setB.has(k) && !seen.has(k)) { seen.add(k); out.push(x) } }
  return out
}
...
export function stableSetHashes(rows: readonly string[]): string[] {
  const out = new Set<string>()
  for (const r of rows) out.add(createHash('sha1').update(String(r).trim(), 'utf8').digest('hex').slice(0, 8))
  return [...out].sort()
}

/** 两个集合指纹的 **Jaccard 相似度**（|交| / |并|；两者皆空 ⇒ 1.0「一致地没有」）。 */
export function jaccardOfHashes(a: readonly string[], b: readonly string[]): number {
