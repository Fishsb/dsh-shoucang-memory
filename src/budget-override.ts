...
export const BUDGET_OVERRIDE_KEYS = ['injectBudgetChars', 'injectLevelCaps', 'injectSituationBudgetChars'] as const
...
export interface ResolvedNumber { value: number; clamped: boolean; source: 'override' | 'registry' }
 ...
 */
export function resolveBudgetNumber(key: BudgetOverrideKey, override: unknown, registry: unknown): ResolvedNumber {
 ...
 */
export function resolveLevelCaps(
  override: unknown,
  registry: Record<string, unknown> | undefined,
): { caps: Record<string, number>; clamped: string[] } {
  ...
  for (const k of Object.keys(reg)) {
    const r = num(reg[k])
    const o = num(ov[k])
    if (o === null) { if (r !== null) out[k] = Math.max(lo, Math.min(hi, r)); continue }
    if (o < lo || o > hi) clamped.push(k)
 ...
 */
export function budgetOverrideOf(suite: Record<string, unknown> | null | undefined): Partial<Record<BudgetOverrideKey, unknown>> {
  const out: Partial<Record<BudgetOverrideKey, unknown>> = {}
  const s = suite || {}
  for (const k of BUDGET_OVERRIDE_KEYS) if (k in s) out[k] = s[k]
  return out
}
 ...
 * **纯函数**：只读入参，零 IO、零抛错。
 */
export function resolveSupplyBudget(
  suite: Record<string, unknown> | null | undefined,
  injection: Record<string, unknown> | null | undefined,
): ResolvedSupplyBudget {
  const ov = budgetOverrideOf(suite)
  const inj = injection || {}
  const clamped: string[] = []

  const totalRes = resolveBudgetNumber('injectBudgetChars', ov.injectBudgetChars, inj.budgetChars)
  if (totalRes.clamped) clamped.push(`injectBudgetChars→${totalRes.value}（原 ${String(ov.injectBudgetChars)}）`)

  const capsRes = resolveLevelCaps(ov.injectLevelCaps, inj.levelCaps as Record<string, unknown> | undefined)

  const sitCfg = (inj.situation && typeof inj.situation === 'object') ? inj.situation as Record<string, unknown> : {}
  const sitRes = resolveBudgetNumber('injectSituationBudgetChars', ov.injectSituationBudgetChars, sitCfg.budgetChars)
  if (sitRes.clamped) clamped.push(`injectSituationBudgetChars→${sitRes.value}（原 ${String(ov.injectSituationBudgetChars)}）`)
