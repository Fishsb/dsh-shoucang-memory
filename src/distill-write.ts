...
const writeDispatch = async (dep: WriteDeps, sid: string, out: any, route: string, workspace: string | null): Promise<{ added: number; rejected: number; failed: number; undigested: number; needsAnchor: number; pairedSkipped: number; items: Array<Record<string, string>>; targetLib: string }> => {
  ...
  if (route === 'memory') {
    ...
    const fileTally: Record<string, { attempted: number; written: number; rejected: number }> = {}
    const bump = (f: string, kind: 'attempted' | 'written' | 'rejected'): void => {
    ...
    for (const a of appends) {
      ...
      if (r.status === 0) { added++; bump(String(a.target), 'written') }
      else {
    ...
    for (const ni of newIndex) {
      ...
      if (!ni || !ni.line) { markRejected('index-baditem', ni?.target, '', '字段缺失（line 为空）'); bump(t, 'attempted'); bump(t, 'rejected'); continue }
      if (!gate(t)) { bump(t, 'attempted'); bump(t, 'rejected'); continue }
      ...
      //   判据 = 指针的 `notes/<file>` 与**首段小节**配对（保守优先）。
      {
        ...
        if (unpaired.length) {
          ...
          pairedSkipped++
          bump(t, 'attempted'); bump(t, 'rejected')
