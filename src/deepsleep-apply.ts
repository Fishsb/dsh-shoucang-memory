 ...
 */
const gateReasonOf = (status: number): string =>
  status === 1 ? '超限=本轮拒收（容量门 enforce=on；收敛由深睡树整理 consolidateTree 负责）'
    : status === 2 ? '指针悬空/未注册'
      : status === 4 ? '行格式违规'
        : status === -1 ? '门禁执行异常'
          : `gate exit=${status}`
 ...
 */
const readPrincipleOrigin = (principlesPath: string, headerOf: Record<string, string>): { content: string; disk: string | undefined } => {
  try {
...
const principlesTmp = `${principlesPath}.tmp-${process.pid}-${Date.now().toString(36)}`
  ...
  if (g2.ok) return commitAccepted(principlesPath, principlesTmp, acceptedItems, attempted, skipped, rejectedLines, log, originOnDisk)
 ...
 */
const commitWithCas = (tmpPath: string, targetPath: string, baseline: string | undefined): { ok: boolean; err?: string } => {
  let cur: string | undefined
  try { cur = readFileSync(targetPath, 'utf8') } catch { cur = undefined }
  if (cur !== baseline) {
    try { unlinkSync(tmpPath) } catch { /* tmp 唯一名，清理失败无害 */ }
...
function commitAccepted(
  ...
  attempted: number, skipped: number, rejectedLines: string[], log: (m: string) => void, baseline: string | undefined,
