...
export async function consolidateTree(d: TreeDeps, memRoot: string): Promise<{ idxExact: number; idxSem: number; secMerged: number; lineDedup: number; archived: number }> {
  ...
  const archive = (o: Record<string, unknown>): void => {
    try {
      ...
      appendFileSync(archFile, envelope(o, `archive.${String((o as { op?: string; action?: string; kind?: string }).op || (o as { action?: string }).action || (o as { kind?: string }).kind || 'event')}`), 'utf8')
      archived++
        ...
        try {
          ...
          for (const raw of lines) {
            ...
            for (const [nf, chain] of mergesByFile) {
              const upd = rewriteRowPointersByChain(cur, nf, finalChain(chain))
              if (upd !== null) cur = upd
            }
            if (cur !== raw) {
              fileChanged = true
...
// 核心名 / 双向包含：**单一实现**（S4Z 收敛）—— 此处原为 `treeops` 的**第二份副本**
//   （正则字面相同但各自手写；根因是 `treeops#biContains` 当时**模块私有未导出** ⇒ 用方只能复制）。
//   现 `treeops` 已导出二者，本件 import 复用（L4 → L2，高依赖低）。

/** 单行索引指针改写（原为 consolidateTree 内闭包，提到模块级以降单函数跨度）。 */
// S4Z（2026-09-14）改名：原名 `rewriteRowPointers` 与 `treeops#rewriteRowPointers` **同名不同义**
//   （此处按 `finals` **批量**改写；treeops 那处是**单对** old→new）⇒ 由 `audit-impl-drift.mjs` 发现，
//   改名消除混淆（避免读者以为两处是同一实现）。
export const rewriteRowPointersByChain = (raw: string, nf: string, finals: Array<{ old: string; next: string }>): string | null => {
