/**
 * panel-eval.ts — 「可配置评估通道」面板面（M1 · 2026-09-21 · ACT-283）
 *
 * ## 为什么单独成件（**门禁指路，不是我的偏好**）
 * 初版把这两条路由与 handler 直接加在 `panel-inject.ts` ⇒ 该件实测 624 行 ≥ 600，
 * `check-module-growth` / `check-pane-sections` 同时报：
 *   「**[未登记] `src/panel-inject.ts` 实测 624 行 ≥ 600 ⇒ 必须登记进 FREEZE 基线
 *     （新功能应落新模块，而不是堆大旧模块）**」。
 * 抬基线属 R3（须用户拍板）；而本仓既有出路就是**按领域接缝切分面板面**——
 *   `panel-config`（根目录+配置）· `panel-memory`（记忆只读）· `panel-observe`（观测/判据）·
 *   `panel-arch`（架构观测）· `panel-inject`（注入+向量+写回）。本件补齐 **eval 领域**，同族切法。
 *
 * ## 职责
 * 评估通道的**配置读写**与**连通性测试**（两条路由）。**只读**：本件不落库、不写账
 *   （判定写账属 M2）；出网闸由 `eval-channel#evaluate` 内部先过，本件**不绕过**。
 * ⚠ 与 `/embed/config` 同口径：白名单补丁（`required: []`）、GET 回 **persisted + effective + isDefault**。
 */
...
import { evalOptionsOf, EVAL_TIERS, type EvalConfigFields } from './eval-config.js'
...
import { EVAL_CONFIG_KEYS as EVAL_CONFIG_KEYS_REF, contractFor } from './panel-contract.js'
...
const EVAL_CONFIG_KEYS = ['evalEnabled', 'evalBaseUrl', 'evalModel', 'evalApiKeyEnv', 'evalTier', 'evalEgressAllow'] as const
/** 编译期同集合断言：契约导出新增键而此处漏加 ⇒ typecheck 失败（不靠人记） */
type _EvalKeysEqContract =
  (typeof EVAL_CONFIG_KEYS)[number] extends (typeof EVAL_CONFIG_KEYS_REF)[number]
    ? ((typeof EVAL_CONFIG_KEYS_REF)[number] extends (typeof EVAL_CONFIG_KEYS)[number] ? true : never)
    : never
const _evalKeysExhaustive: _EvalKeysEqContract = true
void _evalKeysExhaustive

const validateEvalConfig = (patch: Record<string, unknown>): string | null => {
  if ('evalEnabled' in patch && typeof patch.evalEnabled !== 'boolean') return 'evalEnabled must be boolean'
  if ('evalBaseUrl' in patch && typeof patch.evalBaseUrl !== 'string') return 'evalBaseUrl must be string'
  if ('evalModel' in patch && typeof patch.evalModel !== 'string') return 'evalModel must be string'
  if ('evalApiKeyEnv' in patch && typeof patch.evalApiKeyEnv !== 'string') return 'evalApiKeyEnv must be string'
  if ('evalTier' in patch && !(EVAL_TIERS as readonly string[]).includes(String(patch.evalTier))) return `evalTier must be one of ${EVAL_TIERS.join('|')}`
  if ('evalEgressAllow' in patch && typeof patch.evalEgressAllow !== 'boolean') return 'evalEgressAllow must be boolean'
  return null
}

/** 评估通道**有效配置**（落盘 ∪ 缺省）—— **单一实现**（沿 `effectiveEmbed` 的教训：避免两处各写一份缺省） */
const effectiveEval = (p: Record<string, unknown>): EvalConfigFields => evalOptionsOf({
  evalEnabled: p.evalEnabled === true,
  evalBaseUrl: String((p as { evalBaseUrl?: unknown }).evalBaseUrl || 'http://127.0.0.1:11434/v1'),
  evalModel: String((p as { evalModel?: unknown }).evalModel || 'qwen3:8b'),
  evalApiKeyEnv: String((p as { evalApiKeyEnv?: unknown }).evalApiKeyEnv || 'EVAL_API_KEY'),
  evalTier: (EVAL_TIERS as readonly string[]).includes(String(p.evalTier)) ? (p.evalTier as EvalConfigFields['evalTier']) : 'local',
  evalEgressAllow: p.evalEgressAllow === true,
})

/** 配置读写（GET 回 persisted/effective/isDefault；POST 白名单补丁） */
async function evalConfigRoute(d: EvalPanelDeps, req: IncomingMessage, res: ServerResponse): Promise<void> {
try {
  if (req.method === 'POST') {
    const body = (await readBody(req).catch(() => ({}))) as Record<string, unknown>
    const patch: Record<string, unknown> = {}
    for (const k of EVAL_CONFIG_KEYS) if (k in body) patch[k] = body[k]
    if (!Object.keys(patch).length) return sendJson(res, 400, { error: 'no-eval-keys' })
...
function appendEvalDecision(rec: Parameters<typeof decisionEventOf>[0]): void {
  try {
...
async function evalTestRoute(d: EvalPanelDeps, _req: IncomingMessage, res: ServerResponse): Promise<void> {
try {
  const cfg = effectiveEval(d.suite.read() as Record<string, unknown>)
...
async function evalStatsRoute(d: EvalPanelDeps, req: IncomingMessage, res: ServerResponse): Promise<void> {
try {
  ...
  try {
    ...
    if (Number.isFinite(n) && n >= 0) want = Math.min(100, Math.max(0, Math.trunc(n)))
  } catch { /* 缺省 20 */ }
  let lines: string[] = []
  try { lines = readLedgerVolumes(join(knowledgeRoot(), 'audit', 'ledger.jsonl')) } catch { lines = [] }
...
export function registerEvalRoutes(d: EvalPanelDeps): void {
