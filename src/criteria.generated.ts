...
export const WIRING: { note: string; why: string; entryOk: readonly string[]; typeOnlyOk: readonly string[]; pending: ReadonlyArray<{ name: string; until: string; reason?: string }>; pendingNote?: string; unreachableValues?: readonly string[]; unreachableValuesNote?: string } = {
  ...
  "unreachableValuesNote": "**`evaluateL0` 不可达面**（`check-l0-conflict-wiring` ② 的事实源）——两类，**均属「声明了但无人产生」**：<br>【一类·入参不可达（2026-09-20）】判定口径：入参若**在所有调用点都不被传**，则它产生的取值（`days30>=2 ⇒ stability='cross-day'` · `sessions>=2 ⇒ reuse='cross-project'`）**结构性不可达**。实测：全仓 `evaluateL0` **只 1 处调用**（`distill-agent.ts`），从不传 `days30` / `sessions`；两者数据源**现成存在**（`activity.ts:29/174`），但**蒸馏调用点未接入活性表**（改动面大于本轮）⇒ 如实登记。**until**：接入后**须删除本表对应项**。<br>【二类·取值无产生式（**2026-09-21 新增** · `OPEN-ITEMS §0p`）】**6 个取值在可执行代码里零产生路径**：`reuse` 的 `cross-task`/`session-only` · `generality` 的 `contract-fact`/`detail` · `stability` 的 `same-day-repeat` · `conflict` 的 `coexist`。⚠ **这不是「有意不可达」，而是断链被显式登记** —— 门此前一直**假绿**：`produces()` 的正则含「冒号后跟空白」那一路匹配，会命中 **`CONF` 常量表**（`coexist: l0Pick('conflict','coexist')`）与**类型声明行**，于是「声明了但无人产生」照样判绿（`distill-agent.ts:435` 的注释**早已写着**「`coexist`/`supersede` 永不产生」）。**2026-09-21 已修门**（三类声明态中和：类型/接口行 · `l0Pick(...)` 调用 · `new Set([...])` 白名单），修后**门如实报出这 6 项** ⇒ 据「登记非豁免，是让断链可见」登记于此。<br>⚠ **残留语义问题（未决 · 属 L0 语义变更，须拍板）**：`JUDGEMENT_VALUES` 的 prompt **要求模型分清 `coexist`（与既有并存）与 `supersede`（取代既有）**，而 `evaluateL0` **只能算出 `none`/`supersede`** ⇒ **模型判了 `coexist` 也无处落**（§0p 原话：「判了也不落库」）。修它=**给 L0 冲突语义补一条产生路径**（行为变更）⇒ 不在本轮范围。<br>**until**：本表任一项取得产生路径后**须删除该项**（棘轮只许收紧）。\n\n【✅ **2026-09-21 D2 收口：`coexist` 已取得产生路径，已从本表删除**（此行留痕以示棘轮确实只许收紧）】判因（真机 + 读码双证）：`coexist` 无产生路径，且**更坏的是模型判了也会被抹掉** —— 调用点（`distill-agent.ts`）已把模型输出**按三义校验**（`CONFLICT_VALUES` 含 `coexist`），随后却只传 `supersedes: conflictNorm === 'supersede'` ⇒ **`coexist` 被静默压成 `none`**（声明三义、实现两义、第三义静默降级）。修法：`L0Input` 增可选 `conflict`（**原始三义**），`evaluateL0` 按「显式优先、布尔兜底」取值 ⇒ `coexist` 可达 · 旧调用方（只传 `supersedes`）**零迁移、取值逐字不变**（实测：`supersedes=true/false` ⇒ `supersede/none`；`conflict=coexist` ⇒ `coexist` + basis `l0.conflict.coexist`）。⚠ 真机 `coexist` 产量现为 **0**（历史从未产生）⇒ 这是「**通路已建、样本未到**」，与「断链」已可分辨（前者 basis 会出现 `l0.conflict.coexist`）。"
}
...
export interface ContentTypeConsumer { layer: string; criterion: string }
...
export interface ContentTypeVocab { note?: string; layer: string[]; timing: string[]; criterion: string[]; budget: string[]; memClass: string[]; reserved: { criterion: string[]; budget: string[] } }
export const CONTENT_TYPES: { note?: string; vocab: ContentTypeVocab; types: Record<string, ContentType>; structural: Record<string, ContentTypeStructural> } = {
  "vocab": {
