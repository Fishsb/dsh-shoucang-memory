...
export interface HooksDeps {
  ...
  agent: AgentApi
  st: DistillState
  ds: { sessions: Map<string, any>; getDeepSleepStatus(): any }
  ctx: any
  config: any
  kRoot: string
}
...
const sweepBacklog = async (dep: HooksDeps, ): Promise<void> => {
  try {
    ...
    try { await dep.write.flushDeferCards() } catch { /* 回流失败不阻断扫尾 */ }
    ...
    for (const a of roots) {
      try {
        ...
        const sid = a.id
        if (dep.parent.hasActiveSubagents(sid)) continue // 子代理在跑：任务未完，扫尾勿抢蒸（2026-09-10）
        if (dep.st.distilling.has(sid)) continue
        const rec = dep.ds.sessions.get(sid)
        if (rec) {
          if (rec.state === 'running' || rec.state === 'probing' || rec.state === 'suspect') continue
          if (rec.lastEndAt && Date.now() - rec.lastEndAt < dep.config.idleWakeMs) continue // 仍在宽限期，等 idle 定时器
        }
        ...
        if (maxSeq > lastSeq) {
          ...
          if (dep.write.claimHeld(sid)) { continue } // 在途，其他 fiber 已接管
          dep.infra.log(`sweep: ${dep.infra.sidShort(sid)} 水位 ${lastSeq}→${maxSeq} 有未消化增量，补蒸馏`)
          void dep.agent.distillAgent(a).catch(() => { /* distillAgent 内部已兜底 */ })
        } else {
          try { unlinkSync(join(dep.kRoot, 'audit', 'claims', sid + '.json')) } catch { /* 无 claim 可清 */ }
        }
      } catch { /* 单会话扫尾失败静默 */ }
    }
  } catch { /* 扫尾零抛出 */ }
