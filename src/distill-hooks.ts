...
// S-P4（2026-09-20）：定时自检「到期」判据（持久事实；自 deepsleep-core 按领域接缝抽出）
import { dueSelfCheck } from './trigger-plan.js'
  ...
  try {
    ...
    for (const a of roots) {
      try {
        ...
        if (rec && (rec.state === 'running' || rec.state === 'probing' || rec.state === 'suspect')) continue
        ...
        const quiet = quiescenceOf({ ctx: dep.env.ctx, hasActiveSubagents: (x: string) => dep.dom.parent.hasActiveSubagents(x) }, a)
        ...
        if (!adm.run) {
          ...
          if (adm.reason !== 'no-increment') dep.io.infra.audit({ sid, kind: 'distill-skip', reason: adm.reason, fclass: 'admission', evidence: adm.evidence })
          if (adm.reason === 'no-increment') { try { unlinkSync(join(dep.io.kRoot, 'audit', 'claims', sid + '.json')) } catch { /* 无 claim 可清 */ } }
          continue
        }
        if (adm.reason === 'unknown-quiescence-fallback') dep.io.infra.audit({ sid, kind: 'distill-skip', reason: adm.reason, fclass: 'admission', evidence: adm.evidence })
...
//   ★ S-P4（2026-09-20）：节拍改由**持久事实**判定 —— 原实现每次装配都新设「启动后 3 分钟首跑」，
//   与 6h 周期并联 ⇒ 热重载是常态（09-18 单日 28 次）时**每挂载必跑一次**
//   （实测 09-19 十次挂载 → 十次自检，间隔恒 179s）。现「到期」由 `selfcheck-latest.json` 的 mtime
//   （自检自身每次覆盖写）判定，定时器只当**唤醒节拍**：启动 3 分钟后探一次，此后每巡检周期探一次。
ctx.effect(() => {
  ...
  const intervalMs = Math.max(30 * 60 * 1000, hours * 3600 * 1000)
  const latest = join(dep.io.kRoot, 'audit', 'selfcheck-latest.json')
  const run = (): void => {
    try {
      let lastMs = 0
      try { lastMs = statSync(latest).mtimeMs } catch { /* 从未跑过 ⇒ 0 ⇒ due */ }
      const due = dueSelfCheck(Date.now(), lastMs, intervalMs)
      if (!due.due) { dep.io.infra.log(`selfcheck: 未到点（${due.reason}${due.sinceMs === null ? '' : ' · 距上次 ' + Math.round(due.sinceMs / 60000) + 'min'}）`); return }
      void dep.dom.bank.runSelfCheck('timer')
    } catch { /* 节拍零抛出 */ }
  }
  const t0 = setTimeout(run, 3 * 60 * 1000)
