...
export interface HotMemory { build(query?: string): string; invalidate(): void; supplyUsage(): SupplyUsage }
 ...
 * 把候选集交给 `assembleSupply` 算账，返回 `SupplyUsage` + 情境块；**其余槽的 blocks 不进注入面**
 * （⇒ 除情境槽外，注入文本逐字节不变）。
 * 阶段 3（2026-09-14）追加：额外把装配器口径的渲染文本放进 `usage.assemblerText`（只读诊断）。
 *   注入面**仍只并入情境块**——加的是取证面，不是切换。
 * 边界纪律（与 `supply-assembly` 头注一致）：本件**不重排、不打分、不选行** —— 候选集与顺序由调用方给定；
 *   情境候选的顺序由 `ring-supply` 给（情境命中 → 环优先级 → due → 新鲜），本件不插手。
 * 预算口径**镜像活路径**（调用方传 cap*），否则账与真实裁切不可比。
 *
 * ✅ **账已是真实裁切**（S4R/R2 · 2026-09-14 起）：`real` 参数直接吃主路径自己的 `clampLines` 结果 ⇒
 *   `kept` = 实际保留行数、`droppedRows` = **实际被丢弃的行内容**（逐条可比）。
 *   `dropped`（原因标签）仍来自装配器口径，**仅供对照** —— 两者数量**故意可能不同**，
 *   差值本身就是"旧近似值偏差多大"的度量（实测：装配器报 **6** 条，主路径真实只丢 **1** 条）。
 *   **R1+R2 后**：渲染与账均已收敛；装配器不再只是"影子"。
 *   ⚠ `kept.stable` 计的是**保留行数**（**含块标题行**：「agent 画像（…）：」/「用户画像（…）：」），
 *     核对时须减去标题数 —— 实测 `- ` 前缀行 = 42，而 `kept.stable + kept.dynamic = 44`（差 2 = 两个标题）。
 */
function supplyUsageMeta(
  cand: { stable: readonly string[]; dynamic: readonly string[]; oneshot: readonly string[]; situation: readonly string[] },
  budget: { stable: number; dynamic: number; oneshot: number; situation: number },
  cues: readonly string[],
  /** S4R/R2：**主路径的真实裁切结果**（保留行数 + 被丢行内容）。缺省 ⇒ 回落装配器口径（兼容旧调用） */
  real?: { stable: number; dynamic: number; oneshot: number; dropped: readonly string[] },
): { usage: SupplyUsage; situationBlock: string } {
  const b: SupplyBudget = { stable: budget.stable, dynamic: budget.dynamic, oneshot: budget.oneshot, serendipity: 0, situation: budget.situation }
  const r = assembleSupply(
    { stable: [...cand.stable], dynamic: [...cand.dynamic], oneshot: [...cand.oneshot], situation: [...cand.situation] },
    b,
  )
  return {
    usage: {
      at: Date.now(),
      chars: r.meta.chars,
      budgetTotal: r.meta.budgetTotal,
      overBudget: r.meta.overBudget,
      kept: { stable: real?.stable ?? r.meta.kept.stable, dynamic: real?.dynamic ?? r.meta.kept.dynamic, oneshot: real?.oneshot ?? r.meta.kept.oneshot, situation: r.meta.kept.situation },
      dropped: r.meta.dropped.map((x) => x.why),
      ...(real ? { droppedRows: [...real.dropped] } : {}),
      cues: [...cues],
      situationEnabled: r.meta.situationEnabled,
      // 阶段 3 取证：装配器口径渲染文本（只读诊断，不进注入面）。
      assemblerText: renderSupplyText(r),
    },
    situationBlock: r.blocks.situation,
  }
}
...
/**
 * 阶段 1 及其**修复**（2026-09-14）：**上游介质签名** —— 产线落盘自动失效注入缓存（用签名非回调：
 *   产线在 scheduler 半区、注入在 panel 半区，回调等于重造跨域通道；签名零耦合，产线只管写文件）。
 ...
 * 介质只取**变更罕见且不绑定查询**的两条：`activity.jsonl` · `delta.md`；**不含 `warm-recall.json`**：
 *   它一次只装**一个 query** 的行，签它会「为 query B 的写入失效 query A 的缓存」而 A 输出不变 ⇒ 平白失效。
 * 零抛出、缺席回落 `x:-`：**签名失败不得影响注入**。
 */
const upstreamStampOf = (memRoot: string): string => {
  const kRoot = (() => { try { return knowledgeRoot() } catch { return '' } })()
  const parts = [sizeStampOf(memRoot, ['audit/activity.jsonl'])]
  if (kRoot) parts.push(sizeStampOf(kRoot, ['delta.md']))
  return parts.join('|')
}
...
function buildHotMemoryText(d: HotMemoryDeps, cache: HotMemoryCache, query = ''): string {
  ...
  const cacheKey = memRoot + '|' + q + '|' + upstreamStampOf(memRoot)
  ...
  const readCarrier = (name: string, opts: { profile?: boolean; maxProfile?: number } = {}): string[] => {
    try {
      ...
      const dropped = prof.length - picked.length
  ...
  const memLines = selectDynamicLines({
    ...
    // ⚠ 候选源必须是**全层索引行**（`[路径]` 属 R 层、在 AGENT.md）；且**仅开启时**才读，缺省零开销
    processRows: (SURFACE.injection as unknown as { process?: { enabled?: boolean } }).process?.enabled === true ? scanIndexRows(memRoot).map((r) => r.line) : undefined,
  })
  ...
  const totalBudget = Math.max(1200, Number((SURFACE.injection as unknown as { budgetChars?: number })?.budgetChars) || 4000)
  ...
  // 阶段 1 修复（2026-09-14）：稳定面键**不含**上游介质签名——那三条介质对画像段**无内容贡献**
  //   （实测：改 activity.jsonl 后稳定面逐字节不变）⇒ 签在这里只会平白重算，不增收益。
  const stableKey = `${memRoot}|${personaMode}|${profileCap}|${playbookEnabled(d.suite)}|${sizeStampOf(memRoot, ['AGENT.md', 'USER.md'])}`
  let stableLines: string[]
  if (cache.stableKey === stableKey && now - cache.stableAt < 120000) {
    stableLines = cache.stableText
  } else {
    const head = `[守藏·热记忆] 记忆库指针（${memRoot}；详情按指针 get_file 拉对应 notes §小节）：`
    const agentBlock: string[] = []
    if (agentLines.length) {
      agentBlock.push('agent 画像（AGENT.md；含 [原则]/[路径] 与成长画像行 `← 源:`——①③步优先读）：')
      for (const l of agentLines) agentBlock.push(/^-\s/.test(l) ? l : `- ${l}`)
    }
    const userBlock: string[] = []
    if (userLines.length) {
      userBlock.push('用户画像（USER.md）：')
      for (const l of userLines) userBlock.push(/^-\s/.test(l) ? l : `- ${l}`)
    }
    // ★运行态缺陷修复（2026-09-13 重启验证实测）：**两块各自配额**。
    //   原实现对「agent+user 合并后的 stable 数组」做一次 clamp ⇒ agent 画像一超预算就把 USER 画像
    //   **整段挤掉**（/inject/preview 实测 `含用户画像段 = false`），违反本方案 I2a「恒定面每步必在」。
    //   现按块切分（55% / 45%，各自下限 120 字符）：**块内裁行、不丢块**，且各自留痕可见化。
    const capAgent = Math.max(120, Math.round(capStable * 0.55))
    const capUser = Math.max(120, capStable - capAgent)
    const [a2, dA] = clampLines(agentBlock, capAgent)
    const [u2, dU] = clampLines(userBlock, capUser)
    const sl: string[] = playbookEnabled(d.suite) ? [head, ...MEMORY_PLAYBOOK_LINES, ...a2] : [head, ...a2]
    if (dA.length > 0) sl.push(`（agent 画像另省略 ${dA.length} 行，按需 get_file 读取）`)
    sl.push(...u2)
    if (dU.length > 0) sl.push(`（用户画像另省略 ${dU.length} 行，按需 get_file 读取）`)
    cache.stableKey = stableKey
    cache.stableAt = now
    cache.stableText = sl
    cache.realCut = { stable: a2.length + u2.length, dropped: [...dA, ...dU] }
    stableLines = sl
  }
  const stablePart = stableLines
  ...
  if (memLines.length) {
    if (m.length) {
      dynBlock.push(`知识索引（MEMORY.md，热取前 ${m.length} 条）：`)
  ...
  const text = renderSupplyText({
    ...
    tailNote: droppedAll > 0 ? `（本步受预算 ${totalBudget} 字符约束省略 ${droppedAll} 行；按需 get_file 读取）` : undefined,
  })
  // ── P0a/P3（2026-09-14）：影子记账 + 情境槽 ──
  //   候选取**原始集**（clamp 之前）：stable = P 层索引行 + 画像行；dynamic = 知识索引候选；oneshot = 成长 delta。
  //   预算镜像活路径三顶 ⇒ 账与真实裁切可比。结果进 cache.usage，经 hot.supplyUsage() 只读出去。
  ...
  const shadow = supplyUsageMeta(
    { stable: [...agentLines, ...userLines], dynamic: memLines, oneshot: deltaText ? deltaText.split('\n') : [], situation: sitLines },
    { stable: capStable, dynamic: capDynamic, oneshot: capOneshot, situation: sitBudget },
    sitCues,
    // S4R/R2：传**主路径自己的裁切结果** —— 有它，账才不是"按装配器口径重算的近似值"（B2）
    { stable: cache.realCut?.stable ?? 0, dynamic: m.length, oneshot: oneshotPart.length, dropped: [...(cache.realCut?.dropped ?? []), ...droppedOneshot, ...droppedMem] },
  )
  cache.usage = shadow.usage
  // 情境块只在开启且有内容时并入 ⇒ 缺省（budgetChars=0）下**输出与开启前逐字节相同**（P3 验收）
  const finalText = shadow.situationBlock ? `${text}\n\n${shadow.situationBlock}` : text
