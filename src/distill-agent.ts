...
const distillAgent = async (dep: AgentDeps, agent: any): Promise<void> => {
  ...
  const quiet = quiescenceOf({ ctx: dep.env.ctx, hasActiveSubagents: (x: string) => dep.parent.parent.hasActiveSubagents(x) }, agent)
  ...
  if (quiet.state === 'busy') { dep.io.infra.log(`distill: ${dep.io.infra.sidShort(sid)} 已恢复活跃（${quiet.why}），跳过`); return }
  ...
  try {
    ...
    if (!chunks.length || totalChars < (dep.env.config.minTurnChars ?? 200)) {
      ...
      if (!noIncrement) dep.io.infra.ledger({ domain: 'ingest', sid: sid.replace(/^session-/, '').slice(0, 8), decision: { route: 'skip', reason: skipReason, chars: totalChars, heldForUndigested: !skipPlan.write }, result: { added: 0, rejected: 0, failed: 0 } })
  ...
  try {
    ...
    for (const a of roots) {
      try {
        ...
        let maxSeq = lastSeq
        try { for (const e of a.session.snapshotEvents()) { const s = Number((e as any)?.seq ?? 0); if (s > maxSeq) maxSeq = s } } catch { /* 快照不可用 ⇒ maxSeq 保持基线 */ }
        const quiet = quiescenceOf({ ctx: dep.env.ctx, hasActiveSubagents: (x: string) => dep.parent.parent.hasActiveSubagents(x) }, a)
        const adm = planIngestAdmission({
          lastSeq, maxSeq,
          sinceLastEndMs: null,
          idleWakeMs: dep.env.config.idleWakeMs,
        ...
        if (!adm.run) { dep.io.infra.log(`distill(manual): ${dep.io.infra.sidShort(a.id)} 跳过（${adm.reason}）`); continue }
