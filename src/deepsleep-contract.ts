...
/** 子进程执行结果（深睡与蒸馏共享的形态） */
export interface RunNodeResult { status: number | null; out: string; err: string }
...
export interface SleepLlm {
  runNode(nodeBin: string, scriptPath: string, args: string[], opts?: { cwd?: string; env?: Record<string, string>; timeout?: number }): Promise<RunNodeResult>
...
export interface SleepState {
   ...
   */
  epoch: { v: string | null; since: number | null; pendingChunk: number | null }
}
