 ...
 */
export const quiescenceOf = (d: QuiescenceDeps, agent: any): { state: Quiescence; why: string } => {
  try {
    const sid = agent && agent.id
    if (!sid) return { state: 'unknown', why: 'no-sid' }
    if (d.hasActiveSubagents(sid)) return { state: 'busy', why: 'active-subagent' }
    const live = (() => { try { return d.ctx && d.ctx.agents && typeof d.ctx.agents.get === 'function' ? d.ctx.agents.get(sid) : null } catch { return null } })()
    const status = (live && live.status) || (agent && agent.status)
    if (!status) return { state: 'unknown', why: 'status-unavailable' }
    if (status !== 'idle') return { state: 'busy', why: `status=${status}` }
    ...
    const queued = (() => {
      try {
        ...
        if (q && typeof q.size === 'number') return q.size
        return null
      } catch { return null }
    })()
    if (typeof queued === 'number' && queued > 0) return { state: 'busy', why: `inbox=${queued}` }
    return { state: 'quiet', why: 'idle' }
  } catch { return { state: 'unknown', why: 'probe-failed' } }
}
...
export const lastTurnEndMsOf = (agent: any): number => {
  try {
 ...
 */
export const planIngestAdmission = (i: IngestAdmissionInput): IngestAdmission => {
  const lastSeq = Number(i.lastSeq) || 0
  const maxSeq = Number(i.maxSeq) || 0
  const quiet: Quiescence = i.quiet === 'busy' || i.quiet === 'quiet' ? i.quiet : 'unknown'
  ...
  if (maxSeq <= lastSeq) return { run: false, reason: 'no-increment', evidence }
  if (Number(i.snapshotStreak) >= Number(i.circuitBreakN)) return { run: false, reason: 'snapshot-unavailable-circuit-break', evidence }
  if (i.claimHeld) return { run: false, reason: 'claim-held', evidence }
  if (quiet === 'busy') return { run: false, reason: 'not-quiet', evidence }
  if (!i.manual && since !== null && since < idle) return { run: false, reason: 'grace-period', evidence }
  if (i.manual) return { run: true, reason: 'manual', evidence }
  if (quiet === 'unknown') return { run: true, reason: 'unknown-quiescence-fallback', evidence }
