...
const KNOWN_DIMS: readonly CueDim[] = ['scope', 'task', 'subject', 'event']
 ...
 */
export function cuesOf(ctx: SituationCtx, dims: readonly string[] = KNOWN_DIMS): string[] {
  const out: string[] = []
...
export const DEFAULT_CUE_DIMS: readonly CueDim[] = KNOWN_DIMS
 ...
 */
export function taskCueOf(query: string, known?: readonly string[]): string {
  try {
    ...
    for (const raw of known) {
      ...
      if (parts.length && parts.every((p) => seg.has(p))) hit.push(k)
    }
