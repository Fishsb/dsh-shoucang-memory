...
export interface DedupResult {
  ...
  note: string
  stat: { coarse: number; fine: number; embedded: number }
}
...
export function embedCfgFromSuite(home = homedir()): { enabled: boolean; baseUrl: string; model: string; apiKeyEnv: string } {
  try {
    const s = JSON.parse(readFileSync(join(home, '.dsh', 'suite', 'scheduler.json'), 'utf8')) as Record<string, unknown>
  ...
  } catch { return { enabled: false, baseUrl: '', model: '', apiKeyEnv: '' } }
}
 ...
 */
export async function crossFormCovered(
  memRoot: string,
  embedCfg: unknown,
  ...
  for (const f of ['AGENT.md', 'USER.md']) {
    ...
    for (const raw of text.split(/\r?\n/)) {
      const l = raw.trim()
      if (!l) continue
      if (isCoarse(l)) coarse.push(l)
      else if (isFine(l)) fine.push(l)
    }
  }
  const stat = { coarse: coarse.length, fine: fine.length, embedded: 0 }
  const empty = (note: string): DedupResult => ({ skip: [], reasons: [], note, stat })
  if (!fine.length || !coarse.length) return empty('无判定面（细粒度或粗粒度行集为空）')
  const cfg = embedCfg as { enabled?: boolean } | null
  if (!cfg || !cfg.enabled) return empty('向量未配置 ⇒ 未判（fail-open：注入面回基线）')
  let vec: { embedMany?: unknown; cosine?: unknown }
  try { vec = (await import('./vec.js')) as unknown as { embedMany?: unknown; cosine?: unknown } } catch { return empty('vec 模块不可用 ⇒ 未判') }
  ...
  if (typeof embedMany !== 'function' || typeof cosine !== 'function') return empty('vec 缺 embedMany/cosine ⇒ 未判')
  let all: number[][]
  try { all = await embedMany(embedCfg, [...coarse, ...fine]) } catch (e) {
    return empty('embedMany 失败（' + String((e as Error)?.message || e).slice(0, 50) + '）⇒ 未判')
  }
  if (!all || all.length !== coarse.length + fine.length) return empty('embed 返回长度不符 ⇒ 未判')
  stat.embedded = all.length
  const skip: string[] = []
  const reasons: DedupSkipReason[] = []
...
export function filterInjected(lines: readonly string[], skip: readonly string[]): string[] {
  if (!skip.length) return [...lines]
  const set = new Set(skip.map((s) => s.trim()))
  return lines.filter((l) => !set.has(l.trim()))
}

/** 过滤**注入文本**（整段；逐行逐字比对，保留行序）。fail-open：`skip` 空 ⇒ **原样返回**。 */
export function filterInjectedText(text: string, skip: readonly string[]): string {
...
export async function warmInjectDedup(memRoot: string, opts?: { threshold?: number; embedCfg?: unknown }): Promise<void> {
  if (BOX.warming) return
  BOX.warming = true
  try {
    ...
    BOX.note = '预热异常（fail-open）：' + String((e as Error)?.message || e).slice(0, 50)
  } finally {
    BOX.warming = false
  }
}
...
export function dedupState(): { skip: readonly string[]; reasons: readonly DedupSkipReason[]; note: string; warmedAt: number } {
