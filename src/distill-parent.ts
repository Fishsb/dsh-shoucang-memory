...
export function createParentApi(d: ParentDeps) {
  return {
    CHILD_ACTIVE_MS,
    isValidParent: (...a: Tail<Parameters<typeof isValidParent>>) => isValidParent(d, ...a),
    rememberAgent: (...a: Tail<Parameters<typeof rememberAgent>>) => rememberAgent(d, ...a),
    isSubagentAgent: (...a: Tail<Parameters<typeof isSubagentAgent>>) => isSubagentAgent(d, ...a),
    parentSidOf: (...a: Tail<Parameters<typeof parentSidOf>>) => parentSidOf(d, ...a),
    noteChildActivity: (...a: Tail<Parameters<typeof noteChildActivity>>) => noteChildActivity(d, ...a),
    dropChild: (...a: Tail<Parameters<typeof dropChild>>) => dropChild(d, ...a),
    hasActiveSubagents: (...a: Tail<Parameters<typeof hasActiveSubagents>>) => hasActiveSubagents(d, ...a),
    pickParent: (...a: Tail<Parameters<typeof pickParent>>) => pickParent(d, ...a),
    resolveDefaultModel: (...a: Tail<Parameters<typeof resolveDefaultModel>>) => resolveDefaultModel(d, ...a),
    ensureDaemonParent: (...a: Tail<Parameters<typeof ensureDaemonParent>>) => ensureDaemonParent(d, ...a),
  }
}
...
const parentSidOf = (d: ParentDeps, a: any): string | null => {
  try {
    const cands = [
...
const noteChildActivity = (d: ParentDeps, parentSid: string | null, childSid: string): void => {
  ...
  if (!m) { m = new Map(); d.st.childSeen.set(parentSid, m) }
  m.set(childSid, now)
}
...
const hasActiveSubagents = (d: ParentDeps, sid: string): boolean => {
  ...
  try {
    ...
    if (m) {
      ...
      if (!m.size) d.st.childSeen.delete(sid)
    }
  ...
  try {
    for (const a of d.ctx.agents.list() || []) {
      if (!isSubagentAgent(d, a)) continue
      const live = d.ctx.agents.get(a.id)
      if (!(live && live.status === 'running')) continue
      let p = parentSidOf(d, a) || parentSidOf(d, live); let depth = 0
      if (!p) return true // 归属解析失败：宁少蒸勿切碎（保守）
      while (p && depth++ < 4) {
        if (p === sid) return true
        const pa: any = d.ctx.agents.get(p); p = pa ? parentSidOf(d, pa) : null
      }
    }
 ...
 */
const resolveDefaultModel = (d: ParentDeps, ): { provider: string; model: string } | undefined => {
  try {
    const c: any = d.ctx as any
...
const ensureDaemonParent = async (d: ParentDeps, signal: AbortSignal, agentOptions?: { provider: string; model: string }): Promise<any | null> => {
  ...
  try {
    ...
    const handle: any = await d.ctx.agents.create({
      sessionId: `session-${randomUUID()}`,
      ...(agentOptions ? { agentOptions } : {}),
      signal,
    })
    const a = handle && handle.agent ? handle.agent : handle
    if (isValidParent(d, a)) { d.st.daemonParent = a; d.log('deep sleep: 已建立守护 parent（无会话场景承载归纳子代理）'); return a }
    d.log('deep sleep: 守护 parent 创建结果不可用')
  } catch (e) { d.log(`deep sleep: 守护 parent 创建失败：${String((e as Error)?.message || e).slice(0, 120)}`) }
  return null
}
