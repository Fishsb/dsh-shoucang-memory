...
export function makeEvent(seq: number, op: RingOp, id: string, data: Record<string, string>, at: string): RingEvent {
  return { seq, op, id, data, at }
}

/** 事件流 → JSONL（append-only 的落盘形态） */
export function serializeEvents(events: readonly RingEvent[]): string {
  return events.map((e) => JSON.stringify(e)).join('\n') + (events.length ? '\n' : '')
}

export function parseEvents(text: string): RingEvent[] {
  const out: RingEvent[] = []
  for (const line of text.split(/\r?\n/)) {
    if (!line.trim()) continue
    try { out.push(JSON.parse(line) as RingEvent) } catch { /* 坏行跳过：解析失败不得让整个账本不可用 */ }
  }
  return out
}
 ...
 */
export function replayEvents(events: readonly RingEvent[]): ReplayResult {
  const errors: string[] = []
  let applied = 0
  let recs: MemRecord[] = []
  ...
  for (const e of sorted) {
    try {
      switch (e.op) {
        ...
        default:
          errors.push(`未知 op：${String((e as RingEvent).op)}`)
          continue
      }
      applied++
    } catch (err) {
      errors.push(`${e.op} ${e.id}: ${String((err as Error).message).slice(0, 120)}`)
    }
  }
 ...
 */
export function eventsFromDiff(prev: readonly MemRecord[], next: readonly MemRecord[], at: string, startSeq = 1, atOf?: (r: MemRecord) => string): RingEvent[] {
  const before = new Map(prev.map((r) => [r.id, r]))
  const out: RingEvent[] = []
  let seq = startSeq
  const push = (op: RingOp, id: string, data: Record<string, string>, atOverride?: string) => {
   ...
   *  ⇒ 静默回落成统一时刻 —— 实测踩过）。 */
  const atOfTransition = (r: MemRecord, field: 'settledAt' | 'landedAt'): string => M(r)[field] || r.updatedAt || at
  const M = (r: MemRecord) => r.meta ?? {}
  ...
  for (const r of next) {
    ...
    if (!b) {
      ...
      if (r.kind === 'association') {
        if (M(r).accepted !== undefined) push('collision.accept', r.id, { collisionId: r.id, accepted: M(r).accepted }, r.updatedAt || at)
...
export function ringRecordsOf(records: readonly MemRecord[]): MemRecord[] {
 ...
 */
export function reconcileRing(records: readonly MemRecord[], events: readonly RingEvent[]): ReconcileResult {
