 ...
 */
export function knownTaskCuesOf(memRoot: string): string[] {
  try {
    ...
    for (const r of records) {
      for (const k of parseCues(r.meta?.cues)) {
        if (k.startsWith('task=')) out.add(k.slice(5).trim())
      }
    }
    out.delete('')
    return [...out]
 ...
 */
export function situationLinesOf(memRoot: string, cues: readonly string[], at: string, budget: number): string[] {
