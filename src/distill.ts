...
export function registerDistill(ctx: AppContext, config: DistillConfig): {
  ...
  const ensureFlowCandidate = (sid: string, intent: string): void => {
    ...
    try {
      ...
      if (existsSync(f)) return // 同会话/同意图去重（一任务一条）
    ...
    try {
      ...
      try {
        ...
        if (disp.added > 0) {
          ...
          if (route === 'memory') ensureFlowCandidate(sid, intent)
        }
        if (stop === 'completed' && out) {
          ...
          log(`distill: ${sid.slice(0, 8)} stop=${stop} out=${out ? 'ok' : 'null'}，水位保留 ${lastSeq} 待下轮重试`)
        }
      } catch (e) {
        clearTimeout(timeout)
  ...
  const gatherDeepSleepTraces = (memRoot: string, since: number): string => {
    ...
    try {
      ...
      if (candFiles.length) {
        const blocks: string[] = ['### 跨窗口任务候选（背景材料，非源指针；同类型重复 → [路径] 判据参考）']
        let cbudget = 2400
