 ...
 *              ┌───────────┬────────────┬─────────────┐
 *     输出在增长│  无增长+会话还在│ 无增长+会话已消失│ 无转录文件 │
 *              ▼           ▼            ▼             ▼
 *          RUNNING      STALLED       ENDED        RUNNING
 *        (正常长任务)   (卡住/异常)   (异常退出)   (保守视为活跃)
 *
 * 探测未决期间一律**保守视为活跃**（不睡）；已确认 STALLED 的会话不阻塞睡眠（另发审计告警）。
 */
...
interface SessRec {
  ...
  probeResult?: 'long-run' | 'stall' | 'exit' | 'no-transcript' // 探测结论（审计可查）
}
  ...
  const probeSession = (rec: SessRec): void => {
    ...
    void (async () => {
      try {
        ...
        const s1 = snap()
        if (!s1) {
          rec.probeResult = 'exit'; rec.state = 'ended'; rec.lastEndAt = Date.now()
        ...
        log(`deep sleep probe err ${short}: ${String((e as Error)?.message || e).slice(0, 120)}`)
      }
    })()
  }
