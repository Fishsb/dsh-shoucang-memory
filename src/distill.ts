 ...
 // 并发守卫：同会话蒸馏在途标记（防 turn/end 重武装导致双写/竞态）
  ...
  const distillAgent = async (agent: any): Promise<void> => {
    ...
    try {
      ...
      try { await write.flushDeferCards() } catch { /* 回流失败不阻断本轮蒸馏 */ }
    ...
    } finally { st.distilling.delete(sid); if (claimed) write.releaseClaim(sid) }
  }
  ...
  const sweepBacklog = async (): Promise<void> => {
    try {
      ...
      try { await write.flushDeferCards() } catch { /* 回流失败不阻断扫尾 */ }
      ...
      for (const a of roots) {
        try {
          ...
          if (maxSeq > lastSeq) {
            ...
            if (write.claimHeld(sid)) { continue } // 在途，其他 fiber 已接管
