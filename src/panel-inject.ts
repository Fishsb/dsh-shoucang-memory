...
const gateWrite = (target: string, tmpPath: string): Promise<{ ok: boolean; reason?: string; out?: string }> =>
  new Promise((done) => {
    ...
    execFile(
      'node',
      [gate, target, tmpPath],
      { encoding: 'utf8', timeout: 30000, windowsHide: true, env: { ...process.env, MEMORY_ROOT: memoryLibRoot(), SHOUCANG_CAP_STRICT: '0' } },
      (err, stdout, stderr) => {
...
const snapshotBank = (d?: InjectDeps): void => {
  try {
    ...
    execFile(process.execPath, [script, '--message', `memory: panel-write @ ${new Date().toISOString().slice(0, 19)}`],
