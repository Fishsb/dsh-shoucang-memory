...
export const stateSha8Of = (state: string): string =>
  createHash('sha256').update(String(state || ''), 'utf8').digest('hex').slice(0, 8)
...
export function shapeOf(state: string, qTypes: readonly string[]): { stateChars: number; stateSha8: string; qCount: number; qTypes: string[] } {
  return {
...
export function confidenceMinOf(answers: Record<string, { confidence: number | null }> | undefined): number | null {
  if (!answers) return null
  const vals = Object.values(answers).map((a) => a.confidence).filter((v): v is number => typeof v === 'number' && Number.isFinite(v))
 ...
 */
export function decisionEventOf(r: EvalDecisionRecord): Record<string, unknown> {
  return {
    type: 'eval.decision',
    source: r.source,
    tier: r.tier,
...
export function statsOfLines(lines: readonly string[], recentN = 0): EvalLedgerStats & { recent: EvalDecisionRow[] } {
  const byOutcome: Record<string, number> = {}
  const bySource: Record<string, number> = {}
  const rows: EvalDecisionRow[] = []
  ...
  for (const l of lines) {
    ...
    if (String(o.type || '') !== 'eval.decision') continue
    total++
    const oc = String(o.outcome || 'unknown')
    byOutcome[oc] = (byOutcome[oc] || 0) + 1
    const src = String(o.source || 'unknown')
    bySource[src] = (bySource[src] || 0) + 1
    if (oc === 'ok' && o.destLoopback === false) egressOk++
    if (o.fellBack === true) fellBack++
    if (recentN > 0) {
      rows.push({
        at: String(o.at || ''),
        source: src,
...
export const isCalibratedTier = (tier: string): boolean => String(tier) === 'native'
 ...
 */
export function evaluateGateOf(o: { outcome: string; tier: string; confidence: number | null }): { gate: EvalGate; why: string } {
  if (o.outcome !== 'ok') return { gate: 'reject', why: `outcome:${o.outcome}` }
  ...
  if (o.confidence >= thr.high) return { gate: 'accept', why: `conf>=${thr.high}` }
  if (o.confidence >= thr.low) return { gate: 'review', why: `conf>=${thr.low}` }
