 ...
 */
export function thresholdValue<T = unknown>(id: string, fallback: T): T {
  const e = (THRESHOLDS.entries as readonly { id: string; value: unknown }[]).find((x) => x.id === id)
  if (!e || e.value === null || e.value === undefined) return fallback
  return e.value as T
}

/** 对象型阈值的取键读口（如 `activity.statusDays` 的 `warm`/`cold`/`archive`）。
 *  对象整值也有 `thresholdValue`，但消费点通常只需要其中一维 —— 本函数避免每处各写一遍取键与兜底。 */
export function thresholdParam<T = unknown>(id: string, key: string, fallback: T): T {
  const v = thresholdValue<Record<string, unknown> | null>(id, null)
  const got = v && typeof v === 'object' ? v[key] : undefined
  return got === undefined || got === null ? fallback : (got as T)
}
 ...
 */
export function splitLawOf(): { R: number; K: number } {
  const id = 'ingest.granularity.split-law'
  return {
    R: paramOf<number>(id, 'R', 1000),
    K: paramOf<number>(id, 'K', 6),
  }
}
...
export function activationOf(distinctDays: number, params = MATURATION): number {
...
export function maturationVerdict(A: number, enforce: boolean, params = MATURATION): Verdict {
...
export function layeredScore(input: { relevance: number; importance: number; recency: number }): number {
...
const TAG_IMP: Record<string, number> = { lesson: 1.0, 原则: 1.0, flow: 0.9, 路径: 0.9, 经验: 0.9, 教训: 0.9, env: 0.7, tool: 0.7 }
export function importanceOf(line: string): number {
