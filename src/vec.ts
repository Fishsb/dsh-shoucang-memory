 ...
 */
export async function embedMany(cfg: EmbedCfg, texts: readonly string[]): Promise<number[][] | null> {
...
export async function recallRanked(
  ...
  try {
    ...
    if (!pool.length) pool = scanIndexRows(root, scope === 'all' ? ['AGENT.md', 'MEMORY.md', 'USER.md'] : ['AGENT.md'])
