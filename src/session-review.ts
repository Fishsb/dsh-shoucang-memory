...
export const REVIEW_DEFAULTS = { idleMs: 30 * 60 * 1000, minNewEntries: 3, minNewEvents: 600 } as const
...
export interface ReviewRunResult {
  ...
  appended: number
  skipped: number
  counts: { manifest: number; failures: number; skeleton: number; proposals: number }
  reviewedSeq: number
}

export const sidShortOf = (sid: string): string => String(sid ?? '').replace(/^session-/, '')
export const proposalPathOf = (bankRoot: string, sid: string): string =>
  join(bankRoot, 'audit', 'session-review', `proposals-${sidShortOf(sid)}.jsonl`)
export const statePathOf = (kRoot: string): string => join(kRoot, 'audit', 'session-review-state.jsonl')
export const manifestPathOf = (kRoot: string, sid: string): string =>
  join(kRoot, 'audit', 'distill-manifest', `${sidShortOf(sid)}.jsonl`)

/** 提案指纹：`op|target|section|before|after` 的稳定指纹（复用 `supply-ledger.fingerprintOf`，不另造哈希）。 */
export function opHashOf(p: ReviewProposal): string {
...
const tailLines = (file: string, n: number): string[] => {
  try { return readTailLines(file, n).map((l) => l.replace(/\s+$/, '')).filter(Boolean) } catch { return [] }
}

/** 读本会话的复盘状态（**末行生效**：本流按 sid 追加，读数取该 sid 最后一条）。 */
export function readReviewState(d: ReviewDeps, sid: string): { reviewedSeq: number; formatVersion?: string; fp?: string } | null {
  const rows = tailLines(statePathOf(d.kRoot), 500)
  for (let i = rows.length - 1; i >= 0; i--) {
    try {
      const o = JSON.parse(rows[i]) as Record<string, unknown>
      if (o.sid === sid) return { reviewedSeq: Number(o.reviewedSeq || 0), formatVersion: o.formatVersion as string | undefined, fp: o.fp as string | undefined }
    } catch { /* 坏行跳过 */ }
  }
  return null
}
...
/** 触发判定（三态可分辨）。 */
export function decideReview(d: ReviewDeps, i: { sid: string; lastActivityMs: number; newEntries: number; newEvents: number; disposed: boolean }): { state: 'not-triggered' | 'skipped-by-threshold' | 'ready'; reason: string } {
  const idleOk = d.now() - Number(i.lastActivityMs || 0) >= REVIEW_DEFAULTS.idleMs
  const due = idleOk || !!i.disposed
  if (!due) return { state: 'not-triggered', reason: idleOk ? 'idle' : `空闲不足（<${Math.round(REVIEW_DEFAULTS.idleMs / 60000)}min）` }
  const contentOk = Number(i.newEntries || 0) >= REVIEW_DEFAULTS.minNewEntries || Number(i.newEvents || 0) >= REVIEW_DEFAULTS.minNewEvents
  if (!contentOk) return { state: 'skipped-by-threshold', reason: `内容维不足（新增条目 ${i.newEntries} / 事件 ${i.newEvents}）` }
  return { state: 'ready', reason: i.disposed ? 'disposed' : 'idle+content' }
}

/** 材料装配：**只读**（清单流 + 失败明细 + 骨架由调用方经 S2 侧提供，本件不读会话原文）。 */
export function buildMaterials(d: ReviewDeps, sid: string, skeleton: string[] = []): { manifest: string[]; failures: string[]; skeleton: string[]; counts: { manifest: number; failures: number; skeleton: number } } {
  const manifest = tailLines(manifestPathOf(d.kRoot, sid), 400)
  const failures: string[] = []
  for (const l of tailLines(join(d.kRoot, 'audit', 'ledger.jsonl'), 2000)) {
    try {
      const o = JSON.parse(l) as Record<string, unknown>
      if (o.kind !== 'distill-run' || o.sid !== sid) continue
      const fi = Array.isArray(o.failedItems) ? (o.failedItems as Array<Record<string, unknown>>) : []
      for (const f of fi) failures.push(`${String(f.k || '')} ${String(f.target || '')}§${String(f.section || '')} ${String(f.reason || '')}`.trim())
    } catch { /* 坏行跳过 */ }
  }
...
export function existingOpHashes(d: ReviewDeps, sid: string): Set<string> {
  const s = new Set<string>()
  for (const l of tailLines(proposalPathOf(d.bankRoot, sid), 500)) {
    try { const o = JSON.parse(l) as { opHash?: string }; if (o.opHash) s.add(String(o.opHash)) } catch { /* 坏行跳过 */ }
  }
  return s
}

/** 追加提案（**append-only**，幂等：同 `(sid, reviewedSeq, opHash)` 第二次即跳过）。 */
export function appendProposals(d: ReviewDeps, sid: string, reviewedSeq: number, proposals: ReviewProposal[], extra: { formatVersion?: string; fp?: string } = {}): { appended: number; skipped: number } {
  ...
  for (const p of proposals) {
    ...
    try {
      ...
      d.log(`session-review: 提案落盘失败（跳过该条）：${String((e as Error)?.message || e).slice(0, 80)}`)
      skipped++
    }
  }
...
export function writeReviewState(d: ReviewDeps, sid: string, reviewedSeq: number, extra: { formatVersion?: string; fp?: string } = {}): void {
  try {
  ...
  } catch (e) { d.log(`session-review: 状态写失败（水位不前移，下轮重试）：${String((e as Error)?.message || e).slice(0, 80)}`) }
}
...
export async function runSessionReview(
  d: ReviewDeps,
  input: { sid: string; lastActivityMs: number; newEntries: number; newEvents: number; disposed: boolean; skeleton?: string[]; reviewedSeq?: number; fp?: string },
  review: (m: { manifest: string[]; failures: string[]; skeleton: string[] }) => Promise<ReviewProposal[]>,
  ...
  const base: ReviewRunResult = { state: dec.state === 'ready' ? 'reviewed' : dec.state, reason: dec.reason, appended: 0, skipped: 0, counts: { manifest: 0, failures: 0, skeleton: 0, proposals: 0 }, reviewedSeq }
  ...
  try {
    ...
    d.audit({ kind: 'session-review', sid: input.sid, state: 'error', reason: `复盘失败：${String((e as Error)?.message || e).slice(0, 80)}`, reviewedSeq, ...m.counts })
  ...
  if (w.appended > 0) writeReviewState(d, input.sid, curSeq, { formatVersion: input.fp, fp: input.fp })
