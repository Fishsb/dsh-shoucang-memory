...
export interface ReviewDeps {
  ...
  thresholds?: { idleMs?: number; minNewEntries?: number; minNewEvents?: number }
}
/** 一条校正提案（**只提案不执行**）。 */
...
export function decideReview(d: ReviewDeps, i: { sid: string; lastActivityMs: number; newEntries: number; newEvents: number; disposed: boolean }): { state: 'not-triggered' | 'skipped-by-threshold' | 'ready'; reason: string } {
  ...
  if (!due) return { state: 'not-triggered', reason: idleOk ? 'idle' : `空闲不足（<${Math.round(th.idleMs / 60000)}min）` }
  const contentOk = Number(i.newEntries || 0) >= th.minNewEntries || Number(i.newEvents || 0) >= th.minNewEvents
  if (!contentOk) return { state: 'skipped-by-threshold', reason: `内容维不足（新增条目 ${i.newEntries} / 事件 ${i.newEvents}；现任阈值 ${th.minNewEntries} 条 或 ${th.minNewEvents} 事件）` }
