...
export const EVAL_TIERS = ['local', 'fast', 'native', 'llm'] as const
...
export const evalOptionsOf = (config: EvalConfigFields): EvalConfigFields => ({
...
export function isLoopbackUrl(raw: string): boolean {
  let h = ''
  try { h = new URL(String(raw || '').trim()).hostname.toLowerCase() } catch { return false }
  if (!h) return false
 ...
 */
export function egressAllowed(cfg: { evalBaseUrl: string; evalEgressAllow: boolean }): { ok: boolean; why: string } {
  if (!cfg.evalBaseUrl) return { ok: false, why: 'endpoint-empty' }
  if (isLoopbackUrl(cfg.evalBaseUrl)) return { ok: true, why: 'loopback' }
  if (!cfg.evalEgressAllow) return { ok: false, why: 'egress-not-allowed' }
