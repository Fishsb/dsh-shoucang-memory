...
const isFineRow = (l: string): boolean => /^\s*-\s/.test(l) && !/\[[^\]\s]{1,8}\]/.test(l)
const isCoarseRow = (l: string): boolean => /\[[^\]\s]{1,8}\]/.test(l)
 ...
 */
export async function applyConvergeOps(
  memRoot: string,
  ops: ConvergeOp[],
  hooks?: { audit?: (o: Record<string, unknown>) => void; log?: (m: string) => void },
): Promise<ConvergeResult> {
  const audit = hooks?.audit || (() => {})
  const log = hooks?.log || (() => {})
  const res: ConvergeResult = { applied: 0, skipped: 0, tried: 0, archived: 0, reasons: [] }
  ...
  for (const op of list) {
    ...
    // ① 只允许这两个画像文件（本通道的射程）
    if (file !== 'AGENT.md' && file !== 'USER.md') { res.skipped++; res.reasons.push(`file 越界：${file}`); continue }
    // ④ 成对取证：两条都必须逐字存在
    let lines: string[]
    try { lines = readFileSync(join(memRoot, file), 'utf8').split(/\r?\n/) } catch { res.skipped++; res.reasons.push(`不可读：${file}`); continue }
    const iFine = lines.findIndex((l) => norm(l) === fine)
    const iCoarse = lines.findIndex((l) => norm(l) === coarse)
    if (!coarse || !fine || iFine < 0 || iCoarse < 0) { res.skipped++; res.reasons.push(`取证失败（未逐字命中）：${file}`); continue }
    // ② / ③ 形态门
    if (!isFineRow(lines[iFine])) { res.skipped++; res.reasons.push('细粒度行含标签（不属本通道）'); continue }
  ...
  if (res.applied) audit({ kind: 'converge-summary', applied: res.applied, skipped: res.skipped, tried: res.tried, archived: res.archived })
  return res
}
