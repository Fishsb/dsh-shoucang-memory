 ...
 */
export const idleMsOf = (c: { deepSleepIdleMs?: unknown }): number => Number(c.deepSleepIdleMs) || TRIGGER.idleMs
 ...
 */
export const epochIdOf = (startedAt: number): string => `epoch-${Math.floor(Number(startedAt) || 0)}`
...
export const planTriggerDim = (
  sinceHottestMs: number,
  idleMs: number,
  contentBytes: number,
  contentMin: number,
): TriggerDim => {
  if (Number(sinceHottestMs) >= Number(idleMs)) return 'time'
  if (Number(contentMin) > 0 && Number(contentBytes) >= Number(contentMin)) return 'content'
  return 'none'
}
 ...
 */
export function splitByCap(segments: readonly string[], capChars: number): string[][] {
  const segs = segments || []
  const cap = Number(capChars) || 0
  if (cap <= 0 || segs.length === 0) return [segs.slice()]
  const out: string[][] = []
  let cur: string[] = []
  let curLen = 0
  for (const s of segs) {
    const len = String(s || '').length
    if (cur.length > 0 && curLen + len > cap) { out.push(cur); cur = []; curLen = 0 }
    cur.push(s)
    curLen += len
  }
 ...
 */
export function windowMaterialBytes(dirs: { pendDir: string; candidateDir: string }, since: number): number {
  let total = 0
  for (const dir of [dirs.pendDir, dirs.candidateDir]) {
    ...
    for (const n of names) {
      ...
      try {
        ...
        if (st.isFile() && st.mtimeMs > since) total += st.size
      } catch { /* 单个文件失败不推翻整体统计 */ }
    }
  }
  return total
...
export interface SessRec {
  ...
  probeEvidence?: { rounds: number; samples: number; deltaBytes: number; alive: boolean; active: boolean; viaChildren?: boolean } // 末次证据（排查用；`viaChildren` = 因**子代理活跃**而判长任务）
...
export const planSegmentWatermark = (
  outcome: SegmentOutcome, attempt: number, maxRetry: number,
): { advance: boolean; forced: boolean; attempt: number; reason: string } => {
  ...
  if (tries >= cap) return { advance: true, forced: true, attempt: tries, reason: 'forced-after-retries' }
