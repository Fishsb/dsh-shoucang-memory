...
/**
 * 深睡作用域：createDeepSleep 的闭包状态 + 外部依赖的**显式装箱**。
 * 为什么需要：内部函数迁到模块级后不再共享闭包，依赖必须显式传入 —— 依赖从"隐式可见 60+ 个名字"
 *   变成"函数签名上一行 DsScope"，改一个函数要理解多少上下文由此可读。
 * ⚠ streak 为什么是对象不是 number：runDeepSleep 会**写**它，标量经解构传递是快照，写不回闭包。
 */
type DsScope = Omit<DeepSleepCtx, 'appCtx'> & {
  ctx: any
  streak: { v: number }
  traceSince: () => number
}
...
function gatherDeepSleepTraces(S: DsScope, memRoot: string, since: number): string {
...
async function applyPrinciples(S: DsScope, memRoot: string, out: any): Promise<{ attempted: number; added: number; replaced: number; skipped: number; gate: string; gateExit: number; rejectedLines: string[] }> {
...
async function applyPointerOps(S: DsScope, memRoot: string, out: any): Promise<{ updated: number; skipped: number; gate: string }> {
...
async function consolidateTree(S: DsScope, memRoot: string): Promise<{ idxExact: number; idxSem: number; secMerged: number; lineDedup: number; archived: number }> {
...
async function runDeepSleep(S: DsScope, sinceArg?: number): Promise<'done' | 'failed' | 'no-traces'> {
...
function probeSession(S: DsScope, rec: SessRec): void {
...
export function createDeepSleep(C: DeepSleepCtx) {
  ...
  const deepSleepCheck = (): void => {
    ...
    for (const [sid, rec] of sessions) {
      ...
      if (config.deepSleepProbe) {
        if (rec.state === 'running' && now - rec.lastEventAt >= probeAfter) probeSession(S, rec)
        else if (rec.state === 'suspect') probeSession(S, rec)
      }
  ...
  return { DEEP_SLEEP_CHECK_MS, sessions, noteEvent,
    ...
    runDeepSleep: (sinceArg?: number) => runDeepSleep(S, sinceArg),
    probeSession: (rec: SessRec) => probeSession(S, rec),
