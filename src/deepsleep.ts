...
import { probeSession as probeSessionImpl, type ProbeDeps } from './deepsleep-probe.js'
import { runDeepSleep as runDeepSleepImpl, type RunDeps } from './deepsleep-run.js'
...
export function createDeepSleep(C: DeepSleepCtx) {
  ...
  const state: SleepState = { streak, traceSince }
  ...
  const deepSleepCheck = (): void => {
    ...
    for (const [sid, rec] of sessions) {
      ...
      if (sess.hasActiveSubagents(sid)) { rec.state = 'running'; rec.lastEventAt = now; running++; continue }
      // 状态机推进：running 且无事件 ≥ probeAfter → 发起探测；suspect → 下轮巡检复核（卡住需连续确认）
      if (config.deepSleepProbe) {
        if (rec.state === 'running' && now - rec.lastEventAt >= probeAfter) probeSessionImpl(probeDeps, rec)
        else if (rec.state === 'suspect') probeSessionImpl(probeDeps, rec)
      }
  ...
  return { DEEP_SLEEP_CHECK_MS, sessions, noteEvent,
    ...
    runDeepSleep: (sinceArg?: number) => runDeepSleepImpl(runDeps, sinceArg),
    probeSession: (rec: SessRec) => probeSessionImpl(probeDeps, rec),
