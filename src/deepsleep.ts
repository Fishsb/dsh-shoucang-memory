...
export function createDeepSleep(C: DeepSleepCtx) {
  ...
  const metaCache = new Map<string, { workspace: string; title: string }>()
  const metaOf = (fullSid: string): { workspace: string; title: string } => {
    ...
    try {
      ...
      for (let i = evs.length - 1; i >= 0; i--) {
        if (evs[i] && evs[i].type === 'session/title') { title = String((evs[i].data && evs[i].data.title) || ''); break }
      }
    } catch { /* 取不到 ⇒ 不富化（面板退化显示编码，不影响状态机） */ }
    const v = { workspace, title }
    if (workspace || title) metaCache.set(fullSid, v) // 只在**拿到东西**时缓存，否则会把"空"钉死
    return v
  }
  ...
  return {
    ...
    getDeepSleepStatus: () => {
      ...
      try {
        ...
        for (const k of [...metaCache.keys()]) if (!live.has(k)) metaCache.delete(k) // 会话出表即清缓存
