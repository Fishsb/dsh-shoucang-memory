...
export function apply(ctx: Context, config: any): void {
  // **唯一 composition root**（G0 · 2026-09-13）：root 显式创建跨域句柄盒，交给两个装配点。
  // 原做法是模块级全局 holder（3 个 `-share` 桥）；现改为显式参数 —— 依赖可静态追，桥逐条退役。
  ...
  applyPanel(ctx, { ...config }, comp)
  applyScheduler(ctx, config, comp)
}
