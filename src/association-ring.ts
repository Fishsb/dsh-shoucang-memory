...
export function sectionOf(anchor: string): string {
 ...
 */
export function hasSection(anchor: string): boolean {
 ...
 */
export function anchorOfRecord(r: MemRecord): string {
 ...
 */
export function recordCollision(records: readonly MemRecord[], c: CollisionInit): { records: MemRecord[]; ok: boolean; id?: string; reason?: string } {
  const a = String(c.a || '').trim()
  const b = String(c.b || '').trim()
  if (!a || !b) return { records: records.slice(), ok: false, reason: '锚点缺失（碰撞须有两条锚点）' }
  if (a === b) return { records: records.slice(), ok: false, reason: '两锚点相同（不是碰撞）' }
  if (!hasSection(a) || !hasSection(b)) return { records: records.slice(), ok: false, reason: `锚点无 § 小节（无地址的两行不算碰撞：${!hasSection(a) ? a : b}）` }
  if (sectionOf(a) === sectionOf(b)) return { records: records.slice(), ok: false, reason: `跨度不足：两锚点同属 §${sectionOf(a)}（同一主题内的归纳归 principles 通道）` }
  ...
  if (i < 0) return { records: [...records, rec], ok: true, id }
 ...
 */
export function collideRecords(
  records: readonly MemRecord[],
  aId: string,
  bId: string,
  opts: { context: string; insight?: string; accepted?: boolean; evidence?: string; at: string },
): { records: MemRecord[]; ok: boolean; id?: string; reason?: string } {
  const a = records.find((r) => r.id === aId)
  const b = records.find((r) => r.id === bId)
  if (!a || !b) return { records: records.slice(), ok: false, reason: `锚点记录不存在（${!a ? aId : bId}）` }
  return recordCollision(records, { ...opts, a: anchorOfRecord(a), b: anchorOfRecord(b) })
}

/** 记/改认可状态（价态） */export function markAccepted(records: readonly MemRecord[], id: string, accepted: boolean, at: string): { records: MemRecord[]; ok: boolean; reason?: string } {
  const i = records.findIndex((r) => r.id === id && r.kind === 'association')
  if (i < 0) return { records: records.slice(), ok: false, reason: `碰撞记录不存在（${id}）` }
  const next = records.slice()
 ...
 */
export function landCollision(records: readonly MemRecord[], id: string, l: LandInit): { records: MemRecord[]; ok: boolean; reason?: string } {
  const i = records.findIndex((r) => r.id === id && r.kind === 'association')
  if (i < 0) return { records: records.slice(), ok: false, reason: `碰撞记录不存在（${id}）` }
...
export function openCollisions(records: readonly MemRecord[]): MemRecord[] {
  const l = (r: MemRecord) => r.meta?.landed
...
export function associationCensus(records: readonly MemRecord[]): AssociationCensus {
 ...
 */
export function serendipityPairs(
  records: readonly MemRecord[],
  opts: { seed?: string; k?: number; maxPerSection?: number; now?: string } = {},
): SerendipityPair[] {
  ...
  const coldOf = (r: MemRecord): number => {
    if (!r.lastHit) return 999
    const d = (nowMs - Date.parse(r.lastHit)) / 86400000
    return Number.isFinite(d) ? Math.max(0, d) : 999
  }
  const ref = (r: MemRecord): AnchorRef => ({ id: r.id, text: r.text, section: sectionOf(anchorOfRecord(r)), file: r.file, cold: Math.round(coldOf(r)) })

  // 每 § 最多取 perSection 条（冷者优先）⇒ 防"同一个筐里自己撞自己"
  const bySection = new Map<string, MemRecord[]>()
  for (const r of pool) {
    const s = sectionOf(anchorOfRecord(r))
    if (!bySection.has(s)) bySection.set(s, [])
    bySection.get(s)!.push(r)
  }
  const picked: MemRecord[] = []
  ...
  // 已撞过的对不再推荐
  const seen = new Set(records.filter((r) => r.kind === 'association').map((r) => r.meta?.pairKey).filter((x): x is string => !!x))

  const out: SerendipityPair[] = []
  for (let i = 0; i < picked.length; i++) {
    for (let j = i + 1; j < picked.length; j++) {
      const ra = picked[i], rb = picked[j]
      if (sectionOf(anchorOfRecord(ra)) === sectionOf(anchorOfRecord(rb))) continue
      if (seen.has(pairKey(anchorOfRecord(ra), anchorOfRecord(rb)))) continue
