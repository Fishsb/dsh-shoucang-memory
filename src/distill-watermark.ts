...
export function createWmApi(d: WmDeps) {
  return {
    ...
    readSegFlowState: (...a: Tail<Parameters<typeof readSegFlowState>>) => readSegFlowState(d, ...a),
  }
}
 ...
 */
export interface SegFlowState { segKey: string; phase: string; attempt: number; lastSeq: number; updatedAt: string }

const readSegFlowState = (d: WmDeps, sid: string): SegFlowState | null => {
  try {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      try {
        ...
        if (!segKey) continue // 只认**带段身份**的行（无 segKey = 与会话级状态同类的行，非段级）
        return {
          segKey,
