...
export type DeepSleepChannelName =
  | 'profiles' | 'pointers' | 'tree' | 'forget' | 'outcomes' | 'episodes' | 'converge'

/** 单通道一轮的计数：`tried` = 有提案且未落地数 · `done` = 成功落地数。 */
export interface DeepSleepChannelStat { name: DeepSleepChannelName; tried: number; done: number }
 ...
 */
export const zeroLandedChannels = (other: DeepSleepOtherChannels): DeepSleepChannelName[] =>
  (other.byChannel || []).filter((c) => c.tried > 0 && c.done === 0).map((c) => c.name)

/** 是否存在**任何**通道有提案却零落地（等价于"这一轮有东西被堵住了"）。 */
export const anyZeroLanded = (other: DeepSleepOtherChannels): boolean =>
  zeroLandedChannels(other).length > 0
