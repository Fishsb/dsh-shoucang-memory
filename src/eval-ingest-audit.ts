...
import type { EvalDeps } from './eval-channel.js'
 ...
 */
export function ingestAuditOf(
  config: IngestAuditConfig | undefined,
  io: { log?(m: string): void; audit(rec: EvalDecisionRow): void },
): ((line: string, at: { sid: string; target: string }) => Promise<void>) | undefined {
  const cfg = config || {}
  if (cfg.evalEnabled !== true) return undefined          // fail-closed：缺省/关 ⇒ 不接
  const d: EvalIngestDeps = {
    cfg: {
      evalEnabled: true,
  ...
  return async (line, at) => {
    try { await auditIndexLine(d, line, at) } catch { /* 宁可不记，不可打断主线 */ }
  }
}

/** 索引行 → { 主题, 概况 }（沿索引行文法：`[tag] 主题 · 概况 → 指针`）。 */
export function splitIndexLine(line: string): { topic: string; summary: string } | null {
  let body = String(line || '')
  const pmi = body.search(/\s*(?:→|←)\s*/)
  if (pmi >= 0) body = body.slice(0, pmi)
 ...
 */
export async function auditIndexLine(
  d: EvalIngestDeps,
  line: string,
  at: { sid: string; target: string },
): Promise<'off' | 'ok' | 'mismatch'> {
  if (!d.cfg.evalEnabled) return 'off'
  const parts = splitIndexLine(line)
  if (!parts) return 'off'

  let r: Awaited<ReturnType<typeof evaluate>>
  try {
    r = await evaluate(d as unknown as EvalDeps, `${parts.topic} · ${parts.summary}`, {
      matches: {
        type: 'boolean',
  ...
  const conf = confidenceMinOf(r.answers as never)
  ...
  try {
    ...
    }) as unknown as EvalDecisionRow)
  } catch { /* 落账失败静默 */ }

  if (r.outcome !== 'ok') return 'off'                     // 未成功判出 ⇒ 既不放行也不拒收
