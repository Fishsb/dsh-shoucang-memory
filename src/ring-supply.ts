...
import { ringOfKind, type Ring } from './rings.js'
...
const KIND_LABEL: Partial<Record<RecordKind, string>> = {
  decision: '决策',
  outcome: '后果',
  relation: '关系',
  commitment: '承诺',
 ...
 */
function dueRank(meta: Record<string, string> | undefined, at: string): number {
  const due = String(meta?.due ?? '').trim()
  if (!due) return 1
  const t = Date.parse(due)
  const now = Date.parse(at)
  if (!Number.isFinite(t) || !Number.isFinite(now)) return 1
  return t < now ? 0 : 2
}
 ...
 *
 * ★ 2026-09-20 **实现已下移至 `due-window.ts`**（按领域接缝抽出，**纯移动**）：
 *   动因 = 册三/册四需要 `relation-ring#trustOf` 统计逾期数，而 `relation-ring → ring-supply`
 *   是逆向依赖（实测把分层深度顶到 13 > 10，`audit-architecture --gate` 当场红）。
 *   此处**再导出**保持既有 API 与调用点零迁移（判据 `check-layering` / 本件单测均按原符号）。
 *   判因与纪律（坏时间戳按"无 due"处理、窗口缺省 7 天唯一）随实现一并迁走，勿在此另写第二份。
 */
export { dueSoon } from './due-window.js'
/** 本件内部调用点用（`export … from` **不引入本地作用域** ⇒ 需单独 import，实测踩过）。 */
import { dueSoon } from './due-window.js'

/** 渲染一行（人读可辨：环标签 + 正文 + 关键 meta）。**渲染只此一处**，panel/深睡/工具共用。 */
export function renderRingLine(r: MemRecord): string {
  ...
  if (dir === 'owed-by-me') bits.push('我欠')
  else if (dir === 'owed-to-me') bits.push('欠我')
  const hit = String(r.meta?.hit ?? '').trim()
  if (hit) bits.push(hit === '1' ? '已应验' : '未应验')
...
export function ringCandidates(
  ...
  const order = (opts.rings && opts.rings.length ? opts.rings : DEFAULT_RING_ORDER) as readonly Ring[]
  ...
  cands.sort((a, b) => {
    ...
    const grp = (c: typeof a): number => (c.why === 'cue' ? 0 : c.why === 'due' ? 1 : 2)
    const ga = grp(a)
    const gb = grp(b)
    if (ga !== gb) return ga - gb
    if (ga === 0 && a.hits.length !== b.hits.length) return b.hits.length - a.hits.length
...
export const DEFAULT_RING_ORDER: readonly Ring[] = ['relation', 'decision', 'association', 'fact']
...
export function createRingSupplyApi(d: RingSupplyDeps = {}): RingSupplyApi {
