...
export const RINGS = ['fact', 'decision', 'relation', 'association', 'value', 'none'] as const
export type Ring = (typeof RINGS)[number]

/** kind → 环（**唯一声明处**） */
export const RING_OF_KIND: Readonly<Record<RecordKind, Ring>> = {
 ...
 * 当前为空 —— 五个环全部已落 kind。
 */
export const RING_PENDING: readonly Ring[] = []
 ...
 */
export const FREQUENCY_FREE_RINGS: readonly Ring[] = ['decision', 'association']

export function ringOfKind(kind: RecordKind): Ring {
  return RING_OF_KIND[kind] ?? 'none'
}

export function kindsOfRing(ring: Ring): RecordKind[] {
  return KINDS.filter((k) => ringOfKind(k) === ring)
}

/** 未登记环归属的 kind（应为空；非空即机检 FAIL） */
export function uncoveredKinds(): RecordKind[] {
  return KINDS.filter((k) => !RING_OF_KIND[k])
}

/** 已登记环却无 kind 且未申报为 pending 的环（应为空；非空即机检 FAIL） */
export function emptyUndeclaredRings(): Ring[] {
 ...
 */
export function stalePendingRings(): Ring[] {
  return RING_PENDING.filter((r) => kindsOfRing(r).length > 0)
}

export function isFrequencyFree(ring: Ring): boolean {
 ...
 */
export const RING_KPI: Readonly<Record<Exclude<Ring, 'none'>, { module: string; fn: string; what: string }>> = {
...
export function ringCensus(records: readonly MemRecord[]): Record<Ring, number> {
  const out = {} as Record<Ring, number>
  for (const r of RINGS) out[r] = 0
  for (const rec of records) out[ringOfKind(rec.kind)]++
  return out
}
