...
/** 已归一化的头表（内部用；外部入参可为 undefined，由 `judgePanelRequest` 兜底为 `{}`）。 */
type HeaderBag = Record<string, string | string[] | undefined>

/** 取单值头（数组取首项；缺省 undefined）。 */
...
/**
 * 归一化 hostname：去掉端口、去方括号（IPv6）、小写。
 * 传入的是 `Host` / `Origin` 里的 authority 部分。
 */
function hostnameOf(authority: string): string | undefined {
  ...
  if (s.startsWith('[')) {
    const end = s.indexOf(']')
    return end > 0 ? s.slice(0, end + 1).toLowerCase() : undefined
  }
 ...
 */
export function judgePanelRequest(facts: PanelRequestFacts): { trusted: boolean; reason?: string } {
  const headers: HeaderBag = facts?.headers ?? {}
  const host = headerOf(headers, 'host')
  // ① Host 缺失 ⇒ 拒（Host 是 rebinding 唯一无法伪造的头，缺失即不可裁决）
  if (!host) return { trusted: false, reason: 'missing-host' }
  const hostName = hostnameOf(host)
  if (!hostName) return { trusted: false, reason: 'malformed-host' }
  // ② Host 非 loopback ⇒ 拒（DNS rebinding 的典型形态）
  if (!isLoopbackHostname(hostName)) return { trusted: false, reason: 'non-loopback-host' }
  // ③ 显式跨站标记 ⇒ 拒
  if (headerOf(headers, 'sec-fetch-site') === 'cross-site') return { trusted: false, reason: 'cross-site' }
  // ④ Origin 存在则须与 Host 同源；缺失放行（非浏览器客户端不带它）
  ...
  try {
    ...
    return { trusted: false, reason: 'malformed-origin' }
  }
  if (originHost !== host.toLowerCase()) return { trusted: false, reason: 'cross-origin' }
  return { trusted: true }
}
