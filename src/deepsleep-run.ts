...
async function attributeRecallMisses(ctx: any, parent: any, agentOptions: any): Promise<any> {
    ...
    try {
        ...
        }, (o: any) => o?.kind === 'mcl-step' && isAttributionRow(o), ATTRIBUTION_SAMPLE_LIMIT)
        missRows = scan.rows
    ...
    try {
        ...
        return { verdict: null, agrees: null, samples: samples.length, scanned, calibration: calibVerdict, note: '调用异常：' + String((e as Error)?.message || e).slice(0, 60) }
    }
}
...
async function judgeYieldRounds(ctx: any, parent: any, agentOptions: any, bankRoot: string): Promise<any> {
    ...
    try {
        ...
        appendLedgerLine(join(knowledgeRoot(), 'audit', 'ledger.jsonl'), envelopeEvent(buildYieldVerdictEvent(judged, rounds, String((rows[rows.length - 1] as any)?.sid || '')), YIELD_VERDICT_TYPE))
        return {
        ...
        return { ...blank, error: 'threw', note: '调用异常：' + String((e as Error)?.message || e).slice(0, 60) }
    }
}
