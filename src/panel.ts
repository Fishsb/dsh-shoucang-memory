...
export function applyPanel(ctx: Context, config: Config): void {
  ...
  const injectCache: { key: string | null; at: number; text: string } = { key: null, at: 0, text: '' }
  ...
  const buildHotMemoryText = (query = ''): string => {
    const now = Date.now()
    const memRoot = memoryRootOf()
    const q = String(query || '').replace(/\s+/g, ' ').trim().slice(0, 200)
    const cacheKey = memRoot + '|' + q
    if (injectCache.key === cacheKey && now - injectCache.at < 30000) return injectCache.text
  ...
  const memoryCaps = (base: string): Record<string, number> => {
    ...
    try {
      const s = readSuiteConfig() as Record<string, unknown>
  ...
  ctx.effect(() => {
    ...
    if (sp && typeof sp.context === 'function') {
      ...
      disposers.push(sp.context({
        ...
        text: (context?: unknown) => { injectMeta.calls++; injectMeta.lastAt = Date.now(); return buildHotMemoryText(taskTextOf(context)) },
      }))
