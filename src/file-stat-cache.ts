...
/** 缓存条目（value 类型由调用点决定，读回时由调用方按同一 produce 断言） */
...
const keyOf = (kind: string, path: string): string => kind + '\u0000' + path

function statOf(path: string): { mtimeMs: number; size: number } | null {
  try { const st = statSync(path); return { mtimeMs: st.mtimeMs, size: st.size } } catch { return null }
}

function cached<T>(kind: string, path: string, produce: (st: { mtimeMs: number; size: number }) => T): T | null {
  ...
  if (hit && hit.mtimeMs === st.mtimeMs && hit.size === st.size) {
    fileReadStats.hits++
    cache.delete(key); cache.set(key, hit) // LRU 提升（Map 保序）
...
/** 清空全部缓存（自测/校准用） */
export function clearFileStatCache(): void { cache.clear() }

/**
 * 非空行数（缓存）。文件不存在或不可读 ⇒ `-1`（调用方按既有 `existsSync` 守卫处理）。
 * ⚠ 语义 = 「非空行数」，与原先 `readFileSync(...).split('\n').filter(trim).length` **逐字等价**。
 */
export function nonEmptyLineCount(path: string): number {
  const v = cached<number>('lines', path, (st) => {
 ...
 */
export function readTailLines(path: string, n: number, maxBytes = 256 * 1024): string[] {
  const v = cached<string[]>('tail:' + n + ':' + maxBytes, path, (st) => {
    if (st.size === 0) return []
    ...
    try {
      ...
      if (pos > 0 && lines.length) lines.shift()
...
export function statSize(path: string): number {
  const v = cached<number>('size', path, (st) => st.size)
  return v === null ? -1 : v
}
