...
export function registerMcl(
  ...
  const counters = { steps: 0, fast: 0, slow: 0, injected: 0, nudged: 0, lastAt: 0, lastChannel: '' as '' | 'fast' | 'slow', lastSim: 0 }
  ...
  const captureFromEvent = (session: any, event: any): void => {
    try {
      if (!event || event.type !== 'user/message') return
      const sid = String(session?.id || '')
      if (!sid) return
      const d = event.data || {}
      if (String(d?.source?.kind || '') !== 'user') return
