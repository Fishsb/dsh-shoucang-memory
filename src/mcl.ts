...
import { foldZeroGain, shouldSwitchSource, YIELD_VERDICT_TYPE, shortSidOf, type YieldVerdict } from './recall-yield.js'
...
interface SessMcl {
  ...
  deciding?: boolean
  /* ── 2026-09-23（圆桌会议定稿 · 消费链拟态落地方案 §3）：回流三态 ─────────────────────────
   * 判因（实测）：`mcl-switch` 10 行（全在 D5 前）与 `yieldSwitchSemantic=true` 14 次**跨日错开**，
   *   配对检验 60s → 0/10、3600s → 0/10；且本文件**全文零 `readFileSync`** ⇒ 回流侧**物理上不存在**。
   *   ⇒ 本组字段即那条缺失的回流边：离线模型判定 → 台账行 → 本会话步内判据。 */
  ...
  lastJudge?: { step: number; zeroGain: number; switchSource: boolean; topicEcho: boolean | null }
 ...
 */
export function planStepJudgement(input: { zeroGain: number; hasTopics: boolean; switchEmitted: boolean; judged?: boolean }): { zeroGain: number; switchSource: boolean; emitSwitch: boolean; stopSource: boolean } {
 ...
 */
export function readLatestVerdict(ledgerFile: string, sid: string): { verdict: YieldVerdict; at: string } | undefined {
  ...
  if (!s8) return undefined
  const row = readLatestLedgerRow<{ verdict?: unknown; at?: unknown }>(ledgerFile, (o) =>
    String(o?.type) === YIELD_VERDICT_TYPE && String(o?.sid) === s8)
  const v = row?.verdict
  if (v !== 'helped' && v !== 'not-helped' && v !== 'unjudged') return undefined
...
export async function handlePreStep(payload: any, next: () => Promise<any>, dep: PreStepDeps): Promise<any> {
    ...
    try {
      ...
      if (!st.channel) {
        ...
        if (!res.decided || res.channel !== 'slow' || !res.material) { auditJudge(null, 'decide-none', { judged: false }); return decision }
      ...
      if (st.channel === 'fast') { auditJudge(null, 'fast', { judged: false }); return decision }
      ...
      if (step <= Number(st.materialStep || 0)) { auditJudge(null, 'material-step', { judged: false }); return decision }
      ...
      try {
        ...
        if (v) { st.verdict = v.verdict; st.verdictAt = v.at }
      ...
      const folding = alreadyFolded
        ? { zeroGain: Math.max(0, Number(st.zeroGain) || 0), signal: 'none' as const, judged: false, reason: 'no-evidence' as const }
      ...
      if (folding.judged && verdict) st.verdictFolded = foldKey
