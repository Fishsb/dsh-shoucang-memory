...
export async function applyForgetOps(
  ...
  const record = (o: Record<string, unknown>): void => {
    try { appendFileSync(archFile, envelope(o, `archive.${String((o as { op?: string; action?: string; kind?: string }).op || (o as { action?: string }).action || (o as { kind?: string }).kind || 'event')}`), 'utf8') } catch { /* 静默 */ }
  }
