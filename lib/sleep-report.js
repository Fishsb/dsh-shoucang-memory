...
export const sleepLedgerStreamOf = (kRoot) => join(kRoot, 'audit', 'sleep-report-ledger.jsonl');
/** 复算某份报告的指纹（**与 `writeSleepRound` 同一实现** ⇒ 账与真身不可能漂移）。 */
export function fingerprintOf(file, date, at, firstSeenAt) {
...
export function firstSeenOf(kRoot, dateFile) {
    let out = '';
    for (const r of parseRows(sleepLedgerStreamOf(kRoot), 2000)) {
        if (String(r.dateFile || '') !== dateFile)
            continue;
...
const tail = (file, n, maxBytes = TAIL_MAX_BYTES) => { try {
    return readTailLines(file, n, maxBytes).filter(Boolean);
}
catch {
    return [];
} };
const parseRows = (file, n, maxBytes = TAIL_MAX_BYTES) => {
    const out = [];
    for (const l of tail(file, n, maxBytes)) {
        try {
...
export function buildImpactRows(d, limit = 3000) {
    ...
    const act = new Map();
    for (const r of parseRows(join(d.bankRoot, 'audit', 'activity.jsonl'), limit))
...
export function issueRowsOf(memRoot, impact, at = new Date().toISOString(), kRoot) {
    const out = [];
    const seen = kRoot ? firstObservedMap(kRoot) : new Map();
...
export function firstObservedMap(kRoot) {
    const out = new Map();
...
export function statsOf(impact, issues, produced = 0) {
...
export function buildReportSection(i, stats, issues) {
    ...
    for (const s of issues.slice(0, 20))
        lines.push(`- \`${s.tag}\` ${s.file}${s.section ? ' §' + s.section : ''} · ${s.evidence} · **未处理**（${s.why}） · 首次标记 ${s.unusedAtFirstObservation} · ${s.stillUnused ? '**仍未真读**' : '已恢复真读'}\n`);
...
export function roundInputFromLedger(d, fallbackDate = new Date().toISOString()) {
    /* 反向扫描全部档位（最旧 → 主档）里的行；`kind` 不在预筛条件里 —— 必须先 parse 才能看 kind。
     * ⚠ 与 `parseRows` 的差别：那次是"取末 4000 行再 parse"，这次是"parse 全量再筛 kind"。
     *   台账单行 ~200–400B、总量 MB 级，且本函数**每轮睡眠只调一次** ⇒ 全量 parse 的代价可接受
     *   （对照：`readLedgerVolumes` 已被 `pointer-deficits` / `panel-observe` / `deepsleep-run` 采用，同量级）。 */
    const all = readLedgerVolumes(join(d.kRoot, 'audit', 'ledger.jsonl'));
    const rows = [];
    for (const l of all) {
        try {
...
export function writeSleepReportFromLedger(d, untilMs) {
    ...
    if (!input)
        return { ok: false, reason: 'no-deep-sleep-round（尾读窗口 2MB/4000 行内未见 kind=deep-sleep —— 见 sleep-report#TAIL_MAX_BYTES 的口径说明）' };
