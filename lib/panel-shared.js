...
export function createSuiteConfig(warn) {
    ...
    return {
        read: () => {
            try {
                ...
                warn?.(`[shoucang] scheduler.json 读取失败 ⇒ 面板按空配置渲染（配置可能已损坏）：${String(e?.message || e).slice(0, 120)}`);
                return {};
            }
        },
...
const readDawnGrowth = (personaMode) => {
    if (personaMode === 'off')
        return '';
    try {
        const rows = latestDerivation({ kRoot: knowledgeRoot() }).slice(0, 3).filter((r) => !!String(r).trim());
        if (rows.length)
            return ['🧠 最近成长（上次深睡归纳 · 数值同睡眠汇报）：', ...rows.map((r) => `  ${String(r).trim()}`)].join('\n');
...
function buildHotMemoryText(d, cache, query = '', omitStable = false) {
    ...
    const resolved = resolveSupplyBudget(sched, SURFACE.injection);
    const budgetClamped = [...resolved.clamped];
    ...
    const rowCaps = resolved.levelCaps;
    ...
    const procCfg = SURFACE.injection.process;
...
export const probeLocalEmbed = async (baseUrl) => {
    ...
    try {
        ...
        const r = await runProcAsync(process.execPath, ['-e', probe], { timeoutMs: 12000, env: env2 });
