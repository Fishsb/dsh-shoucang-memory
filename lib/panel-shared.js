...
import { assembleSupply, budgetOf, clampLines, renderSupplyText } from './supply-assembly.js';
...
import { cuesOf } from './situation-key.js';
 ...
 */
function situationLinesOf(memRoot, cues, at, budget) {
 ...
 */
function supplyUsageMeta(cand, budget, cues, 
/** S4R/R2：**主路径的真实裁切结果**（保留行数 + 被丢行内容）。缺省 ⇒ 回落装配器口径（兼容旧调用） */
real) {
    const b = { stable: budget.stable, dynamic: budget.dynamic, oneshot: budget.oneshot, serendipity: 0, situation: budget.situation };
    const r = assembleSupply({ stable: [...cand.stable], dynamic: [...cand.dynamic], oneshot: [...cand.oneshot], situation: [...cand.situation] }, b);
    return {
        usage: {
            at: Date.now(),
            chars: r.meta.chars,
...
const upstreamStampOf = (memRoot) => {
    ...
    if (kRoot)
        parts.push(sizeStampOf(kRoot, ['delta.md']));
    return parts.join('|');
};
/** P2：逐行累计字符数并裁到 cap 以内；返回 [保留行, 被丢弃行数]（供调用方**可见化**留痕） */
function buildHotMemoryText(d, cache, query = '') {
    const now = Date.now();
    ...
    const memLines = selectDynamicLines({
        ...
        processRows: SURFACE.injection.process?.enabled === true ? scanIndexRows(memRoot).map((r) => r.line) : undefined,
    });
    ...
    if (cache.stableKey === stableKey && now - cache.stableAt < 120000) {
        ...
        if (agentLines.length) {
            ...
            for (const l of agentLines)
                agentBlock.push(/^-\s/.test(l) ? l : `- ${l}`);
        }
        const userBlock = [];
        if (userLines.length) {
            userBlock.push('用户画像（USER.md）：');
            for (const l of userLines)
                userBlock.push(/^-\s/.test(l) ? l : `- ${l}`);
        }
        ...
        const sl = playbookEnabled(d.suite) ? [head, ...MEMORY_PLAYBOOK_LINES, ...a2] : [head, ...a2];
        if (dA.length > 0)
            sl.push(`（agent 画像另省略 ${dA.length} 行，按需 get_file 读取）`);
        sl.push(...u2);
        if (dU.length > 0)
            sl.push(`（用户画像另省略 ${dU.length} 行，按需 get_file 读取）`);
    ...
    const shadow = supplyUsageMeta({ stable: [...agentLines, ...userLines], dynamic: memLines, oneshot: deltaText ? deltaText.split('\n') : [], situation: sitLines }, { stable: capStable, dynamic: capDynamic, oneshot: capOneshot, situation: sitBudget }, sitCues, 
...
export const probeLocalEmbed = (baseUrl) => {
