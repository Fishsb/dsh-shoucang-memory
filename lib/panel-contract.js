...
export const DEEPSLEEP_CONFIG_KEYS = ['enableDeepSleep', 'deepSleepProbe', 'deepSleepIdleMs', 'deepSleepProbeAfterMs', 'deepSleepProbeWindowMs'];
/** 嵌入配置补丁白名单（与 panel-inject 的 EMBED_CONFIG_KEYS 一致；门禁比对防漂移） */
export const EMBED_CONFIG_KEYS = ['embedEnabled', 'embedBaseUrl', 'embedModel', 'embedApiKeyEnv'];
...
export const EVAL_CONFIG_KEYS = ['evalEnabled', 'evalBaseUrl', 'evalModel', 'evalApiKeyEnv', 'evalTier', 'evalEgressAllow'];
export const DISTILL_CONFIG_KEYS = ['enableDistill', 'idleWakeMs', 'minTurnChars', 'distillPrescan', 'llmProvider', 'llmModel', 'distillProvider', 'distillModel', 'distillEffort', 'sleepProvider', 'sleepModel', 'sleepEffort'];
...
export const MCL_CONFIG_KEYS = ['mclEnabled', 'mclFamiliarThreshold', 'mclMaxNudges', 'mclBudgetChars', 'mclTopK', 'mclAudit', 'mclMaterialInSystem', 'enableRemPass'];
const zAny = z.any();
...
export const SET_SCALAR_KEYS = [
    ...
    // ADR-333（2026-09-22）：容量门是否阻断（枚举 on|off → 布尔 capacityEnforce）。
    //   ⚠ 本表与 `panel-config.ts` 的 `allowed` 两处由 `check-budget-override` ⑦c **逐键断言锁死**，
    //     必须同批改——只加一处 ⇒ 机检红（且实测过"只加白名单没加映射 ⇒ 400 假可控"两次）。
...
export const PANEL_ROUTES = [
    ...
    /* S2S3 册四（2026-09-19）：睡眠汇报 + 问题统计（**只读两条**；册四白名单即这两条）。
     *  前者 = 日历式汇报列表（份数/段数/最近一份正文）；后者 = 问题分布（召回面 vs 记忆面，分母=睡眠轮数）。 */
    { path: '/sleep/reports', summary: '睡眠汇报列表（日历式留存：份数 · 段数 · 最近一份）' },
    { path: '/sleep/issues', summary: '睡眠问题统计（suspect-recall=召回面 / suspect-quality=记忆面）' },
    ...
    /* M1（ACT-283）可配置评估通道：白名单补丁（同 embed 口径——整表 required 会误拒"只改 key"的合法补丁）。 */
    { path: '/eval/config', summary: '评估通道配置读写（白名单补丁）', contract: {
            body: z.object(Object.fromEntries(EVAL_CONFIG_KEYS.map((k) => [k, zAny.required(false)])))
        } },
    { path: '/eval/test', summary: '评估通道连通性测试（只读；不落库）', contract: { required: [] } },
    { path: '/eval/stats', summary: '评估通道判定统计（只读；折统一台账 type=eval.decision）' },
...
export function contractFor(path) {
    return BY_PATH.get(path)?.contract;
}
//# sourceMappingURL=panel-contract.js.map