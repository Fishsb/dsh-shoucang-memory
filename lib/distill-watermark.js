...
export function createWmApi(d) {
    return {
        readWatermarks: (...a) => readWatermarks(d, ...a),
        writeWatermark: (...a) => writeWatermark(d, ...a),
        sessionFormatVersionOf: (...a) => sessionFormatVersionOf(d, ...a),
        agentFingerprintAt: (...a) => agentFingerprintAt(d, ...a),
        discardWatermark: (...a) => discardWatermark(d, ...a),
        resolveWatermark: (...a) => resolveWatermark(d, ...a),
        readRunState: (...a) => readRunState(d, ...a),
        readSegFlowState: (...a) => readSegFlowState(d, ...a),
    };
}
const readWatermarks = (d) => {
    const map = new Map();
    try {
...
const readSegFlowState = (d, sid) => {
    try {
        // 尾部窗口取足够大以覆盖「段级行被若干无 segKey 行隔开」的常见距离（每轮 2–4 行写入）
        const lines = readTailLines(d.watermarkFile, 2000);
        for (let i = lines.length - 1; i >= 0; i--) {
            try {
                const o = JSON.parse(lines[i]);
                if (o.sessionId !== sid)
                    continue;
                const segKey = typeof o.segKey === 'string' ? o.segKey : '';
                if (!segKey)
                    continue; // 只认**带段身份**的行（无 segKey = 与会话级状态同类的行，非段级）
                return {
                    segKey,
                    phase: typeof o.phase === 'string' ? o.phase : 'unknown',
                    attempt: Number(o.attempt) || 0,
                    lastSeq: Number(o.lastSeq) || 0,
                    updatedAt: typeof o.at === 'string' ? o.at : '',
                };
            }
            catch { /* 坏行跳过 */ }
        }
...
const writeWatermark = (d, sessionId, lastSeq, agent, run) => {
    try {
...
const sessionFormatVersionOf = (d, agent) => {
    try {
...
const agentFingerprintAt = (d, agent, seq) => {
    try {
 ...
 */
const discardWatermark = (d, sid, reason, agent, wm) => {
 ...
 */
const resolveWatermark = (d, sid, agent, mapCache) => {
...
//# sourceMappingURL=distill-watermark.js.map