...
import { relationCensus, trustOf, overdueCommitments } from './relation-ring.js';
...
const DISTILL_CONFIG_KEYS = ['enableDistill', 'idleWakeMs', 'minTurnChars', 'distillPrescan', 'llmProvider', 'llmModel', 'distillProvider', 'distillModel', 'distillEffort', 'sleepProvider', 'sleepModel', 'sleepEffort'];
...
const validateDistillConfig = (c) => {
    ...
    if ('distillEffort' in c && typeof c.distillEffort !== 'string')
        return 'distillEffort 须为字符串';
    if ('sleepEffort' in c && typeof c.sleepEffort !== 'string')
        return 'sleepEffort 须为字符串';
    return null;
};
const runSelfCheckNow = async (d, lock) => {
    const script = join(memoryLibRoot(), 'scripts', 'sleep-selfcheck.mjs');
    if (!existsSync(script))
        return { active: false, error: 'sleep-selfcheck.mjs 未部署' };
    if (lock.running)
        return { active: false, error: 'already-running', note: '上一次自检仍在跑（并发被拒绝，不排队）' };
    lock.running = true;
    try {
        ...
        if (!r.ok)
            return { active: false, error: (r.timedOut ? '自检超时（180s）' : '自检失败') + (r.err ? '：' + r.err.trim().slice(0, 200) : '') };
        return { active: true, ...JSON.parse(readFileSync(out, 'utf8')) };
    }
    catch (e) {
        return { active: false, error: String(e).slice(0, 200) };
    }
    finally {
        lock.running = false;
    }
 ...
 */
function ringsRoute(_d, _req, res) {
    try {
        ...
        sendJson(res, 200, {
            ...
            relation: { ...relationCensus(records), trust: who.map((w) => trustOf(records, w, at)), pendingAdjudication: overdueCommitments(records, at).map((o) => ({ id: o.record.id, who: o.record.meta?.who ?? '', what: o.record.meta?.what ?? '', due: o.due, overdue: o.overdue })) },
...
function suiteRoute(d, _req, res) {
    try {
        const api = d.scheduler.current;
        if (!api)
            return sendJson(res, 200, { members: [], summary: '调度器未就绪（装配面经 composition root 显式传入；空成员=如实空态）' });
 ...
 */
function mclAuditRecent(root, n = 10) {
    const out = [];
...
function mclStatusRoute(d, _req, res) {
    try {
        const recent = mclAuditRecent(knowledgeRoot());
        const mclApi = d.mcl.current;
        if (!mclApi)
            return sendJson(res, 200, { active: false, reason: 'mcl-not-ready', recent });
...
async function reconcileRoute(d, _req, res) {
    try {
        const script = join(memoryLibRoot(), 'scripts', 'memory-reconcile.mjs');
        if (!existsSync(script))
            return sendJson(res, 200, { active: false, error: 'memory-reconcile.mjs 未部署（跑 npm run build 后同步 skill/scripts）' });
        ...
        if (!r.ok)
            return sendJson(res, 200, { active: false, error: (r.timedOut ? 'reconcile 超时（30s）' : 'reconcile 失败') + (r.err ? '：' + r.err.trim().slice(0, 200) : '') });
        const out = JSON.parse(readFileSync(tmp, 'utf8'));
        sendJson(res, 200, { active: true, ...out });
    }
    catch (e) {
        sendJson(res, 200, { active: false, error: String(e).slice(0, 200) });
    }
}
...
async function maturationScanRoute(d, _req, res) {
    try {
        const script = join(memoryLibRoot(), 'scripts', 'maturation-scan.mjs');
        if (!existsSync(script))
            return sendJson(res, 200, { active: false, error: 'maturation-scan.mjs 未部署（库内 scripts/ 缺失）' });
        const tmp = join(knowledgeRoot(), 'audit', '.maturation-tmp.json');
        const r = await runProcAsync(process.execPath, [script, '--json', '--out', tmp], { timeoutMs: 30000 }); // D-I4：原 execFileSync 同步阻塞宿主 30s；2026-09-25 'node'→execPath
        if (!r.ok)
            return sendJson(res, 200, { active: false, error: (r.timedOut ? 'maturation 超时（30s）' : 'maturation 失败') + (r.err ? '：' + r.err.trim().slice(0, 200) : '') });
        const out = JSON.parse(readFileSync(tmp, 'utf8'));
        sendJson(res, 200, { active: true, ...out });
    }
    catch (e) {
...
function selfcheckRoute(d, _req, res) {
    try {
        const f = join(knowledgeRoot(), 'audit', 'selfcheck-latest.json');
        if (!existsSync(f))
            return sendJson(res, 200, { active: false, error: '尚未跑过自检（POST /selfcheck/run 立即跑一次）' });
        sendJson(res, 200, { active: true, ...JSON.parse(readFileSync(f, 'utf8')) });
    }
    catch (e) {
        sendJson(res, 500, { error: String(e) });
    }
}
async function selfcheckRunRoute(d, lock, _req, res) {
    try {
        sendJson(res, 200, await runSelfCheckNow(d, lock));
    }
    catch (e) {
        sendJson(res, 500, { error: String(e).slice(0, 200) });
    }
}
function configRecentRoute(d, _req, res) {
    try {
        const recent = [];
        try {
            ...
            for (const l of lines.slice(-5).reverse()) {
                ...
                if (m)
                    recent.push({ at: Number(m[1]) * 1000, msg: m[2] });
            }
        }
        catch { /* 无库 git */ }
        let configMtime = 0;
        try {
            configMtime = statSync(join(dshHome(), 'suite', 'scheduler.json')).mtimeMs;
        }
        catch { /* 无配置文件 */ }
 ...
 */
function contentTypesRoute(_d, _req, res) {
    try {
        ...
        const byBudget = all.reduce((m, x) => { m[x.type.budget] = (m[x.type.budget] || 0) + 1; return m; }, {});
        sendJson(res, 200, {
            total: all.length,
            byLayer,
            byBudget,
...
function criteriaRoute(d, _req, res) {
    try {
        let reg = null;
        try {
            reg = JSON.parse(readFileSync(join(memoryLibRoot(), 'engine', 'criteria-gate.json'), 'utf8'));
        }
        catch { /* 投影未部署 */ }
        let ledgerRows = 0;
        let lastAt = 0;
        try {
            ...
            if (last)
                lastAt = Date.parse(JSON.parse(last).at || '') || 0;
        }
        catch { /* 无台账 */ }
        sendJson(res, 200, {
            active: !!reg,
            ...
            rerankGate: (() => {
                let indexRows = 0;
                try {
                    indexRows = readFileSync(join(memoryLibRoot(), 'MEMORY.md'), 'utf8').split(/\r?\n/).filter((l) => /^\[/.test(l.trim())).length;
                }
                catch { /* 缺库 */ }
 ...
 */
export function sleepSummaryOf(sleeps) {
    const byResult = {};
    const byGate = {};
    ...
    for (const s of sleeps) {
        ...
        if (s.stop == null && s.gate == null && s.result == null) {
            legacy++;
            continue;
        }
                ...
                for (const l of readDistillAuditText(f).split('\n')) {
                    ...
                    try {
                        ...
                        sleeps.push({
                            ...
                            gateDetail: Array.isArray(o.rejectedLines) ? o.rejectedLines.slice(0, 5) : [],
                        });
                    }
                    catch { /* 坏行跳过 */ }
                }
...
async function llmModelsRoute(d, _req, res) {
    try {
        const api = d.scheduler.current;
        if (!api || typeof api.llmModels !== 'function')
            return sendJson(res, 200, { models: [], note: '调度器未就绪（llmModels 不在装配面）' });
...
function deepsleepRoute(d, _req, res) {
    try {
...
async function deepsleepTriggerRoute(d, _req, res) {
    try {
        if (TRIGGER.manual === false)
            return sendJson(res, 403, { ok: false, error: 'manual-trigger-disabled-by-registry' }); // B 档：读注册表 criteria.trigger.manual
        const sleep = d.deepSleep.current;
        if (!sleep)
...
async function distillRunRoute(d, _req, res) {
    try {
        const sleep = d.deepSleep.current;
        if (!sleep || typeof sleep.runDistillNow !== 'function')
            return sendJson(res, 400, { ok: false, error: 'distill-not-ready' });
...
async function deepsleepConfigRoute(d, req, res) {
    try {
        if (req.method === 'POST') {
        ...
        const sleep = d.deepSleep.current;
        if (sleep)
            return sendJson(res, 200, { active: true, running: sleep.getConfig(), persisted: d.suite.read() });
...
async function distillConfigRoute(d, req, res) {
    try {
        if (req.method === 'POST') {
            const body = (await readBody(req).catch(() => ({})));
            const patch = {};
...
export function registerObserveRoutes(d) {
    ...
    const selfcheckLock = { running: false };
...
function sleepReportsRoute(_d, _req, res) {
    try {
...
function sleepIssuesRoute(_d, _req, res) {
    try {
        const file = join(knowledgeRoot(), 'audit', 'sleep-issues.jsonl');
        const rows = readTailLines(file, 2000).map((l) => { try {
            return JSON.parse(l);
        }
        catch {
            return null;
        ...
        const byTag = {};
        for (const r of rows) {
            const t = String(r.tag || 'unknown');
            byTag[t] = (byTag[t] || 0) + 1;
        }
        const roundFile = join(knowledgeRoot(), 'audit', 'sleep-reports.jsonl');
        const rounds = readTailLines(roundFile, 2000).map((l) => { try {
            return JSON.parse(l);
        }
        catch {
            return null;
        } }).filter((x) => !!x).filter((r) => r.kind === 'sleep-round');
...
//# sourceMappingURL=panel-observe.js.map