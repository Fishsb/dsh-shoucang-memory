...
async function attributeRecallMisses(ctx, parent, agentOptions) {
    ...
    if (samples.length < ATTRIBUTION_MIN_SAMPLES)
        return { verdict: 'insufficient', agrees: null, samples: samples.length, calibration: calibVerdict, note: `未命中样本 ${samples.length} < ${ATTRIBUTION_MIN_SAMPLES}` };
    ...
    try {
        ...
        return { verdict: null, agrees: null, samples: samples.length, calibration: calibVerdict, note: '调用异常：' + String(e?.message || e).slice(0, 60) };
    }
}
