...
import { DEEP_SLEEP_PROMPT, deepSleepLanded, liveFailPolicy, planDeepSleepVerdict, splitByCap, windowMaterialBytes } from './deepsleep-core.js';
...
async function reviewReleaseSemantics(ctx, parent, agentOptions, bankRoot) {
    ...
    try {
        ...
        return { candidates: plan.candidates.length, approved: 0, rejected: 0, unjudged: 0, note: '调用异常：' + String(e?.message || e).slice(0, 60) };
    }
}
...
async function reviewReleaseSemanticsBatched(ctx, parent, agentOptions, bankRoot) {
    ...
    try {
        ...
        return { ...base, batches: reqs.length, note: '调用异常：' + String(e?.message || e).slice(0, 60) };
    }
}
