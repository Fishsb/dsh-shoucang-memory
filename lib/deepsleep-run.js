...
import { planRelease, buildSemanticReviewRequest, buildSemanticReviewBatches, parseSemanticReview, intersectApprovals, semanticApprovedRows, stableSetHashes } from './essence-release.js';
...
async function reviewReleaseSemanticsBatched(ctx, parent, agentOptions, bankRoot) {
    const plan = planRelease(bankRoot);
    const base = { candidates: plan.candidates.length, approved: 0, rejected: 0, unjudged: 0, batches: 0, sample: [] };
    if (!plan.candidates.length)
    ...
    try {
        ...
        return { candidates: plan.candidates.length, approved: stable.length, rejected: m.rejected, unjudged: m.unjudged, ungrounded: m.ungrounded, batches: reqs.length * 2, sample, hashes, note: `${reqs.length}×2 批（并发）⇒ 候选 ${plan.candidates.length} · 单组通过 ${m.approved.length}${m2 ? ` / ${m2.approved.length}` : '（第二组缺席）'} · **交集通过 ${stable.length}** · 否 ${m.rejected} · 未判 ${m.unjudged}（引文接地失败 ${m.ungrounded}）${alarm}` };
    }
    catch (e) {
...
async function assembleSegs(deps) {
    ...
    // ── S-P1c 分片计划（2026-09-15）────────────────────────────────────────────
    // 消费 `config.materialChunkChars`（缺省 null=关 ⇒ **单片**，行为与改造前逐字等价）。
    // ⚠ **本步只做「计划 + 可观测」，多轮执行尚未接线**（下一期）：`cap>0` 且实切出多片时，
    //   本函数**仍按单片跑完**（不截断、不丢料），并**显式告警**"多轮未接线" ——
    //   绝不静默地把"计划了 3 片"当成"跑了 3 片"（那是仓内最忌的假绿）。
