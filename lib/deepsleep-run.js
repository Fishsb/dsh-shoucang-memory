...
import { readLedgerVolumes, appendLedgerLine } from './ledger-compact.js';
...
import { DEEP_SLEEP_PROMPT, deepSleepLanded, liveFailPolicy, planDeepSleepVerdict, splitByCap, windowMaterialBytes, zeroLandedChannels } from './deepsleep-core.js';
import { gatherDeepSleepTraces, countWindowTraces } from './deepsleep-traces.js';
...
import { applyPrinciples, applyPointerOps, applyNarratives } from './deepsleep-apply.js';
...
import { buildAttributionRequest, parseAttribution, calibrationVerdictOf, samplesFromMclRows, isAttributionRow, takeAttributionScan, ATTRIBUTION_MAX_SCAN } from './recall-diagnosis.js';
import { buildYieldRequest, parseYieldJudgements, switchFromJudgements, shouldSwitchSource, buildYieldVerdictEvent, sampleYieldRounds, YIELD_VERDICT_TYPE } from './recall-yield.js';
// 2026-09-23（圆桌会议定稿）：回流写侧需统一信封（**不新开 .jsonl 流**，并入既有 ledger 以 type 区分）
import { envelopeEvent } from './event-envelope.js';
...
const agentTextOf = (r) => (!r || !Array.isArray(r.output)) ? '' : r.output.filter((b) => b && b.type === 'text').map((b) => b.text).join('').trim();
/* ── **归因样本的两个常量（必须同源 · 2026-09-20 round 9）** ────────────────────────────
 *  判因（真机实测）：二者此前是**两处字面量**（取值上限 `20`、门槛 `30`）⇒ **上限 < 门槛**
 *    ⇒ `samples.length < 门槛` **结构性恒真** ⇒ 判定**永不可达**（真机 35 行 verdict 全 `insufficient`，
 *    `attributionSamples` max 仅 **18**）。这与本仓反复出现的
 *    「**读成"在等样本"，其实是链断了**」**同族**（门4「零样本」· S-P1b′「纪元 3/10」）。
 *  ⇒ 提为模块级常量并**把不变量写成断言**（`check-attribution-samples` 机检）：
 *    **取值上限必须 ≥ 门槛**，否则判定永不发生。
 *  ⚠ 两者相等（30）是**有意的**：既让判定可达，又**不放大**单次子代理输入面
 *    —— 30 条正是门槛本身要求的量，不是新加的量。 */
/** 单次归因请求最多取多少条未命中样本（**必须 ≥ `ATTRIBUTION_MIN_SAMPLES`**）。 */
const ATTRIBUTION_SAMPLE_LIMIT = 30;
/** 可支撑判断的最小未命中样本数（**必须 ≤ `ATTRIBUTION_SAMPLE_LIMIT`**）。 */
const ATTRIBUTION_MIN_SAMPLES = 30;
...
async function attributeRecallMisses(ctx, parent, agentOptions) {
    ...
    try {
        ...
        /* ⚠⚠ **2026-09-21 修（窗口被高频行挤空 · ADR-324）** —— 本轮真机实测发现：
         *   原实现按「末 **60 条 `mcl-step`**」倒扫截断，而 M3a（ADR-303）新增的 `phase:'judge'`
         *   行**不带 `missReason`**（它只记通道/分支/echo）⇒ 该行占切点后总量的 **77.7%**
         *   ⇒ **窗口内可用样本被挤到 0**（门槛 30）⇒ 归因判定**再次结构性不可达**。
         *   实测复刻（末 60 条 `mcl-step`）⇒ **60 条全是 `judge`，samples = 0**；
         *   而按消费谓词收满 30 条只需扫 ≈503 行（同刻台账 13610 行）⇒ **窗口太小才是真因**。
         *
         *   **判据修正**：窗口不按"产者是谁 / 行数多少"截断，而**按消费谓词收满即停**
         *   （`takeAttributionScan` + `isAttributionRow`，与 `samplesFromMclRows` **同源**）。
         *   ⇒ 任何"更近但不满足消费谓词"的行（今日 `judge`，明日别的）**再也挤不掉样本**。
         *   ⚠ 我最初拟的修法是"按 `phase === 'inject'` 过滤后再截 60 条"——**实测只得 27 条，
         *     仍 < 门槛 30**（因窗口内 ok : 未命中 ≈ 33 : 27）⇒ 已改为按消费谓词收满。
         *     **教训：按产者名过滤是脆弱解（产者会变），按消费谓词收满是收敛解。** */
        const scan = takeAttributionScan(rows, (raw) => {
        ...
        }, (o) => o?.kind === 'mcl-step' && isAttributionRow(o), ATTRIBUTION_SAMPLE_LIMIT);
        missRows = scan.rows;
        scanned = scan.scanned;
    }
    catch { /* 台账不可读 ⇒ 样本 0 */ }
    const samples = samplesFromMclRows(missRows, ATTRIBUTION_SAMPLE_LIMIT);
     ...
     *
     * ⚠⚠ **2026-09-20 round 9 修（结构性不可达 · 本轮新发现）**：
     *   本行原为 `samplesFromMclRows(missRows, 20)`，而下面判 `samples.length < 30`
     *   ⇒ **上限 20 永远不可能 ≥ 30** ⇒ 该分支**结构性恒真** ⇒ `attributionVerdict`
     *   **永远只可能是 `insufficient`**（后面那段真调子代理的代码**从未执行过**）。
     *   **真机证据**：跨档 **35** 条带 attribution 字段的深睡行，`attributionSamples` 实测
     *     **min 0 / max 18 / ≥30 者 0 条**，出非 `insufficient` verdict 的 **0 条**。
     *   ⇒ 这与本仓反复出现的「**读成"在等样本"，其实是链断了**」**同族**
     *     （前例：门4「零样本」实为产生路径断在入参 · S-P1b′「纪元 3/10」实为单档读）。
     *   **修法**：两处常量**收敛为同一对事实源**，并加**编译期可读的约束注释**：
     *     上限（`ATTRIBUTION_SAMPLE_LIMIT`）必须 **≥** 门槛（`ATTRIBUTION_MIN_SAMPLES`），
     *     否则判定**永不可达**。取两者相等（30）：既让判定可达，又**不放大**单次子代理的输入面
     *     （30 条正是门槛本身要求的量，不是新加的量）。 */
    if (samples.length < ATTRIBUTION_MIN_SAMPLES) {
        /* ⚠ **「扫到上限而没收满」必须与「样本本来就不够」区分开**（2026-09-21 补）：
         *   两者在读数上**同形**（`samples` 都 < 门槛），但成因与处置相反：
         *     · 样本本来就不够 ⇒ **等时间**（台账里真的没有那么多未命中行）；
         *     · 扫描触顶     ⇒ **窗口太小/密度下降 ⇒ 调 `ATTRIBUTION_MAX_SCAN`**，等多久都不会够。
         *   实测密度**在漂**：修此缺陷当时收满 30 条只需扫 **≈503 行**，数十分钟后同库需 **≈1514 行**
         *   （**密度降到 1/3**）—— 上限是 4000，余量已从 8× 缩到 2.6×。
         *   ⇒ 故把两种情形**写成不同的 note**，并在触顶时显式点名上限：
         *     后续若见「扫满上限」字样，直接调上限即可，不必再重新归因一次。 */
        const hitCap = scanned >= ATTRIBUTION_MAX_SCAN;
        return {
            verdict: 'insufficient', agrees: null, samples: samples.length, scanned, calibration: calibVerdict,
            note: hitCap
                ? `**扫描触顶**：扫满上限 ${ATTRIBUTION_MAX_SCAN} 行仍未收满 ${ATTRIBUTION_MIN_SAMPLES} 条（仅 ${samples.length} 条）⇒ 调 ATTRIBUTION_MAX_SCAN，非等时间`
                : `未命中样本 ${samples.length} < ${ATTRIBUTION_MIN_SAMPLES}（扫 ${scanned} 行 · 未触顶 ⇒ 属样本真不足，等时间）`,
        };
    }
    ...
    try {
        ...
        if (!parsed)
            return { verdict: null, agrees: null, samples: samples.length, scanned, calibration: calibVerdict, note: `无法解析（stop=${String(res3?.stopReason || '?')} · 输出 ${raw.length} 字符）` };
        return { verdict: parsed.verdict, agrees: parsed.verdict === calibVerdict, samples: samples.length, scanned, calibration: calibVerdict, note: parsed.reason.slice(0, 60) };
    }
    catch (e) {
        return { verdict: null, agrees: null, samples: samples.length, scanned, calibration: calibVerdict, note: '调用异常：' + String(e?.message || e).slice(0, 60) };
    }
}
...
async function judgeYieldRounds(ctx, parent, agentOptions, bankRoot) {
    /* ⚠ 2026-09-23（圆桌会议定稿）：**三态失败原因**（照 ACT-352 的 `releaseSemanticError` 范式）。
     * 判因（obs 实测）：「判不准」（模型说 helped=null）与「判不了」（输出 0 字符 / 无法解析）
     *   在**现有读数里完全同形** —— 59 行真机判定中 `yieldUnjudged`/`yieldError` 字段**根本不存在**，
     *   「未判 10」只活在 `yieldNote` 的中文串里（要靠中文正则才取得出）。
     *   ⇒ 落独立字段，**键缺席 = 判定完成**（`undefined` 会被 `envelopeEvent` 静默丢键 ⇒ 消费侧必须用
     *     `'yieldError' in o` 判定，**不得**用 `o.yieldError !== undefined` —— 两者在键缺席时同形）。
     *   词汇取既有 `'unparsable'`/`'threw'`（release 侧已用），另加本链特有的 `'unreadable'`/`'insufficient-rows'`。 */
    const blank = { judged: 0, helpedTrue: 0, unjudged: 0, switchSemantic: false, switchCounter: false };
    let rows = [];
    try {
        ...
        return { ...blank, error: 'unreadable', note: 'yield-rounds.jsonl 不可读（未跑过采集器）' };
    }
    if (rows.length < 5)
        return { ...blank, error: 'insufficient-rows', note: `取证行 ${rows.length} < 5` };
     ...
     * ⚠⚠ **ACT-363（2026-09-23 复验）**：原实现 `sort(idleSteps desc).slice(0,10).reverse()` 有**真缺陷** ——
     *   `idleSteps` 被采集器封顶在 `YIELD_NEXT_N=3`，实测分布 `{0:948, 1:20, 2:26, 3:5739}`
     *   ⇒ **85.2% 并列最大** ⇒ 排序键饱和，稳定排序取到的是**最老的 10 行**（实测全为 2026-09-15）
     *   ⇒ 回流 sid 恒指向**已死会话**（实测 `6be5ac2e`，末次活动 09-16）⇒ 活会话**永远读不到**。
     *   ⇒ 改走**单一实现** `sampleYieldRounds`（筛有动作 + 按时间序取最近），本件不再自写取样。 */
    rows = sampleYieldRounds(rows, 10);
    ...
    try {
        ...
        if (!judged)
            return { ...blank, error: 'unparsable', note: `无法解析（stop=${String(res4?.stopReason || '?')} · 输出 ${raw4.length} 字符）` };
        const helpedTrue = judged.filter((j) => j.helped === true).length;
        const unjudged = judged.filter((j) => j.helped === null).length;
        /* ── 2026-09-23（圆桌会议定稿 · 方案 §3.1）：**回流写侧** —— 落一条 `yield.verdict` 到既有统一台账。
         * 判因：离线判定与步内出口之间**不存在回流通道**（`mcl-switch` 与 `yieldSwitchSemantic` 跨日错开，
         *   配对 60s/3600s 均 0/10）⇒ 本步即补那条边。
         * ⚠ 装配归 `recall-yield#buildYieldVerdictEvent`（纯函数）· 写口归 `ledger-compact#appendLedgerLine`
         *   （台账域单一出口，已内置 fail-safe）⇒ 本件只留一行，不堆大旧模块（`check-module-growth` 阈值 600）。 */
        appendLedgerLine(join(knowledgeRoot(), 'audit', 'ledger.jsonl'), envelopeEvent(buildYieldVerdictEvent(judged, rounds, String(rows[rows.length - 1]?.sid || '')), YIELD_VERDICT_TYPE));
        return {
            judged: judged.length, helpedTrue, unjudged,
        ...
        return { ...blank, error: 'threw', note: '调用异常：' + String(e?.message || e).slice(0, 60) };
    }
}
/* P5c 语义门（2026-09-16）：**字面自足 ≠ 语义自足** —— 对释放候选（判据行）问
 *   "丢掉正文后还能不能据以复现做法"，与字面判据**对账**；**不一致以更严者为准**（未判 ⇒ 不通过）。
 *   ⚠ 无候选 ⇒ **直接返回、不调 LLM**（释放默认关闭，不该每轮白烧一次子代理）。 */
async function reviewReleaseSemantics(ctx, parent, agentOptions, bankRoot) {
    ...
    try {
        ...
        return { candidates: plan.candidates.length, approved: 0, rejected: 0, unjudged: 0, error: 'threw', note: '调用异常：' + String(e?.message || e).slice(0, 60) };
    }
}
...
async function reviewReleaseSemanticsBatched(ctx, parent, agentOptions, bankRoot) {
    const plan = planRelease(bankRoot);
    ...
    try {
        ...
        const runBatches = async (batchReqs, tag) => await Promise.all(batchReqs.map(async (text, k) => {
            const ac = new AbortController();
        ...
        return { ...base, batches: reqs.length, error: 'threw', note: '调用异常：' + String(e?.message || e).slice(0, 60) };
    }
}
...
function liveAutoSwitch(key, envName) {
...
function releaseAutoOn() { return liveAutoSwitch('releaseAuto', 'SHOUCANG_RELEASE_AUTO'); }
/**
...
async function runSleepMaintenance(env) {
    ...
    const brief = (e) => String(e?.message || e).slice(0, 120);
    try {
        await consolidateTree({ log, audit, embedCfgOf }, resolved);
    }
    catch (e) {
        log(`deep sleep: consolidation 失败（跳过，继续深睡）: ${brief(e)}`);
    }
    try {
        ...
        log(`deep sleep: 真实读采集失败（跳过，按既有日志聚合）: ${brief(e)}`);
    }
    try {
        ...
        log(`deep sleep: activity 聚合失败（跳过，继续深睡）: ${brief(e)}`);
    }
}
/**
 ...
 */
function runSupersedeChannelSleep(env) {
    const zero = { applied: 0, skipped: 0, archived: 0, ran: false, reasons: [] };
    const sup = env.out && env.out.supersedes;
    if (env.stop !== 'completed' || !Array.isArray(sup))
        return zero;
    ...
    if (plan.rejected)
        env.audit({ kind: 'supersede-plan-rejected', rejected: plan.rejected, accepted: plan.accepted, reasons: plan.reasons.slice(0, 4) });
    if (!plan.ops.length)
        return { ...zero, reasons: plan.reasons };
    return applySupersedeOps({ root: env.root, at: new Date().toISOString(), log: env.log, audit: env.audit }, plan.ops, { enabled: liveAutoSwitch('supersedeApply', 'SHOUCANG_SUPERSEDE_APPLY') });
}
async function runRelease(env) {
    const { root, relReview, log, audit } = env;
    const zero = { released: 0, skipped: 0, reasons: [], ran: false, executed: false };
    if (!relReview || !Array.isArray(relReview.ops) || !relReview.ops.length)
        return { ...zero, reasons: ['无释放提案（候选为空 / 字面判据未过）'] };
    if (!relReview.executable)
        return { ...zero, reasons: [`语义门未放行（交集 ${Number(relReview.approved) || 0} 条 · 空集或 over-permissive 警报）⇒ **零释放**`] };
    if (!releaseAutoOn())
        return { ...zero, reasons: ['release 自动执行**默认关闭**（开启：scheduler.json 的 `releaseAuto:true` 或 env `SHOUCANG_RELEASE_AUTO=1`）⇒ 本轮零写入'] };
    const r = await applyRelease(root, relReview.ops, relReview.approvedRows, (ops) => applyForgetOps(root, ops, { audit, log }), { maxPerRun: RELEASE_MAX_PER_RUN });
...
async function assembleSegs(deps) {
...
function emitDeepSleepAudit(audit, x) {
    ...
    return { kind: 'deep-sleep', sleepEpoch: state.epoch.v, epochSince: since, materialBytes, materialChars: userInput.length, traceFiles, chunk: chunkIdx, totalChunks: chunks.length,
        ...
        attributionVerdict: attribution.verdict, attributionAgrees: attribution.agrees, attributionSamples: attribution.samples, attributionScanned: attribution.scanned, attributionCalibration: attribution.calibration, attributionNote: attribution.note,
        // J5/U3：收益判定与**计数法对账**（`yieldHelpedTrue` > 0 ⇒ 计数法把"其实帮上了"的轮次误当零增益）
        yieldJudged: yieldRes.judged, yieldHelpedTrue: yieldRes.helpedTrue, yieldUnjudged: yieldRes.unjudged, yieldSwitchSemantic: yieldRes.switchSemantic, yieldSwitchCounter: yieldRes.switchCounter, yieldNote: yieldRes.note,
        // ⚠ **`yieldError`**（2026-09-23 · 圆桌会议定稿，照 ACT-352 的 `releaseSemanticError` 范式）：
        //   **键缺席 = 判定完成**；键存在且为字符串 = **判不了**，值即原因
        //   （`unreadable` | `insufficient-rows` | `unparsable` | `threw`）。
        //   ⇒ 消费侧**必须**写 `'yieldError' in o`，**不得**写 `o.yieldError !== undefined`
        //     —— `undefined` 会被 `envelopeEvent` 的 `JSON.stringify` 静默丢键，两者在键缺席时同形。
        //   ⚠ 仍**不得**据 `yieldHelpedTrue === 0` 推断语义（那是**代理指标**；release 侧已有实证：
        //     把"12 批全失败"计成"通过率 0%"，极差虚增一倍）。
        //   ⚠ **只在此处写「有值」形态**：`undefined` 时**不写该键**（写了也会被丢，等于假装有契约）。
        ...(yieldRes.error ? { yieldError: String(yieldRes.error) } : {}), // P5c 语义门：**通过数**（= 可释放面）与未判数；⚠ **仍未执行任何释放**（applyRelease 默认关闭）
...
export async function runDeepSleep(d, sinceArg) {
    ...
    try {
        ...
        /* ⚠ **2026-09-20 round 9 名实订正 → round 10 判定落地（S-P1b″）**：
         *   · 注册项 `trigger.newTracesMin` 的名字与 note 都说「窗口内最少新痕迹**数**」；
         *   · 而本轮之前这里比的是 `traces.length` —— `traces` 是 `gatherDeepSleepTraces` 返回的**字符串**，
         *     `.length` 是**材料段字符数**（旧 log 自己就写着"痕迹 N **字符**"）。
         *   ⇒ 值 `1` 时两者恰好同效（材料非空 ⇔ 至少一条痕迹），**所以一直没暴露**；
         *     但它是**潜伏的假旋钮**：把值调到 3，名字说"至少 3 个痕迹文件"，
         *     行为却是"材料段至少 3 个字符"（几乎必然通过 ⇒ 改了没反应）。
         *   · round 9 只把真轴落成可测量量（`countWindowTraces` → 审计 `traceFiles`），
         *     **未改判定**（属 R3 行为变更）；**round 10 经用户授权落地**（「继续全部收尾」）。
         *   · **本轮行为变更的已知代价（授权时已具名陈述）**：真机 49 纪元里 **46** 个 `materialBytes=0`
         *     （窗口内零文件），其中 **25** 个此前靠"窗口内任务运行统计/access 命中"撑出非空材料而仍入睡
         *     ⇒ 改按文件数判后这 25 轮**不再入睡**（**砍掉约 51% 入睡纪元的输入面**）。
         *     代价可接受的理由：那些材料是**背景上下文**（运行统计/检索命中），不是**新增知识**；
         *     S-P1b″ 定案的轴正是"与新增材料同源"的量。**不丢数据**：跳过会把窗口滑到当前，
         *     未来文件的 mtime 必然晚于水位 ⇒ 仍会被后续纪元捕获。 */
        const traceFiles = countWindowTraces(traceDeps, resolved.root, since);
        const minTraces = Number(TRIGGER.newTracesMin ?? 1); // 注册语义 = 痕迹**文件数**；判定已按此轴（本轮）
        if (traceFiles < minTraces) {
            log(`deep sleep: 窗口内痕迹文件不足（${traceFiles} 个 < ${minTraces}，材料 ${traces ? traces.length : 0} 字符，起点 ${new Date(since).toLocaleString()}），窗口滑到当前，本轮不睡`);
            return 'no-traces';
        }
        log(`deep sleep: 窗口内痕迹文件 ${traceFiles} 个 · 材料 ${traces.length} 字符（起点 ${new Date(since).toLocaleString()}）`);
        ...
        if (remOn) {
            try {
                ...
                log(`deep sleep: 联想候选失败（不影响本轮）: ${String(e?.message || e).slice(0, 100)}`);
            }
        }
        ...
        if (cc.candidates.length) {
            segs.push('## 跨粒度收敛候选（**宿主已用向量预筛**；请**逐条判定**是否「同一知识」——是 ⇒ 填 convergeOps，两条**逐字抄自下方**）\n'
                + cc.candidates.map((c, i) => `- 候选 ${i + 1}（相似度 ${c.sim} · file=${c.file}）\n  粗（带标签）：${c.coarse}\n  细（无标签）：${c.fine}`).join('\n'));
        }
        ...
        const agentOptions = useProvider
            ? withEffort({ provider: resolvedLlm.provider, model: resolvedLlm.model }, config.sleepEffort)
            : undefined;
        const ac = new AbortController();
        ...
        try {
            ...
            const episodeItems = (stop === 'completed' && out && Array.isArray(out.narratives) ? out.narratives : [])
                .filter((n) => n && String(n.title || '').trim() && String(n.text || '').trim())
                .map((n) => ({
            ...
            const otherChannels = {
                ...
                byChannel: [
                    { name: 'profiles', tried: profileTried, done: profileAdded },
                    { name: 'pointers', tried: ptrRes.skipped, done: ptrRes.updated },
                    { name: 'tree', tried: treeRes.skipped, done: treeRes.applied },
                    { name: 'forget', tried: forgetRes.skipped, done: forgetRes.archived },
                    { name: 'outcomes', tried: outcTried, done: ringRes.outcomes },
                    { name: 'episodes', tried: epiTried, done: ringRes.episodes },
                    { name: 'converge', tried: conv.skipped + conv.applied, done: conv.applied },
                ],
            };
            ...
            audit({ kind: 'deep-sleep', sleepEpoch: state.epoch.v, epochSince: since, error: String(e?.message || e).slice(0, 160) });
            return 'failed';
        }
    }
    catch (e) {
