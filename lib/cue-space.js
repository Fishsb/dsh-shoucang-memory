...
const dimOf = (s) => String(s ?? '').split('=')[0].trim().toLowerCase();
/** kebab 归一（`task`/`event`）：小写 + 空白/下划线→`-` + 折叠 + 去首尾 */
const kebab = (v) => String(v ?? '').trim().toLowerCase().replace(/[\s_]+/g, '-').replace(/-{2,}/g, '-').replace(/^-+|-+$/g, '');
/**
 * **键归一**（**唯一实现** · 读侧写侧共用）。`''` = 空键/无值（调用方应丢弃）。
 * 只做**确定性、可逆性无损**的归一：不做别名归一（`<盘符>:\<别名>\<a>` 是否等于 `<盘符>:\<a>`
 *   **无确定性依据，须人裁决**），不做大小写折叠（理由见上）。
 */
export function normalizeCueKey(raw) {
    try {
        ...
        const eq = s.indexOf('=');
        if (eq <= 0)
            return ''; // 无 `=`（或维名为空）⇒ 不是键，丢弃（旧实现原样返回，会把垃圾带进库）
...
let dimCache = null;
export function declaredCueDims() {
    if (dimCache)
        return dimCache;
    const d = SURFACE?.injection?.situation?.cueDims;
 ...
 */
export function validateCueKey(raw, dims = declaredCueDims()) {
    const key = normalizeCueKey(raw);
    if (!key)
        return { ok: false, key: '', reason: 'empty' };
    const dim = dimOf(key);
    if (!dims.includes(dim))
        return { ok: false, key, reason: `undeclared-dim:${dim}` };
...
export function cueKeyOf(dim, value) {
 ...
 */
export function cueSetOf(raw, dims = declaredCueDims()) {
    ...
    for (const x of arr) {
        ...
        else if (v.key || String(x ?? '').trim())
            rejected.push({ key: v.key || String(x).trim(), reason: v.reason });
    }
    return { keys, rejected };
}
//# sourceMappingURL=cue-space.js.map