...
const distillStatsOf = () => {
    ...
    const isoDateOf = (iso) => String(iso || '').slice(0, 10);
    for (let i = 6; i >= 0; i--) {
    ...
    for (const r of rows) {
        ...
        if (r.kind === 'gate-reject') {
            ...
            if (at >= gateLastAt) {
                gateLastAt = at;
                gateLastTarget = t;
                gateLastReason = String(r.reason || '').slice(0, 120);
            }
            continue;
        }
...
const dirSizeOf = (p, countBytes = true) => {
    let files = 0, bytes = 0;
    try {
        for (const e of readdirSync(p, { withFileTypes: true })) {
            const full = join(p, e.name);
            if (e.isDirectory()) {
                const s = dirSizeOf(full, countBytes);
                files += s.files;
                bytes += s.bytes;
            }
            else {
...
const libraryStructureOf = (base) => {
    const entries = [];
    let names = [];
    try {
        names = readdirSync(base, { withFileTypes: true }).map((e) => e.name).sort();
    }
    catch { /* 库根不可读 */ }
    for (const name of names) {
        ...
        const kind = STRUCT_TOOLING.has(name) ? 'tooling' : STRUCT_ARTIFACT.has(name) ? 'artifact' : 'content';
        if (isDir) {
...
export const parseIndexLines = (text) => {
    const out = [];
...
const charsOf = (text) => capacityCharsOf(text);
...
const pendingOfRoot = (base) => {
    const out = { count: 0, last24h: 0, recent: [] };
    try {
        ...
        for (const n of names.slice(0, 8))
            out.recent.push({ name: n, mtime: statMtime(join(pendDir, n)) });
    }
    catch { /* pending 缺失 */ }
    return out;
};
/** flow-candidates 子区（待转正候选；蒸馏采集不递归，故与根 pending 分列） */
const flowCandidatesOf = (base) => {
    const out = { count: 0, recent: [] };
    try {
        ...
        out.count = names.length;
        for (const n of names.slice(0, 8))
            out.recent.push({ name: n, mtime: statMtime(join(dir, n)) });
    }
    catch { /* 无该子区 */ }
    return out;
};
...
function memoryOverviewRoute(d, _req, res) {
    const base = memoryHomeOf();
    if (!base)
        return sendJson(res, 200, { present: false, error: '未检测到记忆库根（<DSH_HOME>/suite/memory，可用 MEMORY_ROOT 覆盖）——记忆插件蒸馏事实源不在本机默认位' });
    try {
    ...
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (h && h.level >= 2) { // 记忆小节从 ## 起（# 是文件标题）
                ...
                const node = { title: h.title, titleLevel: h.level, line: i + 1, body: '', children: [], foldKey: '', core: '' };
        ...
        const fillKeys = (list, path) => {
            for (const s of list) {
                s.core = coreName(s.title);
                s.foldKey = `note:${rel}:${path}§${s.title}`;
                if (s.children.length)
                    fillKeys(s.children, (path ? path + '/' : '') + s.title);
            }
        };
        fillKeys(sections, '');
        ...
        const pointerIndex = (() => {
            ...
            const walkKeys = (list) => {
                for (const s of list) {
                    keyByTitle.set(s.title, s.foldKey);
                    if (s.children.length)
                        walkKeys(s.children);
                }
            };
                ...
                for (const l of raw.split(/\r?\n/)) {
                    ...
                    for (const p of pointersOfRow(l)) {
                        ...
                        for (let i = parts.length - 1; i >= 0; i--) {
                            if (parts[i].res.state === 'exists') {
                                title = parts[i].res.cands[0]?.title ?? '';
                                break;
                            }
                        }
                        const candsOf = () => {
