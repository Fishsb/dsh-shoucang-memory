...
const auditDir = () => join(knowledgeRoot(), 'audit');
/** `ledger.jsonl` 实况：行数 / 按 type 分布 / 缺 `at` 的行数（信封完整性）
 *
 * ⚠ **跨档读（G13 轮转失明 · 2026-09-20 修）**：本函数原**只读主档** `readFileSync(path).split('\n')`，
 *   而台账按大小轮转（`ledger-compact#rotateBySize` 保留 3 档）⇒ **旧卷 `.1/.2/.3` 全看不见**。
 *   实测（本轮）：主档 `deep-sleep` 行 **0** / 跨档真值 **49**；`compliance` 行 635 / **4653**（漏 87%）。
 *   该读数正是面板「台账实况」卡片的数据源 ⇒ **面板一直在少报历史**（且不报错）。
 *   ⇒ 现改为复用唯一实现 `ledger-compact#readLedgerVolumes`（最旧档 → 主档，按时间序）。
 *   判据：`scripts/check-ledger-read.mjs`（源码面硬门 + 真机形态反例自证）。 */
const ledgerFacts = () => {
    ...
    let missingAt = 0;
    try {
        for (const l of readLedgerVolumes(path)) {
            if (!l.trim())
                continue;
            lines++;
            try {
                ...
                byType.set('(坏行)', (byType.get('(坏行)') || 0) + 1);
            }
        }
    }
    catch { /* 台账缺席=空 */ }
...
const auditDirFacts = () => {
    try {
        return readdirSync(auditDir())
            ...
            .map((f) => {
            try {
                const s = statSync(join(auditDir(), f));
                return { name: f, bytes: s.size, mtime: s.mtimeMs };
            }
            catch {
                return { name: f, bytes: 0, mtime: 0 };
            }
        })
...
function archRecordsRoute(_d, _req, res) {
    try {
        ...
        sendJson(res, 500, { error: String(e).slice(0, 300) });
    }
}
/** ② 断言图：节点/边/各 rel/悬空证据（纯计数，零向量）——同样读**记忆库**根 */
function archGraphRoute(_d, _req, res) {
    try {
...
function archObservabilityRoute(_d, _req, res) {
    try {
...
function archAssemblyRoute(d, _req, res) {
    try {
        let version = '(未知)';
        let libFiles = 0;
        try {
            version = String(JSON.parse(readFileSync(join(LIB_DIR, '..', 'package.json'), 'utf8')).version || '(未知)');
        }
        catch { /* 读不到就给未知 */ }
        try {
            libFiles = readdirSync(LIB_DIR).filter((f) => f.endsWith('.js')).length;
        }
        catch { /* 同上 */ }
        sendJson(res, 200, {
            composition: {
            ...
            store: {
                mode: String(d.suite.read().storeMode || 'md'),
                allowed: ['md', 'dual'],
...
const validateMclConfig = (c) => {
    for (const k of ['mclEnabled', 'mclAudit', 'mclMaterialInSystem', 'enableRemPass']) {
        if (k in c && typeof c[k] !== 'boolean')
            return `${k} 须为布尔`;
    }
    const ranges = { mclFamiliarThreshold: [0, 1], mclMaxNudges: [0, 3], mclBudgetChars: [120, 4000], mclTopK: [1, 5] };
...
async function mclConfigRoute(d, req, res) {
    try {
        if (req.method === 'POST') {
            const body = (await readBody(req).catch(() => ({})));
            const patch = {};
            for (const k of MCL_CONFIG_KEYS)
                if (k in body)
                    patch[k] = body[k];
            if (!Object.keys(patch).length)
                return sendJson(res, 400, { error: 'no-mcl-keys' });
...
export function registerArchRoutes(d) {
...
//# sourceMappingURL=panel-arch.js.map