...
import { applyForgetOps, applyTreeOps, sectionExists } from './treeops.js';
...
import { demoteVerdict, maturationVerdict, promoteVerdict } from './criteria.js';
import { COMMIT_FAILED_GATE, DEEP_SLEEP_PROMPT, commitPrinciples, deepSleepLanded, deepSleepReplayable, liveFailPolicy, planDeepSleepVerdict, } from './deepsleep-core.js';
import { resolveTarget } from './targets.js';
// 当天痕迹收集（深度睡眠作用域=本日）
// since 由调用方显式传入：**必须在推进 lastDeepSleepAt 之前取值**（否则窗口起点=当前时刻 → 恒零痕迹，
// 见 deepSleepCheck 的调用处注释；此坑曾让深度睡眠自上线起从未真正归纳过任何材料）。
/** gatherDeepSleepTraces（自 createDeepSleep 迁出；依赖经 DsScope 显式注入） */
function gatherDeepSleepTraces(S, memRoot, since) {
    const { candidateDir, pendDir, auditFile } = S;
    const parts = [];
...
/** applyPrinciples（自 createDeepSleep 迁出；依赖经 DsScope 显式注入） */
async function applyPrinciples(S, memRoot, out) {
    const { config, log, PROFILE_HEADER, kRoot, runNode, capEnv, textOf } = S;
...
async function applyPointerOps(S, memRoot, out) {
    const { runNode, config, capEnv, log, textOf } = S;
    const gateScript = join(memoryLibRoot(), 'scripts', 'memory_write_gate.mjs');
    if (!existsSync(gateScript))
...
/** consolidateTree（自 createDeepSleep 迁出；依赖经 DsScope 显式注入） */
async function consolidateTree(S, memRoot) {
    const { log, audit, embedCfgOf } = S;
...
/** runDeepSleep（自 createDeepSleep 迁出；依赖经 DsScope 显式注入） */
async function runDeepSleep(S, sinceArg) {
    const { ctx, config, log, audit, ledger, streak, llmState, traceSince, runNode, capEnv, textOf, kRoot, probeScriptPath, validateProvider, resolveLlm, resolveDefaultModel, pickParent, parseAgentJson, normalizeProfileTarget, writeProfileLine, ensureDaemonParent, bankSnapshot, runSelfCheck } = S;
    try {
        ...
        try {
            await consolidateTree(S, resolved.root);
        }
        catch (e) {
        ...
        const traces = gatherDeepSleepTraces(S, resolved.root, since);
        ...
        const forgetCandidates = (() => {
            try {
                ...
                for (const idx of ['MEMORY.md', 'USER.md', 'AGENT.md']) {
                    let raw = '';
                    try {
                        raw = readFileSync(join(resolved.root, idx), 'utf8');
                    }
                    catch {
                        continue;
                    }
                ...
                for (const l of readFileSync(join(resolved.root, 'audit', 'activity.jsonl'), 'utf8').split(/\r?\n/)) {
                    ...
                    try {
                        const o = JSON.parse(l);
                        if (String(o.status) !== 'cold')
                            continue;
                        const days = o.lastHit ? Math.round((Date.now() - Number(o.lastHit)) / 86400000) : 'never';
                        if (days !== 'never' && days <= 90)
                            continue;
                        const f = String(o.f || '').replace(/^notes\//, '');
                        const s = String(o.s || '');
                        if (PROFILE.has(f.toLowerCase()))
                            continue;
                        // R2（审查项）：剔除**悬空候选**（节不存在）——否则白占材料 top-10 名额（口径与 matchSection 同源）
                        if (!sectionExists(resolved.root, f, s))
                            continue;
                ...
                const v = (d) => (d === 'never' ? Number.MAX_SAFE_INTEGER : d);
        ...
        try {
            ...
            const app = (stop === 'completed' && out)
                ? await applyPrinciples(S, resolved.root, out)
            ...
            const ptrRes = (stop === 'completed' && out)
                ? await applyPointerOps(S, resolved.root, out)
...
/** probeSession（自 createDeepSleep 迁出；依赖经 DsScope 显式注入） */
function probeSession(S, rec) {
    const { log, audit, config, ctx, locateTranscript } = S;
...
export function createDeepSleep(C) {
    const { log, audit, ledger, kRoot, auditFile, pendDir, candidateDir, PROFILE_HEADER, capEnv, llmState, 
    // appCtx 重命名回 ctx：让迁出的 1419 行代码里的 `ctx.xxx` **一个字都不用改**
    appCtx: ctx, config, runNode, textOf, embedCfgOf, probeScriptPath, distillAgent, writeDispatch, writeProfileLine, validateProvider, runSelfCheck, resolveLlm, resolveDefaultModel, pickParent, parseAgentJson, normalizeProfileTarget, locateTranscript, hasActiveSubagents, ensureDaemonParent, bankSnapshot, } = C;
    ...
    // 深睡作用域：闭包状态 + 外部依赖装箱，供模块级函数**显式**取用（不再靠隐式闭包）
    const S = {
        ctx, config, log, audit, ledger, kRoot, auditFile, pendDir, candidateDir, PROFILE_HEADER,
        capEnv, llmState, runNode, textOf, embedCfgOf, probeScriptPath,
    ...
    const deepSleepCheck = () => {
        ...
        // 窗口起点必须在推进水位**之前**取：lastDeepSleepAt 一旦置为 now，S.traceSince() 会退化成
        // max(今日 0 点, now)=now，痕迹扫描窗口变成 [now, now] → 恒「本日无痕迹」（2026-09-09 实修）。
        const since = S.traceSince();
        lastDeepSleepAt = now;
    ...
    const runDeepSleepNow = async () => {
        ...
        try {
            ...
            const since = S.traceSince();
            const r = await runDeepSleep(S, since);
            if (r !== 'failed')
                lastDeepSleepAt = Date.now();
    ...
    return { DEEP_SLEEP_CHECK_MS, sessions, noteEvent,
        ...
        runDeepSleep: (sinceArg) => runDeepSleep(S, sinceArg),
        probeSession: (rec) => probeSession(S, rec),
