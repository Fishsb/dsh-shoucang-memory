...
export async function consolidateTree(d, memRoot) {
    ...
    const atomicWrite = (p, text) => atomicWriteFile(p, text).ok;
            ...
            try {
                ...
                if (ecfg.enabled && embedBudget > 0) {
                    for (const g of groups.values()) {
                        ...
                        const idxSemSim = thresholdValue('tree.indexSemanticSim', 0.9);
                ...
                for (let guard = 0; guard < 8; guard++) { // 每并一次结构变化 → 重解析再找；上限防病态循环
                    ...
                    for (let a = 0; a < leaves.length && !merged; a++) {
                        for (let b = a + 1; b < leaves.length && !merged; b++) {
                            ...
                            const secMergeSim = thresholdValue('tree.sectionMergeSim', 0.95);
                            if (sim === null || sim < secMergeSim)
...
export const rewriteRowPointersByChain = (raw, nf, finals) => {
