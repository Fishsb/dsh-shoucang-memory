...
import { INGEST_JUDGE, JUDGEMENT_HINT, JUDGEMENT_VALUES, LEDGER_FILE } from './criteria.generated.js';
...
${JUDGEMENT_HINT}
${JUDGEMENT_VALUES}
- **supersedes（门4 · 时态失效）**：当本段知识**取代**了「相关既有记忆」里的某条旧断言（旧说法已不成立，而非仅仅重复或可并存）⇒ 填 supersedes，target **必须逐字抄自「相关既有记忆」段的原文**（宿主按逐字唯一命中定位；抄不准 ⇒ 整条被拒，且**不许自己拼一条像既有行的文本**——那会误标真事实失效）。只写"与旧条不冲突"或"只是更啰嗦"的**不要**填（那是 coexist/合并，不是取代）。拿不准就留空数组——**错标失效比不标更坏**（读者再也见不到真事实）。`;
 ...
 */
export function distillEntryOf(agent, enableDistill) {
    if (enableDistill !== false)
        return agent;
    return {
        ...agent,
        armIdleTimer: () => { },
...
export function registerDistill(ctx, config) {
    ...
    const infra = createInfraApi({
        ...
        onWriteFail: (kind, e) => infra.log('[write-fail] ' + kind + ': ' + String(e?.message || e).slice(0, 160)),
    });
