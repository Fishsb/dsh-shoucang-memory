...
export function registerDistill(ctx, config) {
    ...
    const extractDelta = (agent, lastSeq) => {
        ...
        for (const e of events) {
            ...
            if (seq > maxSeq)
                maxSeq = seq;
            for (const p of textPartsOfEvent(e))
                parts.push(p);
        }
    ...
    const writeDispatch = async (sid, out, route, workspace) => {
        ...
        if (route === 'project') {
            ...
            const cardDupOf = (title) => {
                const tn = intentTokens(title);
                if (tn.length < 2)
                    return null;
                for (const ex of existingCardTitles) {
                    if (ex === title)
                        return ex;
                    const te = intentTokens(ex);
                    if (te.length < 2)
                        continue;
                    const inter = tn.filter((x) => te.includes(x)).length;
                    const union = new Set([...tn, ...te]).size;
                    if (union > 0 && inter / union >= 0.66)
                        return ex;
                }
                return null;
    ...
    const parentSidOf = (a) => { try {
        const p = a?.session?.header?.parentSession;
                ...
                try {
                    ...
                    if (maxSeq > lastSeq) {
                        ...
                        try {
                            let claim = null;
                            try {
                                claim = JSON.parse(readFileSync(claimFile, 'utf8'));
                            }
                            catch { /* 无 claim */ }
                            if (claim && Date.now() - (claim.at || 0) < 25 * 60000) {
                                continue;
                            } // 在途，其他 fiber 已接管
