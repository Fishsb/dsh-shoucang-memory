...
export function registerDistill(ctx, config) {
    ...
    const ensureFlowCandidate = (sid, intent) => {
        ...
        try {
            ...
            if (existsSync(f))
                return; // 同会话/同意图去重（一任务一条）
            writeFileSync(f, `# 任务候选（低置信 · 跨窗口记忆）\n\n- 类型线索：${intent}\n- 源会话：${sid}\n- 状态：候选（非源指针；仅供深睡跨窗口同型判断——同类第二次成功由深睡归纳为 [路径]）\n`, 'utf8');
        }
        catch { /* 候选落盘失败静默 */ }
    };
    const log = (msg) => { try {
        ...
        if (route === 'project') {
            ...
            if (!workspace) {
                for (const pc of cards) {
                    rejected++;
                    audit({ sid, kind: 'gate-reject', target: pc?.title, reason: '项目事实无归属（workspace 反解失败），不落全局库（跨工作区红线）' });
                }
    ...
    const distillAgent = async (agent) => {
        ...
        try {
            ...
            try {
                ...
                // 路线②：成功蒸馏即留轻 episode + 低置信任务候选（跨窗口记忆；同类二次出现供深睡跨窗归纳 [路径]）
                if (disp.added > 0) {
                    const intent = intentOf(deltaText);
                    recordEpisode({ sid, intent: intent.slice(0, 120), route, added: disp.added, rejected: disp.rejected, failed: disp.failed, lib: disp.targetLib });
                    if (route === 'memory')
    ...
    const gatherDeepSleepTraces = (memRoot, since) => {
        ...
        try {
            const candFiles = existsSync(candidateDir) ? readdirSync(candidateDir).filter((f) => f.endsWith('.md')).sort().slice(-8) : [];
            if (candFiles.length) {
                const blocks = ['### 跨窗口任务候选（背景材料，非源指针；同类型重复 → [路径] 判据参考）'];
                let cbudget = 2400;
    ...
    ctx.on('session/event', (session, event) => {
        try {
            ...
            if (config.activationShadow !== false)
                activationStep(sid, event); // 路线④ 打扰度影子观察
        }
        catch { /* 状态迁移零抛出 */ }
    });
    ctx.effect(() => {
