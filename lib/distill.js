 ...
 *   → PROBING（采样 transcript 两次比对 mtime/size）→ 增长=长任务（刷新水位不睡）/ 不增长+会话在=STALLED（不阻塞，
 *   发审计告警）/ 不增长+会话没了=EXIT（计入停滞）/ 探针缺失=保守不睡。探测未决期间一律不睡（保守）。
...
export function registerDistill(ctx, config) {
    ...
    const noteEvent = (sid, isTurnEnd) => {
        const now = Date.now();
        lastActivityAt = now;
        const rec = sessions.get(sid) || { sid, state: 'running', lastEventAt: now, lastEndAt: 0, probeAt: 0 };
        rec.lastEventAt = now;
        if (isTurnEnd) {
            rec.state = 'ended';
            rec.lastEndAt = now;
    ...
    try {
        for (const l of readFileSync(auditFile, 'utf8').split('\n')) {
            ...
            try {
                ...
                if (t > lastActivityAt)
                    lastActivityAt = t;
                if (o.kind === 'deep-sleep' && t > lastDeepSleepAt)
                    lastDeepSleepAt = t;
            }
            catch { /* 坏行跳过 */ }
        }
    ...
    const probeSession = (rec) => {
        ...
        void (async () => {
            try {
                ...
                if (!file) {
                    // 探针不可用 → 保守视为活跃（宁可不睡也不错睡），并留审计告警提示排查
                    rec.probeResult = 'no-transcript';
                    rec.state = 'running';
                    rec.lastEventAt = Date.now();
                    log(`deep sleep probe: ${short} 探针不可用（无转录路径），保守视为活跃——请检查记忆仓 locate-transcript-probe 是否就位`);
                    audit({ kind: 'deep-sleep-probe', sid: short, result: 'no-transcript', note: '探针不可用，保守不睡' });
                    return;
                }
                const snap = () => {
                    try {
                ...
                if (!s2) {
                    ...
                    rec.state = 'ended';
                    rec.lastEndAt = Date.now();
                    log(`deep sleep probe: ${short} 转录不可读 → 视为结束`);
                ...
                if (s2.mtimeMs > s1.mtimeMs || s2.size !== s1.size) {
                    ...
                    rec.state = 'ended';
                    rec.lastEndAt = Date.now();
                    log(`deep sleep probe: ${short} 会话已消失且无输出 → 视为结束（计入停滞）`);
                ...
                rec.state = 'running';
                rec.lastEventAt = Date.now(); // 探测异常 → 保守视为活跃
                log(`deep sleep probe err ${short}: ${String(e?.message || e).slice(0, 120)}`);
            }
        })();
    };
    const deepSleepCheck = () => {
        ...
        for (const [sid, rec] of sessions) {
            ...
            if (rec.state === 'running' && now - rec.lastEventAt >= probeAfter && config.deepSleepProbe)
                probeSession(rec);
            if (rec.state === 'probing') {
                probing++;
                continue;
        ...
        runDeepSleep().catch((e) => log(`deep sleep err: ${String(e?.message || e).slice(0, 120)}`)).finally(() => { deepSleepRunning = false; });
    };
    ...
    ctx.effect(() => {
        ...
        log(`deep sleep 巡检启动（enable=${config.enableDeepSleep} · 停滞阈值 ${Math.round((Number(config.deepSleepIdleMs) || 10800000) / 60000)}min · 探测 ${config.deepSleepProbe ? '开' : '关'}${config.deepSleepProbe ? `（无事件 ${Math.round((Number(config.deepSleepProbeAfterMs) || 10800000) / 60000)}min 后发起，探针${probeOk ? '就位' : '缺失→保守不睡'}）` : ''}）`);
