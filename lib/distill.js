...
export function registerDistill(ctx, config) {
    ...
    try {
        ...
        for (let i = lines.length - 1; i >= 0; i--) {
            if (!lines[i].trim())
                continue;
            try {
                const t = Date.parse(JSON.parse(lines[i]).at);
                if (!Number.isNaN(t)) {
                    if (t > lastActivityAt)
                        lastActivityAt = t;
                    break;
    ...
    const gatherDeepSleepTraces = (memRoot) => {
        ...
        try {
            ...
            touched = readdirSync(notesDir).filter((f) => f.endsWith('.md') && f !== 'INDEX.md').filter((f) => {
                try {
                    return statSync(join(notesDir, f)).mtimeMs >= startOfDay.getTime();
                }
                catch {
                    return false;
                }
        ...
        try {
            const hits = readFileSync(join(memRoot, 'audit', 'access.log'), 'utf8').split('\n').filter((l) => l.trim())
                ...
                .filter((o) => !!o && typeof o.t === 'string' && o.t.startsWith(today));
            if (hits.length) {
                const agg = new Map();
                for (const h of hits) {
    ...
    const deepSleepCheck = () => {
        if (!config.enableDeepSleep || deepSleepRunning)
            return;
        const now = Date.now();
        if (now - lastActivityAt < config.deepSleepIdleMs)
            return;
        if (lastActivityAt <= lastDeepSleepAt)
            return; // 本轮停滞窗口已消化（新活动推进 lastActivityAt 后重新武装）
        try {
            const roots = ctx.agents.roots() || [];
            if (roots.some((a) => a.status && a.status !== 'idle'))
                return; // 尚有活跃会话，推迟到下轮巡检
        }
        catch { /* 探测失败按无活跃处理 */ }
