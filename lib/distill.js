...
import { applyTreeOps, applyForgetOps, sectionExists } from './treeops.js';
...
export function registerDistill(ctx, config) {
    ...
    const resolveWatermark = (sid, agent, mapCache) => {
        ...
        if (recVer === undefined || liveVer === undefined) {
            discardWatermark(sid, 'unverifiable-legacy', agent, wm);
            return null;
        }
        if (liveVer !== recVer) {
            discardWatermark(sid, `format-migrated ${recVer}→${liveVer}`, agent, wm);
            return null;
        }
        const fp = agentFingerprintAt(agent, lastSeq);
        if (fp === null || wm.fp === undefined || wm.fp === null) {
            discardWatermark(sid, 'fingerprint-unavailable', agent, wm);
            return null;
        }
        if (fp !== wm.fp) {
            discardWatermark(sid, `seq-space-shifted (${String(wm.fp).slice(0, 40)} ≠ ${fp.slice(0, 40)})`, agent, wm);
            return null;
        }
    ...
    const applyPrinciples = async (memRoot, out) => {
        ...
        if (!existsSync(gateScript))
            return { attempted: 0, added: 0, replaced: 0, skipped: 0, gate: 'write_gate 未就位', rejectedLines: [] };
        let content = '';
        try {
        ...
        if (!added && !replaced)
            return { attempted, added, replaced, skipped, gate: 'no-op', rejectedLines };
        ...
        try {
            ...
            if (g.status === 0) {
                ...
                return { attempted, added, replaced, skipped, gate: 'pass', rejectedLines };
            }
            try {
                unlinkSync(tmpPath);
            }
            ...
            return { attempted, added: 0, replaced: 0, skipped: attempted, gate: '落盘异常: ' + String(e.message).slice(0, 80), rejectedLines: [...rejectedLines, ...accepted.map((t) => `[io] ${t}`)] };
        }
    };
    ...
    const applyPointerOps = async (memRoot, out) => {
        const gateScript = join(memoryLibRoot(), 'scripts', 'memory_write_gate.mjs');
        if (!existsSync(gateScript))
     ...
     */
    const consolidateTree = async (memRoot) => {
        ...
        try {
            ...
            try {
                ...
                const app = (stop === 'completed' && out)
                    ...
                    : { attempted: 0, added: 0, replaced: 0, skipped: 0, gate: `stop=${stop}`, rejectedLines: [] };
