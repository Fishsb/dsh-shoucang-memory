...
export async function applyConvergeOps(memRoot, ops, hooks) {
    ...
    for (const op of list) {
        ...
        try {
            ...
            if (!ar.ok) {
                res.skipped++;
                res.reasons.push(`归档失败（拒绝移除）：${String(ar.error).slice(0, 60)}`);
                continue;
            }
            res.archived++;
        }
        ...
        if (!ed.ok) {
            res.skipped++;
            res.reasons.push(ed.refused ? '库锁被占用（拒写）' : `写回失败：${String(ed.error).slice(0, 60)}`);
            continue;
        }
        if (!ed.changed) {
            res.skipped++;
            res.reasons.push(`写前复检未命中（并发变更）：${fine.slice(0, 30)}`);
            continue;
        }
        res.applied++;
