...
function runNodeBin(args, opts) {
    return new Promise((resolve) => {
        ...
        const child = spawn(process.execPath || 'node', args, {
            cwd: opts?.cwd, env: env, windowsHide: true,
        });
        ...
        const sink = makeCappedSink(OUT_CAP, () => { try {
            child.kill();
        }
        catch { /* */ } });
...
export const coreName = (t) => String(t || '').trim().replace(CORE_TAIL_RE, '').trim();
/** 双向包含（核心名口径）：x===y || x.includes(y) || y.includes(x) */
export const biContains = (a, b) => {
    const x = coreName(a);
    const y = coreName(b);
...
export function parseSections(ls) {
    const heads = [];
...
export function normalizeNotesFile(f) {
...
export const atomicWrite = (p, text) => atomicWriteFile(p, text).ok;
/** 既有行整理口径：空行压缩 + 末尾单换行（与 applyPrinciples/applyPointerOps/consolidateTree 一致） */
export const finalize = (ls) => ls.join('\n').replace(/\n{3,}/g, '\n\n').replace(/\s+$/, '\n');
...
async function runIndexGate(memRoot, target, tmp) {
    ...
    if (!existsSync(gateScript))
        return 'absent';
    const env = { MEMORY_ROOT: memRoot, SHOUCANG_CAP_STRICT: '0' };
...
export function safeAudit(hooks, o) { try {
    hooks.audit(o);
}
catch { /* */ } }
export function safeLog(hooks, m) { try {
    hooks.log(m);
}
catch { /* */ } }
/**
...
export async function applyTreeOps(memRoot, ops, hooksIn) {
    ...
    const archive = (o) => {
        try {
            appendFileSync(ctx.archFile, envelope(o, `archive.${String(o.op || o.action || o.kind || 'event')}`), 'utf8');
            ctx.archived++;
        }
...
export function matchSection(sections, kw) {
 ...
 */
const OP_SCOPE = { rename: [2, 3], merge: [2, 3], splitTarget: [2], splitConflict: [3] };
/** 按 op 取节范围（**唯一 filter 落点**；口径见 `OP_SCOPE`） */
export const scopeOf = (sections, op) => sections.filter((s) => OP_SCOPE[op].includes(s.level));
