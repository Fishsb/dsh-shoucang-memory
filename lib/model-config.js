...
export const modelOptionsOf = (config) => ({
 ...
 */
export function resolveRoute(cfg, kind) {
    ...
    if (sp && sm)
        return { provider: sp, model: sm, effort: se || '' };
    if (cfg.llmProvider && cfg.llmModel)
        return { provider: cfg.llmProvider, model: cfg.llmModel, effort: '' };
    return null;
}
/**
 ...
 */
export function withEffort(opts, effort) {
