 ...
 *
 *  ⚠ **容量开关的显式声明**（ADR-333 册三 · B3 的第二半，2026-09-22）：
 *  本处此前传 `{ ...process.env, MEMORY_ROOT }` ⇒ **继承宿主环境**。若部署侧恰好设了
 *  `SHOUCANG_CAP_STRICT=1`，**人在面板上手工改文件也会被容量闸拒**——而面板写路径的
 *  语义恰恰是「**用户显式触发，非热路径**」（本件 `:63` 原文），安全网是「file 白名单 +
 *  前端 confirm」，**不该再叠一层自动容量闸**（人已经明确表达了意图）。
 *  ⇒ 显式置 `SHOUCANG_CAP_STRICT='0'`：**不让环境碰运气决定**，而是把"人工路径不参与
 *    自动容量阻断"写成代码里的**显式事实**。
 *  ⚠ 其余门禁**照旧生效**：指针（exit 2）/格式（4）/凭据（5）不因本行而放宽——
 *     本行只声明**容量**这一支在此路径不阻断（与 `capacityEnforce` 的"只关容量支路"同一取向）。 */
const gateWrite = (target, tmpPath) => new Promise((done) => {
    ...
    execFile('node', [gate, target, tmpPath], { encoding: 'utf8', timeout: 30000, windowsHide: true, env: { ...process.env, MEMORY_ROOT: memoryLibRoot(), SHOUCANG_CAP_STRICT: '0' } }, (err, stdout, stderr) => {
        if (!err)
...
const snapshotBank = (d) => {
    try {
        ...
        if (!existsSync(script))
            return;
        execFile(process.execPath, [script, '--message', `memory: panel-write @ ${new Date().toISOString().slice(0, 19)}`], { env: { ...process.env, MEMORY_ROOT: memoryLibRoot() }, windowsHide: true, timeout: 20000 }, (err) => {
