...
function vectorStatus2Route(d, _req, res) {
    try {
...
function injectPreviewRoute(d, req, res) {
    let q = '';
    try {
...
function mountHotMemoryInjection(ctx, d) {
    ...
    if (sp && typeof sp.context === 'function') {
        ...
        d.disposers.push(sp.context({
            ...
            text: (context) => { d.injectMeta.calls++; d.injectMeta.lastAt = Date.now(); return d.hot.build(taskTextOf(context)); },
        }));
