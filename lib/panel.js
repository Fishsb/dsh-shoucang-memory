...
export function applyPanel(ctx, config) {
    ...
    const buildHotMemoryText = (query = '') => {
        ...
        const alwaysProfileTags = new Set(Object.entries((CARRIERS.tags) || {})
    ...
    route('/toggle', async (req, res) => {
        ...
        if (SUITE_BOOL[key]) {
            ...
            const cur = readSuiteConfig()[prop] !== false; // 缺省 true
            writeSuiteConfig({ ...readSuiteConfig(), [prop]: !cur });
            injectCache.at = 0;
    ...
    route('/mcl/status', (_req, res) => {
        try {
            ...
            const persisted = readSuiteConfig();
    ...
    route('/selfcheck/run', (_req, res) => {
        try {
            sendJson(res, 200, runSelfCheckNow());
        }
        catch (e) {
    ...
    ctx.effect(() => {
        ...
        const snapshotBank = () => {
            try {
                ...
                if (!existsSync(script))
                    return;
                execFile('node', [script, '--message', `memory: panel-write @ ${new Date().toISOString().slice(0, 19)}`], { env: { ...process.env, MEMORY_ROOT: memoryLibRoot() }, windowsHide: true, timeout: 20000 }, () => { });
            }
            catch { /* 静默 */ }
        };
