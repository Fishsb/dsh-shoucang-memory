...
export function applyPanel(ctx, config) {
    ...
    const buildHotMemoryText = () => {
        ...
        if (hasSchedInject) {
            ...
            if (typeof sched.injectMaxTokens === 'number')
                maxTokens = sched.injectMaxTokens;
            if (typeof sched.injectAgentMaxChars === 'number')
                capAgent = sched.injectAgentMaxChars;
            if (typeof sched.injectUserMaxChars === 'number')
                capUser = sched.injectUserMaxChars;
            if (typeof sched.injectMemoryMaxChars === 'number')
                capMemory = sched.injectMemoryMaxChars;
        }
        else {
        ...
        try {
            ...
            const vectorMini = (() => {
                try {
                    ...
                    const baseUrl = String(p.embedBaseUrl || 'http://127.0.0.1:9915/v1');
                    ...
                    if (enabled && /^https?:\/\/(127\.0\.0\.1|localhost):9915/.test(baseUrl))
    ...
    ctx.effect(() => {
            ...
            try {
                ...
                if (enabled && local) {
                    try {
                        ...
                        const r = execFileSync('node', ['-e', 'fetch("http://127.0.0.1:9915/health").then(r=>r.json()).then(j=>console.log(JSON.stringify(j))).catch(()=>process.exit(1))'], { encoding: 'utf8', timeout: 8000, windowsHide: true, stdio: ['ignore', 'pipe', 'ignore'], env: env2 });
        ...
        const gateWrite = (target, tmpPath) => {
            try {
                const gate = join(memoryLibRoot(), 'scripts', 'memory_write_gate.mjs');
                if (!existsSync(gate))
                    return { ok: false, reason: 'write_gate 未就位' };
                const r = execFileSync('node', [gate, target, tmpPath], { encoding: 'utf8', timeout: 30000, windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'], env: { ...process.env, MEMORY_ROOT: memoryLibRoot() } });
                return { ok: true, out: String(r) };
            }
            catch (e) {
                const ee = e;
                return { ok: false, reason: `gate exit=${ee.status ?? '?'}`, out: typeof ee.stderr === 'string' ? ee.stderr : (ee.stderr || Buffer.from('')).toString() || ee.message || '' };
            }
        };
        ...
        const writeMemViaGate = (file, nextText) => {
            const abs = join(memoryLibRoot(), file);
