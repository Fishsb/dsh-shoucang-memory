...
function walkMarkdown(rootDir, skipDirs = []) {
    const out = [];
...
export function applyPanel(ctx, config) {
    ...
    const pointerTitle = (f) => {
        const m = f.text.match(/^---\r?\n([\s\S]*?)\r?\n---/);
        if (m) {
            for (const l of m[1].split(/\r?\n/)) {
                if (l.startsWith('title:')) {
                    const v = l.replace(/^title:\s*/, '').replace(/^['"]|['"]$/g, '');
                    if (v)
                        return v.slice(0, 30);
                }
                if (l.startsWith('name:')) {
                    const v = l.replace(/^name:\s*/, '').replace(/^['"]|['"]$/g, '');
                    if (v)
                        return v.slice(0, 30);
                }
            }
    ...
    const buildHotMemoryText = () => {
        ...
        const root = activeRootOf();
        const key = root?.path ?? '';
        if (injectCache.root === key && now - injectCache.at < 30000)
        ...
        if (level === 'off' || (!personaOn && !memoryOn)) {
            injectCache.text = '';
            return '';
        }
        const lines = ['[守藏·热记忆] 画像与记忆指针（缩略注入·按需 get_file 拉全文）：'];
        const seen = new Set(); // 防御性去重：同一 rel 绝不重复注入（审查 2026-08-27）
        const pushLine = (rel, line) => {
            if (seen.has(rel))
                return false;
            seen.add(rel);
            lines.push(line);
        ...
        if (personaOn) {
            ...
            if (me && injectMe)
                pushLine(`画像/${me.rel}`, `- [[画像/${me.rel}|用户画像]] — ${pointerDesc(me)}`);
            if (you && you !== me && injectYou)
                pushLine(`画像/${you.rel}`, `- [[画像/${you.rel}|agent画像]] — ${pointerDesc(you)}`);
        }
        ...
        if (memoryOn && hotMemoryOn) {
            ...
            const caps = { low: 2, medium: 4, high: 8, smart: 10 };
    ...
    const memoryCaps = (base) => {
        const out = { 'MEMORY.md': 3000, 'USER.md': 2000, 'AGENT.md': 2000 };
        try {
        ...
        const scanDir = (dir) => {
            ...
            for (const e of entries) {
                ...
                for (const c of GM_CARD_FILES) {
                    try {
                        cards[c.type] = readFileSync(join(cardsDir, c.file), 'utf8').split(/\r?\n/).filter((l) => /^##\s+/.test(l)).length;
                    }
                    catch {
                        cards[c.type] = 0;
                    }
    ...
    route('/pmg/cards', (req, res) => {
        try {
            ...
            if (!cardFile)
                return sendJson(res, 400, { error: 'bad card type' });
            const lib = gmDiscover().find((l) => l.id === id);
            if (!lib)
                return sendJson(res, 404, { error: 'card library not found' });
            const full = join(lib.path, 'docs', 'devref', 'cards', cardFile.file);
            if (!existsSync(full))
                return sendJson(res, 200, { present: false, error: '该册尚不存在（空卡册）' });
            const text = readFileSync(full, 'utf8');
            const cards = [];
            let cur = null;
            for (const l of text.split(/\r?\n/)) {
                ...
                if (m) {
                    if (cur)
                        cards.push({ title: cur.title, body: cur.body.join('\n').trim() });
    ...
    const suiteScan = () => {
        ...
        let profDirs = [];
        try {
            profDirs = readdirSync(profilesDir, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => d.name);
        }
        catch { /* none */ }
        const profileHits = {};
        for (const prof of profDirs) {
            try {
                const pj = JSON.parse(readFileSync(join(profilesDir, prof, 'package.json'), 'utf8'));
                ...
                for (const m of SUITE_MEMBERS) {
                    ...
                    if (names.has(m.package) || names.has(short))
                        (profileHits[m.id] = profileHits[m.id] || []).push(prof);
                }
            }
