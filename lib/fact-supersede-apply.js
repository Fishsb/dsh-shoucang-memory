...
/** 留档目录（每轮一个文件，一行一记录原文）。 */
export const supersedeArchiveDirOf = (root) => join(String(root), 'audit', 'supersede');
 ...
 */
export function applySupersedeOps(d, ops, opts = {}) {
    const list = Array.isArray(ops) ? ops : [];
    const res = { ran: false, applied: 0, skipped: 0, archived: 0, reasons: [], verdicts: [], ok: true };
    if (!opts.enabled) {
        ...
        res.verdicts = list.map((o) => ({ targetId: str(o?.targetId), verdict: 'skipped', reason: 'disabled' }));
        return res;
    }
    if (!list.length) {
        res.ran = true;
    ...
    try {
        ...
        if (!existsSync(recordStorePath(d.root))) {
            ...
            res.skipped = list.length;
            res.verdicts = list.map((o) => ({ targetId: str(o?.targetId), verdict: 'skipped', reason: 'no-store' }));
        ...
        if (cur.error) {
            ...
            res.skipped = list.length;
            res.verdicts = list.map((o) => ({ targetId: str(o?.targetId), verdict: 'skipped', reason: 'store-unreadable' }));
            return res;
        }
        ...
        if (judged.applied) {
            try {
                ...
                res.reasons.push(`留档失败 ⇒ 整批拒改（未写库）：${String(e?.message || e).slice(0, 80)}`);
                // 逐条把已裁决的 applied 改判为 skipped（**可见**，不是默默不算）
                res.verdicts = judged.verdicts.map((x) => (x.verdict === 'applied' ? { ...x, verdict: 'skipped', reason: 'archive-failed' } : x));
        ...
        res.ok = false;
        res.reasons.push(String(e?.message ?? e).slice(0, 160));
 ...
 */
function judgeOps(at, input, list) {
    let records = input.slice();
    const out = { applied: 0, skipped: 0, reasons: [], verdicts: [], rows: [] };
...
function locate(records, id, text) {
    if (id) {
    ...
    if (m.length === 0)
        return { reason: 'no-match', why: '陈旧指令：目标正文在当下库中逐字未命中（库已变）' };
    if (m.length > 1)
        return { reason: 'ambiguous-text', why: `歧义：目标正文命中 ${m.length} 处（需唯一）⇒ 拒` };
    return { record: m[0], reason: 'ok', why: '' };
}
/** 读取留档（回滚入口用：`revive` 只需 id，此处给出 id 与原文供人工核对）。 */
export function readSupersedeArchive(root, file) {
    const out = [];
    try {
 ...
 *   ③ 超长目标 ⇒ 拒（模型倾向把整段话当目标）。
 */
export function planSupersedeOps(proposals, seenLines, limits = {}) {
    ...
    const out = { ops: [], accepted: 0, rejected: 0, reasons: [] };
    if (!list.length)
        return out;
    for (const raw of list.slice(0, Math.max(maxOps, 0) || 0)) {
        ...
        if (!target) {
            out.rejected++;
            out.reasons.push('空目标 ⇒ 拒');
            continue;
        }
        if (target.length > maxChars) {
            out.rejected++;
            out.reasons.push(`目标超长（${target.length} > ${maxChars}）⇒ 拒`);
            continue;
        }
        if (!seen.has(target)) {
    ...
    if (list.length > maxOps)
        out.reasons.push(`配额截断：提案 ${list.length} 条，上限 ${maxOps}`);
    return out;
}
//# sourceMappingURL=fact-supersede-apply.js.map