...
export function fileSectionsOf(memRoot) {
    ...
    for (const f of ordered) {
        ...
        for (const t of titles) {
            if (t.level === 2) {
                ...
                paths.push(curTop ? `${curTop}/${t.title}` : t.title);
        }
...
export const addressKeyOf = (file, section) => `${String(file || '').trim()}§${String(section ?? '').split('/').map((s) => sectionCore(s)).filter(Boolean).join('/')}`;
/**
 * 生成地址清单（纯读、零抛出）。
 * 降级链：① 全量（`##` + `##父/子`）→ ② 仅 `##` 级 → ③ 仅摘要计数（连 `##` 级都放不下时）。
 * ⚠ 任何一级都**按行裁剪**（不截断到半行），且降级时**显式标注**（不静默变少）。
 */
export function sectionAddressSupply(memRoot, maxChars = 3000) {
    const per = fileSectionsOf(memRoot);
    const set = new Set();
    for (const f of per)
        for (const t of f.tops)
            set.add(addressKeyOf(f.file, t));
    for (const f of per)
        for (const p of f.paths)
            set.add(addressKeyOf(f.file, p));
    ...
    for (const f of per) {
        ...
        if (f.paths.length)
            full.push(`  └ ${f.paths.map((p) => `§${p}`).join(' §')}`);
    }
    const build = (rows) => [head, ...rows];
    const size = (rows) => build(rows).join('\n').length;
    if (size(full) <= budget)
        return { lines: build(full), set, sections, files, degraded: false, summary: `地址清单 ${sections} 条（全量 · ${files} 文件）` };
    // ② 仅 ## 级
    const topsOnly = per.map((f) => `${f.file} §${f.tops.join(' §')}`);
    const note2 = '（预算不足 ⇒ 已降级为**仅 ## 级**；### 级需写「父/子」路径时，按 ## 父节名推）';
    if (size([note2, ...topsOnly]) <= budget)
        return { lines: build([note2, ...topsOnly]), set, sections, files, degraded: true, summary: `地址清单 ${sections} 条（降级：仅 ## 级 · ${files} 文件）` };
    // ③ 逐行裁剪（保留完整行，末尾显式标注被裁）
    const kept = [note2];
    let used = size(kept);
    ...
    if (dropped > 0)
        kept.push(`…（预算所限，另有 ${dropped} 个文件未列出；地址集仍按全量校验）`);
...
export function buildDistillUserInput(p) {
    return [
...
//# sourceMappingURL=section-supply.js.map