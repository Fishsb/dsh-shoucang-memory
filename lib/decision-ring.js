...
export function openDecision(records, d) {
...
export function outcomeOf(records, decisionId) {
 ...
 */
export function collectOutcome(records, decisionId, o) {
    const idx = records.findIndex((r) => r.id === decisionId && r.kind === 'decision');
    if (idx < 0)
        return { records: records.slice(), ok: false, reason: `裁决不存在（${decisionId}）` };
    if (outcomeOf(records, decisionId))
        return { records: records.slice(), ok: false, reason: '该裁决已回收（不重复产记录）' };
...
export function recordValence(records, v) {
...
//# sourceMappingURL=decision-ring.js.map