...
export function evaluateL0(input) {
    ...
    const conflict = (() => {
        ...
        if (explicit === CONF.coexist || explicit === CONF.supersede || explicit === CONF.none)
            return explicit;
        return input.supersedes ? CONF.supersede : CONF.none;
    })();
    if (conflict === CONF.supersede)
        basis.push('l0.conflict.supersede');
    else if (conflict === CONF.coexist)
        basis.push('l0.conflict.coexist');
