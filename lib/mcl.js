...
export function registerMcl(ctx, cfg, hooksIn) {
    ...
    ctx.on('agent/pre-step', async (payload, next) => {
        ...
        try {
            ...
            if (!ready.has(sid)) {
                ready.add(sid);
                hooks.audit({ kind: 'mcl-ready', sid: sid.replace(/^session-/, '').slice(0, 8), step });
            }
            ...
            if (!st.channel && step !== 1) {
                if (!st.lateLogged) {
                    st.lateLogged = true;
                    hooks.audit({ kind: 'mcl-skip', sid: sid.replace(/^session-/, '').slice(0, 8), step, reason: 'late-step' });
                }
                return decision;
            }
            ...
            if (!compliant)
                hooks.audit({ kind: 'mcl-step', sid: sid.replace(/^session-/, '').slice(0, 8), step, channel: 'slow', sim: Number(st.sim.toFixed(3)), compliant: false, nudge: 0, nudges: st.nudges, topics: st.topics });
            return decision;
        }
        catch (e) {
            try {
