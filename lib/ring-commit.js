 ...
 */
const cuesOfItem = (it, audit) => {
    const raw = it?.cues;
    const { keys, rejected } = cueSetOf(raw);
    if (rejected.length && audit) {
        try {
            audit({ kind: 'cue.rejected', count: rejected.length, keys: rejected.map((r) => r.key).slice(0, 8), reasons: [...new Set(rejected.map((r) => r.reason))].slice(0, 4) });
        }
        catch { /* 审计失败不影响落库 */ }
    }
...
export function commitRingChannels(d, root, out, at, source = 'unknown') {
    ...
    try {
        ...
        for (const it of rel) {
            ...
            const r = assertRelation(records, { who, note, ...(Number.isFinite(lv) ? { level: lv } : {}), evidence: str(it?.evidence), cues: cuesOfItem(it, d.audit), at });
            records = r.records;
            res.relations++;
        }
        ...
        for (const it of val) {
            ...
            if (!trigger || !Number.isFinite(v))
                continue;
            const r = recordValence(records, { trigger, valence: v, evidence: str(it?.evidence), cues: cuesOfItem(it, d.audit), at });
            records = r.records;
            res.valences++;
        }
