...
export function noteEvent(dep, m, sid, isTurnEnd) {
    const now = Date.now();
    m.lastActivityAt = now;
    const rec = m.sessions.get(sid) || { sid, state: 'running', lastEventAt: now, lastEndAt: 0, probeAt: 0, probeRound: 0, stallRound: 0 };
    rec.lastEventAt = now;
...
export function replayWatermark(dep, m) {
    ...
    try {
        ...
        for (const o of readDistillAudit(dep.io.auditFile)) {
            try {
                const t = Date.parse(String(o.at));
                if (Number.isNaN(t))
                    continue;
                if (t > m.lastActivityAt)
                    m.lastActivityAt = t;
                ...
                if (deepSleepReplayable(o) && t > m.lastDeepSleepAt)
                    m.lastDeepSleepAt = t;
            }
            catch { /* 坏行跳过 */ }
        }
 ...
 */
export function deepSleepCheck(dep, m) {
    if (!dep.config.enableDeepSleep || m.deepSleepRunning)
        return;
    const now = Date.now();
    ...
    for (const [sid, rec] of m.sessions) {
        ...
        if (dep.session.hasActiveSubagents(sid)) {
            rec.state = 'running';
            rec.lastEventAt = now;
            continue;
        }
    ...
    if (probing > 0) {
        dep.io.log(`deep sleep: ${probing} 个会话探测未决，本轮跳过（保守不睡）`);
        return;
    }
    ...
    const runAllChunks = async () => {
        ...
        while (r !== 'failed' && dep.state.epoch.pendingChunk !== null)
            r = await runDeepSleep(dep.run, since);
    ...
    runAllChunks().then((r) => {
        ...
        if (r === 'failed') {
            ...
            dep.io.log('deep sleep: 本轮未消化（failed），水位回滚（同一批痕迹下轮可重试）');
        }
        else
            m.lastDeepSleepAt = now;
    }).catch((e) => {
        m.lastDeepSleepAt = prevDeepSleepAt;
        dep.io.log(`deep sleep err: ${String(e?.message || e).slice(0, 120)}（水位回滚）`);
    }).finally(() => { m.deepSleepRunning = false; });
}
/** 状态机快照——面板展示 / 手动触发（POST /deepsleep/trigger）/ 配置读写（/deepsleep/config）均读这里 */
export function getDeepSleepStatus(dep, m) {
    ...
    return {
        ...
        phase: (() => {
            if (!dep.config.enableDeepSleep)
                return 'disabled';
            if (m.deepSleepRunning)
                return 'running';
            if (probing > 0 || suspect > 0)
                return 'probing';
            if (running > 0)
                return 'active';
            if (stalled > 0)
...
export async function runDeepSleepNow(dep, m) {
    if (m.deepSleepRunning)
...
//# sourceMappingURL=deepsleep-machine.js.map