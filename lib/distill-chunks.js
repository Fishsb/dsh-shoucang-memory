 ...
 */
export const MATERIAL_EVENT_TYPES = new Set(['user/message', 'assistant/message']);
/** 该事件是否属于**材料面**（只有材料事件能进段文本、能推段边界） */
export const isMaterialEvent = (e) => MATERIAL_EVENT_TYPES.has(String(e && e.type));
export const textPartsOfEvent = (e) => {
    const parts = [];
    if (!isMaterialEvent(e))
        return parts; // ★ 册一：非材料事件（管理事件）零字进入材料
    if (e.type === 'user/message') {
        if (!isRealUserEvent(e))
            return parts; // ★ 宿主注入块不进蒸馏材料 / episode.intent
        const d = e.data || {};
        ...
        for (const c of arr)
            if (c && c.type === 'text' && typeof c.text === 'string')
                parts.push('[assistant] ' + c.text.slice(0, 3000));
    }
    return parts;
};
/** 事件元信息（段身份构造用；与水位双证 `type|time|dataLen` 同思路，但不做 JSON 序列化以免大正文开销） */
const metaOfEvent = (e) => `${e.seq ?? 0}|${e.type || '?'}|${Number(e.time ?? -1)}`;
/** 段身份 = 首末材料事件元信息 + 段文本长度（内容变了必变；引用同一内容重试必稳） */
const segKeyOf = (head, tail, textLen) => createHash('sha1').update(`${head}|${tail}|${textLen}`).digest('hex').slice(0, 12);
/**
...
export function buildEventChunks(agent, lastSeq, chunkChars = CHUNK_CHARS, eventsOf) {
    ...
    for (const e of events) {
        ...
        if (seq <= lastSeq)
            continue;
        if (seq > maxSeq)
            maxSeq = seq; // 扫描边界：一切事件（含管理事件）都推进它 ⇒ 不丢事件
        ...
        if (cur && cur.len + addCost > chunkChars)
            flush();
        if (!cur)
            cur = { startSeq: seq, endSeq: seq, parts: [], len: 0, events: 0, head: metaOfEvent(e), tail: metaOfEvent(e) };
...
export const manifestLineFor = (endSeq, route, out) => {
    const seen = new Set();
    const topics = [];
...
export const manifestPush = (manifest, line, cap) => {
...
//# sourceMappingURL=distill-chunks.js.map