...
export function gatherMaterials(root, sinceMs) {
    ...
    const sectionRef = { droppedMissing: 0, ambiguousKept: 0, ambiguous: [], missing: [] };
    const forgetCandidates = (() => {
        try {
            ...
            for (const idx of ['MEMORY.md', 'USER.md', 'AGENT.md']) {
                let raw = '';
                try {
                    raw = readFileSync(join(root, idx), 'utf8');
                }
                catch {
                    continue;
                }
            ...
            for (const l of readFileSync(join(root, 'audit', 'activity.jsonl'), 'utf8').split(/\r?\n/)) {
                ...
                try {
                    ...
                    if (ref.state === 'missing') {
                        sectionRef.droppedMissing++;
                        sectionRef.missing.push(`${f} §${s}`);
                        continue;
                    }
                    const amb = ref.state === 'ambiguous';
                    if (amb) {
                        sectionRef.ambiguousKept++;
                        sectionRef.ambiguous.push(`${f} §${s}（${ref.cands.length} 个同名候选）`);
                    }
    ...
    const toolUsage = (() => {
        try {
            ...
            for (const r of inWin)
                byTool.set(r.tool, (byTool.get(r.tool) || 0) + (Number(r.n) || 0));
...
//# sourceMappingURL=deepsleep-materials.js.map