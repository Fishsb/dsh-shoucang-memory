...
const writeDispatch = async (dep, sid, out, route, workspace) => {
    ...
    const failedItems = [];
    const markFailed = (k, target, section, reason, tag = '') => {
        failed++;
        if (failedItems.length < 20)
            failedItems.push({ k, target: String(target || ''), section: String(section || ''), tag, reason: String(reason || '').slice(0, 80) });
    };
    ...
    if (route === 'memory') {
        ...
        for (const a of appends) {
            ...
            if (r.status === 0)
                ...
                markFailed('append', String(a.target), _sec, textOf(r));
        ...
        for (const ni of newIndex) {
            ...
            if (r.status === 0) {
                ...
                markFailed('index', t, '', textOf(r), mNew ? mNew[1] : '');
        ...
        for (const p of profiles) {
            if (!p || !p.target || !p.section || !p.text) {
                markFailed('profile-baditem', p?.target, p?.section, '字段缺失');
                continue;
            }
            ...
            if (w.st === 'added')
                ...
                markFailed('profile', String(p.target), String(p.section), '画像行落盘失败');
            // dedup：静默不计
        }
    ...
    if (route === 'project') {
        ...
        const cardDupOf = (title) => {
            ...
            for (const ex of existingCardTitles) {
                if (ex === title)
                    return ex;
                if (dep.cand.cardSimilar(title, ex) >= 0.42)
                    return ex;
            }
            return null;
        };
