...
export function readLockInfo(bankRoot) {
    ...
    try {
        const o = JSON.parse(readFileSync(join(dir, 'owner.json'), 'utf8'));
        ...
        try {
            return { owner: '', pid: 0, at: '', note: '(owner.json 缺失)', ageMs: Date.now() - statSync(dir).mtimeMs };
        }
        catch {
            return null;
        }
...
export function logLockEvent(bankRoot, ev) {
    ...
    try {
        appendFileSync(lockLogPathOf(bankRoot), `${new Date().toISOString()} ${body}\n`, 'utf8');
    }
    catch { /* 日志失败不影响锁语义 */ }
}
/** 原子取锁：mkdir 成功即持有。 */
...
/** 陈旧锁改名接管（留证不直删）。 */
function takeOverStale(bankRoot, owner, ageMs) {
...
export function acquireBankLock(bankRoot, opts = {}) {
    ...
    if (HELD.has(bankRoot))
        return { owner: HELD.get(bankRoot), root: bankRoot, path: lockPathOf(bankRoot), reentrant: true, staleTakenOver: false, waitedMs: 0 };
    ...
    if (token) {
        ...
        if (cur && cur.owner === token)
            return { owner: token, root: bankRoot, path: lockPathOf(bankRoot), reentrant: true, staleTakenOver: false, waitedMs: 0 };
    }
    const owner = String(opts.owner || newOwnerId());
    const info = { owner, pid: process.pid, at: new Date().toISOString(), atMs: Date.now(), note };
    const t0 = Date.now();
    const taken = (staleTakenOver) => {
    ...
    for (;;) {
        if (tryTake(bankRoot, info))
            return taken(false);
        const cur = readLockInfo(bankRoot);
        const age = cur?.ageMs ?? 0;
        if (age > staleMs && takeOverStale(bankRoot, owner, age) && tryTake(bankRoot, info))
            return taken(true);
...
export function releaseBankLock(handle) {
    if (!handle || handle.reentrant)
        return false;
    const cur = readLockInfo(handle.root);
    if (cur && cur.owner && cur.owner !== handle.owner)
        return false;
    try {
...
export function withBankLock(bankRoot, note, fn, opts = {}) {
    ...
    if (!h)
        return { ok: false, refused: true, waitedMs: Number(opts.waitMs ?? DEFAULT_WAIT_MS), staleTakenOver: false, owner: '' };
    try {
...
//# sourceMappingURL=bank-lock.js.map