...
const distillAgent = async (dep, agent) => {
    ...
    try {
        ...
        try {
            candFiles = readdirSync(dep.io.pendDir).filter((f) => /^\d{4}-\d{2}-\d{2}-.*\.md$/.test(f) && !f.includes('-project-defer-')).sort();
        }
        catch {
            candFiles = [];
        }
        ...
        for (let k = 0; k < segLimit; k++) {
            ...
            try {
                ...
                const disp = route === 'discard'
                    ? { added: 0, rejected: 0, failed: 0, undigested: 0, needsAnchor: 0, items: [], targetLib: 'none' }
