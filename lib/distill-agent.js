...
export function createAgentApi(dep) {
    return {
        distillAgent: (...a) => distillAgent(dep, ...a),
        armIdleTimer: (...a) => armIdleTimer(dep, ...a),
        parseAgentJson: (...a) => parseAgentJson(dep, ...a),
        runDistillNow: (...a) => runDistillNow(dep, ...a),
    };
}
/**
 * 册四（2026-09-19）：**重试计数与段身份的持久读**（守卫与被守卫事实同寿）。
 *
 * 判因：`dispatchFailStreak` 是内存 Map，而插件热重载实测 **220 次**（09-18 单日 28 次）⇒ 计数每轮清零
 ...
 */
const retryAttemptFor = (dep, sid, segKey) => {
    try {
        const p = dep.wm.wm.readSegFlowState(sid);
        if (p && p.segKey === segKey && Number(p.attempt) > 0)
            return Number(p.attempt);
    }
    catch { /* 读失败 ⇒ 回落内存缓存 */ }
...
const retryRowHeld = (dep, sid) => {
    try {
        return dep.wm.wm.readSegFlowState(sid)?.phase === 'retry';
    }
    catch {
        return false;
    }
 ...
 */
const liveSupersedeSwitch = () => {
    if (process.env.SHOUCANG_SUPERSEDE_APPLY === '1')
        return true;
    try {
        ...
        return false; /* 配置不可读 ⇒ 关闭（fail-closed） */
    }
};
/**
 ...
 */
const runSupersedeChannel = (dep, a) => {
    const sup = a.out?.supersedes;
    if (a.stop !== 'completed' || !Array.isArray(sup))
        return;
    ...
    if (!sp.ops.length)
        return;
    const sr = applySupersedeOps({ root: memoryLibRoot(), at: new Date().toISOString(), log: (m) => dep.io.infra.log(m), audit: (o) => dep.io.infra.audit({ sid: a.sid, ...o }) }, sp.ops, { enabled: liveSupersedeSwitch() });
    dep.io.infra.ledger({
    ...
    if (sr.applied > 0)
        dep.io.infra.log(`distill: ${dep.io.infra.sidShort(a.sid)} 时态剔除 ${sr.applied} 条（旧的已失效，读者不再见到）`);
};
/**
...
/** 蒸馏子代理（段循环 + 派单 + 写回 + 审计）。 */
const distillAgent = async (dep, agent) => {
    const sid = agent.id;
    if (dep.wm.st.distilling.has(sid))
        return; // 并发守卫（本 fiber 内）：蒸馏在途（最长 10min）内再触发直接跳过
    // ★ 册二（2026-09-19）：**静默判据**收敛到单一实现 `quiescenceOf`（子代理 / status / inbox 三证据）——
    //   旧实现把「status !== idle」与「有活跃子代理」写成两句各自为政的判定，与扫尾/手动入口口径不一。
    const quiet = quiescenceOf({ ctx: dep.env.ctx, hasActiveSubagents: (x) => dep.parent.parent.hasActiveSubagents(x) }, agent);
    ...
    if (quiet.state === 'busy' && quiet.why === 'active-subagent') {
    ...
    if (quiet.state === 'busy') {
        dep.io.infra.log(`distill: ${dep.io.infra.sidShort(sid)} 已恢复活跃（${quiet.why}），跳过`);
        return;
    }
    ...
    try {
        ...
        try {
            await dep.write.write.flushDeferCards();
        }
        catch { /* 回流失败不阻断本轮蒸馏 */ }
        ...
        // 阶段 2：**本轮 run 标识**（跨段共享）。一次蒸馏 = 一个 runId，使「同一轮的多段」可被识别为
        //   同一次流程实例；也让外部能区分「卡住了」与「已经换了一轮」（runId 变了=新轮）。
        ...
        // ★ 册二（2026-09-19）：**准入判定单一实现**（与扫尾 / 手动入口同源 `planIngestAdmission`）。
        //   idle 路径的「等够时间」由定时器本身承担（武装于 turn/end 之后）⇒ `sinceLastEndMs: null`，不二次计宽限；
        //   claim 在此**预读**（便宜），权威写仍在下方 `tryClaim`（幂等、跨实例）。
        const adm = planIngestAdmission({
            lastSeq, maxSeq,
            sinceLastEndMs: null,
            idleWakeMs: dep.env.config.idleWakeMs,
            quiet: quiet.state,
            claimHeld: dep.write.write.claimHeld(sid),
        ...
        if (!adm.run) {
            ...
            dep.io.infra.log(`distill: ${dep.io.infra.sidShort(sid)} 准入未通过（${adm.reason} · ${adm.evidence}），本轮跳过`);
            return;
        }
        ...
        try {
            candFiles = readdirSync(dep.io.pendDir).filter((f) => /^\d{4}-\d{2}-\d{2}-.*\.md$/.test(f) && !f.includes('-project-defer-') && !f.includes('-knowledge-defer-')).sort();
        }
        catch {
            candFiles = [];
        }
        ...
        if (!chunks.length || totalChars < (dep.env.config.minTurnChars ?? 200)) {
            ...
            // 册四：未消化判据**持久化** —— 内存记账之外再看水位流末行是否 `retry`（重载后不再误判为"已消化"）
            const undigestedPending = dep.write.write.hasPendingUndigested(sid) || retryRowHeld(dep, sid);
            const skipPlan = planSkipWatermark(undigestedPending, dep.wm.st.skipHoldStreak.get(sid) || 0, maxSeq);
            dep.wm.st.skipHoldStreak.set(sid, skipPlan.holdRounds);
            if (skipPlan.write)
                dep.wm.wm.writeWatermark(sid, skipPlan.seq, agent);
            // 跳过也留审计痕（观测盲区修复 2026-09-09：此前门槛/预筛跳过只进日志，审计里只见真实 run，
            // 「蒸馏为什么没跑」无法从数据区分——是没触发还是被挡）
            // ⚠ 2026-09-14（P7 · 治 C6/D-19「ingest 空转」）：**「无增量」与「被门槛挡下」不是一回事**——
            //   前者根本没有内容可裁决，后者是"有内容但不足门槛"。旧实现把两者混成 `below-min-chars` 并都写
            //   `decision.ingest` 台账行 ⇒ 实测 137/643 = **21%** 的台账行是 `route:skip / chars:0` 噪声，
            //   且成了**单一最大 type**，把 route 分流质量分析的主语从"真实路由"挤成了"空转"。
            //   现：reason 区分（`no-increment` vs `below-min-chars`）；**无增量不写 decision.ingest**
            //   （那不是对内容的裁决），有内容而被挡下**照旧写**。
            const noIncrement = !chunks.length || totalChars === 0;
            const skipReason = noIncrement ? 'no-increment' : 'below-min-chars';
            dep.io.infra.audit({ sid, kind: 'distill-skip', reason: skipReason, fclass: 'below-min', chars: totalChars, skipPlan: skipPlan.reason, heldForUndigested: !skipPlan.write, starved: skipPlan.holdRounds, watermarkTo: skipPlan.write ? skipPlan.seq : lastSeq });
            if (!noIncrement)
                dep.io.infra.ledger({ domain: 'ingest', sid: sid.replace(/^session-/, '').slice(0, 8), decision: { route: 'skip', reason: skipReason, chars: totalChars, heldForUndigested: !skipPlan.write }, result: { added: 0, rejected: 0, failed: 0 } });
        ...
        if (dep.env.config.distillPrescan !== false) {
            ...
            if (!hasSig && candFiles.length === 0) {
                // G-4a：同上——有未消化段时不得推到 maxSeq（册四：未消化判据含水位流 `retry` 末行）
                const skipPlan = planSkipWatermark(dep.write.write.hasPendingUndigested(sid) || retryRowHeld(dep, sid), dep.wm.st.skipHoldStreak.get(sid) || 0, maxSeq);
                dep.wm.st.skipHoldStreak.set(sid, skipPlan.holdRounds);
                if (skipPlan.write)
                    dep.wm.wm.writeWatermark(sid, skipPlan.seq, agent);
        ...
        const candText = candFiles.map((f) => {
            let body = '';
            try {
                body = readFileSync(join(dep.io.pendDir, f), 'utf8');
            }
            catch {
                return '';
            }
        ...
        for (let k = 0; k < segLimit; k++) {
            ...
            try {
                ...
                //   外界无从判断"正在蒸"还是"已经卡死"。
                // ⚠ 此处**不推** lastSeq（写的是 wmNow），故即使崩溃也**不影响续传正确性**：
                //   末行仍是 wmNow ⇒ 下轮从本段重蒸，与既有语义逐字一致（唯一变化是多了一条同 seq 的历史行）。
                // 册四（2026-09-19）：**本段已累计的重试次数随行落盘**（`attempt` + `segKey`）——计数与段身份同寿，
                //   热重载/崩溃后由 `readRunState` 重建（旧行缺字段 ⇒ 0，与旧行为逐字等价）。
                carriedAttempt = retryAttemptFor(dep, sid, chunk.segKey);
                dep.wm.wm.writeWatermark(sid, wmNow, agent, { runId, phase: 'spawn', attempt: carriedAttempt, segKey: chunk.segKey });
                ...
                const sectionMiss = (out && Array.isArray(out.appends))
                    ? out.appends.filter((a) => a && a.section && !supply.set.has(addressKeyOf(String(a.target || ''), a.section))).length
                    : 0;
                if (stop === 'completed' && out)
                    dep.llm.llmState.providerFailCount = 0;
                else if (useProvider && (stop !== 'completed' || !out))
                    dep.llm.llmState.providerFailCount++;
                ...
                const disp = route === 'discard'
                    ? { added: 0, rejected: 0, failed: 0, undigested: 0, needsAnchor: 0, pairedSkipped: 0, items: [], targetLib: 'none' }
                ...
                const conflictRaw = jd ? String(jd.conflict ?? '') : '';
                ...
                dep.io.infra.ledger({
                    ...
                    l0After: evaluateL0({ text: String(chunk.text || '').slice(0, 400), traces: 1, supersedes: conflictNorm === 'supersede', conflict: conflictNorm }),
                ...
                if (disp.added > 0 || (out?.newIndex || []).length > 0)
                    void dep.write.bankSnapshot('distill'); // v2：写后库快照（best-effort，不阻塞）
                ...
                dep.io.infra.recordStub({ sid, watermark: [wmNow, chunk.endSeq], chars: chunk.text.length, route, stop, fclass, llm: llmLabel, segKey: chunk.segKey, disp: { added: disp.added, rejected: disp.rejected, failed: disp.failed, targetLib: disp.targetLib }, outShape: out ? { appends: (out.appends || []).length, newIndex: (out.newIndex || []).length, profiles: (out.profiles || []).length, projectCards: (out.projectCards || []).length, skipped: (out.skipped || []).length, ...countRingChannels(out) } : null, chunk: k + 1, chunkStart: chunk.startSeq, chunkEnd: chunk.endSeq, totalChunks: chunks.length });
                if (stop === 'completed' && out && disp.undigested === 0) {
                    // ★ 册三（2026-09-19）：**「已裁决」即推进** —— 入册 0 条但 `rejected`/`needsAnchor` 非零也照推
                    //   （内容已被看过并裁决，只是地址/格式不合规或需人工建锚）；只有 `undigested`（I/O 类）才扣水位。
                    if (disp.added > 0)
                        anyAdded = true;
                    ...
                    dep.io.infra.recordEpisode({ sid, intent: intent.slice(0, 120), route, fclass, llm: llmLabel, outcome: disp.targetLib, added: disp.added, rejected: disp.rejected, failed: disp.undigested, lib: disp.targetLib });
                    if (route !== 'discard')
                        await dep.cand.cand.ensureFlowCandidate(sid, intent);
                    // v18 核心：段成功立即推水位到该段 endSeq（断点续传——失败/截断不再丢尾；整窗处理完自然到达 maxSeq）
                    dep.wm.wm.writeWatermark(sid, chunk.endSeq, agent, { runId, phase: 'segment-done' });
                    // G-4a：本段已消化 ⇒ 清掉它的失败记账与「扣住」计数（否则 hasPendingUndigested 永久为真 ⇒ 跳过分支被无谓扣住）
                    dep.wm.st.dispatchFailStreak.delete(`${sid}#${chunk.segKey}`);
                ...
                else if (stop === 'completed' && out && disp.undigested > 0) {
                    // A1（2026-09-11 审查修复）：stop/JSON 都 OK 但**条目级落盘失败** → 本段不算消化，水位不前移。
                    // 防死循环：同一段连续失败满 MAX_DISPATCH_RETRY 次 → 强制推进并落审计（丢失显式记账）。
                    // ★ 册三（2026-09-19）：**只有 `undigested`（I/O 类真失败）扣水位** —— 判据收敛为纯函数
                    //   `planSegmentWatermark`（单一实现，可单测）；`needsAnchor`/`rejected` 的段走上面"已裁决即推进"分支。
                    //   旧实现用 `disp.failed > 0`（一个数承载四类东西）⇒ 模型侧不合规也能永久锁水位。
                    // ★ 册一（2026-09-19）：键从 `sid#chunkEndSeq` 改为 **`sid#segKey`**（内容指纹）。
                    //   判因（真机实测）：段边界原先被**蒸馏自己 spawn 的子代理记录**（宿主在父会话落的
                    //   `subagent/catalog`）每轮 +1 ⇒ 键每轮都变 ⇒ 本防死循环闸**结构性失效**
                    //   （全日志「第 3/3 次」0 次、「强制推进」17 次）。内容指纹对"同段重试"稳定、对"内容变了"必变，
                    //   故"连败 3 次"重新成为可达状态。
                    // ★ 册四（2026-09-19）：`attempt` 先读**水位流**（持久），读不到才回落内存计数 ⇒ 热重载不归零。
                    const streakKey = `${sid}#${chunk.segKey}`;
                    const plan = planSegmentWatermark(disp, carriedAttempt, dep.write.write.MAX_DISPATCH_RETRY);
                    if (plan.advance) {
                        dep.wm.st.dispatchFailStreak.delete(streakKey);
                        dep.wm.st.skipHoldStreak.delete(sid); // 本段已放弃 ⇒ 不再因它扣住跳过分支
                        dep.wm.wm.writeWatermark(sid, chunk.endSeq, agent, { runId, phase: 'forced', attempt: plan.attempt, segKey: chunk.segKey });
                        wmNow = chunk.endSeq;
                        ...
                        segOk = true;
                        dep.io.infra.audit({ sid, kind: 'distill-run', route, stop, fclass: 'dispatch-failed-forced', llm: llmLabel, targetLib: disp.targetLib, added: disp.added, rejected: disp.rejected, failed: disp.undigested, undigested: disp.undigested, needsAnchor: disp.needsAnchor, chunk: k + 1, chunkStart: chunk.startSeq, chunkEnd: chunk.endSeq, totalChunks: chunks.length, segKey: chunk.segKey, tries: plan.attempt, reason: plan.reason });
                        dep.io.infra.log(`distill: ${dep.io.infra.sidShort(sid)} 段${k + 1}/${segLimit} 落盘失败 ${disp.undigested} 条、已连续 ${plan.attempt} 轮——强制推进水位 → ${chunk.endSeq}（丢失已审计 dispatch-failed-forced）`);
                    }
                    else {
                        dep.wm.st.dispatchFailStreak.set(streakKey, plan.attempt);
                        // 册四：把本轮次数落到水位流（重载后可重建；`phase:'retry'` 与前缀 `spawn` 行同键）
                        dep.wm.wm.writeWatermark(sid, wmNow, agent, { runId, phase: 'retry', attempt: plan.attempt, segKey: chunk.segKey });
                        dep.io.infra.log(`distill: ${dep.io.infra.sidShort(sid)} 段${k + 1}/${segLimit} 落盘失败 ${disp.undigested} 条（第 ${plan.attempt}/${dep.write.write.MAX_DISPATCH_RETRY} 次，${plan.reason}）——水位保留 ${wmNow}，下轮从本段（seq ${chunk.startSeq}）续传`);
                    }
                }
                else {
        ...
        if (anyAdded && candIncluded.length) {
            ...
            try {
                ...
                for (const f of candIncluded) {
                    try {
                        renameSync(join(dep.io.pendDir, f), join(procDir, f));
                    }
                    catch { /* */ }
                }
            }
        ...
        dep.io.infra.log(`distill agent err ${dep.io.infra.sidShort(sid)}: ${String(e?.message || e).slice(0, 120)}`);
    }
    finally {
        dep.wm.st.distilling.delete(sid);
        if (claimed)
            dep.write.write.releaseClaim(sid);
    }
};
const armIdleTimer = (dep, agent) => {
    ...
    const t = setTimeout(() => {
        ...
        distillAgent(dep, agent).catch((e) => dep.io.infra.log(`distill agent err ${dep.io.infra.sidShort(sid)}: ${String(e?.message || e).slice(0, 120)}`));
    }, dep.env.config.idleWakeMs);
    dep.wm.st.idleTimers.set(sid, t);
    // 观测补口（2026-09-14）：此前"武装成功"**零痕迹** ⇒ 外部无法区分「定时器没武装」与「武装了但没到点」，
    //   这次排障只能靠"日志里一条 distill 行都没有"反推。武装必须留一行**正证**，否则同类断链仍不可观测。
    dep.io.infra.log(`distill: ${dep.io.infra.sidShort(sid)} 空闲定时器已武装（${Math.round(Number(dep.env.config.idleWakeMs) / 60000)}min 后到点）`);
};
// 子代理输出 → JSON（剥离代码栅栏 + 容错提取首个 {...}；蒸馏/深度睡眠共用）
const parseAgentJson = (dep, result, label) => {
    ...
    try {
        ...
        if (m) {
            try {
                ...
                dep.io.infra.log(`${label} JSON 解析失败: ${String(e2.message).slice(0, 80)}`);
            }
        }
        else
            dep.io.infra.log(`${label} JSON 解析失败: ${String(e1.message).slice(0, 80)}`);
        return null;
    }
};
// ── 手动蒸馏触发（2026-09-10：pending 回流闭环——参数调节「立即处理 pending」调此）──
const runDistillNow = async (dep) => {
    try {
        ...
        for (const a of roots) {
            try {
                ...
                if (origin === 'subagent')
                    continue;
                if (dep.wm.st.distilling.has(a.id))
                    continue;
                // ★ 册二：手动入口也走**同一准入函数**（唯一差异 = `manual: true` 豁免宽限期，其余约束照旧）
                const base = dep.wm.wm.resolveWatermark(a.id, a);
                ...
                catch { /* 快照不可用 ⇒ maxSeq 保持基线 */ }
                const quiet = quiescenceOf({ ctx: dep.env.ctx, hasActiveSubagents: (x) => dep.parent.parent.hasActiveSubagents(x) }, a);
                const adm = planIngestAdmission({
                    lastSeq, maxSeq,
                    sinceLastEndMs: null,
                    idleWakeMs: dep.env.config.idleWakeMs,
                    quiet: quiet.state,
                    claimHeld: dep.write.write.claimHeld(a.id),
                    snapshotStreak: 0,
                    circuitBreakN: DISCARD_SNAPSHOT_CB_N,
                    manual: true,
                });
                if (!adm.run) {
                    dep.io.infra.log(`distill(manual): ${dep.io.infra.sidShort(a.id)} 跳过（${adm.reason}）`);
                    continue;
                }
                await distillAgent(dep, a).catch(() => { });
                n++;
            }
            catch { /* 单会话跳过 */ }
        }
...
//# sourceMappingURL=distill-agent.js.map