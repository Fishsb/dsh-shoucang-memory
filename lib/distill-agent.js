...
const distillAgent = async (dep, agent) => {
    ...
    if (agent.status && agent.status !== 'idle') {
        dep.io.infra.log(`distill: ${dep.io.infra.sidShort(sid)} 已恢复活跃（status=${agent.status}），跳过`);
        return;
    }
    ...
    try {
        ...
        for (let k = 0; k < segLimit; k++) {
            ...
            try {
                ...
                dep.wm.wm.writeWatermark(sid, wmNow, agent, { runId, phase: 'spawn' });
                ...
                const failReason = fclass === 'dispatch-failed' ? `dispatch failed=${disp.failed} added=${disp.added} target=${disp.targetLib}`
                    : fclass === 'gate-reject' ? `gate rejected=${disp.rejected} target=${disp.targetLib}`
                        : fclass === 'json-parse' ? `json-parse stop=${stop}`
                            : (fclass === 'provider-fail' || fclass === 'agent-stop') ? `agent stop=${stop} llm=${llmLabel}`
                                : '';
                dep.io.infra.audit({ sid, kind: 'distill-run', route, stop, fclass, llm: llmLabel, targetLib: disp.targetLib, added: disp.added, rejected: disp.rejected, failed: disp.failed, chunk: k + 1, chunkStart: chunk.startSeq, chunkEnd: chunk.endSeq, totalChunks: chunks.length, ...(failReason ? { reason: failReason } : {}) });
