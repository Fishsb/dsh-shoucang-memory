...
export function rowFingerprint(row) {
    return fingerprintOf(String(row?.line || ''));
}
export function createSupplyLedger(now = Date.now) {
    const sessions = new Map();
    let marked = 0;
    let swept = 0;
    let calls = 0;
    const sweep = (maxAgeMs = DEFAULT_TTL_MS) => {
    ...
    const tick = () => { calls++; if (calls % SWEEP_EVERY === 0)
        sweep(); };
    const entryOf = (sid, at) => {
        let e = sessions.get(sid);
        if (!e) {
            e = { epoch: 0, rows: new Map(), lastAt: at };
            sessions.set(sid, e);
        }
        return e;
    };
...
//# sourceMappingURL=supply-ledger.js.map