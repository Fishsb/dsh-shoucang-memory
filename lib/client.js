  ...
  window.__ModuleLoader__.load({
    ...
    factory: function (require) {
      ...
      var CSS = [
        ...
        '--sc-w-control:280px;--sc-w-read:72ch;--sc-w-sm:180px;--sc-w-md:200px;--sc-w-lg:300px;--sc-w-xl:320px;--sc-shadow-2:0 2px 12px rgba(0,0,0,.28);',
        ...
        /* v9 认知可视化（睡眠回执 / 材料预估 / 趋势 / 归档区）：两列数值行，无进度条无百分比 */
        '.sc-comp{margin:8px 0 4px;}',
        '.sc-comp-row{display:grid;grid-template-columns:1fr auto;gap:10px;align-items:baseline;margin:3px 0;}',
        '.sc-comp-k{font-size:12px;color:var(--sc-text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}',
        '.sc-comp-v{font-size:11px;color:var(--sc-faint);font-variant-numeric:tabular-nums;text-align:right;}',

        '.sc-comp-row:hover .sc-comp-k{color:var(--sc-accent);}',
        ...
        /* ═══ U2.5b：JS 内联样式 → 类/CSS 变量（样式只在 CSS，JS 只管状态）═══
         * 保留内联的只有两类：**数据驱动的颜色**（标签色相/状态色）与 **CSS 变量赋值**（--sc-indent 动态缩进）。 */
        '.sc-hidden{display:none !important;}',
        '.sc-on{color:var(--sc-accent);}',
        '.sc-inline-note{margin:var(--sc-sp-1) 0 var(--sc-pad-card);font-size:var(--sc-fs-xs);color:var(--sc-accent);}',
        '.sc-col-end{display:flex;flex-direction:column;gap:var(--sc-sp-1);flex:none;align-items:flex-end;}',
        '.sc-row-gap{display:flex;gap:var(--sc-sp-1);flex:none;align-items:center;}',
        '.sc-toolbar{display:flex;gap:var(--sc-gap-row);margin-top:var(--sc-sp-1);}',
        '.sc-box{border:1px solid var(--sc-border);border-radius:var(--sc-r-md);padding:var(--sc-sp-1) var(--sc-sp-2);margin-bottom:var(--sc-sp-1);background:var(--sc-bg1);}',
        '.sc-input.sc-w-md{width:var(--sc-w-md);}',
        '.sc-max-xl{max-width:var(--sc-w-xl);}',
        '.sc-btn-xs{padding:var(--sc-sp-1) var(--sc-sp-3);font-size:var(--sc-fs-xs);flex:none;}',
        '.sc-btn-ok{color:var(--sc-ok,var(--sc-accent));}',
        '.sc-btn-muted{color:var(--sc-muted);}',
        '.sc-card-head{display:flex;align-items:baseline;gap:var(--sc-sp-1);cursor:pointer;padding-left:calc(var(--sc-sp-2) + var(--sc-indent,0));}',
        '.sc-edit-sec{display:inline-block;margin:var(--sc-sp-1) 0 var(--sc-sp-1) calc(var(--sc-sp-2) + var(--sc-indent,0));padding:var(--sc-sp-1) var(--sc-sp-2);font-size:var(--sc-fs-xs);color:var(--sc-accent);}',
        '.sc-ta{width:100%;min-height:calc(var(--sc-sp-6) * 3 + var(--sc-sp-5));font-family:ui-monospace,monospace;font-size:var(--sc-fs-xs);}',
        '.sc-ic-lg{width:calc(var(--sc-sp-6) - var(--sc-sp-2));height:calc(var(--sc-sp-6) - var(--sc-sp-2));display:block;margin:var(--sc-sp-2) auto;pointer-events:none;}',
        '.sc-ic-sm{width:var(--sc-sp-4);height:var(--sc-sp-4);display:block;pointer-events:none;}',
        '.sc-fab{position:fixed;left:var(--sc-sp-4);bottom:var(--sc-sp-4);z-index:9800;width:calc(var(--sc-sp-6) + var(--sc-sp-2));height:calc(var(--sc-sp-6) + var(--sc-sp-2));border-radius:50%;border:none;cursor:pointer;font-size:var(--sc-fs-md);font-weight:600;color:#fff;background:hsl(254deg 80% 68%);box-shadow:var(--sc-shadow-2);}',
        /* ---- UI 重构新增（2026-09-12）：按钮 / 徽标 / 折叠 / 进度 / 日志 / 密度 / 状态分级 ---- */
        '.sc-btn{appearance:none;border:1px solid var(--sc-border);background:var(--sc-bg2);color:var(--sc-text);',
        'border-radius:6px;padding:5px 11px;font-size:12px;cursor:pointer;line-height:1.4;}',
        '.sc-btn:hover:not(:disabled){background:var(--sc-hover);}',
        '.sc-btn:disabled{opacity:.55;cursor:default;}',
        '.sc-btn-primary{background:var(--sc-accent);border-color:var(--sc-accent);color:#fff;}',
        '.sc-btn-danger{color:var(--dsw-alias-state-error,#e05561);border-color:var(--dsw-alias-state-error,#e05561);}',
        '.sc-badge{display:inline-block;padding:1px 7px;border-radius:9px;font-size:10.5px;line-height:16px;',
        'background:var(--sc-hover);color:var(--sc-muted);white-space:nowrap;}',
        '.sc-badge-ok{background:rgba(80,200,120,.16);color:#3fa76a;}',
        '.sc-badge-warn{background:rgba(230,170,60,.18);color:#c98a1e;}',
        '.sc-badge-error{background:rgba(224,85,97,.16);color:#e05561;}',
        /* 可折叠（D2 概览—详情分层） */
        '.sc-fold{border:1px solid var(--sc-border);border-radius:7px;margin:6px 0;background:var(--sc-bg1);}',
        '.sc-fold-head{display:flex;align-items:center;gap:7px;padding:8px 11px;cursor:pointer;',
        'font-size:12.5px;font-weight:600;user-select:none;}',
        '.sc-fold-head:hover{background:var(--sc-hover);}',
        '.sc-fold-arrow{color:var(--sc-muted);width:10px;}',
        '.sc-fold-summary{margin-left:auto;font-weight:400;font-size:11px;color:var(--sc-muted);}',
        '.sc-fold-body{display:none;padding:2px 11px 10px;}',
        '.sc-fold.open .sc-fold-body{display:block;}',
        /* 进度（C1） */
        '.sc-prog{margin:6px 0;}',
        '.sc-prog-bar{height:5px;border-radius:3px;background:var(--sc-hover);overflow:hidden;}',
        '.sc-prog-fill{height:100%;width:0;border-radius:3px;background:var(--sc-accent);transition:width .25s ease;}',
        '.sc-prog-txt{font-size:11px;color:var(--sc-muted);margin-top:3px;}',
        /* 键值（信息密度） */
        '.sc-kv{display:grid;grid-template-columns:auto 1fr;gap:2px 12px;font-size:11.5px;}',
        '.sc-kv-k{color:var(--sc-muted);white-space:nowrap;}',
        '.sc-kv-v{color:var(--sc-text);word-break:break-all;}',
        /* 日志面板（C2） */
        '.sc-logwrap{border-top:1px solid var(--sc-border);background:var(--sc-bg2);max-height:150px;overflow:auto;}',
        '.sc-log-head{display:flex;align-items:center;gap:8px;padding:5px 12px;font-size:10.5px;',
        'text-transform:uppercase;letter-spacing:.06em;color:var(--sc-faint);position:sticky;top:0;background:var(--sc-bg2);}',
        '.sc-log-row{display:flex;gap:8px;padding:2px 12px;font-size:11px;font-family:var(--font-monospace,monospace);}',
        '.sc-log-t{color:var(--sc-faint);flex:none;}',
        '.sc-log-lv{flex:none;width:38px;text-transform:uppercase;font-size:10px;}',
        '.sc-log-info .sc-log-lv{color:var(--sc-muted);}',
        '.sc-log-warn .sc-log-lv{color:#c98a1e;}',
        '.sc-log-error .sc-log-lv{color:#e05561;}',
        '.sc-log-msg{color:var(--sc-text);word-break:break-word;}',
        /* 状态分级（D1） */
        '.sc-status-info{color:var(--sc-muted);}',
        '.sc-status-warn{color:#c98a1e;}',
        '.sc-status-error{color:#e05561;font-weight:600;}',
        /* 密度（D3） */
        '.sc-density-compact .setting-item{padding:6px 0;}',
        '.sc-density-compact .setting-item-desc{display:none;}',
        '.sc-density-compact .sc-fold-head{padding:5px 11px;}',
        '.sc-density-compact .sc-kv{font-size:10.5px;}',

        /* ══════════ 2026-09-12 样式系统优化（纯新增，不改动既有规则） ══════════
         * 目标：① 补齐缺失设计令牌 ② 统一交互反馈与边界态 ③ 响应式适配 ④ 视觉层次。
         * 原则：只增不改——既有选择器一律不动，避免布局回退；新令牌供后续渐进替换硬编码。
         */

        /* ── ① 设计令牌补充（阴影 / 过渡 / 焦点环 / 禁用 / 层级 / 字重） ── */
        '#scpanl-root{',
        '--sc-shadow-1:0 1px 3px rgba(0,0,0,.18);--sc-shadow-2:0 2px 12px rgba(0,0,0,.28);--sc-shadow-3:0 8px 32px rgba(0,0,0,.40);',
        '--sc-t-fast:.12s;--sc-t-base:.18s;--sc-t-slow:.28s;--sc-ease:cubic-bezier(.4,0,.2,1);',
        '--sc-ring:0 0 0 2px var(--sc-accent);--sc-ring-offset:2px;',
        '--sc-disabled-op:.45;--sc-readonly-op:.7;',
        '--sc-z-modal:1000;--sc-z-mask:999;--sc-z-toast:1001;',
        '--sc-fw-normal:400;--sc-fw-medium:500;--sc-fw-semibold:600;--sc-fw-bold:700;',
        '--sc-ls-tight:-.01em;--sc-ls-wide:.06em;',
        /* 状态色统一派生（避免各处直接写 rgba 十六进制 —— 实测原有 16 种十六进制散落） */
        '--sc-ok-bg:rgba(63,185,80,.14);--sc-warn-bg:rgba(232,163,61,.16);--sc-err-bg:rgba(229,83,75,.15);',
        '--sc-ok-fg:var(--sc-ok);--sc-warn-fg:var(--sc-warn);--sc-err-fg:var(--sc-err);',
        '}',

        /* ── ② 交互反馈：focus-visible（键盘可达，鼠标不显环） ── */
        '#scpanl-root :focus{outline:none;}',
        '#scpanl-root :focus-visible{outline:none;box-shadow:var(--sc-ring);border-radius:var(--sc-r-sm);}',
        '.sc-trigger:focus-visible{box-shadow:var(--sc-ring);}',
        '.sc-btn:focus-visible,.sc-fold-head:focus-visible,.sc-nav-item:focus-visible{box-shadow:var(--sc-ring);}',
        ...
        '#scpanl-root .sc-btn:disabled:hover{background:var(--sc-bg2);}',
        '#scpanl-root [aria-disabled="true"]{opacity:var(--sc-disabled-op);pointer-events:none;}',

        /* 过渡统一：默认开启，尊重系统「减少动效」 */
        '#scpanl-root .sc-btn,#scpanl-root .sc-nav-item,#scpanl-root .sc-fold-head,#scpanl-root .checkbox-container{',
        'transition:background var(--sc-t-base) var(--sc-ease),color var(--sc-t-base) var(--sc-ease),',
        'border-color var(--sc-t-base) var(--sc-ease),box-shadow var(--sc-t-fast) var(--sc-ease),transform var(--sc-t-fast) var(--sc-ease);}',
        '@media (prefers-reduced-motion:reduce){#scpanl-root *{transition-duration:.01ms !important;animation-duration:.01ms !important;}}',

        /* ── ③ 边界态：加载 / 空 / 溢出 ── */
        /* 加载：骨架微光 + 行内 spinner */
        '.sc-loading{position:relative;pointer-events:none;}',
        '.sc-loading::after{content:"";position:absolute;inset:0;border-radius:inherit;',
        ...
        '.sc-spinner{display:inline-block;width:13px;height:13px;flex:none;vertical-align:-2px;',
        'border:2px solid var(--sc-border);border-top-color:var(--sc-accent);border-radius:50%;animation:sc-spin .7s linear infinite;}',
        '@keyframes sc-spin{to{transform:rotate(360deg)}}',

        /* 空态：居中 + 图标位 + 说明 */
        '.sc-empty{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:var(--sc-sp-2);',
        'padding:var(--sc-sp-6) var(--sc-sp-4);color:var(--sc-muted);text-align:center;}',
        '.sc-empty-icon{opacity:.5;}',
        '.sc-empty-title{font-size:var(--sc-fs-md);font-weight:var(--sc-fw-semibold);color:var(--sc-text);}',
        '.sc-empty-desc{font-size:var(--sc-fs-sm);color:var(--sc-muted);max-width:42ch;line-height:var(--sc-lh-normal);}',

        /* 溢出：文本超长省略 + 多点截断；容器可控滚动 */
        '.sc-ellipsis{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0;}',
        '.sc-clamp-2{display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;}',
        '.sc-clamp-3{display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden;}',
        '.sc-scroll-y{overflow-y:auto;overscroll-behavior:contain;}',
        '.sc-break{overflow-wrap:anywhere;word-break:break-word;}',

        /* ── ④ 视觉层次：主次按钮 / 状态徽标 / 分区标题统一 ── */
        '#scpanl-root .sc-btn{min-height:30px;display:inline-flex;align-items:center;justify-content:center;gap:6px;}',
        '#scpanl-root .sc-btn-primary:hover:not(:disabled){background:var(--sc-accent-hover);}',
        '#scpanl-root .sc-btn-danger:hover:not(:disabled){background:var(--sc-err-bg);}',
        '.sc-badge-ok{background:var(--sc-ok-bg);color:var(--sc-ok-fg);}',
        '.sc-badge-warn{background:var(--sc-warn-bg);color:var(--sc-warn-fg);}',
        '.sc-badge-error{background:var(--sc-err-bg);color:var(--sc-err-fg);}',
        '.sc-badge-info{background:var(--sc-hover);color:var(--sc-muted);}',
        /* 分区节奏：相邻分区之间留白一致（留白即分组） */
        '#scpanl-root .sc-section{margin-bottom:var(--sc-gap-group);}',
        '#scpanl-root .sc-section:last-child{margin-bottom:0;}',
        '#scpanl-root .sc-h1{font-size:var(--sc-fs-xl);font-weight:var(--sc-fw-bold);letter-spacing:var(--sc-ls-tight);}',
        '#scpanl-root .sc-h2{font-size:var(--sc-fs-lg);font-weight:var(--sc-fw-semibold);}',

        /* ── ⑤ 响应式：移动 / 平板 / 大屏（原 0 处 @media） ── */
        /* 中小屏（≤900px）：导航收窄、内容区占满 */
        '@media (max-width:900px){',
        '#scpanl-modal{--sc-nav-w:160px;}',
        '#scpanl-root .sc-nav{width:var(--sc-nav-w) !important;padding:10px 6px;}',
        '#scpanl-root .sc-nav-item{padding:8px 8px;gap:7px;}',
        '#scpanl-root .sc-main{padding:0;}',
        '}',
        /* 小屏（≤640px）：导航转顶部横向条，模态全屏，避免左右挤压 */
        '@media (max-width:640px){',
        '#scpanl-modal{flex-direction:column !important;width:100vw !important;height:100vh !important;',
        'max-width:100vw !important;max-height:100vh !important;border-radius:0 !important;}',
        '#scpanl-root .sc-nav{width:100% !important;flex-direction:row !important;overflow-x:auto;',
        'border-right:none !important;border-bottom:1px solid var(--sc-border);padding:8px;gap:4px;}',
        '#scpanl-root .sc-nav-title,#scpanl-root .sc-nav-group,#scpanl-root .sc-nav-spacer{display:none !important;}',
        '#scpanl-root .sc-nav-item{flex:none;white-space:nowrap;border-radius:var(--sc-r-pill);padding:6px 12px;}',
        '#scpanl-root .sc-view{padding:var(--sc-sp-4) var(--sc-sp-3) !important;}',
        '#scpanl-root .setting-item{flex-direction:column;align-items:stretch !important;gap:var(--sc-sp-2);}',
        '#scpanl-root .setting-item-control{width:100%;justify-content:flex-start;}',
        '#scpanl-root .sc-kv{grid-template-columns:1fr;gap:0;}',
        '#scpanl-root .sc-kv-k{padding-top:4px;}',
        '#scpanl-root textarea{font-size:13px;}',
        '}',
        /* 大屏（≥1440px）：放宽阅读宽度，避免内容被拉得太长 */
        '@media (min-width:1440px){',
        '#scpanl-modal{--sc-nav-w:232px;}',
        '#scpanl-root .sc-view{max-width:1100px;}',
        '}',
        /* 触摸设备：加大点击热区 */
        '@media (hover:none){',
        '#scpanl-root .sc-nav-item{padding:10px;}',
      ...
      var UI = {
        ...
        toggle: function (checked, onChange, label) {
          var wrap = el('label', 'checkbox-container');
          var cb = document.createElement('input');
          cb.type = 'checkbox'; cb.checked = !!checked;
          if (label) cb.setAttribute('aria-label', label);
          cb.onchange = function () { onChange(cb.checked); };
          var mk = el('span', 'checkbox-material');
          wrap.appendChild(cb); wrap.appendChild(mk);
          return wrap;
        },
        /* 文本/数字输入 */
        input: function (value, onChange, opts) {
          ...
          i.style.maxWidth = o.width || '220px';
        ...
        collapsible: function (title, bodyNode, opts) {
          var o = opts || {};
          var box = el('div', 'sc-fold' + (o.open ? ' open' : ''));
          var head = el('div', 'sc-fold-head');
          var arrow = el('span', 'sc-fold-arrow', o.open ? '▾' : '▸');
          head.appendChild(arrow);
          head.appendChild(el('span', null, title));
          if (o.summary) head.appendChild(el('span', 'sc-fold-summary', o.summary));
          box.appendChild(head);
          var body = el('div', 'sc-fold-body');
        ...
        progress: function (id) {
          ...
          function render(p) {
            var s = (p || {})[id];
            if (!s || !s.on) { box.style.display = 'none'; return; }
            box.style.display = '';
      ...
      function scheduleFold(host, mark, label, openDefault) {
        ...
        if (!open) body.classList.add('sc-hidden');
      ...
      function renderViewToggles(view, parsed, global) {
        ...
        (function () {
          ...
          q.oninput = function () {
            ...
            items.forEach(function (it) {
              ...
              it.classList.toggle('sc-hidden', !show);
              if (show) hit++;
            });
        ...
        function refreshVecZone() {
          api('/vector/status2').then(function (s2) {
            ...
            urlInfo.appendChild(el('div', 'setting-item-desc', '如 http://127.0.0.1:11434/v1（Ollama）或 http://127.0.0.1:9915/v1（自建桥）；改完回车自动探测。'));
          ...
          if (autoSection) {
            ...
            heads.forEach(function (h) {
              if (h.textContent.indexOf(autoSection) !== -1) {
                ...
                if (body && body.classList.contains('sc-card-body') && body.classList.contains('sc-hidden')) h.click();
              }
            });
          }
        }).catch(fail);
      ...
      function renderCognitionReport(view, mode) {
        api('/cognition/report').then(function (r) {
          ...
          var sleeps = r.sleeps || [], last = sleeps.length ? sleeps[sleeps.length - 1] : null;
          if (mode === 'sleep') {
      ...
      function renderMemoryExpanded(view, data) {
        ...
        if (data.vector && data.vector.enabled !== false) {
          ...
          bVec.appendChild(el('span', null, '向量 ' + (vp === 'fusion' ? '融合' : vp === 'gpu-ready' ? '本机就绪' : vp === 'lexical' ? '词法' : vp === 'cloud' ? '云端' : vp === 'unreachable' ? '服务未连' : '关')));
        ...
        if (memoryFile) g2.appendChild(mkStat('知识索引', String((memoryFile.lines || []).length) + ' 行', 'MEMORY.md'));
      ...
      function renderNoteSections(view, data) {
        ...
        function renderSecNode(sec, depth, container) {
          ...
          body.classList.add('sc-hidden');
          body.textContent = sec.body || (sec.children && sec.children.length ? '' : '（空小节）');
          var editSec = el('button', 'sc-btn subtle sc-edit-sec sc-hidden', '✎ 编辑此小节');
          editSec.type = 'button';
          if (pad) editSec.style.setProperty('--sc-indent', pad + 'px');
          editSec.addEventListener('click', function () { editNoteSection(data, sec, view); });
          var kidsWrap = el('div', 'sc-hidden');
          var toggle = function () {
            var open = !body.classList.contains('sc-hidden') || !kidsWrap.classList.contains('sc-hidden');
            if (open) { body.classList.add('sc-hidden'); kidsWrap.classList.add('sc-hidden'); editSec.classList.add('sc-hidden'); }
            else {
              body.classList.remove('sc-hidden');
              editSec.classList.remove('sc-hidden');
              if (sec.children && sec.children.length) kidsWrap.classList.remove('sc-hidden');
            }
            arrow.textContent = open ? '▸' : '▾';
      ...
      function renderViewObserve(view) {
        view.textContent = '';
        view.appendChild(el('div', 'sc-h1', '运行观测'));
        view.appendChild(el('div', 'sc-desc', '执行进度、调用日志、错误定位与关键指标。日志保留最近 500 条，错误带端点/参数/堆栈，便于定位而非只剩一行提示。'));

        // 1) 进度
        var pbox = el('div');
        var p = Store.get('progress') || {};
        var ids = Object.keys(p);
        if (!ids.length) pbox.appendChild(el('div', 'sc-desc', '当前无进行中的任务。'));
        ...
        var ebox = el('div');
        if (!errs.length) ebox.appendChild(el('div', 'sc-desc', '暂无错误。'));
        else {
          var list = el('div');
        ...
        if (!mk.length) mbox.appendChild(el('div', 'sc-desc', '暂无指标（切换各视图会自动采集）。'));
        else mbox.appendChild(UI.kv(mk.map(function (k) { return [k, String(m[k])]; })));
        view.appendChild(UI.collapsible('关键指标', mbox, { open: true }));

        // 4) 日志（C2）
        view.appendChild(UI.collapsible('调用日志', buildLogPanel(400), { open: true, summary: (Store.get('logs') || []).length + ' 条' }));
          ...
          UI.button('测试连接', function () {
            ...
            return api('/embed/config').then(function (c) {
              ...
              return apiCtx('/embed/test', {
                ...
                var n = (r && r.models && r.models.length) || 0;
      ...
      function buildLogPanel(maxH) {
        ...
        var spacer = el('span'); spacer.style.marginLeft = 'auto'; head.appendChild(spacer);
        ...
        function render() {
          ...
          body.textContent = '';
          if (!all.length) { body.appendChild(el('div', 'sc-log-row', '（无）')); return; }
      ...
      function renderViewSettings(view) {
        view.textContent = '';
        view.appendChild(el('div', 'sc-h1', '界面设置'));
        view.appendChild(el('div', 'sc-desc', '把此前写死的行为参数化：显示密度、导航宽度、自动刷新与轮询频率、日志与概览方式。设置保存在浏览器 localStorage，立即生效。'));
        ...
        look.appendChild(UI.item('导航宽度', '左侧导航像素宽度（140–320）。',
          UI.input(Cfg.get('navWidth', 200), function (v) {
      ...
      function renderSuite(view, data) {
        ...
        members.forEach(function (m) {
          ...
          var badgeText = m.status === 'both' ? '✓✓ 双基准' : m.status === 'injected' ? '✓ 注入器' : m.status === 'profile' ? '✓ profile' : '✗ 未装配';
          var badgeKind = m.status === 'both' ? 'ok' : m.status === 'missing' ? 'error' : 'info';
        ...
        api('/deepsleep').then(function (r) {
          ...
          api('/selfcheck').then(function (s) {
            ...
            if (!s || !s.active) {
              ...
              scBox.appendChild(scCard('白名单调整', ((s.adjustments || []).length ? (s.adjustments || []).map(function (a) { return a.id; }).join(' · ') : '无'), '仅窄动作且可回滚；改 α/gate/判据 一律只建议'));
            }
          ...
          advBody.classList.add('sc-hidden');
          advBtn.onclick = function () {
            advOpen = !advOpen;
            advBody.classList.toggle('sc-hidden', !advOpen);
      ...
      function applyNavWidth() {
        var n = document.querySelector('.sc-nav');
        if (n) n.style.width = Cfg.get('navWidth', 200) + 'px';
      }
