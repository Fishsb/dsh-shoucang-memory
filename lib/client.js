  ...
  window.__ModuleLoader__.load({
    ...
    factory: function (require) {
      ...
      function renderViewToggles(view, parsed, global) {
        ...
        // 2026-09-10 用户拍板：上限默认=容量门硬边界；实际量=动态参考（随内容成长），二者区分展示
        var actualChars = (g && g.actual) || { agent: 0, user: 0, memory: 0 };
        view.appendChild(numSetting('agent 画像上限 injection.agent_max_chars', 'AGENT.md（含 [原则]/[路径]）注入字符上限；当前实际 ' + (actualChars.agent || 0) + ' 字符（上限默认 3000=容量门全量，设小裁切）——全局 scheduler.json', gVal('agent_max_chars', 3000), 'injection.agent_max_chars', '字符'));
        view.appendChild(numSetting('用户画像上限 injection.user_max_chars', 'USER.md 注入字符上限；当前实际 ' + (actualChars.user || 0) + ' 字符（上限默认 2000=容量门全量，设小裁切）——全局 scheduler.json', gVal('user_max_chars', 2000), 'injection.user_max_chars', '字符'));
        view.appendChild(numSetting('知识索引上限 injection.memory_max_chars', 'MEMORY.md 注入字符上限（档位行数后二次裁切）；当前实际 ' + (actualChars.memory || 0) + ' 字符（上限默认 3000=容量门全量，设小裁切）——全局 scheduler.json', gVal('memory_max_chars', 3000), 'injection.memory_max_chars', '字符'));
