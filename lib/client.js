    ...
    factory: function (require) {
      ...
      var Log = (function () {
        ...
        function add(level, msg, ctx) {
          ...
          Store.set('logs', a);
          Bus.emit('log', e);
          if (level === 'error') status(msg, 'error');
          return e;
        }
        return {
          add: add,
      ...
      function status(msg, level) {
        ...
        var lv = level || 'info';
        n.textContent = (msg || '') + '';
        n.className = 'sc-statusbar sc-status-' + lv;
      ...
      function fail(e, ctx) {
        ...
        status(msg, 'error');
        var rec = { t: Date.now(), message: err.message, ctx: ctx || null, stack: (err.stack || '').split('\n').slice(0, 4).join(' | ') };
        var a = (Store.get('errors') || []).concat([rec]);
        Store.set('errors', a.slice(-100));
        Log.error(err.message, { where: ctx || null, stack: rec.stack });
        return rec;
      }
