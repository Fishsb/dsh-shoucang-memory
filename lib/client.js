  ...
  window.__ModuleLoader__.load({
    ...
    factory: function (require) {
      ...
      function renderViewToggles(view, parsed, global) {
        ...
        // 2026-09-10 用户拍板：总预算 max_tokens 退役（注入=双画像+记忆指针薄行，无需总预算）；
        // 板块上限默认=当前实际文件量（全量语义），设小于实际才裁切
        var actualChars = (g && g.actual) || { agent: 0, user: 0, memory: 0 };
        view.appendChild(numSetting('agent 画像上限 injection.agent_max_chars', 'AGENT.md（含 [原则] 习得原则与 [路径]）注入字符上限；默认=当前实际 ' + (actualChars.agent || 0) + ' 字符（全量），设更小才逐行裁切——全局 scheduler.json', gVal('agent_max_chars', 0), 'injection.agent_max_chars', '字符'));
        view.appendChild(numSetting('用户画像上限 injection.user_max_chars', 'USER.md 注入字符上限；默认=当前实际 ' + (actualChars.user || 0) + ' 字符（全量），设更小才裁切——全局 scheduler.json', gVal('user_max_chars', 0), 'injection.user_max_chars', '字符'));
        view.appendChild(numSetting('知识索引上限 injection.memory_max_chars', 'MEMORY.md 注入字符上限（档位行数后二次裁切）；默认=当前实际 ' + (actualChars.memory || 0) + ' 字符（全量），设更小才裁切——全局 scheduler.json', gVal('memory_max_chars', 0), 'injection.memory_max_chars', '字符'));
