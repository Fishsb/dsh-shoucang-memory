...
export function stableStashOf(memRoot, capStable, playbookOn, key, agentLines, userLines) {
    return {
 ...
 */
export function stableLinesOf(s, now, cache) {
    ...
    const sl = s.playbookOn ? [head, ...MEMORY_PLAYBOOK_LINES, ...a2] : [head, ...a2];
    if (dA.length > 0)
...
export function stableTextOf(s, now, cache) {
    return stableLinesOf(s, now, cache).join('\n');
}
//# sourceMappingURL=hot-stable.js.map