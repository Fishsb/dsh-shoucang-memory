...
import { withBankLock } from './bank-lock.js';
let TMP_SEQ = 0;
/**
 ...
 */
export function atomicWriteFileCas(p, text, expected) {
    let current = null;
    try {
    ...
    if (expected === undefined) {
        if (current !== null)
            return { ok: false, conflict: true, error: 'CAS 冲突：文件已被他人创建' };
    }
...
export function atomicWriteFile(p, text) {
    ...
    try {
        ...
        if (got !== want)
            return { ok: false, error: `回读字节不符：${got} ≠ ${want}` };
        ...
        if (gotText !== text)
            return { ok: false, error: '回读内容不符（长度相同但内容不同 ⇒ 可能被并发覆盖）' };
    }
    catch (e) {
 ...
 * 门禁脚本本身只读，不会与锁冲突。
 */
export async function gatedWriteFile(p, text, gate) {
    const tmp = `${p}.tmp-${process.pid}-${TMP_SEQ++}-${Math.random().toString(36).slice(2, 6)}`;
    try {
        writeFileSync(tmp, text, 'utf8');
    }
    ...
    const want = Buffer.byteLength(text, 'utf8');
    try {
        if (statSync(p).size !== want)
            return { ok: false, gate: '回读不符', error: `字节 ${statSync(p).size} ≠ ${want}` };
        // ADR-333 册三：与 `atomicWriteFile` **同批**补内容校验（孪生形态，不同批会造成两套"可验"语义）。
        if (readFileSync(p, 'utf8') !== text)
            return { ok: false, gate: '回读不符', error: '内容不符（长度相同但内容不同 ⇒ 可能被并发覆盖）' };
    }
    catch (e) {
...
export function appendFileUnderLock(bankRoot, absPath, text, opts = {}) {
    ...
    if (r.refused)
        return { ok: false, error: '库锁被占用（拒写）' };
    if (!r.ok)
        return { ok: false, error: String(r.error || '追加失败') };
 ...
 */
export function editFileUnderLock(bankRoot, absPath, mutate, opts = {}) {
    let changed = false;
    ...
    if (r.refused)
        return { ok: false, refused: true, changed, error: '库锁被占用（拒写）' };
    if (!r.ok)
        return { ok: false, changed, error: String(r.error || '改写失败') };
    changed = !!r.value?.changed;
    return { ok: true, changed };
}
//# sourceMappingURL=section-rewrite.js.map