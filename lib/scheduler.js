...
function readMigrationHints(limit = 20) {
    ...
    try {
        ...
        for (let i = lines.length - 1; i >= 0 && hits.length < limit; i--) {
            const line = lines[i];
            if (!line.includes('迁移调度提示'))
                continue;
            const at = (line.match(/^\[([^\]]+)\]/) || [])[1] || '';
            hits.push({ at, line });
        }
        for (const h of hits.reverse())
            out.push({ at: h.at, text: h.line.slice(0, 300) });
    }
    catch {
        // 日志不可读 → 空（只读工具如实报）
    }
...
function readPendingBacklog(project) {
    try {
...
export function applyScheduler(ctx, config) {
    ...
    ctx.effect(() => {
        if (!config.migrate_enabled)
            return () => { };
        return ctx.tools.register(defineTool({
            name: 'shoucang_migrate',
            description: '守藏 migrationHint 消费（只读）：读记忆插件日志（$DSH_HOME/super-injector/dsh-managing-memory.log）的「迁移调度提示」行，聚合近期提示 + 目标项目 pending 积压（devref/pending），并给出 devref-card 派发命令（目标=参数 project 或 config default_project；写卡由 pmg devref-card 执行，本工具不写）。',
            parameters: {
            ...
            output: {
                ...
                render: (_a, v) => [
                    {
                        type: 'text',
                        text: `migrationHint 消费：近 24h 提示 ${v.hint_count_24h} 条（共 ${v.hints.length} 条）；目标 ${v.target_project || '(未配置)'} pending ${v.backlog} 张（阈值 ${v.backlog_threshold_met ? '已过' : '未过'}）` +
    ...
    ctx.effect(() => ctx.tools.register(defineTool({
        ...
        description: '守藏蒸馏目标层自测（ADR-0002 阶段1）：组合矩阵 4 行路由解析（memory/project × 成员在缺）+ 现网实测落点 + 白名单门禁抽样（各库 whitelist.json 自治，不符合不存）。只读。',
        parameters: {},
    ...
    if (config.enableDistill) {
        const governancePkg = config.members.find((m) => m.id === 'governance')?.package || '@dsh-external/project-map-governance';
        registerDistill(ctx, {
            nodeBin: 'node',
