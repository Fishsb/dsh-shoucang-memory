...
export function createActApi(dep) {
    return {
        activationStep: (...a) => activationStep(dep, ...a),
    };
}
...
const activationStep = async (dep, sid, event) => {
    try {
        if (!event)
            return;
        const d = event.data || {};
        ...
        if (!isRealUserEvent(event))
            return;
        if (!srcKind && isNoiseIntent(text.trim()))
            return;
        ...
        if (actSt.cooldown > 0)
            actSt.cooldown--;
        if (sim >= dep.actConf.on && actSt.cooldown === 0 && actSt.state === 'idle') {
            actSt.state = 'prefetch';
            emit = true;
            actSt.cooldown = dep.actConf.cooldown;
        }
        else if (sim < dep.actConf.off && actSt.state !== 'idle') {
            actSt.state = 'idle';
        }
        actSt.prevScore = sim;
...
//# sourceMappingURL=distill-activation.js.map