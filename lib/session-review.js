...
export function decideReview(d, i) {
    ...
    if (!due)
        return { state: 'not-triggered', reason: idleOk ? 'idle' : `空闲不足（<${Math.round(REVIEW_DEFAULTS.idleMs / 60000)}min）` };
    const contentOk = Number(i.newEntries || 0) >= REVIEW_DEFAULTS.minNewEntries || Number(i.newEvents || 0) >= REVIEW_DEFAULTS.minNewEvents;
    if (!contentOk)
        return { state: 'skipped-by-threshold', reason: `内容维不足（新增条目 ${i.newEntries} / 事件 ${i.newEvents}）` };
