...
const cacheKey = (file, hash, model) => file + '\u0000' + hash + '\u0000' + model;
...
function saveLine(file, line, hash, vec, model) {
    try {
        const f = CACHE_FILE();
        ...
        if (need.length) {
            ...
            if (vecs && vecs.length === batch.length) {
                for (let i = 0; i < batch.length; i++) {
                    const v = vecs[i];
                    if (v && v.length) {
                        saveLine(batch[i].file, batch[i].line, lineHash(batch[i].line), v, mdl);
