...
import { egressAllowed, isLoopbackUrl } from './eval-config.js';
/** 行级准入：只收已提炼断言形态（空文本/自由长文一律拒） */
export function admitStateRow(r) {
    const text = String(r?.text || '').trim();
    const tag = String(r?.tag || '').trim();
    if (!text)
        return { ok: false, why: 'empty' };
    if (!tag)
        return { ok: false, why: 'no-tag' };
    if (text.length > 400)
        return { ok: false, why: 'too-long' }; // 断言应短；长文属"原料"不是断言
 ...
 */
export function buildEvalState(rows, cap = 2000) {
    const kept = [];
    let rejected = 0;
    let chars = 0;
    for (const r of rows) {
...
export function parseEvalAnswers(raw, qs) {
    ...
    for (const [id, q] of Object.entries(qs)) {
        ...
        if (!a || typeof a !== 'object')
            return { ok: false, why: `missing:${id}` };
        ...
        if (q.type === 'choice') {
            ...
            if (!Object.prototype.hasOwnProperty.call(q.criteria, v))
                return { ok: false, why: `type-violation:${id}` };
            ...
            if (!Number.isFinite(v) || v < 0 || v > Math.max(1, q.criteria.length - 1))
                return { ok: false, why: `type-violation:${id}` };
            out[id] = { value: v, confidence };
        }
    }
...
function nativePayloadOf(model, prompt) {
    return {
        model,
...
function postJson(urlStr, payload, headers, timeoutMs) {
...
const chatUrlOf = (base) => String(base || '').replace(/\/+$/, '') + '/chat/completions';
/**
 ...
 */
export async function evaluate(d, state, questions, timeoutMs) {
    const t0 = Date.now();
    const at = (o, why, answers) => ({ outcome: o, why, answers, latencyMs: Date.now() - t0 });
    if (!d.cfg.evalEnabled)
        return at('off', 'disabled');
    if (!d.cfg.evalBaseUrl)
        return at('off', 'endpoint-empty');
    if (!Object.keys(questions).length)
        return at('bad-body', 'no-questions');
    if (!String(state || '').trim())
        return at('bad-body', 'empty-state');
    ...
    if (!gate.ok)
        return at('egress-denied', gate.why);
    ...
    if (!loop && !key)
        return at('key-missing', String(d.cfg.evalApiKeyEnv));
    ...
    if (r.status === 429)
        return at('unreachable', 'rate-limited:429'); // ⚠ 暂并入 unreachable，why 里单列
    if (r.status !== 200)
        return at('unreachable', `http:${r.status}`);
    let content = '';
    try {
        ...
        return at('bad-body', 'resp-not-json');
    }
    if (!content)
        return at('bad-body', 'empty-content');
...
export async function probeEndpoint(d, timeoutMs) {
...
//# sourceMappingURL=eval-channel.js.map