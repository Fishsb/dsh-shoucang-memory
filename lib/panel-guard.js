 ...
 */
export function judgePanelRequest(facts) {
    const headers = facts?.headers ?? {};
    const host = headerOf(headers, 'host');
    // ① Host 缺失 ⇒ 拒（Host 是 rebinding 唯一无法伪造的头，缺失即不可裁决）
    if (!host)
        return { trusted: false, reason: 'missing-host' };
    const hostName = hostnameOf(host);
    if (!hostName)
        return { trusted: false, reason: 'malformed-host' };
    // ② Host 非 loopback ⇒ 拒（DNS rebinding 的典型形态）
    if (!isLoopbackHostname(hostName))
        return { trusted: false, reason: 'non-loopback-host' };
    // ③ 显式跨站标记 ⇒ 拒
    ...
    if (originHost !== host.toLowerCase())
        return { trusted: false, reason: 'cross-origin' };
    return { trusted: true };
}
