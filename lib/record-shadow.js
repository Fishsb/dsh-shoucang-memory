...
const EMPTY_STATS = { writes: 0, verified: 0, diverged: 0, lastAt: '', lastFile: '', lastDivergedAt: '', lastDivergedFile: '' };
/** 计数落点路径 */
export function shadowStatsPath(root) {
    return join(root, RECORD_DIR, SHADOW_STATS_FILE);
}
/** 读计数（缺席/损坏 ⇒ 全零，**不抛**） */
export function readShadowStats(root) {
    try {
 ...
 */
export function bumpShadowStats(root, at, file, roundTrip) {
    try {
...
export function recordStorePath(root) {
...
export function loadStore(root) {
    ...
    if (!existsSync(f))
        return { records: [] };
    try {
        return { records: parseRecordStore(readFileSync(f, 'utf8')) };
    }
    catch (e) {
        return { records: [], error: String(e.message).slice(0, 120) };
    }
}
/** 落盘记录集（环记录用：`file=''` 的记录不会被 mirrorFile 的按文件替换碰到） */
export function saveStoreRecords(root, records) {
    saveStore(root, records);
}
function saveStore(root, records) {
    const dir = join(root, RECORD_DIR);
    mkdirSync(dir, { recursive: true });
    const f = recordStorePath(root);
...
export function parseMdFile(root, file, at) {
 ...
 */
export function carrierFiles(root) {
    const out = [...INDEX_FILES];
    try {
 ...
 */
export function mirrorFile(root, file, at) {
    try {
        const parsed = parseMdFile(root, file, at);
        if (!parsed)
            return { file, ok: false, records: 0, bytes: 0, roundTrip: false, error: 'md 缺席' };
        const roundTrip = renderFile(parsed.records, file) === parsed.raw;
        const { records: existing, error } = loadStore(root);
        if (error)
            return { file, ok: false, records: 0, bytes: parsed.raw.length, roundTrip, error: `影子库损坏：${error}` };
        ...
        return { file, ok: false, records: 0, bytes: 0, roundTrip: false, error: String(e.message).slice(0, 120) };
    }
}
/** 镜像全部三索引（一次性迁移 / 对账 / 自检用） */
export function mirrorAll(root, at, files = INDEX_FILES) {
    const out = [];
    for (const f of files)
        if (existsSync(join(root, f)))
            out.push(mirrorFile(root, f, at));
    return out;
}
/**
 ...
 */
export function mirrorFailuresOf(res) {
    return res.filter((r) => !r.ok || !r.roundTrip).map((r) => `${r.file}: ${r.error || '往返不一致'}`);
}
/**
 * 对账：影子库投影 ⟷ md 原文（**双写期唯一判据**，§10「同一 Record 集导出 md 必须逐字节重现」）。
 * 只读，不写任何文件。
 */
export function parityOf(root, files = INDEX_FILES) {
    ...
    return files.map((f) => {
        ...
        if (error)
            return { file: f, present, ok: false, mdBytes: 0, storeBytes: 0, reason: `影子库损坏：${error}` };
        if (!present)
            return { file: f, present: false, ok: true, mdBytes: 0, storeBytes: 0, reason: 'md 缺席（跳过）' };
        const raw = readFileSync(abs, 'utf8');
...
export function shadowInventory(root) {
    const { records, error } = loadStore(root);
    const inv = inventoryOf(records);
    return error ? { ...inv, error } : inv;
}
//# sourceMappingURL=record-shadow.js.map