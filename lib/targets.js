...
export function memoryChain(memoryMemberPresent) {
...
export function projectChain(governanceMemberPresent) {
...
export function resolveTarget(route, members) {
...
export const BUILTIN_WHITELISTS = {
...
export function loadWhitelist(root, library) {
    try {
...
export function gateProjectCard(pc, wl) {
    const c = String(pc.cardType || '').trim();
    if (!wl.cardTypes.includes(c))
    ...
    if (boardKeys.length > 0) {
        ...
        if (!boardKeys.includes(b))
            return { ok: false, reason: `白名单不符: 板块=${b} 不在 ${wl.library} 收录范围（boards=[${boardKeys.join(',')}]，越界知识宁弃不存）` };
    }
    return { ok: true };
}
