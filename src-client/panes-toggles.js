...
function renderViewToggles(view, parsed, global) {
  ...
  var pT1 = el('div'); var pT2 = el('div'); var pT3 = el('div'); var pT4 = el('div'); var _tb = UI.tabs("toggles", [{ id: 'inject', label: tr("① 注入与画像"), pane: pT1 }, { id: 'cap', label: tr("② 记忆与容量"), pane: pT2 }, { id: 'model', label: tr("③ 模型与向量"), pane: pT3 }, { id: 'sched', label: tr("④ 后台与调度"), pane: pT4 }]);
...
function renderTogglesInject(host, view, parsed, g) {
  ...
  (function () {
    ...
    var q = el('input', 'sc-input'); q.type = 'search'; q.placeholder = tr('检索参数（名称 / 键名 / 说明）…');
  ...
  appState.switchKeys().forEach(function (it) {
    ...
    host.appendChild(appState.makeToggle(it[0], it[1], it[2], cur, function (key, sw) {
      appState.api('/toggle', { method: 'POST', body: JSON.stringify({ key: key }) })
        .then(function () { appState.statusFn(tr("✓ 已切换 ") + key); })
  ...
  host.appendChild(appState.makeToggle('injectRelevance', tr("注入相关性重排 injectRelevance"), tr("开=按相关性选行（缺省）；关=回落「基线 + 新鲜度」选行。即时生效（下次注入即用）"), gVal('injectRelevance', true) !== false, function (key, sw) {
    appState.api('/toggle', { method: 'POST', body: JSON.stringify({ key: key }) })
      .then(function () { appState.statusFn(tr("✓ 已切换 ") + key + tr("（即时）")); })
      .catch(function (e) { appState.failFn(e); sw.checked = !sw.checked; });
  }));
...
function renderTogglesModelVec(host) {
  ...
  function refreshVecZone() {
    appState.api('/vector/status2').then(function (s2) {
      ...
      vb.appendChild(UI.dsBadge(tr("召回模式 ") + Derive.vecLabel(st), 'ended')
        .setTitle(tr("provider → 中文模式名：fusion=融合 / lexical=词法 / gpu-ready=就绪 / off=关")).box);
      ...
      sw.addEventListener('change', function () {
        appState.api('/embed/config', { method: 'POST', body: JSON.stringify({ embedEnabled: sw.checked }) })
          .then(function () { appState.statusFn(tr("✓ 向量 ") + (sw.checked ? tr("开") : tr("关")) + tr("（重载后生效）")); })
          .catch(function (e) { appState.failFn(e); sw.checked = !sw.checked; });
      });
      ...
      var kInp = el('input', 'sc-input sc-w-md'); kInp.value = curKeyEnv; kInp.title = 'embedApiKeyEnv'; kInp.placeholder = tr("key 环境变量名（本地免填）");
      ...
      saveBtn.addEventListener('click', function () {
        ...
        appState.api('/embed/config', { method: 'POST', body: JSON.stringify({ embedBaseUrl: uInp.value.trim(), embedModel: mSel.value || curModel, embedApiKeyEnv: kInp.value.trim() || 'EMBED_API_KEY' }) })
          .then(function () { appState.statusFn(tr("✓ 已保存（重载后生效——若换了服务/模型请点「清缓存重建」）")); saveBtn.disabled = false; saveBtn.textContent = tr("保存配置"); })
          .catch(function (e) { saveBtn.disabled = false; saveBtn.textContent = tr("保存配置"); appState.failFn(e); });
      });
      ...
      function setSelectState(disabled, placeholder) {
        ...
        mSel.textContent = '';
        var opt = el('option'); opt.value = ''; opt.textContent = placeholder || tr("选择模型…");
      ...
      function enumModels() {
        ...
        probeDone = false;
        if (!root) { setSelectState(true, tr("先填写服务地址")); probeHint.textContent = ''; return; }
        // 校验 URL
        try { var u = new URL(root); if (u.protocol !== 'http:' && u.protocol !== 'https:') throw new Error('x'); } catch (e) { setSelectState(true, tr("URL 无效")); probeHint.textContent = tr("需 http(s):// 开头"); return; }
        if (probing) return;
        probing = true; setSelectState(true, tr("加载可用模型中…")); probeHint.textContent = '';
      ...
      var clearBtn = UI.button(tr("清缓存重建"), function () {
        return appState.api('/vector/cache/clear', { method: 'POST', body: '{}' })
          .then(function (r) { appState.statusFn(tr("✓ 向量缓存已清") + (r && r.removed ? tr("（删除 ") + r.removed + '）' : '') + tr("——下次召回按当前模型自动重嵌")); });
    ...
    }).catch(function (e) { vzone.textContent = ''; vzone.appendChild(el('div', 'sc-desc', tr("向量状态不可用: ") + e.message)); });
  }
...
function renderTogglesSched(host, g) {
  ...
  host.appendChild(appState.makeToggle('bankGit', tr("记忆库 git 版本化 bankGit"), tr("每次成功写入后提交库快照（可 diff/revert；库在 ~/.dsh 下，不入公开树）。缺省开"), gVal('bankGit', true) !== false, function (key, sw) {
    appState.api('/toggle', { method: 'POST', body: JSON.stringify({ key: key }) })
      .then(function () { appState.statusFn(tr("✓ 已切换 ") + key + tr("（需重载生效）")); })
      .catch(function (e) { appState.failFn(e); sw.checked = !sw.checked; });
  }));
  ...
  function distillSave(patch, onFail) {
    return appState.api('/distill/config', { method: 'POST', body: JSON.stringify(patch) })
      .then(function () { appState.statusFn(tr("✓ 已写入 ") + Object.keys(patch).join(',') + tr("（重载后生效）")); })
  ...
  function distillInput(key, name, desc, initial, kind, min) {
    ...
    else { inp.type = 'text'; inp.placeholder = tr("留空=继承主会话模型"); inp.className = 'sc-input sc-w-sm'; }
  ...
  appState.api('/distill/config').then(function (d) {
    ...
    dDesc.textContent = tr("写入自持配置 ~/.dsh/suite/scheduler.json（深度睡眠同通道）。改动不会立刻作用到在跑的会话——**需重载插件后生效**。当前：蒸馏 ") + (val('enableDistill', true) ? tr("开") : tr("关")) + tr(" / 空闲 ") + Math.round(val('idleWakeMs', 600000) / 60000) + tr(" 分钟 / 本轮最少 ") + val('minTurnChars', 200) + tr(" 字符 / 预筛 ") + (val('distillPrescan', true) ? tr("开") : tr("关")) + tr(" / 蒸馏模型 ") + curModelTxt + '。';
    dZone.textContent = '';
    ...
    dZone.appendChild(distillInput('idleWakeMs', tr("空闲唤醒 idleWakeMs"), tr("turn 结束后空闲满此时长才蒸馏（≥1 分钟，默认 10 分钟）"), Math.round(val('idleWakeMs', 600000) / 60000), 'minutes', 1));
    ...
    pendBtn.addEventListener('click', function () {
      pendBtn.disabled = true; pendBtn.textContent = tr("蒸馏中…（约 1-2 分钟）");
      appState.api('/distill/run', { method: 'POST', body: '{}' })
        .then(function (rr) {
          pendBtn.disabled = false; pendBtn.textContent = tr("立即蒸馏一次");
          if (rr && rr.ok) appState.statusFn('✓ ' + (rr.note || tr("蒸馏完成")));
          else appState.statusFn('⚠ ' + ((rr && rr.note) || tr("蒸馏未触发")) + tr("——根会话活跃中会跳过，等闲置自动跑"));
        })
        .catch(function (e) { pendBtn.disabled = false; pendBtn.textContent = tr("立即蒸馏一次"); appState.failFn(e); });
    });
